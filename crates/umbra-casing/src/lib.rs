//! Shared case-conversion helpers for the umbra workspace.
//!
//! This crate is intentionally dependency-free so it can be used from
//! `umbra-macros` (a proc-macro crate), `umbra-core`, `umbra-cli`, and
...
//! |---|---|---|
//! | [`to_snake_case`] | `umbra-macros` (table-name derivation) | Full acronym-aware algorithm |
//! | [`pascal_case_from_table`] | `umbra-core/inspect.rs` | SQL table → struct name; lowercases segments |
//! | [`pascal_case_from_ident`] | `umbra-cli/scaffold.rs`, `umbra-openapi` | Ident / name → PascalCase; no lowercasing |
...
///
/// Used by `umbra-core/inspect.rs` (the `inspectdb` command) to generate
/// struct names from introspected table names.
///
/// # Examples
///
/// ```
/// use umbra_casing::pascal_case_from_table;
///
...
///
/// Used by `umbra-cli/scaffold.rs` (to produce `{Name}Plugin`) and
/// `umbra-openapi` (to produce OpenAPI schema names from model names that
