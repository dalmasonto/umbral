...
pub(super) fn hydrate_joined_rels<T: Model + HydrateRelated>(
    ...
    for field_name in join_fields {
        ...
        for idx in (0..hops.len()).rev() {
            ...
            let pk_is_null = crate::orm::dynamic::decode_to_json_aliased(row, pk_col, &pk_alias)
                .map(|v| v.is_null())
                .unwrap_or(true);
            if pk_is_null {
...
pub(super) fn extract_m2m_child_json<T: Model>(
    ...
    let pk_null = crate::orm::dynamic::decode_to_json_aliased(row, pk_col, &pk_alias)
        .map(|v| v.is_null())
        .unwrap_or(true);
    if pk_null {
        return Ok(None);
    ...
    if let Some((_ct, _cpk, onward)) = crate::orm::queryset::resolve_m2m_chain::<T>(field_name) {
        ...
        for i in (0..onward.len()).rev() {
            ...
            let hpk_null = crate::orm::dynamic::decode_to_json_aliased(row, hpk, &hpk_alias)
                .map(|v| v.is_null())
                .unwrap_or(true);
            if hpk_null {
                deeper = None;
