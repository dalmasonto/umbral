    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ReverseError::NoForeignKey {
                ...
                f,
                "umbra::orm::reverse: `{child}` has no foreign key to `{parent_table}` \
            ...
            ReverseError::Ambiguous {
                ...
                f,
                "umbra::orm::reverse: `{child}` has multiple foreign keys to `{parent_table}` \
                 ({}). Disambiguate with `reverse_via::<{child}>(\"<column>\")`",
                candidates.join(", ")
            ),
            ReverseError::UnknownColumn { child, column } => write!(
                f,
                "umbra::orm::reverse_via: `{child}` has no column `{column}`"
            ),
            ReverseError::NotAForeignKey {
                ...
                f,
                "umbra::orm::reverse_via: column `{column}` on `{child}` is not a foreign key \
                 to `{parent_table}`"
            ),
