...
//! phone numbers. `umbra-oauth` uses it for provider access/refresh
//! tokens.
//!
...
//! from `UMBRA_MASK_PUBLIC_KEY` and the optional `UMBRA_MASK_PRIVATE_KEY`
...
pub enum MaskError {
    ...
    /// called nor `UMBRA_MASK_PUBLIC_KEY` is set). Encryption can't run.
    NoKeyring,
    /// The keyring has a public key but no private key, so the value
    /// can be stored but not revealed. Set `UMBRA_MASK_PRIVATE_KEY`.
    NoPrivateKey,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MaskError::NoKeyring => f.write_str(
                "no mask keyring configured (set UMBRA_MASK_PUBLIC_KEY or call set_mask_keyring)",
            ),
            MaskError::NoPrivateKey => f.write_str(
                "mask keyring has no private key; cannot reveal (set UMBRA_MASK_PRIVATE_KEY)",
            ),
...
impl MaskKeyring {
    ...
    /// Read `UMBRA_MASK_PUBLIC_KEY` (+ optional `UMBRA_MASK_PRIVATE_KEY`)
    /// from the environment. Returns `Err(NoKeyring)` if the public key
    /// is unset.
    pub fn from_env() -> Result<Self, MaskError> {
        let public = std::env::var("UMBRA_MASK_PUBLIC_KEY").map_err(|_| MaskError::NoKeyring)?;
        let secret = std::env::var("UMBRA_MASK_PRIVATE_KEY").ok();
...
impl<T> Serialize for Masked<T> {
    ...
    /// missing `UMBRA_MASK_PUBLIC_KEY` loudly at write time instead of
