...
//! `umbra-core` (re-exported from the `umbra` facade at `umbra::auth`)
//! so that `umbra-auth` and `umbra-rest` both depend *inward* on core
//! rather than one depending on the other.
//!
//! This is the architectural fix for gaps2 #76: previously
//! `umbra-auth` depended on `umbra-rest` to get `Identity` and
//! `Authentication`, which forced REST into every app that used auth —
//! even REST-free HTML apps. After this move, `umbra-auth` names
//! `umbra::auth::*` (the facade path), and `umbra-rest` re-exports the
...
pub struct Identity {
    ...
    /// built-in `IsStaff` permission class in `umbra-rest`.
    pub is_staff: bool,
...
/// RestPlugin::default().authenticate(FnAuthentication::new(|headers| async move {
///     let (user, pass) = umbra::auth::parse_basic_credentials(&headers)?;
