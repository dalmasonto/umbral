...
//!
//! - **Derive classification.** `#[umbra(xml)]` / `#[umbra(ltree)]` /
//!   `#[umbra(bit)]` upgrade a `String` / `Option<String>` field from
//!   `SqlType::Text` to the native PG type without changing the Rust
//!   type. A plain `String` field (no attr) stays `Text` (regression
//!   guard). `#[umbra(macaddr)]` confirms a `mac_address::MacAddress`
...
//! - **Backend gating.** An `#[umbra(xml)]` field against SQLite fails
//!   at boot with `field.backend` (ignored — pollutes the registry).
//! - **Live PG round-trip** behind `#[ignore]`.

use umbra::orm::{Model, SqlType};
use umbra_core::migrate::{Column, Operation, render_operation_for};

#[derive(Debug, Clone, sqlx::FromRow, umbra::orm::Model)]
#[umbra(table = "umbra_gaps2_70_doc")]
pub struct Doc {
    pub id: i64,
    /// `#[umbra(xml)]` → `SqlType::Xml` (not Text).
    #[umbra(xml)]
    pub body: String,
    /// `#[umbra(ltree)]` on a non-nullable field → `SqlType::Ltree`.
    #[umbra(ltree)]
    pub path: String,
    /// `#[umbra(bit)]` → `SqlType::Bit` (BIT VARYING).
    #[umbra(bit)]
    pub flags: String,
    /// Nullable XML: `#[umbra(xml)]` on `Option<String>` → nullable Xml.
    #[umbra(xml)]
    pub note: Option<String>,
    /// `#[umbra(macaddr)]` confirms MacAddr (already auto-detected).
    #[umbra(macaddr)]
...
fn field_map() -> std::collections::HashMap<&'static str, &'static umbra::orm::FieldSpec> {
...
fn xml_attr_classifies_as_xml_sqltype() {
    ...
    assert_eq!(body.ty, SqlType::Xml, "#[umbra(xml)] String → Xml");
    ...
    assert_eq!(
        note.ty,
        SqlType::Xml,
        "#[umbra(xml)] Option<String> → nullable Xml"
    );
...
fn ltree_and_bit_attrs_classify_correctly() {
    ...
    assert_eq!(path.ty, SqlType::Ltree, "#[umbra(ltree)] String → Ltree");
    assert!(!path.nullable);

    let flags = by_name.get("flags").expect("flags field");
    assert_eq!(flags.ty, SqlType::Bit, "#[umbra(bit)] String → Bit");
    assert!(!flags.nullable);
}

/// `#[umbra(macaddr)]` is the "free" type — the `mac_address::MacAddress`
...
fn postgres_ddl_renders_xml_ltree_bit_types() {
    let op = Operation::CreateTable {
        table: "umbra_gaps2_70_doc".to_string(),
        columns: vec![
...
fn column_const_module_has_text_pg_types() {
    use umbra::orm::column::{BitCol, LtreeCol, MacAddrCol, NullableXmlCol, XmlCol};
...
fn inspect_renders_text_pg_types_as_string() {
    use umbra::inspect::{IntrospectedColumn, IntrospectedSchema, IntrospectedTable, render_models};
...
#[tokio::test]
#[ignore = "needs UMBRA_TEST_POSTGRES_URL + CREATE EXTENSION ltree"]
async fn text_pg_fields_round_trip_through_postgres() {
    ...
    sqlx::query("CREATE EXTENSION IF NOT EXISTS ltree")
        ...
        .await
        .unwrap();
    sqlx::query("DROP TABLE IF EXISTS umbra_gaps2_70_doc")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
        "CREATE TABLE umbra_gaps2_70_doc ( \
    ...
    sqlx::query(
        "INSERT INTO umbra_gaps2_70_doc (body, path, flags, note, mac, title) \
