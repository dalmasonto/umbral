...
//! `#[umbra(db_constraint = false)]` opt-out.
//!
//! umbra routes models to different databases (named pools). A foreign
...
//!
//! **(b) The opt-out** `#[umbra(db_constraint = false)]` keeps the FK as
...
use umbra::orm::{FkAction, Model, SqlType};
...
#[derive(Debug, Clone, sqlx::FromRow, serde::Serialize, serde::Deserialize, umbra::orm::Model)]
#[umbra(table = "xdb_bad_child", database = "analytics")]
pub struct XdbBadChild {
    pub id: i64,
    #[umbra(no_reverse)]
    pub parent: umbra::orm::ForeignKey<XdbParent>,
}

/// Same cross-DB shape, but the FK opts out of the physical constraint
/// → `App::build()` must accept it.
#[derive(Debug, Clone, sqlx::FromRow, serde::Serialize, serde::Deserialize, umbra::orm::Model)]
#[umbra(table = "xdb_ok_child", database = "analytics")]
pub struct XdbOkChild {
    pub id: i64,
    #[umbra(no_reverse, db_constraint = false)]
    pub parent: umbra::orm::ForeignKey<XdbParent>,
}
...
async fn cross_db_fk_without_optout_fails_build() {
    ...
    let mut settings = umbra::Settings::from_env().expect("settings load");
...
/// (b) The same cross-DB FK with `#[umbra(db_constraint = false)]` →
...
fn db_constraint_lands_on_field_spec() {
    ...
    assert!(
        !ok_fk.db_constraint,
        "#[umbra(db_constraint = false)] sets db_constraint = false"
    );
}
