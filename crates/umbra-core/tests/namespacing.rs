...
//!
//! Verifies that `#[umbra(plugin = "...")]` sets `Model::TABLE` to
...
#[derive(Debug, Clone, sqlx::FromRow, umbra::orm::Model)]
#[umbra(plugin = "blog", table = "custom_posts")]
...
fn no_attribute_stays_bare_snake_case() {
    assert_eq!(
        ...
        "category",
        "a model with no umbra attribute should use bare snake_case struct name"
    );
}
