...
use umbra::orm::{ChoiceField, Model, MultiChoice};
...
#[derive(Debug, Clone, serde::Serialize, sqlx::FromRow, umbra::orm::Model)]
#[umbra(table = "mc_article")]
pub struct Article {
    pub id: i64,
    pub title: String,
    #[umbra(default = "design,frontend")]
...
fn field_spec_marks_multichoice_and_carries_choices() {
    ...
    assert_eq!(tags.ty, umbra_core::orm::SqlType::Text);
...
fn migrate_column_carries_is_multichoice_flag() {
    let col = umbra_core::migrate::Column::from(
        Article::FIELDS
            .iter()
            .find(|f| f.name == "tags")
            .expect("tags field"),
