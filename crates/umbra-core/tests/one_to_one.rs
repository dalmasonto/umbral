...
//!   - Auto FK discovery: no `#[umbra(reverse_one = "...")]` etc.
...
use umbra::orm::{ForeignKey, OneToOne};
use umbra_core::db;

#[derive(Debug, Clone, sqlx::FromRow, Serialize, Deserialize, umbra::orm::Model)]
#[umbra(table = "oto_user")]
pub struct User {
    pub id: i64,
    pub username: String,
    /// Reverse OneToOne — populated by
    /// `.prefetch_related("profile")`. No umbra attribute needed:
...
#[derive(Debug, Clone, sqlx::FromRow, Serialize, Deserialize, umbra::orm::Model)]
#[umbra(table = "oto_profile")]
pub struct Profile {
    pub id: i64,
    /// `#[umbra(unique)]` is what makes the reverse side OneToOne
