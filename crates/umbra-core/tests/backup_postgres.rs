...
//!
//! The test self-skips unless `UMBRA_TEST_POSTGRES_URL` points at a
...
#[umbra(table = "umbra_backup_pg_record")]
...
async fn postgres_dump_and_load_round_trip_core_types() {
    let Ok(url) = std::env::var("UMBRA_TEST_POSTGRES_URL") else {
        eprintln!("skipping Postgres backup test: UMBRA_TEST_POSTGRES_URL is not set");
        return;
    };

    let pool = PgPool::connect(&url)
        .await
        .expect("connect to UMBRA_TEST_POSTGRES_URL");

    sqlx::query("DROP TABLE IF EXISTS umbra_backup_pg_record")
        .execute(&pool)
        .await
        .expect("drop prior backup test table");
    sqlx::query(
        "CREATE TABLE umbra_backup_pg_record (\
    ...
    sqlx::query(
        "INSERT INTO umbra_backup_pg_record \
    ...
    let mut settings = umbra::Settings::from_env().expect("settings defaults load");
    ...
    let dump = umbra::backup::dump()
        .await
        .expect("dump should read from the Postgres pool");
    let table = dump
        .models
        .iter()
        .find(|model| model.table == "umbra_backup_pg_record")
    ...
    sqlx::query("DELETE FROM umbra_backup_pg_record")
        .execute(&pool)
        .await
        .expect("wipe table before load");

    let report = umbra::backup::load(&dump)
        .await
