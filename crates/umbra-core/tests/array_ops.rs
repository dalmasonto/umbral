...
//! gated on `UMBRA_TEST_POSTGRES_URL`.
...
#[umbra(table = "umbra_phase421_event")]
pub struct Event {
    pub id: i64,
    pub tags: Vec<String>,
...
/// `UMBRA_TEST_POSTGRES_URL` and run with `--ignored`.
#[tokio::test]
...
async fn array_operators_filter_real_postgres_rows() {
    ...
    sqlx::query("DROP TABLE IF EXISTS umbra_phase421_event")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
        "CREATE TABLE umbra_phase421_event ( \
    ...
    sqlx::query("INSERT INTO umbra_phase421_event (tags, scores) VALUES ($1, $2)")
