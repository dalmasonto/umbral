...
async fn run_in_applies_pending_migrations_and_creates_tables() {
    ...
    // `migrated_dir()` now seeds two migrations: the autogenerated
    // `0001_auto` for the model registry and the hand-crafted
    // `0002_add_post_*` that the M8 end-to-end test reads. Both must
    // appear here as proof that `run_in` applied each one.
    assert_eq!(
        rows.len(),
        2,
    ...
    let (plugin, name, applied_at, snapshot_hash) = &rows[1];
    assert_eq!(plugin, APP_PLUGIN_NAME);
    ...
    assert_eq!(
        snapshot_hash,
        &Snapshot::current().hash(),
        "M8 AddColumn snapshot_hash should also equal the live snapshot's hash",
    );
}
...
/// M8 — flipping a column from non-nullable to nullable (or back) also
/// surfaces as `MigrateError::UnsafeAlter`. A flip on a populated
/// SQLite table is destructive without the table-recreation dance.
#[test]
fn diff_returns_unsafe_alter_for_a_nullable_flip() {
    let previous = snapshot_of(post_model(vec![id_column(), text_column("title")]));
    let current = snapshot_of(post_model(vec![
        id_column(),
        Column {
    ...
    let err = diff(&previous, &current).expect_err("a nullable flip must be UnsafeAlter");
    match err {
        MigrateError::UnsafeAlter {
            model,
            column,
            ...
            assert!(
                ...
                "UnsafeAlter reason should call out the nullable flip; got {reason:?}",
            );
        }
        other => panic!("expected MigrateError::UnsafeAlter, got {other:?}"),
    }
...
/// via `run_in`, registers in `umbra_migrations`, and the new column
...
async fn run_in_applies_a_hand_crafted_add_column_migration() {
    ...
    let tracked: Vec<(String, String)> =
        sqlx::query_as("SELECT plugin, name FROM umbra_migrations WHERE name = ?")
