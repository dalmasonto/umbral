...
//! `crates/umbra-core/tests/plugin_contract.rs`.

use std::path::PathBuf;

use umbra::Settings;
use umbra::plugin::{Plugin, StaticDir};
...
async fn published_static_round_trips_through_app_build() {
    ...
    assert!(
        umbra::static_files::published_static().is_none(),
    ...
    let published =
        umbra::static_files::published_static().expect("App::build publishes static contributions");
