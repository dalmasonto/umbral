//! Multi-DB routing: per-model `#[umbra(database = "alias")]` + per-DB
//! migration tracking.
//!
...
//! `umbra_migrations` tracking table should record exactly the
...
#[derive(Debug, Clone, sqlx::FromRow, serde::Serialize, umbra::orm::Model)]
#[umbra(table = "primary_article")]
...
#[derive(Debug, Clone, sqlx::FromRow, serde::Serialize, umbra::orm::Model)]
#[umbra(table = "analytics_event", database = "analytics")]
...
async fn boot_app_with_two_dbs() -> &'static PathBuf {
    BOOT.get_or_init(|| async {
        ...
        let settings = umbra::Settings::from_env().expect("settings load");
...
async fn table_alias_resolves_per_model_attribute() {
    ...
    assert_eq!(umbra::migrate::table_alias("primary_article"), "default");
    // `analytics_event` was tagged with `database = "analytics"`.
    assert_eq!(umbra::migrate::table_alias("analytics_event"), "analytics");
    // Unknown tables fall through to "default" so framework book-
    // keeping (`umbra_migrations` itself, orphan schema) lands on
    // the main pool.
    assert_eq!(umbra::migrate::table_alias("not_a_table"), "default");
}
...
async fn each_pool_has_only_its_routed_table() {
    ...
    // `analytics_event`.
    let default_pool = umbra::db::pool_for("default");
    ...
    let analytics_pool = umbra::db::pool_for("analytics");
...
#[tokio::test(flavor = "multi_thread")]
async fn each_pool_has_its_own_umbra_migrations_tracking_table() {
    let _ = boot_app_with_two_dbs().await;
    ...
    for alias in ["default", "analytics"] {
        let pool = umbra::db::pool_for(alias);
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM umbra_migrations")
            .fetch_one(&pool)
            .await
            .unwrap();
        assert!(
