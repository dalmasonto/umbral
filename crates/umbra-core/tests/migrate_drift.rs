...
//!   their side-effects on the shared `umbra_migrations` table never
//!   collide.
//!
//! Functions under test (all from `umbra::migrate`):
...
/// Mark one migration applied in the SHARED pool's `umbra_migrations`.
...
async fn mark_applied_shared(plugin: &str, name: &str) {
    ...
    .execute(&pool)
    .await
    .expect("ensure umbra_migrations table");
    sqlx::query(
...
async fn run_checked_in_proceeds_with_allow_drift() {
    ...
    let rows: Vec<(String, String)> =
        sqlx::query_as("SELECT plugin, name FROM umbra_migrations WHERE name = '0002_pending_rad'")
            .fetch_all(&pool)
            .await
            .expect("select");
    assert_eq!(
...
/// DDL. After the call, `umbra_migrations` has the row but the target table
/// does NOT exist.
#[tokio::test]
async fn fake_apply_in_marks_applied_without_schema_change() {
    ...
    // Tracking row exists.
    let rows: Vec<(String, String)> =
        sqlx::query_as("SELECT plugin, name FROM umbra_migrations WHERE name = '0001_fake_fa'")
            .fetch_all(&pool)
            .await
            .expect("select");
    assert_eq!(
    ...
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM umbra_migrations WHERE name = '0001_fake_fa'")
            .fetch_one(&pool)
            .await
            .expect("count");
    assert_eq!(
...
async fn fake_initial_in_skips_when_tables_absent() {
    ...
    let pool = umbra::db::pool();
    let rows: Vec<(String, String)> =
        sqlx::query_as("SELECT plugin, name FROM umbra_migrations WHERE name = '0001_fi_absent'")
            .fetch_all(&pool)
            .await
            .expect("select");
    assert!(
...
async fn fake_initial_in_marks_applied_when_tables_exist() {
    ...
    let rows: Vec<(String, String)> =
        sqlx::query_as("SELECT plugin, name FROM umbra_migrations WHERE name = '0001_fi_exists'")
            .fetch_all(&pool)
            .await
            .expect("select");
    assert_eq!(
