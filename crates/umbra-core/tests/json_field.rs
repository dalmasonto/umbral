...
#[umbra(table = "umbra_phase4_json_event")]
pub struct Event {
    pub id: i64,
    pub kind: String,
...
async fn json_value_round_trips_through_sqlite() {
    ...
    sqlx::query("INSERT INTO umbra_phase4_json_event (kind, payload, meta) VALUES (?, ?, ?)")
        .bind("startup")
        .bind(&payload)
...
async fn nullable_json_col_is_null_filters_correctly() {
    ...
    let payload_spec = <Event as Model>::FIELDS
        ...
        .unwrap();
    assert_eq!(payload_spec.ty, umbra::orm::SqlType::Json);
}
