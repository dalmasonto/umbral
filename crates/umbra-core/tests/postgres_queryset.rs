...
//!   runs only when `UMBRA_TEST_POSTGRES_URL` is set in the
...
// The Article model is parallel to `umbra_core::orm::Post` (the
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, sqlx::FromRow, umbra::orm::Model)]
#[umbra(table = "umbra_phase25_article")]
pub struct Article {
    pub id: i64,
    pub title: String,
    pub body: String,
...
// --------------------------------------------------------------------- //
// Full round trip — runs only with UMBRA_TEST_POSTGRES_URL set.          //
// --------------------------------------------------------------------- //

/// End-to-end against a real Postgres. Boots the pool, creates the
/// table, seeds two rows, and exercises every terminal.
///
/// Set `UMBRA_TEST_POSTGRES_URL` to a writable Postgres URL to run
/// this. Example:
///
/// ```text
/// UMBRA_TEST_POSTGRES_URL=postgres://umbra:umbra@localhost/umbra_test \
///     cargo test --test postgres_queryset -- --ignored
/// ```
///
...
async fn full_round_trip_against_real_postgres() {
    ...
    sqlx::query("DROP TABLE IF EXISTS umbra_phase25_article")
        .execute(&pool)
        .await
        .expect("drop prior table");

    sqlx::query(
        "CREATE TABLE umbra_phase25_article (\
    ...
    sqlx::query(
        "INSERT INTO umbra_phase25_article (title, body, published_at) \
