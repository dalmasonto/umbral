...
//!    data lands.
//! 2. The data migration is recorded once in `umbra_migrations`; a
...
use umbra::migrate::{MigrationFile, Operation, Snapshot, make_empty_in, make_in, run_in};
...
async fn boot() {
    BOOT.get_or_init(|| async {
        let settings = umbra::Settings::from_env().expect("settings load in test env");
...
async fn run_sql_data_migration_applies_and_is_idempotent() {
    ...
    let pool = match umbra::db::pool_dispatched() {
        umbra::db::DbPool::Sqlite(p) => p,
        umbra::db::DbPool::Postgres(_) => unreachable!("test pool is sqlite"),
    };
    ...
    // 3) Recorded once.
    let tracked: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM umbra_migrations WHERE plugin = 'app' AND name = '0002_run_sql'")
            .fetch_one(pool)
            .await
...
async fn make_empty_writes_a_noop_migration_that_does_not_drift() {
    ...
    match make_in(dir).await {
        Err(umbra::migrate::MigrateError::NoChanges) => {}
