...
//!
//! See `crates/umbra-core/src/check.rs` for the framework being tested.

use std::collections::HashMap;

use umbra_core::backend::SqliteBackend;
use umbra_core::check::{CheckContext, Severity, framework_checks, run_all};
use umbra_core::settings::{Environment, Settings};
...
async fn system_check_failed_build_returns_build_error_system_check_failed() {
    ...
    let result = umbra_core::app::App::builder()
        .settings(settings)
    ...
    let findings = match err {
        umbra_core::app::BuildError::SystemCheckFailed { findings } => findings,
