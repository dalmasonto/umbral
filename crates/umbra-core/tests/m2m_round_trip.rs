...
//! `umbra-permissions`: a parent with `id: i64` joined to a child with
...
//! `RoundTripPermission` pair rather than the real umbra-permissions
...
pub struct RoundTripPermission {
    #[umbra(primary_key, string, max_length = 150)]
    pub codename: String,
    pub label: String,
}
...
async fn pool_handle() -> SqlitePool {
    match umbra::db::pool_dispatched() {
        ...
        umbra::db::DbPool::Postgres(_) => panic!("M2M round-trip test targets SQLite only"),
    }
}
