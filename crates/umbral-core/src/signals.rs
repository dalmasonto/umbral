...
//! directly and that `umbral-signals` builds its typed per-model API on
//! top of. Moving the registry here — inside `umbral-core` — breaks the
//! dependency cycle: the signals plugin depends on the `umbral` facade
//! which depends on `umbral-core`, so `umbral-core` can call
...
// keeps `umbral-core` decoupled from any specific identity shape — auth
...
// tracking, and umbral-storage's replace-cleanup) need the OLD row too — what
