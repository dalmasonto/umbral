        ...
        let field_spec = T::FIELDS
            ...
            .ok_or_else(|| {
                sqlx::Error::Protocol(format!(
                    "umbral::orm::select_related: unknown field `{field_name}` on model `{}`",
                    T::NAME
                ))
            })?;
        let fk_target = field_spec.fk_target.ok_or_else(|| {
            sqlx::Error::Protocol(format!(
                "umbral::orm::select_related: field `{field_name}` on `{}` is not a foreign key",
                T::NAME
            ))
        })?;
...
pub(super) async fn hydrate_select_related_nested<T: Model + HydrateRelated>(
    ...
    for hop in &hops {
        let meta = registered
            ...
            .ok_or_else(|| {
                sqlx::Error::Protocol(format!(
                    "umbral::orm::select_related: model for table `{current_table}` is not registered \
                     (needed for nested traversal of `{path}`)"
                ))
            })?;
        let col = meta.fields.iter().find(|c| c.name == *hop).ok_or_else(|| {
            sqlx::Error::Protocol(format!(
                "umbral::orm::select_related: unknown field `{hop}` on table `{current_table}` \
                 (full path `{path}`)"
            ))
        })?;
        let target = col.fk_target.as_deref().ok_or_else(|| {
            sqlx::Error::Protocol(format!(
                "umbral::orm::select_related: field `{hop}` on table `{current_table}` is not a \
...
///
///   - add `#[umbral(unique)]` to make a non-unique FK unique
...
async fn hydrate_one_to_one_for_field<T: Model + HydrateRelated>(
    ...
    let Some(child_meta) = registered.iter().find(|m| m.table == spec.target_table) else {
        return Err(sqlx::Error::Protocol(format!(
            "umbral::orm::prefetch_related: child model for table `{}` is not \
    ...
    let fk_column = match candidates.len() {
        ...
        0 => {
            return Err(sqlx::Error::Protocol(format!(
                "umbral::orm::prefetch_related: OneToOne field `{}` on `{}` \
                 has no back-link — `{}` needs a `#[umbral(unique)]` \
...
pub(super) async fn hydrate_prefetch_related<T: Model + HydrateRelated>(
    ...
    for field_name in prefetch_fields {
        ...
        // field declared on the parent with `#[umbral(reverse_fk =
        // "<fk_col>")]`).
        ...
        let spec = match m2m_spec {
            ...
            None => {
                ...
                return Err(sqlx::Error::Protocol(format!(
                    "umbral::orm::prefetch_related: unknown field `{field_name}` on model `{}`{hint}",
                    T::NAME
                )));
            }
        };
