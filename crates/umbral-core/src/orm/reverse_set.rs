...
//! by default) and tag it with `#[umbral(reverse_fk = "<fk_col>")]`
...
pub struct ReverseSet<C: Model> {
    ...
    /// Set by the same macro hook from the `#[umbral(reverse_fk =
...
impl<C: Model> ReverseSet<C> {
    ...
    /// hook from the `#[umbral(reverse_fk = "...")]` attribute.
    ...
    /// macro didn't see a `#[umbral(reverse_fk = ...)]` attribute, or
