...
#[umbral(table = "umbral_decimal_invoice")]
pub struct Invoice {
    pub id: i64,
...
#[derive(Debug, Clone, sqlx::FromRow, serde::Serialize, serde::Deserialize, umbral::orm::Model)]
#[umbral(table = "umbral_decimal_quote")]
pub struct Quote {
    pub id: i64,
...
async fn decimal_round_trips_on_postgres() {
    ...
    sqlx::query("DROP TABLE IF EXISTS umbral_decimal_invoice")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("CREATE TABLE umbral_decimal_invoice (id BIGSERIAL PRIMARY KEY, total NUMERIC(19,4) NOT NULL)")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query("INSERT INTO umbral_decimal_invoice (total) VALUES ($1)")
...
fn nullable_decimal_produces_nullable_decimal_col_constant() {
    use umbral::orm::column::{DecimalCol, NullableDecimalCol};
