...
//!   `Option<FileField>`, and explicit `#[umbral(widget)]` override.
...
use umbral::orm::{FieldSpec, FileField, ImageField, Model, SqlType};

#[derive(Debug, Clone, sqlx::FromRow, umbral::orm::Model)]
#[umbral(table = "umbral_file_doc")]
pub struct Doc {
    pub id: i64,
...
fn explicit_widget_overrides_type_default() {
    ...
    assert_eq!(
        banner.widget,
        Some("file"),
        "explicit #[umbral(widget)] must beat the type-derived image default"
    );
}
