...
//! Coverage layers:
//!
//! - **Derive classification.** `umbral::orm::TsVector` lands as
...
use umbral::orm::{Model, SqlType, TsVector};

#[derive(Debug, Clone, sqlx::FromRow, umbral::orm::Model)]
#[umbral(table = "umbral_phase43_doc")]
pub struct Doc {
    pub id: i64,
    pub title: String,
    pub search: umbral::orm::TsVector,
    pub alt_search: Option<umbral::orm::TsVector>,
}
...
fn postgres_ddl_renders_tsvector_type() {
    ...
    let op = Operation::CreateTable {
        table: "umbral_phase43_doc".to_string(),
        columns: vec![
            Column {
                name: "id".to_string(),
                ty: SqlType::BigInt,
    ...
    assert!(
        all.contains("idx_umbral_phase43_doc_search_gin"),
...
fn column_const_module_has_fulltext_types() {
    use umbral::orm::column::{FullTextCol, NullableFullTextCol};
...
async fn fulltext_field_filters_real_postgres_rows() {
    ...
    sqlx::query("DROP TABLE IF EXISTS umbral_phase43_doc")
        .execute(&pool)
        .await
        .unwrap();
    ...
    sqlx::query(
        "CREATE TABLE umbral_phase43_doc ( \
    ...
    sqlx::query("INSERT INTO umbral_phase43_doc (title) VALUES ($1)")
