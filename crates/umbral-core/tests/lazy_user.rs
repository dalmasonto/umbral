...
async fn current_thread_runtime_does_not_panic_returns_anonymous_fallback() {
    ...
    let out = with_current_user_lazy(lazy, async {
        umbral_core::templates::render_str("{{ user.is_staff }}", &serde_json::json!({}))
    })
    .await
...
async fn resolver_runs_once_across_two_renders_that_read_user() {
    ...
    let out = with_current_user_lazy(lazy, async {
        let a = umbral_core::templates::render_str("{{ user.is_staff }}", &serde_json::json!({}))
            .unwrap();
        let b = umbral_core::templates::render_str("{{ user.is_staff }}", &serde_json::json!({}))
            .unwrap();
        format!("{a}-{b}")
    })
    .await;
