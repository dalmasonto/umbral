...
//! Option<ImageField>`) must compile WITHOUT `#[umbral(noform)]` — the
...
use umbral::forms::{FormValidate, InputKind};
...
// NO `#[umbral(noform)]`. The whole point of the fix: this must compile.
#[derive(
    Debug,
    Clone,
    Default,
...
#[umbral(table = "fff_listing")]
pub struct Listing {
    #[umbral(primary_key)]
    pub id: i64,
    pub attachment: umbral::orm::FileField,
    pub avatar: Option<umbral::orm::ImageField>,
}
...
async fn optional_image_present_constructs_some() {
    let form = Listing::validate(&data(&[("avatar", "a.png"), ("attachment", "k1.png")]))
        ...
        .expect("both present");
    assert_eq!(form.attachment.key(), "k1.png");
    assert_eq!(form.avatar, Some(umbral::orm::ImageField::from("a.png")));
}
