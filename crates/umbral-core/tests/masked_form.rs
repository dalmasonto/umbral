...
//! compile error ("unsupported field type … mark with `#[umbral(noform)]`").
...
//! `#[umbral(noform)]` on the masked fields, and the form validates
//! without them.
...
#[umbral(table = "masked_form_contact")]
pub struct Contact {
    pub id: i64,
    #[form(required, length(min = 2, max = 80))]
    pub name: String,
    // No `#[umbral(noform)]` here — the derive skips Masked on its own.
