...
//!      correctly calls `umbral_tasks::register_handler`.
...
async fn boot() {
    BOOT.get_or_init(|| async {
        let settings = umbral::Settings::from_env().expect("settings from env");
...
#[umbral::task(name = "myapp.process_item_v2")]
...
async fn task_macro_registers_handler_under_fn_name() {
    ...
    boot().await;
    drain_queue().await;
    umbral_tasks::_clear_handlers_for_tests();
    ...
    let id = umbral_tasks::enqueue(
        ...
        umbral_tasks::EnqueueOptions::default(),
    )
    .await
    .expect("enqueue");

    let processed = umbral_tasks::run_worker_once().await.expect("worker step");
    ...
    let row: (String,) = sqlx::query_as("SELECT status FROM task_row WHERE id = ?")
        ...
        .await
        .expect("fetch row");
    assert_eq!(row.0, umbral_tasks::STATUS_SUCCEEDED);
}
