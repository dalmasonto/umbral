//! Tests for `#[umbral(display = "...", icon = "...")]` attributes on
...
//!   2. A model with no `#[umbral(...)]` attributes falls back to the
...
#[umbral(display = "Users", icon = "users")]
struct AuthUser {
    id: i64,
    username: String,
}

/// No umbral attributes — all defaults.
#[derive(Debug, sqlx::FromRow, Serialize, Deserialize, Model)]
struct BlogPost {
    id: i64,
    title: String,
...
// =========================================================================
// BUG-9 from bugs/tests/testBugs.md: `#[umbral(singleton)]` flips
...
fn singleton_attribute_flips_const() {
    assert!(
        ...
        "Model::SINGLETON should be true for a model with #[umbral(singleton)]",
    );
    let meta = umbral::migrate::ModelMeta::for_::<SiteSettings>();
    assert!(
        meta.singleton,
...
fn singleton_defaults_to_false_when_unset() {
    ...
    let meta = umbral::migrate::ModelMeta::for_::<BlogPost>();
...
fn singleton_round_trips_through_json_snapshot() {
    ...
    let round: umbral::migrate::ModelMeta = serde_json::from_str(&json).unwrap();
    assert!(round.singleton);
}

// =========================================================================
// IMP-3: `#[umbral(min = N)]` / `#[umbral(max = N)]` propagate from the
...
use umbral::orm::{Email, Slug, Url, ValidatorError};
...
fn validator_types_emit_text_format_marker() {
    ...
    assert_eq!(by_name["slug"].ty, umbral::orm::SqlType::Text);
    assert_eq!(by_name["email"].text_format, Some("email"));
    assert_eq!(by_name["email"].ty, umbral::orm::SqlType::Text);
    assert_eq!(by_name["homepage"].text_format, Some("url"));
    assert_eq!(by_name["homepage"].ty, umbral::orm::SqlType::Text);
    assert_eq!(by_name["plain"].text_format, None);
    assert_eq!(by_name["plain"].ty, umbral::orm::SqlType::Text);
}
...
fn validator_text_format_round_trips_through_meta_snapshot() {
    let meta = umbral::migrate::ModelMeta::for_::<ValidatorRow>();
    let json = serde_json::to_string(&meta).unwrap();
    ...
    let back: umbral::migrate::ModelMeta = serde_json::from_str(&json).unwrap();
...
fn m2m_relations_propagate_to_model_const() {
    ...
    let meta = umbral::migrate::ModelMeta::for_::<PostWithTags>();
...
fn struct_attrs_round_trip_through_model_meta() {
    let meta = umbral::migrate::ModelMeta::for_::<PostWithStructAttrs>();
    assert_eq!(
...
fn min_max_round_trip_through_model_meta_snapshot() {
    let meta = umbral::migrate::ModelMeta::for_::<PersonRow>();
    let json = serde_json::to_string(&meta).unwrap();
