//! Derive and attribute macros for umbral: `#[derive(Model)]`, `#[task]`, etc.
//!
//! Do not depend on this crate directly. Use the `umbral` facade, which
//! re-exports the derives so user code only ever imports `umbral`.
//!
...
/// `impl Model for Post` in `umbral-core::orm::post`, but uses `::umbral`
...
/// - One struct-level `#[umbral(table = "...")]` attribute is
...
struct UmbralStructAttr {
    ...
    singleton: bool,
    /// Feature #72 — `#[umbral(soft_delete)]`. Set when the model
    ...
    /// `#[umbral(indexes = [["a", "b"], ["status"]])]` — multi-column
    /// indexes (single-column already covered by field-level
    /// `#[umbral(index)]`). Closes BUG-7.
    indexes: Vec<Vec<String>>,
    /// `#[umbral(ordering = ["-published_at", "id"])]` — default
    /// `ORDER BY`. A leading `-` flips the direction to DESC.
    /// Closes BUG-8. Stored as `(column, direction)` pairs.
    ordering: Vec<(String, bool)>,
}

/// Field-level `#[umbral(...)]` attribute parsed from a struct field.
#[derive(Default)]
struct UmbralFieldAttr {
    /// `#[umbral(cidr)]` — on an `IpNetwork` / `Option<IpNetwork>` field,
    ...
    cidr: bool,
    /// `#[umbral(macaddr)]` — explicit marker for a `mac_address::
    ...
    macaddr: bool,
    /// `#[umbral(xml)]` — on a `String` / `Option<String>` field, declare
    /// it as a Postgres `XML` column instead of `Text`. gaps2 #70.
    xml: bool,
    /// `#[umbral(ltree)]` — on a `String` / `Option<String>` field,
    ...
    db_constraint: bool,
    /// `#[umbral(noedit)]` — show on edit form read-only, never on create.
    noedit: bool,
    /// `#[umbral(primary_key)]` — explicitly nominate this field as the
    ...
    /// `#[umbral(primary_key)] pub codename: String` instead of
    /// `pub id: String`.
    primary_key: bool,
    /// `#[umbral(no_reverse)]` — suppress the Gap-30 reverse-FK
    /// accessor (`<child>_set` on the parent type) for this FK.
    /// Required when the FK target is defined in another crate
    /// (e.g. `ForeignKey<AuthUser>` from `umbral-auth`), because
    ...
    max_length: u32,
    /// `#[umbral(choices)]` — the field's type implements
    /// [`umbral::orm::ChoiceField`]. The Model derive emits the field
    ...
    /// `#[umbral(default = "...")]` — SQL `DEFAULT` clause for this
    ...
    /// `#[umbral(unique)]` — emit a column-level `UNIQUE` constraint
    ...
    /// constraint flags).
    unique: bool,
    /// `#[umbral(on_delete = "cascade" | "restrict" | "set_null" |
    ...
    /// emitter parses it into `umbral::orm::FkAction` at codegen
    /// time and rejects unknown values with a compile error.
    on_delete: Option<String>,
    /// `#[umbral(on_update = "...")]` — emit `ON UPDATE <action>`.
    /// Same vocabulary as `on_delete`.
    on_update: Option<String>,
    /// `#[umbral(index)]` — single-column index. Closes BUG-4 from
    ...
    help: Option<String>,
    /// `#[umbral(example = "...")]` — sample value rendered as
    ...
    /// `#[umbral(backend = "postgres")]` — restrict this field to a
    ...
    /// `#[umbral(backend = "postgres"), umbral(backend = "mysql")]`.
    backends: Vec<String>,
    /// `#[umbral(min = N)]` — numeric lower bound. Closes IMP-3.
    min: Option<i64>,
    /// `#[umbral(max = N)]` — numeric upper bound. Closes IMP-3.
    max: Option<i64>,
    /// `#[umbral(slug_from = "title")]` — auto-derive this column from
    /// a sibling column at write time. Gap 109. Only meaningful on
    /// `Slug` / `String` columns; the runtime in
    /// `umbral-core::orm::write` computes the slug when the column
    /// itself isn't supplied on the body, leaving explicit
    /// user-supplied slugs alone.
    slug_from: Option<String>,
    /// Gap #44 — `#[umbral(reverse_fk = "post")]` on a
...
/// is the CHILD-side sugar (equivalent to `#[umbral(unique)] pub
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident("cidr") {
                // `#[umbral(cidr)]` — upgrade an `IpNetwork` /
                ...
                // `#[umbral(macaddr)]` — explicit MACADDR marker. The
                ...
                // `#[umbral(xml)]` — declare a String field as Postgres XML.
                parsed.xml = true;
                Ok(())
            } else if meta.path.is_ident("ltree") {
                // `#[umbral(ltree)]` — declare a String field as Postgres LTREE.
                parsed.ltree = true;
                Ok(())
                ...
                // `#[umbral(db_constraint)]` reads as `true` (the
                ...
                // `#[umbral(choices)]` — marker. The Rust field type
                ...
                // `#[umbral(default = "...")]` — SQL DEFAULT clause.
                ...
                // `#[umbral(index)]` — marker. Emits a single-column
                ...
                // `#[umbral(widget = "markdown")]` — form-renderer
                ...
                // `#[umbral(backend = "postgres")]` — restrict to
                ...
                // `#[umbral(slug_from = "title")]` — auto-derive at
...
/// a path token referring to `umbral::orm::FkAction::<Variant>`.
...
fn fk_action_tokens(
    ...
    let Some(raw) = value else {
        return Ok(quote!(::umbral::orm::FkAction::NoAction));
    };
    if !is_fk_field {
        return Err(syn::Error::new_spanned(
            field_ty,
            format!(
                "umbral: `{attr_name}` is only meaningful on `ForeignKey<T>` / \
    ...
    let variant_ident = match normalised.as_str() {
        ...
        other => {
            return Err(syn::Error::new_spanned(
                field_ty,
                format!(
                    "umbral: unknown `{attr_name}` value `{other}` — accepted: \
    ...
    Ok(quote!(::umbral::orm::FkAction::#variant))
}
...
/// e.g. "umbral: `unique_together` entries must be string literals."
...
fn parse_str_array(expr: &syn::Expr, context: &str) -> syn::Result<Vec<String>> {
    match expr {
        syn::Expr::Array(arr) => {
            ...
            for elem in &arr.elems {
                let syn::Expr::Lit(syn::ExprLit {
                    ...
                    return Err(syn::Error::new_spanned(
                        elem,
                        format!("umbral: `{context}` entries must be string literals"),
                    ));
                };
                out.push(lit.value());
            }
        ...
        _ => Err(syn::Error::new_spanned(
            expr,
            format!("umbral: `{context}` must be a string-literal array"),
        )),
    }
}

fn parse_umbral_struct_attr(attrs: &[syn::Attribute]) -> syn::Result<UmbralStructAttr> {
    let mut parsed = UmbralStructAttr {
        table: None,
        plugin: None,
        display: None,
        icon: None,
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident("table") {
                ...
                // `#[umbral(database = "analytics")]` — pin this model
                ...
                // `#[umbral(singleton)]` — single-row marker. Closes
                ...
                // `#[umbral(ordering = ["-published_at", "id"])]` —
...
fn expand_model(input: DeriveInput, mode: EmitMode) -> syn::Result<TokenStream2> {
    ...
    let fields = match &input.data {
        Data::Struct(data) => match &data.fields {
            ...
            _ => {
                return Err(syn::Error::new_spanned(
                    struct_name,
                    "umbral::Model can only be derived on structs with named fields",
                ));
            }
        },
        _ => {
    ...
    //   1. First-match: any field carrying `#[umbral(primary_key)]`.
    //      The explicit marker lets a model name its PK something
    //      domain-specific — e.g. `Permission` with
    //      `#[umbral(primary_key)] pub codename: String`.
    ...
    // here: any type implementing `umbral::orm::PrimaryKey` works. The
    ...
    let explicit_pk = fields
        .iter()
        .find(|f| match parse_umbral_field_attr(&f.attrs) {
    ...
    let table_name = if let Some(explicit) = struct_attr.table {
        ...
        // Tagging a model `#[umbral(plugin = "app")]` is a no-op for
    ...
    // `#[umbral(display = "...")]`. ICON defaults to `"database"`;
    // overridden by `#[umbral(icon = "...")]`.
    ...
    for field in fields.iter() {
        ...
        if let FieldKind::Many2Many(ref inner_ty) = kind {
            ...
            m2m_specs.push(quote! {
                ::umbral::orm::M2MRelationSpec {
                    ...
                    target_table: <#inner as ::umbral::orm::Model>::TABLE,
                    target_name: <#inner as ::umbral::orm::Model>::NAME,
                }
            });
            continue;
        }
        ...
        // `#[umbral(reverse_fk = "...")]` attribute is REQUIRED on
        ...
        if let FieldKind::ReverseSet(ref inner_ty) = kind {
            ...
            let Some(fk_col) = field_attr.reverse_fk.as_ref() else {
                let err = syn::Error::new_spanned(
                    field,
                    "ReverseSet<C> fields require `#[umbral(reverse_fk = \"<fk_column>\")]` \
            ...
            reverse_fk_specs.push(quote! {
                ::umbral::orm::ReverseFkRelationSpec {
                    field_name: #field_name_str,
                    target_table: <#inner as ::umbral::orm::Model>::TABLE,
                    target_name: <#inner as ::umbral::orm::Model>::NAME,
                    fk_column: #fk_col,
                    soft_delete: <#inner as ::umbral::orm::Model>::SOFT_DELETE,
                }
            });
                ...
                #field_name_str => {
                    ...
                    for row in rows {
                        if let ::core::result::Result::Ok(c) =
                            ::umbral::_serde_json::from_value::<#inner>(row)
                        {
                            decoded.push(c);
                        }
                    }
        ...
        //    equivalent to `#[umbral(unique)] pub <f>: ForeignKey<T>`.
            ...
            && has_sqlx_skip(&field.attrs)
        {
            ...
            one_to_one_specs.push(quote! {
                ::umbral::orm::OneToOneRelationSpec {
                    field_name: #field_name_str,
                    target_table: <#inner as ::umbral::orm::Model>::TABLE,
                    target_name: <#inner as ::umbral::orm::Model>::NAME,
                }
        ...
        // gaps2 #70 — text-backed Postgres types. `#[umbral(xml)]` /
        // `#[umbral(ltree)]` / `#[umbral(bit)]` upgrade a `String` /
        // `Option<String>` field from Text → the native PG column type.
        // No-op on any other kind. `#[umbral(macaddr)]` is an explicit
        ...
        // When `#[umbral(choices)]` is set, the field's Rust type is
        ...
        let (sql_ty_tokens, nullable_lit) = if is_choices_field {
            ...
            if is_option_type(&field.ty) {
                let err = syn::Error::new_spanned(
                    &field.ty,
                    "umbral: `#[umbral(choices)]` on `Option<T>` is deferred. \
            ...
            (quote!(::umbral::orm::SqlType::Text), quote!(false))
        } else {
        ...
        let fk_target_tokens = match &kind {
            FieldKind::ForeignKey(inner_ty) => {
                quote! { Some(<#inner_ty as ::umbral::orm::Model>::TABLE) }
            }
        ...
        let (choices_tokens, choice_labels_tokens) = if is_choices_field {
            let ty = &field.ty;
            (
                quote! { <#ty as ::umbral::orm::ChoiceField>::VALUES },
                quote! { <#ty as ::umbral::orm::ChoiceField>::LABELS },
            )
        } else if let FieldKind::MultiChoice(ref inner) = kind {
            let inner_ty = inner.as_ref();
            (
                quote! { <#inner_ty as ::umbral::orm::ChoiceField>::VALUES },
                quote! { <#inner_ty as ::umbral::orm::ChoiceField>::LABELS },
            )
        } else {
            (quote! { &[] }, quote! { &[] })
        };

        // `#[umbral(default = "...")]` lifts to a static-str default.
        ...
        // model author writing `#[umbral(widget = ...)]`. An explicit
        // `#[umbral(widget = "...")]` always wins — it's checked first.
    ...
    // (field ident, parent type, child FK column name). The column is the
    // reverse-O2O hop's driving column, captured in the child's derive.
    let mut reverse_o2o_entries: Vec<(syn::Ident, syn::Type, String)> = Vec::new();
    for field in fields.iter() {
        let field_name = field.ident.as_ref().unwrap();
        let field_name_str = field_name.to_string();
        ...
        match &kind {
            ...
            FieldKind::ForeignKey(inner_ty) => {
                if !field_attr.no_reverse {
                    ...
                    // still works (at most one row); the o2o variant returns
                    // a chainable `Relation<Child>` (Task 5) so a traversal
                    // can continue past the reverse O2O. Capture the child's
                    // FK column (honouring `#[sqlx(rename)]`) — the reverse
                    // hop's driving column, known here in the child's derive.
                    if field_attr.unique {
                        let o2o_fk_col =
                            sqlx_rename(&field.attrs).unwrap_or_else(|| field_name.to_string());
                        reverse_o2o_entries.push((
                            field_name.clone(),
                            (**inner_ty).clone(),
                            o2o_fk_col,
                        ));
                ...
                fk_id_arms.push(quote! {
                    #field_name_str => ::umbral::_serde_json::to_value(&self.#field_name.id()).ok(),
                });
            }
            FieldKind::NullableForeignKey(inner_ty) => {
                hydrate_arms.push(quote! {
                ...
                fk_id_arms.push(quote! {
                    #field_name_str => self.#field_name
                        .as_ref()
                        .and_then(|fk| ::umbral::_serde_json::to_value(&fk.id()).ok()),
                });
            }
            _ => {}
        }
        ...
        && one_to_one_parent_arms.is_empty()
    {
        ...
        let typed_pk_decl = if needs_typed_pk {
            quote! { let __pk = <Self as ::umbral::orm::Model>::primary_key(self); }
        } else {
            quote! {}
        };
        let json_pk_decl = if needs_json_pk {
            quote! {
                let __pk_json = ::umbral::_serde_json::to_value(
                    &<Self as ::umbral::orm::Model>::primary_key(self),
                )
                .unwrap_or(::umbral::_serde_json::Value::Null);
            }
        } else {
            quote! {}
        };
        ...
        quote! {
            fn write_pending_m2m<'a>(
                ...
                dyn ::std::future::Future<
                    Output = ::core::result::Result<(), ::umbral::orm::write::WriteError>,
                > + ::core::marker::Send + 'a,
            >> {
        ...
        .map(|(ident, child_ty)| {
            ...
            quote! {
                ...
                pub async fn #contains_any_fn(
                    parent_ids: &[<Self as ::umbral::orm::Model>::PrimaryKey],
                    child_pk: <#child_ty as ::umbral::orm::Model>::PrimaryKey,
                ) -> ::core::result::Result<bool, ::umbral::_sqlx::Error> {
                    ::umbral::orm::M2M::<#child_ty, <Self as ::umbral::orm::Model>::PrimaryKey>::any_holds(
                        Self::#junction_fn(),
                        parent_ids,
                        child_pk,
                    ).await
                ...
                pub async fn #union_for_fn(
                    ...
                    ::std::vec::Vec<<#child_ty as ::umbral::orm::Model>::PrimaryKey>,
                    ::umbral::_sqlx::Error,
                >
                where
                    <#child_ty as ::umbral::orm::Model>::PrimaryKey:
                        for<'r> ::umbral::_sqlx::Decode<'r, ::umbral::_sqlx::Sqlite>
                        + for<'r> ::umbral::_sqlx::Decode<'r, ::umbral::_sqlx::Postgres>
                        + ::umbral::_sqlx::Type<::umbral::_sqlx::Sqlite>
                        + ::umbral::_sqlx::Type<::umbral::_sqlx::Postgres>
                        + ::core::marker::Send
                        + ::core::marker::Unpin,
                {
                    ::umbral::orm::M2M::<#child_ty, <Self as ::umbral::orm::Model>::PrimaryKey>::holders_of_any(
                        Self::#junction_fn(),
                        parent_ids,
                    ).await
                }
    ...
    //  - The opt-out `#[umbral(no_reverse)]` still works: it skips
    ...
    // deeper traversal (`user.developer().software_groups()…`) AND still awaits
    // to a single child. This UNIFIES the reverse-O2O accessor with Task 5's
    // parent-side-`OneToOne` accessor: there is now exactly ONE `parent.child()`
    // method, no name collision, and it works zero-config (no parent-side field
    // required) for the framework's blessed O2O fixtures.
    //
    // Return-shape / backward-compat note: `Relation<T>` awaits (`IntoFuture`)
    // to `Result<T, sqlx::Error>` — the REQUIRED shape (a missing row is
    // `Err(RowNotFound)`), NOT `Result<Option<T>>`. A reverse O2O is nullable,
    // so callers that want the "absent → None" shape use the explicit
    // `.get_opt().await` terminal (same `sqlx::Error`); `.await` alone yields
    // the child or errors on absence. This is the single deliberate call-site
    // change from the pre-Task-5 `Future<Option<Child>>` accessor.
    //
    // Disambiguation is per-parent-type among the UNIQUE FKs only, separate
    // from the reverse-FK count (a child with `user: FK<U> + unique` AND
    // `manager: FK<U>` gets `customer()` for the o2o and `customer_via_user_set()`
    // + `customer_via_manager_set()` for the FK reverses).
    let mut o2o_parent_type_counts: std::collections::HashMap<String, usize> =
        std::collections::HashMap::new();
    for (_, parent_ty, _) in &reverse_o2o_entries {
        let key = quote!(#parent_ty).to_string();
        ...
        .map(|(field_ident, parent_ty, fk_col)| {
            ...
            let trait_doc = format!(
                "Reverse-OneToOne trait emitted by `#[derive(Model)]` for the \
                 ...
                 `parent.{}()` to get a chainable `Relation<{}>` for the \
                 (at-most-one) child whose `{}` FK points at the parent — await \
                 it for the child (`.await?`, erroring if absent) or use \
                 `.get_opt().await?` for `Option<{}>`, and continue the \
                 traversal with the child's own relation accessors. Trait-based \
                 emission (mirrors gap #105) sidesteps the orphan rule, so the \
                 accessor works even when the parent type is defined in another \
                 crate — the canonical case is a model in an app crate declaring \
                 `#[umbral(unique)] pub user: ForeignKey<AuthUser>`.",
                struct_name, field_ident, accessor_name, struct_name, field_ident, struct_name,
            );
            quote! {
                #[doc = #trait_doc]
                pub trait #trait_name {
                    fn #accessor_name(&self) -> ::umbral::orm::Relation<#struct_name>;
                }
                impl #trait_name for #parent_ty {
                    fn #accessor_name(&self) -> ::umbral::orm::Relation<#struct_name> {
                        ::umbral::orm::relation::to_one_hop(
                            self,
                            ::umbral::orm::relation::HopSpec {
    ...
    //   - forward FK / O2O child-side (a unique forward FK)
    //         → fn <field>(&self) -> Relation<Target>   (via `to_one_hop`)
    //   - M2M forward
    //         → fn <field>(&self) -> QuerySet<Target>   (via `to_many_hop`)
    //
    // The PARENT-side reverse O2O (`user.developer()`) is NOT emitted in this
    // trait: the upgraded reverse-O2O machinery above (`reverse_o2o_impls`)
    // owns that chainable accessor so it also covers zero-config / cross-crate
    // parents. Reverse-FK likewise keeps its separate `<child>_set()` accessor
    // (emitted above); neither is re-emitted here. The trait is LOCAL to the
    // defining crate and
    ...
    for field in fields.iter() {
        ...
        match &kind {
            ...
            // accessor is generated by the UPGRADED reverse-O2O machinery above
            // (`reverse_o2o_impls`), which returns the same `Relation<Child>`
            // and works zero-config even when the parent declares no back-link
            // field (the cross-crate case). Emitting a second `child()` here
            // would collide with it. `ReverseSet` keeps its existing
            // `<child>_set()`; flatten bases and scalar fields contribute no
            // forward relation accessor.
            _ => {}
        }
    }
    ...
    let output = quote! {
        ...
        impl ::umbral::orm::Model for #struct_name {
            ...
            const REVERSE_FK_RELATIONS: &'static [::umbral::orm::ReverseFkRelationSpec] = &[
                #(#reverse_fk_specs),*
            ];
        ...
        impl ::umbral::orm::HydrateRelated for #struct_name {
            fn fk_id_for(
                &self,
                field_name: &str,
            ) -> ::core::option::Option<::umbral::_serde_json::Value> {
                match field_name {
                    #(#fk_id_arms)*
                    _ => ::core::option::Option::None,
                }
            }
            fn hydrate_fk(&mut self, field_name: &str, row: &::umbral::_serde_json::Value) {
                match field_name {
                    #(#hydrate_arms)*
                    _ => {}
                }
            ...
            fn set_m2m_resolved_json(
                &mut self,
                field_name: &str,
                rows: ::std::vec::Vec<::umbral::_serde_json::Value>,
            ) {
            ...
            fn set_one_to_one_resolved_json(
                &mut self,
                field_name: &str,
                row: ::core::option::Option<::umbral::_serde_json::Value>,
            ) {
        ...
        impl #struct_name {
            pub fn objects() -> ::umbral::orm::Manager<Self> {
                ::umbral::orm::Manager::default()
            }
...
enum FieldKind {
    ...
    /// `#[umbral(reverse_fk = "<fk_col>")]` attribute names that
    ...
    /// back. Unlike ReverseSet, no `#[umbral(...)]` attribute is
    ...
    /// derive picks Inet by default; the `#[umbral(cidr)]` field-level
    ...
    MacAddr,
    NullableMacAddr,
    /// `String` declared as a Postgres `XML` column via `#[umbral(xml)]`.
    ...
    /// `#[umbral(ltree)]`. Text-backed. gaps2 #70.
    Ltree,
    NullableLtree,
    /// `String` declared as a Postgres `BIT VARYING` column via
    /// `#[umbral(bit)]`. Text-backed. gaps2 #70.
    Bit,
    NullableBit,
    /// `umbral::orm::TsVector` — Postgres full-text-search tsvector
    /// column (Phase 4.3).
    FullText,
    NullableFullText,
    ...
    Slug,
    /// `umbral::orm::Email` — TEXT with structural email validation.
    /// Closes BUG-12.
    Email,
    /// `umbral::orm::Url` — TEXT with `http(s)://...` validation.
    /// Closes BUG-13.
    Url,
    /// `umbral::orm::FileField` — TEXT storing a storage key. Storage is
    ...
    NullableFileField,
    /// `umbral::orm::ImageField` — a `FileField` with the default
    ...
    NullableImageField,
    /// `umbral::orm::Masked<String>` — an encrypt-at-rest TEXT column.
    ...
    fn sql_type_tokens(&self) -> Option<(TokenStream2, TokenStream2)> {
        let sql = match self {
            FieldKind::SmallInt | FieldKind::NullableSmallInt => {
                quote!(::umbral::orm::SqlType::SmallInt)
            }
            FieldKind::Integer | FieldKind::NullableInteger => {
                quote!(::umbral::orm::SqlType::Integer)
            }
            FieldKind::BigInt | FieldKind::NullableBigInt => quote!(::umbral::orm::SqlType::BigInt),
            FieldKind::Real | FieldKind::NullableReal => quote!(::umbral::orm::SqlType::Real),
            FieldKind::Double | FieldKind::NullableDouble => quote!(::umbral::orm::SqlType::Double),
            FieldKind::Bool | FieldKind::NullableBool => quote!(::umbral::orm::SqlType::Boolean),
            FieldKind::Str | FieldKind::NullableStr => quote!(::umbral::orm::SqlType::Text),
            ...
            FieldKind::Date | FieldKind::NullableDate => quote!(::umbral::orm::SqlType::Date),
            FieldKind::Time | FieldKind::NullableTime => quote!(::umbral::orm::SqlType::Time),
            ...
            FieldKind::Array(elem) | FieldKind::NullableArray(elem) => {
                ...
                quote!(::umbral::orm::SqlType::Array(#elem_tokens))
            }
            FieldKind::Inet | FieldKind::NullableInet => quote!(::umbral::orm::SqlType::Inet),
            FieldKind::Cidr | FieldKind::NullableCidr => quote!(::umbral::orm::SqlType::Cidr),
            FieldKind::MacAddr | FieldKind::NullableMacAddr => {
                quote!(::umbral::orm::SqlType::MacAddr)
            }
            FieldKind::Xml | FieldKind::NullableXml => quote!(::umbral::orm::SqlType::Xml),
            FieldKind::Ltree | FieldKind::NullableLtree => quote!(::umbral::orm::SqlType::Ltree),
            FieldKind::Bit | FieldKind::NullableBit => quote!(::umbral::orm::SqlType::Bit),
            FieldKind::FullText | FieldKind::NullableFullText => {
                quote!(::umbral::orm::SqlType::FullText)
            }
            FieldKind::ForeignKey(_) | FieldKind::NullableForeignKey(_) => {
                quote!(::umbral::orm::SqlType::ForeignKey)
            }
            FieldKind::MultiChoice(_) => quote!(::umbral::orm::SqlType::Text),
            FieldKind::Bytes | FieldKind::NullableBytes => quote!(::umbral::orm::SqlType::Bytes),
    ...
    fn error_message(&self) -> &'static str {
        match self {
            FieldKind::Unsupported(UnsupportedReason::WideOrUnsignedInt) => {
                "umbral M3 doesn't support 128-bit ints or u64 (no SQL backend handles \
            ...
            FieldKind::Unsupported(UnsupportedReason::NotInCatalogue) => {
                "umbral M3 doesn't yet support this field type; see \
...
fn classify_field_type(ty: &Type) -> FieldKind {
    ...
    // leaf ident so both `FileField` (after `use umbral::orm::FileField`)
    ...
    // `Some("image")` unless `#[umbral(widget = "...")]` overrides.
    ...
    // future `#[umbral(cidr)]` attribute or by writing the FieldSpec
    ...
    // Phase 4.3 — `umbral::orm::TsVector`. The qualifier is `orm`
    // (matching the umbral facade re-export). Bare `TsVector` won't
    ...
    // code write `ForeignKey<User>` after `use umbral::orm::ForeignKey`.
...
/// that writes `use umbral::orm::ForeignKey; field: ForeignKey<User>` works.
...
fn vec_element_kind(ty: &Type) -> Option<ArrayElementKind> {
    ...
    // ArrayElementKind enum and the `umbral::orm::ArrayElement`
...
fn array_element_tokens(kind: ArrayElementKind) -> TokenStream2 {
    match kind {
        ArrayElementKind::SmallInt => quote!(::umbral::orm::ArrayElement::SmallInt),
        ArrayElementKind::Integer => quote!(::umbral::orm::ArrayElement::Integer),
        ArrayElementKind::BigInt => quote!(::umbral::orm::ArrayElement::BigInt),
        ArrayElementKind::Real => quote!(::umbral::orm::ArrayElement::Real),
        ArrayElementKind::Double => quote!(::umbral::orm::ArrayElement::Double),
        ArrayElementKind::Boolean => quote!(::umbral::orm::ArrayElement::Boolean),
        ArrayElementKind::Text => quote!(::umbral::orm::ArrayElement::Text),
        ArrayElementKind::Uuid => quote!(::umbral::orm::ArrayElement::Uuid),
    }
}
...
/// (after `use umbral::orm::FileField`) and `orm::FileField` resolve.
...
/// `#[umbral(choices)] field: Option<T>` at derive time (v1 limitation).
...
/// Build the `pub const FOO: ::umbral::orm::column::FooCol<Self> =
...
// `to_snake_case` is imported from `umbral_casing` at the top of this file.
...
// `#[form(...)]` attrs into an `impl umbral::forms::Form`. The validate /
// render bodies call into the Field primitives from umbral-core::forms;
// the macro only does compile-time wiring.
// =========================================================================

/// Derive `umbral::forms::Form` on a struct of named fields.
///
/// Field-type dispatch:
///
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident("email") {
                ...
                Err(meta.error(
                    "umbral::Form derive accepts `required`, `optional`, \
    ...
    for attr in attrs {
        ...
        attr.parse_nested_meta(|meta| {
            if meta.path.is_ident("normalize_strings") {
                ...
                Err(meta.error("umbral::Form container-level attrs accept `normalize_strings`"))
            }
        })?;
    }
    Ok(out)
...
/// forms by default (treated as `#[umbral(noform)]`) because the generic
...
//      `::umbral_tasks::register_handler("name", wrapper)` where `wrapper`
...
/// Mark an `async fn` as an umbral background task.
///
...
/// `umbral_tasks::register_handler`. Call the companion at boot time from
...
///
/// ```ignore
/// #[umbral::task(name = "blog.send_welcome")]
    ...
    if !is_correct_return {
        return Err(syn::Error::new_spanned(
            ...
            "#[task] requires `-> Result<(), String>` as the return type (matches \
             the umbral-tasks handler contract)",
        ));
    }
                ...
                |payload_json: &str| {
                    ...
                    async move {
                        let payload: #payload_ty =
                            ::umbral_tasks::_serde_json::from_str(&owned)
...
fn expand_form(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let fields = match &input.data {
        Data::Struct(data) => match &data.fields {
            ...
            _ => {
                return Err(syn::Error::new_spanned(
                    struct_name,
                    "umbral::Form can only be derived on structs with named fields",
                ));
            }
        },
        _ => {
    ...
    for field in fields.iter() {
        ...
        // gaps2 #19 follow-up: read the Model-level `#[umbral(...)]`
        ...
        let model_attr = parse_umbral_field_attr(&field.attrs).unwrap_or_default();
        ...
        // type" error that forced users onto `#[umbral(noform)]`.
        if let Some((file_kind, is_option)) = classify_file_form_field(&field.ty) {
            ...
            let newtype = match file_kind {
                FileFormKind::File => quote!(::umbral::orm::FileField),
                FileFormKind::Image => quote!(::umbral::orm::ImageField),
            };
            let field_var = format_ident!("_{}_field", field_ident);
            let mut chain = quote! {
                ::umbral::forms::Field::file(#field_name)
            };
            if is_optional {
                chain = quote! { #chain.optional() };
            }
            field_builders.push(quote! {
                let #field_var: ::umbral::forms::Field = #chain;
            });
        ...
        let Some((kind, is_option)) = classify_form_field_type(&field.ty) else {
            return Err(syn::Error::new_spanned(
                &field.ty,
                "umbral::Form derive: unsupported field type for a user-\
        ...
        // emits a `let <ident>_field = ::umbral::forms::Field::<ctor>(name)
        ...
        let mut chain = quote! {
            ::umbral::forms::Field::#ctor(#field_name)
        };
...
fn expand_choices(input: DeriveInput) -> syn::Result<TokenStream2> {
    ...
    let variants = match &input.data {
        ...
        _ => {
            return Err(syn::Error::new_spanned(
                enum_name,
                "umbral::Choices can only be derived on enums",
            ));
        }
    };
                ...
                rename = match lit.value().as_str() {
                    ...
                    other => {
                        return Err(meta.error(format!(
                            "umbral::Choices: unknown `rename_all = \"{other}\"`. Known: \
    ...
    for v in variants {
        if !matches!(v.fields, Fields::Unit) {
            return Err(syn::Error::new_spanned(
                &v.ident,
                "umbral::Choices variants must be unit (no fields)",
            ));
        }
        ...
        for attr in &v.attrs {
            ...
            attr.parse_nested_meta(|meta| {
                if meta.path.is_ident("value") {
                    ...
                    Err(meta.error(
                        "umbral::Choices variant attr accepts `value = \"...\"` or `label = \"...\"`",
                    ))
                }
            })?;
        }
        ...
        for j in (i + 1)..values.len() {
            if values[i] == values[j] {
                return Err(syn::Error::new_spanned(
                    variant_idents[j],
                    format!(
                        "umbral::Choices: duplicate DB value `{}` (also used by `{}`). \
    ...
    Ok(quote! {
        impl ::umbral::orm::ChoiceField for #enum_name {
        ...
        impl ::core::fmt::Display for #enum_name {
            fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
                <Self as ::umbral::orm::ChoiceField>::as_str(self).fmt(f)
            }
        }
        ...
        impl ::core::str::FromStr for #enum_name {
            ...
            fn from_str(s: &str) -> ::core::result::Result<Self, Self::Err> {
                <Self as ::umbral::orm::ChoiceField>::from_str_ok(s)
        ...
        impl<'q> ::umbral::_sqlx::Encode<'q, ::umbral::_sqlx::Sqlite> for #enum_name {
            fn encode_by_ref(
                &self,
                buf: &mut <::umbral::_sqlx::Sqlite as ::umbral::_sqlx::Database>::ArgumentBuffer<'q>,
            ) -> ::core::result::Result<
                ::umbral::_sqlx::encode::IsNull,
                ::std::boxed::Box<dyn ::std::error::Error + ::core::marker::Send + ::core::marker::Sync>,
            > {
                <&str as ::umbral::_sqlx::Encode<'q, ::umbral::_sqlx::Sqlite>>::encode_by_ref(
                    &<Self as ::umbral::orm::ChoiceField>::as_str(self),
                    buf,
                )
            }
        }
        impl<'q> ::umbral::_sqlx::Encode<'q, ::umbral::_sqlx::Postgres> for #enum_name {
            fn encode_by_ref(
                &self,
                buf: &mut <::umbral::_sqlx::Postgres as ::umbral::_sqlx::Database>::ArgumentBuffer<'q>,
            ) -> ::core::result::Result<
                ::umbral::_sqlx::encode::IsNull,
                ::std::boxed::Box<dyn ::std::error::Error + ::core::marker::Send + ::core::marker::Sync>,
            > {
                <&str as ::umbral::_sqlx::Encode<'q, ::umbral::_sqlx::Postgres>>::encode_by_ref(
                    &<Self as ::umbral::orm::ChoiceField>::as_str(self),
                    buf,
                )
            }
        }
        impl<'r> ::umbral::_sqlx::Decode<'r, ::umbral::_sqlx::Sqlite> for #enum_name {
            fn decode(
                value: <::umbral::_sqlx::Sqlite as ::umbral::_sqlx::Database>::ValueRef<'r>,
            ) -> ::core::result::Result<
                Self,
                ::std::boxed::Box<dyn ::std::error::Error + ::core::marker::Send + ::core::marker::Sync>,
            > {
                let s = <&str as ::umbral::_sqlx::Decode<'r, ::umbral::_sqlx::Sqlite>>::decode(value)?;
                <Self as ::umbral::orm::ChoiceField>::from_str_ok(s)
                    .ok_or_else(|| ::std::format!("{}: `{}`", #invalid_msg, s).into())
            }
        }
        impl<'r> ::umbral::_sqlx::Decode<'r, ::umbral::_sqlx::Postgres> for #enum_name {
            fn decode(
                value: <::umbral::_sqlx::Postgres as ::umbral::_sqlx::Database>::ValueRef<'r>,
            ) -> ::core::result::Result<
                Self,
                ::std::boxed::Box<dyn ::std::error::Error + ::core::marker::Send + ::core::marker::Sync>,
            > {
                let s = <&str as ::umbral::_sqlx::Decode<'r, ::umbral::_sqlx::Postgres>>::decode(value)?;
                <Self as ::umbral::orm::ChoiceField>::from_str_ok(s)
                    .ok_or_else(|| ::std::format!("{}: `{}`", #invalid_msg, s).into())
            }
        }
