...
/// ├── plugins/
/// │   ├── .gitkeep          # local app plugins land here (umbra startapp)
/// │   └── README.md
/// └── templates/
///     ├── base.html
///     ├── home.html
...
//!     ../plugins/  — local app plugins (`umbra startapp <name>`)
//!
//! Run with:
...
//!
//! Every handler hands a context to `umbra::templates::render` and wraps
//! DB / template errors with the shared `internal_error` helper.

use umbra::prelude::*;
use umbra::templates::context;
use umbra::web::{Html, Json, StatusCode};
...
pub async fn home() -> Result<Html<String>, (StatusCode, String)> {
    ...
    let body = umbra::templates::render("home.html", &context!(post_count)).map_err(internal_error)?;
    Ok(Html(body))
}
...
pub async fn dashboard(
    user: umbra_auth::LoggedIn<umbra_auth::AuthUser>,
) -> Result<Html<String>, (StatusCode, String)> {
    // Demonstrates a transaction: fetch the user's post list atomically.
    let user_id = user.id;
    let my_posts = umbra::transaction(|tx| {
        Box::pin(async move {
            Post::objects()
    ...
    let body =
        umbra::templates::render("dashboard.html", &context!(user, my_posts)).map_err(internal_error)?;
    Ok(Html(body))
}
"#;
...
"#;
    ...
    // ------------------------------------------------------------------ //
    let seed_credentials_rs = r#"//! First-run convenience: mints a deterministic dev superuser
//! ("admin" / "admin") when no users exist yet, printing the
//! credentials to stderr so you can log in to /admin/ without leaving
//! the terminal. Idempotent — subsequent boots find the user and stay
//! quiet.
//!
//! NEVER ship this in production. It's a dev scaffold; production would
//! call `createsuperuser` interactively.
...
pub async fn test_credentials() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    umbra_auth::create_superuser("admin", "admin@example.com", "admin")
        .await
        .map_err(|e| -> Box<dyn std::error::Error + Send + Sync> { Box::new(e) })?;

    eprintln!();
    eprintln!("======================================================================");
    eprintln!(" DEV CREDENTIALS — seeded because no users existed yet");
    eprintln!("----------------------------------------------------------------------");
    eprintln!(" Username : admin");
    eprintln!(" Password : admin");
    eprintln!(" Log in   : http://127.0.0.1:8000/admin/");
    eprintln!(" Change or remove this in src/seed/credentials.rs before shipping.");
    eprintln!("======================================================================");
    eprintln!();
...
/// the Django-app-style per-concern layout (gaps2 #8):
///
/// ```text
/// plugins/<name>/
/// ├── Cargo.toml
...
///     └── urls.rs    — the URL conf (`router()`), like Django's urls.py
/// ```
///
...
//!
//! A Django-app-style plugin, split one file per concern:
//!
//!   src/
//!     lib.rs     — the `Plugin` impl: glues models + routes together (this file)
//!     models.rs  — `#[derive(Model)]` structs (this app's tables)
//!     views.rs   — HTTP handlers
//!     urls.rs    — the URL conf: maps paths to `views::` handlers (Django's urls.py)
//!
...
"#
    ...
    // src/urls.rs — the plugin's URL conf (Django's urls.py). One place
    // that maps every path to a `views::` handler.
    let urls_rs = format!(
        r#"//! URL conf for the `{name}` plugin — the route table, like Django's
//! `urls.py`. `router()` returns the axum `Router` that
...
mod tests {
    ...
    fn scaffold_project_creates_empty_plugins_dir() {
        ...
        assert!(
            readme.contains("umbra startapp"),
            "plugins/README.md should point at `umbra startapp`",
        );
    }
