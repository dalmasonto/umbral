//! Smoke test for `umbra_cli::dispatch` — proves that argv routes
//! through plugin-contributed `PluginCommand` impls before the
//! built-in `serve` / `migrate` / etc. clap parser ever sees argv.
//!
//! This was a real shipped bug: gap 3's `umbra_cli::dispatch(app)`
...
async fn dispatch_routes_argv_to_plugin_contributed_commands() {
    ...
    // 3. Dispatch. If `umbra_cli::dispatch_with_argv` routes through
    //    `umbra_core::cli::dispatch` (the fix), the plugin's command
    ...
    let result = umbra_cli::dispatch_with_argv(app, argv).await;
    ...
    assert!(
        ...
        "SmokeCommand::run did not fire - dispatch reached the built-in subcommand parser without consulting plugins. The wire between umbra-cli and umbra-core's cli::dispatch is broken.",
    );
}
