...
// =========================================================================
// boot — one app per test process, without the OnceCell dance (feature #79)
// =========================================================================

/// Boot a throwaway app for a test binary: in-memory SQLite, your models and
/// plugins, and a schema derived from those models.
///
...
/// ```
///
/// The schema is created by [`create_tables`], so it is the models' schema — no
...
///
/// [`AppBuilder`]: umbral::AppBuilder
pub async fn boot<F>(configure: F)
where
    F: FnOnce(umbral::AppBuilder) -> umbral::AppBuilder,
{
    static BOOTED: tokio::sync::OnceCell<()> = tokio::sync::OnceCell::const_new();
    BOOTED
        .get_or_init(|| async {
            let pool = umbral::db::connect_sqlite("sqlite::memory:")
                .await
                .expect("umbral-testing: in-memory sqlite pool");
            let mut settings =
                umbral::Settings::from_env().expect("umbral-testing: settings from env");
            settings.database_url = "sqlite::memory:".to_string();

            let builder = umbral::App::builder()
                .settings(settings)
                .database("default", pool);

            configure(builder)
                .build()
                .expect("umbral-testing: App::build");

            create_tables()
                .await
                .expect("umbral-testing: create the test schema");
        })
        .await;
}
