...
//! umbra facade. The macro resolves each type to its
...
//!   - Renaming a model's `#[umbra(table = "...")]` propagates
...
#[derive(Debug, Clone, sqlx::FromRow, Serialize, Deserialize, umbra::orm::Model)]
#[umbra(table = "orders")] // <-- renamed; macro must follow
struct Order {
    id: i64,
    note: String,
}
...
fn renamed_table_is_picked_up_automatically() {
    // `Order` carries `#[umbra(table = "orders")]` — if the macro
