...
The umbral framework's Cargo workspace.

## What's in here

| Crate | Purpose |
|---|---|
| `umbral-core/` | Internals: ORM, migrations, routing, DB backends, the `Plugin` trait. Don't depend on this crate directly. |
| `umbral-macros/` | Proc macros: `#[derive(Model)]`, `#[task]`, etc. Don't depend on this crate directly. |
| `umbral/` | The **facade**. The single stable surface for user code and plugin authors. `use umbral::prelude::*;` brings in everything a typical handler / model / plugin needs. |
| `umbral-cli/` | The `manage.py` equivalent binary (`migrate`, `makemigrations`, `inspectdb`, `worker`, ...). |
...
Arrows point inward. Plugins depend on the facade; the facade depends on the internals. Nothing in `umbral-core` ever depends on a plugin or the facade. Cargo's ban on circular crate deps is what enforces this. See `arch.md §1` for the architectural rationale.
...
cargo test -p umbral-core         # test one crate
cargo test -p umbral-core --test fulltext_field   # one test binary
cargo run -p umbral-cli -- <cmd>  # run the manage.py equivalent
...
Most tests run on in-memory SQLite and need nothing. A handful exercise Postgres-only behaviour (full-text search, native `uuid` relations, array/JSON/network types, PG backup) and are gated `#[ignore]` so they self-skip when no server is configured. To run them, point `UMBRAL_TEST_POSTGRES_URL` at a database and pass `--include-ignored`:

```bash
export UMBRAL_TEST_POSTGRES_URL="postgres://user:pass@localhost/umbral_test"

# all of a crate's tests, including the Postgres-gated ones
cargo test -p umbral-core -- --include-ignored

# just the full-text-search suite against Postgres
cargo test -p umbral-core --test fulltext_field -- --include-ignored
```
...
- **Path deps between members are relative.** Sibling crates reference each other via `umbral-core = { path = "../umbral-core", version = "0.0.1" }`. Moving the workspace under `crates/` preserved these (the crates are still siblings).
