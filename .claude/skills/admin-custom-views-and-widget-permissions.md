...
2. **Registration** (`src/lib.rs`) — `AdminPlugin.custom_views: Vec<AdminView>`; `.view()/.views()` builders. In `routes()`, each view's section widgets are **flattened into the same flat `catalog: Vec<Widget>`** that backs `AdminState.widget_catalog`, so the existing `GET {base}/api/dashboard/widgets/{key}/data` endpoint serves them with zero new plumbing. Dup widget keys → `tracing::warn!` (keys are global). Each view mounts `GET {base}/{path}` via a closure that clones the view's slug per-invocation (so the handler stays `Clone`). axum's static-over-`{param}` precedence means a static view path wins over the `{table}/` changelist route automatically.
