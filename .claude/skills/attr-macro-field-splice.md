...
- **Multi-base is hard.** Nesting `__umbral_base_fields_B!{...}` inside A's argument does NOT pre-expand (macro expansion isn't eager), so A's matcher can't see B's fields. Needs continuation-passing macro_rules. Ship single-base first.
