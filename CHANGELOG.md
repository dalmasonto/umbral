...
- **Bring-your-own email sender (`EmailBackend` trait, features #91).** `umbral-email` now exposes a public `EmailBackend` trait (`#[async_trait] async fn deliver(&self, &EmailMessage) -> Result<(), EmailError>`) and `EmailPlugin::with_backend(my_backend)`, so an app can send through its own transport — a provider SDK (Resend/Postmark/SES), an in-house relay, or a test spy — with just a struct, one trait impl, and one builder call, no fork. `deliver` receives the fully-resolved, header-injection-validated `EmailMessage`; report failures with the new `EmailError::Backend(String)`. A registered backend overrides the settings-driven Console/SMTP/API dispatch, but a bare app still gets Console-in-dev / SMTP-in-prod with zero code. `async-trait` is now a normal (no longer `auth`-gated) dependency. See `plugins/email` → "Bring your own sender".
...
- **Admin changelist columns are drag-resizable (gaps4 #61, pass 7).** Each column header has a grab strip on its right edge; drag to resize, and the width persists per-table in `localStorage` (and survives search/sort/pagination swaps). Implemented by pinning the column via `min/max-width` on its cells under the table's auto layout — no `table-layout: fixed` — so it coexists with column-hide, the density toggle, and the sticky Actions column; a value wider than the column ellipsizes. Also softened the ⌘K command-palette section labels (Records/Models/Commands) to sentence case to match the rest of the admin.

- **Admin dashboard labels match the rest of the admin (gaps4 #61, pass 6).** The KPI/stat/widget eyebrow labels dropped their all-caps `uppercase tracking-wider` treatment for the same sentence-case labels passes 3–5 use, so the dashboard reads as one system with the changelist and forms. Avatar initials stay uppercase (correct there). The dashboard was already tokenized; the two remaining hex values are legitimate — ApexCharts up/down trend colors (green/red by design, theme-independent) and a `var(--primary, …)` fallback — and were left as-is.

- **Admin legacy full-page form/detail tokenized; form widgets de-duplicated (gaps4 #61, pass 5).** The full-page `form.html` and `detail.html` (reached by direct-URL navigation / the non-HTMX fallback) were still on raw inline `style=` + hardcoded hex colors, so they rendered nearly unstyled under the dev Tailwind CDN. Both are now on the umbra-dusk tokens with the same card/breadcrumb chrome as the sheet. `form.html` no longer inlines its own copy of every field widget — it renders through the **shared `field_editor` macro** (the one the slide-over sheet uses), so both surfaces share every tokenized, accessible widget and future field work lands in both at once (~110 lines of duplicated raw-hex markup deleted). The shared `_macros/inlines.html` formset (used by both surfaces) was tokenized too — every `inline-…` field name, the `.inline-group` class, and the `data-*` hooks are unchanged. The macro's per-field error element gained a stable `field-error` hook. Admin templates only. Full suite green.
