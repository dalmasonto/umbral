...
async fn home() -> Result<Html<String>, (StatusCode, String)> {
    ...
    // `umbra_migrations` tracking table; if the framework doesn't
...
/// with `umbra-migrate`. Reading the count without depending on a
...
pub struct PluginRow {
    ...
    pub notes: i64,
    /// True for Umbra- or third-party-reviewed plugins — drives the
...
impl From<pd::Plugin> for PluginRow {
    fn from(p: pd::Plugin) -> Self {
        ...
        let install = p
            ...
            .find(|l| !l.is_empty())
            .map(str::to_string)
            .unwrap_or_else(|| format!("umbra add {}", p.crate_name));
        Self {
            id: p.id,
            status,
            stars: p
            ...
            audited: matches!(
                p.audit_status,
                pd::AuditStatus::UmbraReviewed | pd::AuditStatus::ThirdPartyReviewed
            ),
            install,
