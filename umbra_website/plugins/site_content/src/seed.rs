...
//!
//! Five lengthy, markdown-authored posts about Umbra so the blog renders
...
const POSTS: &[Seed] = &[
    Seed {
        slug: "why-umbra-exists",
        title: "Why Umbra exists",
        excerpt: "Rust has fast routers and solid ORMs. What it didn't have was the Django feeling — declare your data and get migrations, an admin, forms, and a REST API almost for free. Umbra is that feeling, rebuilt on Rust's guarantees.",
...
Umbra (Latin for *shadow* — it lives in Django's shadow in shape, not in code) is a deliberate attempt to recreate that feeling on top of Rust's compile-time guarantees.

![The Umbra plugin directory, built with Umbra](/media/91b8f829-2e2e-4ecb-8c53-4e2fec4a7028-adem_preview2-720x405.jpg)
...
Umbra is pre-alpha, and honest about it. But the shape is settled, and the everyday loop already works. That's why this whole website is built with it.
"#,
    },
    Seed {
        slug: "building-the-umbra-website-with-umbra",
        title: "Building the Umbra website with Umbra",
        excerpt: "The best proof that a framework can build a real, content-heavy site is to build one. This site — directory, reviews, discussions, admin, forms — is an Umbra project using the same apps, migrations, and plugins it documents.",
        kind: BlogPostKind::Community,
        reading_minutes: 8,
        featured: true,
        body: r#"There's a temptation, when you build a web framework, to ship a thin marketing page and call it a day. We did the opposite: **this website is a full Umbra project**, and the important sections prove the framework rather than describe it.

## Apps, not a monolith

The site is scaffolded with `umbra startapp` into focused plugin crates, each owning its own models and migrations:

```text
umbra_website/plugins/
  ...
  showcase/          # sites built with Umbra
...
Contact, plugin submission, plugin reports, plugin comments, and reviews all use Umbra Forms: server-side validation, friendly errors, and a clear moderation state where it matters. Submitted content doesn't appear publicly until it's approved.
...
"#,
    ...
    Seed {
        ...
        reading_minutes: 9,
        featured: false,
        body: r#"Migrations are where a lot of Rust web projects quietly fall back to hand-written SQL. Umbra treats the managed loop as the product, not an add-on.
...
```text
umbra makemigrations: rename detected (column-shape match): `body` → `content`
```
...
"#,
    ...
    Seed {
        ...
        reading_minutes: 8,
        featured: true,
        body: r#"Umbra's strongest public argument is also its simplest internal rule: **the batteries are real plugins.** The official auth, sessions, admin, tasks, and REST implement the exact same `Plugin` trait a community developer would.

## Dependency inversion is the whole game

> Dependencies point inward toward core. Control flows outward through the trait.

- Every plugin depends on the `umbra` facade, never the reverse.
- `umbra-core` defines the `Plugin` trait but never names a concrete plugin — it touches plugins only as `Box<dyn Plugin>`.
- `umbra-core` depends on neither `umbra-rest` nor `umbra-openapi`. That's the structural proof that "serializers are a plugin." Cargo's ban on circular dependencies enforces it for us.
...
"#,
    ...
    Seed {
        ...
        reading_minutes: 7,
        featured: false,
        body: r#"The payoff of declaring your data once is that several systems can read that declaration. In Umbra, the same model struct can drive a validated form, an admin CRUD screen, and a REST resource — each one an opt-in plugin.

## One struct, three surfaces

```rust
#[derive(Debug, Clone, Default, sqlx::FromRow, Serialize, Deserialize, Model, umbra::forms::Form)]
#[umbra(plugin = "directory", display = "Plugins", icon = "package")]
pub struct Plugin {
    pub id: i64,
...
The `Form` derive turns the `#[form(...)]` attributes into server-side validation with friendly errors. Fields marked `#[umbra(noform)]` never appear on the public form — a visitor can't set their own moderation status.
...
Register the model and the admin gives you a CRUD UI — list, filters, create, edit — with the `#[umbra(noform)]` fields editable by staff. The dashboard can add widgets that read straight from the model.
