//! Official Umbral website.
//!
//! The website intentionally dogfoods Umbral's project/app shape:
...
use umbral::web::{Html, SlashRedirect, StatusCode};
...
use umbral_oauth::providers::{GitHubProvider, GoogleProvider};
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // (from `UMBRAL_OAUTH_<PROVIDER>_CLIENT_ID` / `_CLIENT_SECRET`).
    ...
    let app = App::builder()
        ...
        // cache expensive data/fragments via `umbral_cache::ambient()`.
        ...
        // origin — override it in prod with UMBRAL_OAUTH_REDIRECT_BASE.
        .plugin({
        ...
        .plugin(PlaygroundPlugin::new("Umbral").at("/api/playground/"))
        ...
        .plugin(
            AdminPlugin::default()
                ...
                .dashboard_section(
                    umbral_admin::WidgetSection::new("Directory overview")
                ...
                .dashboard_section(
                    umbral_admin::WidgetSection::new("Composition")
                ...
                .dashboard_section(
                    umbral_admin::WidgetSection::new("Recent activity")
...
async fn dashboard(
    _user: umbral_auth::LoggedIn<AuthUser>,
    ...
    // ambient context.
    let body = umbral::templates::render("dashboard.html", &context! {}).map_err(internal_error)?;
    Ok(Html(body))
}
...
async fn notify_admins(kind: &'static str, title: &'static str, url: &'static str) {
    ...
    let ids: Vec<i64> = match AuthUser::objects()
        .filter(umbral_auth::auth_user::IS_SUPERUSER.eq(true))
        .filter(umbral_auth::auth_user::IS_ACTIVE.eq(true))
        .fetch()
        .await
    {
