...
const CATALOG: &[Category] = &[
    Category {
        ...
        features: &[
            ...
            Feat { name: "Managed migrations", summary: "makemigrations diffs models — including unique_together and composite indexes — and migrate applies reversible operations, backfilling NOT NULL tightenings and refusing destructive drops unless you pass --allow-destructive.", status: SH, maturity: STA },
            Feat { name: "Concurrent-safe migrations", summary: "A Postgres advisory lock serializes migrators, so several replicas deploying at once never race the same DDL — one applies, the rest wait then skip.", status: SH, maturity: STA },
            Feat { name: "Multi-database transactions", summary: "begin_for(alias) / transaction_on(alias) run a transaction against a replica or tenant pool, so on_tx writes for a routed model land in the right database — not silently in the default one.", status: SH, maturity: STA },
            Feat { name: "Soft deletes & signals", summary: "deleted_at auto-filtering and pre/post save/delete hooks; mark a field #[umbral(signal_skip)] to keep secrets and PII out of signal payloads.", status: SH, maturity: STA },
        ],
    },
    Category {
        name: "Web & Templates",
        ...
        features: &[
            ...
            Feat { name: "File & image fields", summary: "Multipart upload to a pluggable Storage backend, with a bounded concurrency gate on background processing.", status: SH, maturity: STA },
            Feat { name: "Protected file serving", summary: "Gate media downloads behind an async access-control callback — check a session or ownership before a byte is served, so private uploads aren't world-readable by URL.", status: SH, maturity: STA },
    ...
    Category {
        ...
        features: &[
            Feat { name: "Users & permissions", summary: "Argon2 hashing (behind a concurrency gate that sheds load under a login flood), groups, RBAC, per-object checks.", status: SH, maturity: STA },
            Feat { name: "Sessions", summary: "Server-side session store + middleware, with a configurable SameSite policy and an absolute max-age cap alongside sliding expiry.", status: SH, maturity: STA },
            Feat { name: "Trusted-proxy client IP", summary: "Declare how many reverse proxies sit in front and umbral resolves the real client IP from X-Forwarded-For, so throttles and logs key on the caller — not a forgeable header.", status: SH, maturity: STA },
    ...
    Category {
        ...
        features: &[
            Feat { name: "Auto REST", summary: "Serializers, viewsets, pagination, filtering, auth gates — safe-by-default (writes 403 without an explicit permission).", status: SH, maturity: BETA },
            Feat { name: "Object-level scoping", summary: "Scope every list and detail endpoint to the rows a caller may see with .scope() / .owned_by(), so a REST API can't leak another user's records.", status: SH, maturity: BETA },
    ...
    Category {
        ...
        features: &[
            Feat { name: "Task queue", summary: "#[task] jobs drained by a worker, with retries; the claim query uses FOR UPDATE SKIP LOCKED on Postgres so many workers each grab a different job.", status: EX, maturity: ALPHA },
