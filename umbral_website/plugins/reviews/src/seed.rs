...
const REVIEWS: &[Seed] = &[
    Seed { rating: 5, title: "Familiar workflow, Rust guarantees", body: "I declared my models, ran makemigrations and migrate, and had an admin and a REST API in an afternoon. It feels effortless, and the compiler catches the mistakes I used to find in production.", role: "Staff Engineer", company: "Fintech startup", context: ReviewUsageContext::WorkProject, featured: true },
...
pub async fn seed() -> Result<usize, Box<dyn std::error::Error + Send + Sync>> {
    ...
    for s in REVIEWS {
        ...
        r.umbral_version = Some("0.0.1".to_string());
