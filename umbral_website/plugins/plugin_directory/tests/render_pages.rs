...
//! umbral-admin's `ensure_tables_for_tests`). Every row-level read /
...
use umbral::forms::ValidationErrors;
...
async fn seed() {
    ...
    rest.name = "Umbral REST".to_string();
    rest.slug = "umbral-rest".to_string();
    rest.crate_name = "umbral-rest".to_string();
    rest.author = "Umbral contributors".to_string();
    rest.short_description = "serializers, viewsets, routers".to_string();
    rest.full_content =
        "## Build APIs\n\nSerializers and viewsets the familiar way. `umbral add umbral-rest`."
            .to_string();
    rest.installation_commands = "umbral add umbral-rest".to_string();
    rest.status = PluginStatus::Usable;
    rest.maturity = PluginMaturity::Beta;
    rest.audit_status = AuditStatus::UmbralReviewed;
    ...
    rest.docs_url = Some("https://umbral.dev/docs/rest".to_string());
    rest.source_url = Some("https://github.com/umbral/umbral-rest".to_string());
    rest.issue_tracker_url = Some("https://github.com/umbral/umbral-rest/issues".to_string());
    ...
    tenancy.name = "Umbral Multitenancy".to_string();
    tenancy.slug = "umbral-multitenancy".to_string();
    tenancy.crate_name = "umbral-multitenancy".to_string();
    tenancy.author = "@kanto".to_string();
    tenancy.short_description = "schema-per-tenant scoping".to_string();
    tenancy.full_content = "Row- and schema-level tenancy for Umbral apps.".to_string();
    tenancy.installation_commands = "umbral add umbral-multitenancy".to_string();
    ...
    let compat = PluginCompatibility {
        id: 0,
        plugin: ForeignKey::new(rest.id),
        umbral_version: "0.0.1".to_string(),
...
async fn listing_and_detail_render_real_db_rows() {
    ...
    assert!(community.contains("Umbral Multitenancy"));
    assert!(
        !community.contains("Umbral REST"),
    ...
    assert!(
        searched.contains("Umbral Multitenancy"),
        "?search=multitenancy matches the multitenancy plugin"
    );
    assert!(
        !searched.contains("Umbral REST"),
    ...
    // --- Detail -----------------------------------------------------------
    let detail = render_detail("umbral-rest")
        .await
        .expect("detail renders")
        .expect("plugin exists");
    assert!(
        detail.contains("Umbral REST"),
    ...
    assert!(
        detail.contains("umbral.dev&#x2f;docs&#x2f;rest"),
    ...
    assert!(
        detail.contains("/login?next=/plugins/umbral-rest"),
        "anonymous detail render shows the log-in-to-post CTA"
    );
    assert!(
        !detail.contains("action=\"/plugins/umbral-rest/notes\""),
    ...
    let rest_row = Plugin::objects()
        .filter(plugin_directory::models::plugin::SLUG.eq("umbral-rest"))
        .first()
        .await
        .expect("query rest")
        .expect("rest exists");
    ...
    let after = render_detail_with("umbral-rest", true)
        .await
    ...
    // --- Search fragment --------------------------------------------------
    // A query matching the seeded official plugin (name "Umbral REST",
    // crate "umbral-rest") returns it as a `.pd-search-result` link to its
    ...
    assert!(
        ...
        hits.contains("href=\"&#x2f;plugins&#x2f;umbral-rest\""),
        "matching plugin is a search-result link to its slug"
    );
    assert!(
        hits.contains("<span class=\"pd-search-name\">Umbral REST</span>"),
        "the matching plugin's name renders inside the result"
    );
    assert!(
        !hits.contains("Umbral Multitenancy"),
    ...
    assert!(
        report.contains("Umbral REST"),
        "the resolved plugin name renders in the report form"
    );
    assert!(
        report.contains("value=\"umbral-rest\""),
    ...
    let err = create_report(Some("umbral-rest"), "bug", "   ")
        .await
    ...
    assert!(
        resubmit.contains("value=\"umbral-x\""),
    ...
    // (1 parents + 1 children query). The seeded official "Umbral REST" and
    ...
    assert!(
        prebuilt.contains("/plugins/umbral-rest"),
        "the card links to the plugin detail page"
    );
    assert!(
        !prebuilt.contains("Umbral Multitenancy"),
...
async fn create_note_threads_replies_under_a_visible_top_level_note() {
    ...
    let note = create_note("umbral-rest", "Parent note body.", "general", None, None)
        .await
...
async fn detail_page_nests_replies_under_their_note() {
    ...
    let note = create_note("umbral-rest", "Top note here.", "general", None, None)
        .await
        .expect("ok")
        .expect("payload");
    create_note(
