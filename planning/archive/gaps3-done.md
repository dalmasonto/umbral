...
53. [x] **Soft delete does not cascade.** `#[umbral(soft_delete)]` is otherwise complete (core auto-filter, admin trash UI with restore/purge, REST), but `on_delete = "cascade"` is a **DDL-level FK clause** — it fires on a real SQL `DELETE`, and a soft delete is an `UPDATE`, so a soft-deleted parent leaves its children as live rows. Reads that traverse *from* the parent do hide them (`crates/umbral-core/src/orm/queryset/hydration.rs:741,869`), which masks the problem: the children are invisible via the parent but still returned by their own queries, still counted, still unique-constrained. Kikosi #7 explicitly asked for "cascade-aware". This is a silent data-correctness footgun and the highest-value item left in #7.

**SHIPPED (2026-07-12).** `crates/umbral-core/src/orm/soft_delete_cascade.rs`. Soft-deleting a parent now soft-deletes every `on_delete = "cascade"` descendant, recursively, and restoring it brings back exactly those rows. Wired into BOTH soft-delete paths — the typed `QuerySet` and `DynQuerySet` (which is what admin and REST run on, so scoping only the typed one would have left the admin surface still leaking orphans).

Two ideas carry the implementation. **Subqueries, not primary keys**: children are selected with `child.fk IN (SELECT pk FROM parent WHERE …)`, so no PK marshalling (i64 / String / Uuid all just work) and no IN-list size limit. **One timestamp identifies one cascade**: every row a cascade touches gets the same `deleted_at`, and restore clears only descendants carrying the parent's timestamp — so a child trashed independently last week stays trashed. Without that, restore would resurrect rows the user deliberately deleted, trading an orphan bug for an unrecoverable-delete bug.
...
81. [x] `startcommand` — scaffold a management command, ask where it lives, register it

**Finding.** We need a `startcommand` command - This is to help start a command, it should illustrate how to add args, named kwargs etc. It should ask the name of the command, next it shall ask where to place it either at root or at a specific plugin. The plugin names should be listed so the user can select from them so something like: root, plugin1, plugin2, etc. They shall be able to select, next after that, proceed to show them where to place the command so that it can be registered or even better, can they be auto detected?

**Shipped.** `umbral startcommand [NAME] [--in root|<plugin>]`. Interactive by default (asks the name, then lists `root` + every plugin under `plugins/` as a numbered menu); fully flag-driven when stdin is not a TTY, so CI does not hang on a prompt that can never be answered.

**The contract that had to be built first.** "Place it at root" was not implementable. `umbral_core::cli::dispatch` only ever walked `Plugin::commands()`, so a command belonging to the *binary* — a backfill, a one-off import — could only reach argv by inventing a dummy plugin to carry it. That is the plugin trait being used as a doorway rather than as a packaging unit, and per "fix, don't patch" the scaffolder could not paper over it. So:

- `AppBuilder::command(impl PluginCommand)` and `AppBuilder::commands(Vec<Box<dyn PluginCommand>>)`, plus `App::commands()` mirroring `App::plugins()`.
- `cli::dispatch_with_app_commands` / `cli::command_catalog_with_app_commands`, both routed through one `collect_commands` so the help listing cannot claim a command that would not actually run. App commands are collected first: on a name clash the project's own wins (most specific layer) and the losing plugin is named in a warning.
- `dispatch_with_argv`'s `if !app.plugins().is_empty()` short-circuit became `|| !app.commands().is_empty()` — without that, the plugin-free project (exactly what `--in root` produces) would report "unknown command" for a command sitting in its own `main.rs`.
- **`umbral::cli::clap` re-export.** `PluginCommand::command()` returns a `clap::Command`, so the trait's public signature names a foreign crate while the facade handed out no way to reach it. Every implementor was declaring its own `clap = "4"` and betting on resolving the same major version as the dispatcher; a scaffolded project has no clap dependency at all. A trait that names a foreign type has to hand that type out.

**Generated layout.** Both targets get the same shape — `commands/<name>.rs` beside a `commands/mod.rs` whose `all()` is the registry — and the registry is wired into whatever owns it: `main.rs` gets `mod commands;` + `.commands(commands::all())` inserted before the terminal `.build()`; a plugin's `lib.rs` gets `pub mod commands;` + a `Plugin::commands()` impl returning `commands::all()`. The command file itself is the illustration the gap asked for: a required positional, a typed named option with a default (`--limit 25`), a repeatable one (`--tag a --tag b`), and a boolean flag (`--dry-run`), each read back out of `ArgMatches`, with a commented ORM block showing that the app is fully built by the time `run()` fires.

**On "can they be auto detected?"** Not truly — Rust has no runtime module reflection, so nothing can walk `commands/` at startup and find the structs in it. The honest equivalents are a build script or a linker-section registry, both of which hide the list somewhere unreadable. `all()` IS the auto-detection: the tool maintains it (marker comments give it deterministic insertion points), and it stays a plain function you can read, reorder, or comment a line out of. The second `startcommand` appends to it and touches `main.rs` zero more times. Delete the markers and the tool refuses to guess — it writes the command and prints the two lines to add.

**Guardrails.** A command name that is already a built-in (`migrate`, `serve`, …) or a built-in plugin's (`createsuperuser`, `tasks-worker`, …) is rejected at scaffold time, because dispatch tries app/plugin commands *before* the built-in clap parser: a command named `migrate` would not collide loudly, it would silently take over and migrations would stop applying. The framework half of that reserved set is read off the derived clap parser rather than hand-listed, so a new subcommand reserves its own name with no second list to remember.

**Verified end to end**, not just asserted as strings: a `startproject`d demo with a root command and a plugin command `cargo check`s clean, both run (`backfill_slugs: slug=hello-world limit=3 tags=["a", "b"] dry_run=true`), and both list in `umbral help` next to `migrate`. The integration tests are written in the exact shape the generator emits, so a template that stops compiling takes the test suite with it.
