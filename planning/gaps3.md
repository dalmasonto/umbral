...
71. [ ] **`Masked<T>` is invisible to `ModelMeta`, so no plugin can automatically refuse to serialize it** — `Masked<T>` (field-level encryption, `crates/umbral-core/src/orm/masked.rs`) is exactly the kind of thing that should never cross the wire, and `Column` carries **no marker** distinguishing it from a plain `Text` column. So #70's core denylist cannot cover it: the denylist matches on column *name*, and a `Masked` field can be called anything.

    Needs a `Column.masked: bool` (or a `SqlType` variant) propagated from `FieldSpec`, after which `is_hard_denied_field` grows into a `is_secret(meta, col)` that denies masked columns on every transport by construction, whatever they are named.

    This is the narrow slice of the larger design discussed with the user: a **two-tier** field policy on the model — `#[umbral(private)]` (hidden by default, unlockable at the call site by an explicit `.allow_private([...])`, mirroring how `allow_privileged` unlocks `Column::privileged` on the write path) and `#[umbral(secret)]` (no unlock exists, anywhere — the point of a tier with no escape hatch is that nobody can reach for it under deadline pressure). Presentation stays per-plugin (`RestPlugin::hide` / `GraphqlPlugin::hide`); only the *floor* moves to the model. Open question the user is deciding: per-field unlock (`allow_private("cost")` — verbose, but the verbosity IS the audit trail, and `grep -rn allow_private` becomes a complete inventory of every place confidential data may leave) vs per-audience unlock (a `trusted` mount unlocks everything marked `private` — one line, but adding a new `private` field then silently widens an existing trusted surface).

    Related and deliberately NOT done: allowing the same plugin to be registered twice (e.g. a public `RestPlugin` and a staff `RestPlugin`). `App::build` rejects it (`BuildError::DuplicatePluginName`, `crates/umbral-core/src/app.rs:1994`) and it would fail *silently* if it did not: `RestPlugin` publishes its config into `static CONFIG: OnceLock<RestPlugin>` via `let _ = CONFIG.set(...)` (`plugins/umbral-rest/src/lib.rs:1862`), which swallows the second set — the second instance's routes would mount while answering with the FIRST instance's field policy. Four plugins are singletons this way (`rest`, `openapi`, `logs`, `email`), and `Plugin::name()` additionally keys the migration ledger, settings namespace, command namespace and dependency graph. The right shape is one plugin with N mounts (`Surface`), not N plugin instances — and it is strictly better anyway, because OpenAPI can only describe **one** shape per path: fields that vary by caller identity make the generated spec and TS client wrong for one audience.
