...
33. [x] Auth: a user could register twice differing only by case (`dalmasogembo@gmail.com`/`dalmasonto` AND `Dalmasogembo@gmail.com`/`Dalmasonto`) — usernames/emails were stored and matched case-sensitively. FIXED (2026-07-07): `umbral_auth::normalize_username`/`normalize_email` (trim + lowercase) applied at every write boundary (`insert_user` behind `create_user`/`create_superuser`/`create_user_with_flags`) and every lookup boundary (`authenticate`, `verify_email`, `start_password_reset`, both resend-verification routes) + the oauth policy (rule-3 email-link lookup and rule-4 auto-create). Because every stored row is canonical, the existing `#[umbral(unique)]` on `username`/`email` now enforces case-insensitive uniqueness with no schema change. 3 behavioral tests in `plugins/umbral-auth/tests/integration.rs`. **Residual:** direct AuthUser writes through the generic admin form / REST create path do NOT normalize (they go through `DynQuerySet`, not the auth helpers, and the ORM has no pre-save *mutation* hook — `pre_save` signals are notification-only). Staff-only surface; the proper fix is a framework pre-save normalization hook or a `#[umbral(lowercase)]` field attribute — deferred (see #34).

34. [ ] Framework: no pre-save field-normalization hook. Row values are only transformable by the caller before `.create()`; the ORM's `pre_save` signal hands subscribers an immutable JSON snapshot, so it can't rewrite what gets written. This forces per-call normalization (see #33's auth helpers) and leaves generic write paths (admin form-submit, REST create) un-normalized. Options: (a) a `#[umbral(lowercase)]` / `#[umbral(trim)]` field attribute the macro applies in the write path; (b) a typed `Model::pre_save(&mut self)` mutation hook the ORM calls before INSERT/UPDATE across all paths. Design + decide.
