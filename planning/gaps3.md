...
72. [ ] **GraphQL paginates by `limit`/`offset`, which breaks under concurrent writes** — the plugin ships `products(limit:, offset:)` (default 50, hard cap 200). Correct for a static table, quietly wrong for a live one: OFFSET is positional, so an insert or delete between page 1 and page 2 shifts every row that follows, and the client silently *skips* a row or sees one *twice*. It also degrades — the database walks and discards all N offset rows before returning anything.

    The standard answer is Relay **cursor connections**: `products(first: 20, after: "cursor")` returning `{ edges { node cursor } pageInfo { hasNextPage endCursor } }`, where the cursor encodes the sort key of the last row (`WHERE (sort_key, pk) > (...)`), which is stable across writes and uses the index. Every GraphQL client (Apollo, Relay, urql) already knows this shape and will paginate automatically for you.

    Worth doing properly rather than fast: the cursor must be opaque (base64), must encode the ORDER BY it was minted under (a cursor from one sort order is meaningless in another), and must tie-break on the primary key or rows with equal sort keys straddle the page boundary. Keep `limit`/`offset` alongside it — it is genuinely fine for a small static table and nobody wants a cursor to fetch 10 categories.

73. [ ] **GraphQL has no subscriptions, so `umbral-realtime` and the graph do not meet** — GraphQL's third operation type (after query and mutation) is `subscription`, transported over WebSocket (`graphql-transport-ws`) or SSE. async-graphql supports both, and umbral already has `umbral-realtime` — but nothing connects them, so a client that wants live data has to hold a GraphQL endpoint in one hand and a separate realtime channel in the other, and reconcile the two shapes itself.

    The natural shape: `subscription { productChanged(id: "1") { name price } }`, fed by the ORM signals that already fire on create/update/delete (`post_save`), reusing the same per-request field policy (#71) and `expose_if` gate. SSE is the cheaper half and covers most real use (server→client only, survives proxies, no protocol upgrade); WS is what Apollo/Relay reach for by default.

    Gate this behind the same deny-by-default posture as everything else: a subscription is a long-lived read the CLIENT shapes, so an unexposed model must not be subscribable, and a `private` column must not arrive in a push just because it went out over a socket instead of a response body. Blocked on mutations landing first — a subscription API with no way to cause the events it reports is a demo, not a feature.
