...
26. [ ] Admin sheet reads flake under extreme concurrent load — `current_user` READS error, non-WAL test pool

    Root-caused while chasing the test-suite flake. `require_staff` swallowing the error was fixed (commit 3718206c — it no longer redirects a staff user to login on a DB error). The remaining trigger: under extreme CPU-starvation load, `umbral_auth::current_user` (a session read + `AuthUser::objects().first()`) errors with `database is locked` because a concurrent writer holds SQLite's EXCLUSIVE lock during COMMIT longer than `busy_timeout` while descheduled. This is a READ-contention issue, so `BEGIN IMMEDIATE` (gaps3 #25, a write-path fix) does not address it. **Options:** (a) put the test pools (and/or the ambient dev pool) in WAL mode so readers don't block on a writer's commit — but a naive per-connection WAL PRAGMA contends on a freshly-hammered file DB (observed making `m2m_writethrough` worse); apply WAL once at DB-create time; (b) confirm production is unaffected (`connect_sqlite` already sets WAL + 5s busy_timeout, so real SQLite apps are fine — this is a test-harness artifact under artificial saturation). Realistic priority: low (Postgres-first; only reproduces under `nproc` CPU saturation, ~once per normal full-workspace run).
