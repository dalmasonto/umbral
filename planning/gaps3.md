...
83. [ ] Two report types and two error types where one of each would do. `umbral-cli`'s `ScaffoldReport` is `codegen::Scaffolded` plus `cargo_toml_registered`/`registered`, and `ScaffoldError` mirrors `CodegenError` variant-for-variant (its own `From` impl concedes "they were the same errors, which is why `codegen` exists"). Two consequences: the `registered` honesty flag — added because the CLI used to print "Registered `x`" for a command it had failed to wire — lives on the CLI's type only, so a *plugin* generator returning `Scaffolded` has no way to tell its caller it didn't register anything; and `cargo_toml_registered: Option<bool>` overloads `None` to mean both "not attempted" (startproject) and "no Cargo.toml found" (startapp), which `main.rs::print_report` has to paper over with a `wants_dep` bool. Fix: `Scaffolded` gains `registered`, `ScaffoldReport` collapses onto it, `ScaffoldError` wraps rather than mirrors, and the two `Option<bool>`s become one named enum. Raised by the pre-0.0.10 simplicity sweep.

84. [ ] `startcommand` is project-free, so its reserved-name set is a hardcoded copy of every plugin's command names. `RESERVED_PLUGIN_COMMAND_NAMES` in `umbral-cli/src/scaffold.rs` lists eleven *plugins'* commands (`createsuperuser`, `tasks-worker`, `collectstatic`, `startpermission`, …) inside the plugin-agnostic CLI crate. It rots silently — nothing fails if a new plugin command isn't added — and a **third-party** plugin gets no protection at all: `umbral startcommand <their-command>` scaffolds a command that wins the collect dedup (app commands are collected first) and shadows theirs with only a `tracing::warn!`, not the loud `eprintln!` a built-in shadow gets. Root cause: `startcommand` is dispatched by the global binary rather than forwarded into the project, so it cannot see the live registry. Fix: forward it through `forward_to_project` like every other App-dependent command (the `STANDALONE_COMMANDS` seam already exists) and derive the reserved set from `CommandSet::catalog() + builtin_command_names()`. The hardcoded list then disappears and third-party plugins are covered for free. Cost: `startcommand` would need a compiled project — which it writes into anyway. Raised by the pre-0.0.10 altitude sweep.

85. [ ] The test suite is not trustworthy under load. `plugins/umbral-rest/tests/private_unlock.rs::the_list_endpoint_honours_the_unlock_in_both_directions` fails under a full-workspace `cargo test` and passes in isolation — a timing/`SQLITE_BUSY` flake, not a logic bug. Known root cause: ~150 test pools are opened without `busy_timeout`, so any lock contention under parallel load surfaces as a failed assertion rather than a retry. A suite that cries wolf is a suite people stop reading, and "every visible surface is correct" (the v0.1.0 bar) cannot be claimed on top of one. Fix: set `busy_timeout` on the shared test-pool helper and make the REST test files take the same `test_lock` the sibling suites do.
