...
## Remediation (audit_2 follow-up)

Fixed in `crates/umbral-core/src/orm/`:

- **#2 (HIGH) atomicity** — `DynQuerySet::insert_json` and `update_json` (the auto-commit pool paths) now run the parent write AND the M2M junction writes on ONE transaction (`begin` → execute on tx → junctions on the same tx → `commit`). A junction-write failure rolls the parent back instead of orphaning a committed row. `post_save` / `bulk_post_save` fire only after commit (the `_in_tx` variants still emit nothing — the pool wrapper owns the signal). The now-dead pool-only helpers `write_m2m_junctions` / `hydrate_m2m_into` were removed. Test: `tests/dyn_write_atomicity.rs` (a create/update with a valid child but a missing junction table leaves zero new/changed rows).
- **#4 (MEDIUM) rows_affected** — `update_json` now returns the UPDATE's real `rows_affected()` and collects the matched PKs (for the M2M mirror + `bulk_post_save`) with the *same* `effective_where_clauses()` the UPDATE uses, so soft-deleted rows are no longer double-counted in the return value or the signal payload. Test: `tests/dyn_write_atomicity.rs::update_json_returns_real_rows_affected_excluding_soft_deleted`.
- **#8 (LOW) `filter_sql` contract** — added a loud SECURITY note at the splice site in `orm/search.rs` documenting that `filter_sql()` is emitted verbatim (not parameterized) and the `-> Option<&'static str>` return type is what keeps request data out of it.

#1 was fixed in a prior pass (dynamic write path seals masked columns via `seal_masked_json`). #3, #5, #6, #7, #9 deferred — see the Status column for the per-finding reason.

## Docs updated

- `documentation/docs/v0.0.1/orm/masked.mdx` — Replaced the `danger` callout warning that the dynamic/REST/admin write path stored masked fields in plaintext (finding #1, since fixed) with an `info` callout stating encryption-at-rest now holds on every write path (typed and dynamic) and that a masked write fails closed with no keyring. Kept the earlier softening of the "plaintext never leaves the process through serde" claim.
