...
| P2 | ✓ FIXED (2026-07-07) | permissions / IDOR | `plugins/umbral-permissions/src/{models,perm,membership}.rs` | ~~All checks are model-level~~ → shipped the object-level primitive: `ObjectPermission` grant table + `has_object_perm(user, perm, object_pk)`, which does NOT fall back to the model-level grant, so a handler can scope authorization to the targeted row. Plus `objects_with_perm` (list filter), `grant/revoke_object_permission`, `revoke_object_permissions_for` (delete cleanup), superuser variant. 6 behavioral tests. The model-level `has_perm` and the tower layer stay row-unaware by design; instance-aware checks use the new primitive. | Was: any model-level holder could act on any row → IDOR. Now closed for handlers that use `has_object_perm`. | ~~ship an object-permission primitive~~ DONE. |
...
> **✓ ADDRESSED 2026-07-07 (gaps3 #28).** The boot-time audit already *warned* about app mutating routes with no recorded permission (audit_2 H19, `app.rs::warn_ungated_mutating_routes`). Shipped `AppBuilder::deny_ungated_mutations()` — the opt-in "gated by construction" backstop this finding asked for — which turns that warning into a hard `BuildError::UngatedMutatingRoutes` at `build()`. It gates on the *recorded* permission (`Routes::route_gated`, the metadata the umbral-permissions `require_permission` helper writes), so a properly gated POST doesn't trip it, but a forgotten gate fails the build with the offending `"METHOD /path"` named. Regression tests: `crates/umbral-core/tests/deny_ungated_mutations_{rejects,allows_gated}.rs`. A *global* default-deny router primitive (deny-by-default across plugin routes too) remains a future-major posture change, deferred.
