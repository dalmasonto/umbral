...
| 5 | ✓ FIXED (2026-07-07) | Realtime / DoS amplification | `umbral-realtime/src/lib.rs` (`dispatch_presence`) | ~~On every join the full member list was re-broadcast to the entire group~~ → the `presence:sync` roster is now sent only to the JOINING user's connection(s); existing members track membership via the `presence:join`/`presence:leave` deltas they already receive, so a join storm is O(N), not O(N²). | Was: CPU/bandwidth spike under join storms in large rooms. Now bounded. | ~~Send sync only to the joiner~~ DONE. Wire messages unchanged (same event types/shapes → not a protocol break; the bundled client already tracks deltas). Test `tests/presence_sync_scope.rs`. |
