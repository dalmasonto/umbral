...
| 6 | LOW | Store correctness | lib.rs:714-765 | `set_data`'s out-of-request fallback (`upsert_session_data_key`) writes raw SQL to the `session` table regardless of the active store; on Redis/Cookie stores that write is never read back. | Background/out-of-request `set_data` and `Messages` (which route through `read_session`) silently no-op or lose data on non-DB stores. | Route `set_data`/`Messages` through `active_store().load/save`. | ✅ done — `set_data`'s out-of-request fallback now does a read-modify-write through `active_store()` instead of the raw-SQL `upsert_session_data_key` (deleted, along with `sqlite_json_path` — one fewer raw-SQL path in the plugin). `Messages` reads already route through `read_session` (store-routed since H7). A stateless `CookieStore` out-of-request still can't persist (no response to re-set the cookie), but DB/Redis stores round-trip. Test `set_data_non_db_store` installs a HashMap-backed store with NO SQL table and proves out-of-request `set_data` is visible to `read_session`. |
| 7 | LOW | Transport | lib.rs:513-519, 529-534 | Cookie uses no `__Host-` prefix and no configurable `Domain` (host-only, which is fine) but `SameSite=Lax` is fixed (not configurable to `Strict`). | Lax permits the cookie on top-level cross-site GET navigations; acceptable default but not tunable for high-security deployments. | Optional: allow `SameSite` and a `__Host-` prefix via plugin config. | ✅ done (SameSite) — `SessionsPlugin::same_site(SameSite::Lax\|Strict\|None)` sets the cookie policy; defaults to `Lax` (behavior-preserving). `SameSite=None` forces `Secure` (browsers reject it otherwise). Threaded through `set_cookie_header` / `clear_cookie_header` via an ambient seal. Test `same_site_cookie`. (`__Host-` prefix not added — it forbids `Domain` and mandates `Path=/`+`Secure`, a larger cookie-naming change; left as a future enhancement.) |
