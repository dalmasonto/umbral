...
| 5 | MEDIUM | CSRF | `src/lib.rs:600-604` (deps); `src/handlers/crud.rs`, `sheet.rs`, `actions.rs`, `inline_edit.rs`, `upload.rs`, `prefs.rs`, `dashboard.rs` (all POST/PUT/DELETE handlers) | Only `login_post` self-verifies CSRF (`auth.rs:136-151`). Every other mutating endpoint has no in-handler CSRF check and relies on `SecurityPlugin`'s middleware — but the admin declares deps `["auth","sessions"]` only, not `security`. | If `SecurityPlugin` isn't mounted (or the session cookie isn't `SameSite=Lax/Strict`), all state-changing admin actions (create/update/delete/bulk-action/inline-edit/upload/prefs) are CSRF-forgeable. Login is protected; the actions that matter are not. | Either add `security` to `Plugin::dependencies()` and fail boot without it, or self-verify CSRF in the mutating handlers the way `login_post` does. Document the session-cookie `SameSite` requirement. | deferred: not contained — `Plugin::dependencies()` is hard-enforced at build (`BuildError::DependencyNotFound`, app.rs:1354), so adding `"security"` fails `App::build()` for every consumer that doesn't mount SecurityPlugin, including all 31 App-building admin tests (none register it). The alternative (self-verify CSRF across crud/sheet/actions/inline_edit/upload/prefs/dashboard) is a large multi-handler change. Both exceed a contained/safe fix; real exploitability is also bounded by the session cookie's SameSite flag (umbral-sessions, blind spot). |
