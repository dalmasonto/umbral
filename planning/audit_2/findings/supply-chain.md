...
| SC-5 | LOW | Deps / outdated | `plugins/umbral-livereload/Cargo.toml` (`notify = "8"`) | `notify 6.1.1` is behind current (7.x/8.x) and pulls old transitives: `inotify 0.9.6`, `bitflags 1.3.2`, `mio 0.8.11`. Livereload is opt-in and dev-oriented, limiting blast radius. | Stale watcher stack; source of several SC-4 duplicates. Low because it should never be enabled in production. | Bump `notify` to the current major; document that `umbral-livereload` must not be enabled in prod builds. | **FIXED 2026-07-07.** Bumped `notify 6 → 8.2.0` — the `recommended_watcher`/`EventKind`/`RecursiveMode`/`Watcher::watch` surface livereload uses is stable across majors, so no code change; `inotify 0.9.6 → 0.11.3` and the old `bitflags 1.3.2` / `mio 0.8` transitives drop out (collapses SC-4). Added a "Production" doc section on the plugin noting it's Dev-gated and should stay out of prod builds. |
