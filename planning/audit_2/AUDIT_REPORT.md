...
| H16 | HIGH ⏸DEFERRED | DB pool | core-app-config · `db.rs:382-402`, `app.rs:824` | `UMBRAL_DB_*` pool knobs ignored for the default pool (settings published in `build()`, pool opened before `build()`) | Operator sets `MAX_CONNECTIONS=100`; pool still opens with 10 → saturation + 30s stalls while config appears correct |
| H17 | HIGH ⏸DEFERRED | Routing panic | core-app-config · `settings.rs:273`, `db.rs:239` | `Settings.databases` is documented + deserialized but never consumed to open pools | A model routed to a `settings.databases`-only alias panics `no database registered` on first query → request-path 500. **DEFERRED (needs async pool-open in build);** the false doc claim was corrected. |
