...
| H3 | HIGH ½FIXED | DoS / mass assignment | plugin-rest nested arrays (fixed); core-orm `insert_json`/`update_json` (open) | (rest) nested arrays had no total-node cap; (orm) deny-nothing-by-default write path | **rest FIXED:** whole-tree cap `MAX_NEST_NODES=1000` → 400 before commit. **orm OPEN:** `is_superuser`/ownership FKs writable unless each model opts out (compounds the Form-derive opt-out) |
