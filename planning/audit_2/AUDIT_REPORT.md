...
| H21 | HIGH ⏸DEFERRED | Migrations / SQLite | core-migrate · `migrate.rs:3305,3902` | Combined alter + add/drop on one table emits ops whose `INSERT…SELECT` references not-yet/already-gone columns | A routine model edit produces a migration that can't apply on SQLite; deploy blocked / file must be hand-edited |
