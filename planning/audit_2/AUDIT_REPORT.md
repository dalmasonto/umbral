...
| H10 | HIGH ⏸DEFERRED | Security headers | core-web · `app.rs` wiring + `check.rs:655` | No X-Frame-Options/HSTS/nosniff/CSP by default; they live only in optional `SecurityPlugin`. **DEFERRED (needs design):** shipping default headers or making SecurityPlugin mandatory changes every app's default response / hard-fails existing builds — a framework default-posture decision. |
...
| H24 | HIGH ⏸DEFERRED | Supply chain | supply-chain · `rust-s3 0.35.1` (storage `s3` feature) | Unmaintained; drags EOL `rustls 0.21.12` + `hyper 0.14` onto the object-storage TLS path | Legacy, unpatched TLS on a network-facing path in the expected prod media config *(CVE specifics unverified — `cargo-audit` not installed)* |
