...
| H1 | HIGH ✅FIXED | Authz / IDOR | plugin-rest · `lib.rs`; `permission.rs` | Built-in `retrieve`/`update`/`destroy` scope lookups to the pk only; no `get_queryset`/object-permission hook | Any caller past the table gate reads/mutates any row by id. **FIXED:** new `ResourceConfig::scope(fn)` / `.owned_by(col)` object-level row scope (with `ScopeDecision::{All,Restrict,None}`), ANDed into EVERY built-in CRUD action — list returns only in-scope rows; retrieve/update/delete of an out-of-scope row returns 404 (no oracle). `fetch_rows`' single-row path now applies the filter too. Tests: `object_scope.rs` (5 — list-scoped, retrieve/update/delete cross-owner IDOR → 404, anonymous denied). Doc: `rest/permissions.mdx`. |
