...
| H7 | HIGH ⏸DEFERRED | Session revocation | plugin-sessions · `active_store()` used only in `session_layer` | `revoke_user_sessions`/logout/read hit the SQL table directly → no-ops under `RedisStore`/`CookieStore` | Password reset does not invalidate a stolen Redis/Cookie session. **DEFERRED (needs design):** requires a `SessionStore` trait redesign + a revocation strategy; a stateless `CookieStore` cannot support server-side revocation without token rotation / a denylist. |
