...
105. [MEDIUM] Typed column constants are not generated for `#[umbral(flatten)]`-inherited base columns.
    Evidence: `#[derive(ModelBase)]` + `#[umbral(flatten)]` (shipped) lets a model inherit a base's columns as if inline — they migrate, insert, auto-stamp, and read back correctly, and the base can carry the primary key. But the typed column constants (`article::CREATED_AT`, used for `filter`/`order_by`) are only generated for a model's OWN fields, not the inherited ones, so base columns cannot currently be `filter`/`order_by`'d through the typed `QuerySet` API at all (there is no string-based `order_by`/filter — both require a `Col`/`OrderExpr` const), only through the dynamic `DynQuerySet::for_meta` path (which addresses columns by name) or by declaring the column on the model directly. Root cause: a `#[derive(Model)]` proc-macro sees only its own struct's tokens, so it cannot enumerate the base's field names to emit their consts; the base's field list is only available through a companion macro the `ModelBase` derive would export, and invoking that macro from the embedding model does not resolve cleanly across crates (Rust's flat, `#[macro_export]`-rooted macro namespace) — an automatic version would break the currently-working cross-crate flatten with an "unresolved macro" error. Recommended fix: have `#[derive(ModelBase)]` emit a `#[macro_export] macro_rules!` that expands to the base's column consts parameterized by the target model type, and expose a user-invoked `umbral::mixin_cols!(Article: TimeStamped)` (so the caller supplies the base path, sidestepping cross-crate resolution) that generates `article::ID`/`article::CREATED_AT`/… bound to the model — same-crate first, with a documented cross-crate story. Until then base columns are fully functional; only the typed-query ergonomics are missing. Shipped feature + docs: `documentation/docs/v0.0.1/orm/model-base.mdx`; core: `crates/umbral-core/src/orm/model.rs` (`ModelBase`, `concat_field_specs`), `crates/umbral-macros/src/lib.rs` (`EmitMode`, `#[umbral(flatten)]`).
