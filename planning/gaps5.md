...
22. [x] [tf#235] GIS/PostGIS — SHIPPED (feature-gated core). `SqlType::Geometry(GeometrySpec)` / `Geography(GeometrySpec)` + `GeometryKind` in core (lightweight — DDL/gating/inspect are just string formatting), with the heavy geo codec (`geo-types` + `geozero` EWKB↔GeoJSON) behind the opt-in `postgis` cargo feature so a non-geo app compiles none of it. `#[umbral(geometry|geography = "point", srid = 4326)]` derive attribute → `geometry(Point,4326)` DDL via `ColumnType::custom`; `#[umbral(index)]` on a spatial column emits `USING GIST` (reuses the existing `index: bool`, no new FieldSpec field); migration auto-emits `CREATE EXTENSION IF NOT EXISTS postgis` (the citext precedent). `umbral::orm::gis::Geometry` newtype wraps `geo_types::Geometry<f64>` with sqlx PG Type/Encode/Decode (EWKB) + GeoJSON serde. Wire strategy: writes coerce GeoJSON→EWKT text (bound via PostGIS's text→geometry cast, no query-builder function-wrapping); typed reads decode EWKB; dynamic reads emit GeoJSON. `GeometryCol` spatial predicates (`dwithin`/`intersects`/`contains`/`within`/`bbox_overlaps`) render `ST_*` via `cust_with_values` ($1 placeholders). Backend-gated Postgres-only (SQLite boot check). Plugin arms: REST filtering (spatial filters deferred → rejected), OpenAPI (GeoJSON object), admin (geo widget/kind). **Tested end-to-end against live PostGIS 3.6** with the real amenities-radar Kenyan datasets (800 healthcare-facility Points + 47 county Polygon/MultiPolygon boundaries): writes through the ORM coerce path, typed EWKB + dynamic GeoJSON read-back, `ST_DWithin`/`ST_Intersects` queries, GiST index verified — `crates/umbral-core/tests/postgis_field.rs`. Docs: `documentation/docs/v0.0.1/orm/gis.mdx`. REST spatial filters SHIPPED too: `location__dwithin=lon,lat,meters` (geography-cast → metres) + `location__bbox=minx,miny,maxx,maxy` (GiST `&&`) in umbral-rest filtering, gated so a geometry column accepts only spatial lookups + isnull. Deferred (per the decision doc): raster, 3D/4D, topology, in-ORM `ST_Transform`, distance-as-annotation, interactive admin map picker, and inspectdb SRID/subtype recovery from the `geometry_columns` catalog (v1 maps a `geometry` udt to unconstrained `Geometry(4326)`). Design: `docs/decisions/2026-08-08-postgis-and-content-types.md`.
...
## review_3 remediation (addendum)

106. [x] review_3 sweep — most findings fixed; deferred maintainability items tracked.
    Verified all `planning/review_3/` candidates (codex/ + claude/02_triage.md) against v0.0.11 and marked each with its status in-file (2026-08-16). The confirmed CRITICAL/HIGH security findings were already closed by the gaps4 batch (admin cell-edit privesc, PG write-back leak, REST filter oracle) or this sweep (the Masked re-seal cluster). This sweep additionally FIXED: Decimal on PG dynamic decoders, jsonb `has_key` operator (`??`→`?`), m2m Uuid id (Display not Debug), `StrCol`/`NullableStrCol` `in_`/`in_subquery`, backend-correct `into_subquery`/`combine`, `QuerySetTx::delete` soft-delete + cascade, `update_expr` via `implicit_predicates`, RLS reset-gate memory ordering (Relaxed→Acquire/Release), squash-aware drift detection, unique `ADD COLUMN`→`CREATE UNIQUE INDEX` on SQLite, CORS segment-boundary matching, JSON-500 passthrough, prod error-body blanking, `safe_url` scheme allowlist, `#[derive(Form)]` skips `privileged`, `on_delete=set_null` rejected on non-nullable FK, openapi `filter_parameters` skips hidden columns, auth session resolution routed through the store-aware `current_user_id_str` (store-agnostic + `max_session_age`), `DbStore::save` via ORM upsert, image-decode allocation cap, storage `save*` methods honour the type allow-list, plus doc corrections (rest blocked-tables, realtime max_connections). DEFERRED (marked in the triage, low-risk maintainability / app-level, not shipped in this batch): pure code-dedup (html-escape ×3, PG per-file apply, tracking-table DDL, macros O2O/PascalCase/Validate-parser, permissions↔auth unauth-response, realtime SSE/WS handshake, admin check_action_perm), `backup` per-model alias routing, `Form` multi-value parsing, `file_field` IDOR (app-level ownership via `media_access`), slash-redirect pre-middleware snapshot, `auth_contract` pk/user_pk collapse (breaking), and assorted doc-drift. See `planning/review_3/` for the per-item verdicts.
