...
92. [ ] **umbra-media replace-cleanup — delete the OLD blob when a file field is updated to a new key.** Split out of gaps2 #82 (delete-cleanup landed 2026-06-21). When a row's `FileField`/`ImageField` value is UPDATED from key A to key B, key A's blob is now orphaned and should be deleted. The clean hook would be a `pre_save`/update signal, but `pre_save` (`plugins/umbra-signals/src/lib.rs:163`, emitted at `crates/umbra-core/src/orm/queryset/mod.rs` `emit_pre_save`) carries ONLY the new instance + a `created` bool — **not the previous row's value**. So the handler can't see the old key without an extra DB read-back keyed by PK (load the existing row, compare its file column to the new instance's, delete the old blob if they differ) — which adds a per-save SELECT and a TOCTOU window, the "fragile implementation" gaps2 #82 said to avoid forcing. Options when picked up: (a) a real `pre_update`/`post_update`-with-old-row signal that snapshots the pre-update row (cleanest, touches the ORM update path + signals); (b) a `MediaTracking`-style decorator on the ORM write that diffs old vs new file columns; (c) document delete-and-recreate as the supported reclaim path (current state, see media.mdx "Replace cleanup is deferred"). Tie any implementation into the existing `cleanup_on_delete`/`cleanup_files` opt-in so a model opts into delete AND replace cleanup together.
