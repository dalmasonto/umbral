...
69. Database router - The current flow embeds postgres and sqlite routers in the orm. This is okay but if we were to add mysql, mongodb and other databases, we would need to update the orm to support them. Besides that, any updates that we are to make to the ORM ie multi-tenants for postgres, it might be really difficult to achieve that. But if we move the routing from the orm or atleast make it in a way, even if its in the orm, it still works as a standalone router. This way, a developer who is to implement multi-tenancy can easily swap the router with their own router.
