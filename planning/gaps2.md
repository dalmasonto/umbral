...
73. [~] **Silent wrong-writes + per-request panic.** _PARTIAL (2026-06-20): SHIPPED — float `min`/`max` (`6053fe0`), `inline_edit` 400-on-parse-fail (`e029202`), `Masked` malformed-key → `BadKey` (`d979e9c`), storage de-panicked (`5ffc9ed`), non-i64 M2M child ids (String/Uuid) now carried through form junction writes via `filter_in_strings`+`filter_eq_string` Uuid arm fix (`dynamic.rs`). REMAINING: REST CSV writer errors dropped._ Paths that report success while storing wrong/no data: non-i64 M2M child ids dropped from form junction writes (`crates/umbra-core/src/orm/forms_runtime.rs:226`) — root cause was `filter_in_strings`/`filter_eq_string` binding Uuid values as TEXT strings instead of BLOB, causing the existence check to return no rows; fixed by adding `SqlType::Uuid` arm that parses + binds as `uuid::Uuid` (BLOB); `f32`/`f64` bypass `min`/`max` validation (`orm/dynamic.rs:1348, 2656`); `inline_edit` writes `""` on a parse failure (`plugins/umbra-admin/src/inline_edit.rs:163`, vs `actions.rs:44` which 400s); `Masked` malformed key → silent `None` keyring (`orm/masked.rs:204`); REST CSV writer errors dropped. Plus `storage.rs:186` `.expect` panics every request if `MediaPlugin` wasn't registered (→ boot system-check). Fix: surface each error (400 / boot-fail / log); validate floats; persist non-i64 M2M ids. → `reviews/correctness-domain.md` + `reviews/static-analysis.md`.
