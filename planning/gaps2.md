...
88. [~] **M2M junction parent-id "hardcoded to `i64`" — part (a) DONE (was a false alarm); part (b) open.** (a) **RESOLVED (2026-06-16):** M2M was already PK-agnostic — `M2M<T, P>` is generic over the parent PK, `set_m2m_parent_ids` seeds it via the typed `set_parent_id(self.<pk>)`, and prefetch buckets by `pk_as_json()`. The i64 appearance was a *dead* `HydrateRelated::pk_i64` (zero callers; the prefetch path was lifted to `pk_as_json`) plus stale "i64-only" comments. Removed the dead `pk_i64` (trait + macro emission + `pk_is_i64`), corrected the comments, and added `tests/pk_uuid_m2m.rs` proving a UUID-PK parent does add+prefetch+join_related (String already covered by `pk_string_m2m`). A non-i64 parent declares the field with the matching `P` (`M2M<Child, String>`/`M2M<Child, Uuid>`; `P` defaults to `i64`). Suite 955/0. (b) **OPEN (#69 follow-up):** the M2M junction free-functions (`crates/umbra-core/src/orm/m2m.rs` `set_junction_dynamic`/`load_junction_selection`) select the pool via raw `pool_dispatched()`, bypassing the `DatabaseRouter` — their table refs ARE schema-qualified (schema-per-tenant works), only db-per-tenant/replica pool selection is unrouted, since a junction carries no `ModelMeta`.
