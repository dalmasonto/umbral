...
90. [ ] **Consolidate `umbra-auth::throttle` onto the core `RateLimiter`.** umbra-rest's API throttling (2026-06-21) introduced a reusable sliding-window primitive at `crates/umbra-core/src/ratelimit.rs` (`RateLimiter` + `Rate::parse`). `umbra-auth` still ships its own bespoke login/register throttle (`plugins/umbra-auth/src/throttle.rs`), written before the primitive existed, that re-implements the same per-key sliding-window idea by hand. Follow-up: have `umbra-auth::throttle` adopt `umbra::ratelimit::RateLimiter` so there's one limiter implementation in the tree (and one place to add the Redis-backed multi-instance store both currently lack). Low urgency — the auth throttle works; this is dedup, not a bug.

91. [ ] **Postgres connection management / proper connection pooling.** The framework builds its `DbPool` (sqlx `PgPool`/`SqlitePool`) with mostly default pool settings. Add proper, configurable connection pooling for production Postgres: expose pool options — `max_connections`, `min_connections`/idle floor, `acquire_timeout`, `idle_timeout`, `max_lifetime`, `test_before_acquire`/health-check — via `Settings`/env (`UMBRA_DB_MAX_CONNECTIONS`, etc.) with sensible prod defaults instead of sqlx defaults, and surface the pool lifecycle (graceful close on shutdown). Proper database access: ensure the ambient `DbPool` is built through these options in `App::build`, document the knobs, and add a boot log of the effective pool config. (Requested 2026-06-21.)
