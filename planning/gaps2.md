...
98. [ ] **REST plugin exposes no `OPTIONS` method on resources.** `umbra-rest` registers each resource's collection (`get(list).post(create).patch(bulk_update).delete(bulk_delete)`, `lib.rs` ~1619) and detail routes with the concrete verbs only. `method_filter` (`lib.rs` ~1753) *can* map `OPTIONS`, but only for an explicitly-declared custom `@action`. So an `OPTIONS` request to a built-in resource endpoint returns **405 Method Not Allowed** (axum supplies an `Allow` header listing the registered verbs) rather than a proper `OPTIONS` response. CORS-preflight `OPTIONS` is handled separately by the CORS layer (`crates/umbra-core/src/cors.rs`), so cross-origin browsers aren't broken — but there is **no DRF-style `OPTIONS` metadata endpoint** (204/200 + `Allow`, optionally the resource's field/serializer schema). _Why it matters:_ DRF answers `OPTIONS` on every endpoint with the allowed methods + a machine-readable description (browsable-API / client-codegen affordance). umbra-rest's playground reads the OpenAPI spec instead, so this isn't load-bearing today — but `200/204 + Allow` is the HTTP-spec-correct answer and cheap. _Fix:_ register an `OPTIONS` handler on the collection + detail method-routers returning `204` with an `Allow` header built from the resource's *enabled* operations (respecting `.hide()` / per-op permissions), and optionally a JSON metadata body (name, allowed methods, writable fields) behind a flag. Scope: small (one handler + wiring in `RestPlugin`'s router build); the metadata body can follow. Found 2026-06-24 while reviewing the REST surface (confirmed: only the CORS layer touches `OPTIONS`; resources do not).
