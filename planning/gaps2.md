...
45. [ ] We need to fix the fks relationships. In django you do `Post.objects.get(id=1).comments_set.all()` or even better `Post.objects.annotate(comment_count=Count('comments'))` - We need a seemless way of doing this. The orm should automatically handle reverse keys for all the fks
    - PARTIAL (2026-06-11): the `annotate(Count("child"))` half is shipped — `annotate_count` / `annotate_related` now AUTO-DISCOVER a reverse-FK relation by scanning the model registry for any child whose FK targets the parent's table, with NO `ReverseSet` field declared on the parent. Matches the caller's relation string against the child's conventional name forms (table name, `snake_case` / lowercase struct name, any `_set`-suffixed). All five aggregate kinds (count/sum/avg/min/max) work over the discovered relation; auto-discovered soft-delete children are excluded; declared `REVERSE_FK_RELATIONS` / `M2M_RELATIONS` always take precedence; two candidates (two child models, or a child with two FKs to the parent) → a clear "ambiguous … declare a `#[umbra(reverse_fk=…)]`" error; an unknown name lists the auto-discoverable children. See `crates/umbra-core/src/orm/queryset/mod.rs` (`discover_reverse_relation` / `AutoDiscovery`) + `tests/annotate_autodiscover.rs`. STILL OPEN: the reverse-MANAGER accessor (`post.comments_set.all()` returning a live QuerySet from a fetched instance) — auto-discovery only covers the annotate path so far.
