...
53. [ ] Playground shell hardcodes `/static/playground/assets/` instead of reading `settings.static_url`. When the playground migrated off embedded-bytes serving onto the unified static pipeline (commit on `feat/static-pipeline`), the HTML shell's `<script>`/`<link>` URLs are built from a hardcoded `STATIC_ASSET_PREFIX = "/static/playground/assets"` const in `plugins/umbra-playground/src/routes.rs`. The hardcode is because `Plugin::routes()` (where `PlaygroundState` / `render_shell` run) gets no `Settings` handle, so the configured `static_url` isn't reachable at render time. Consequence: a deploy that overrides `static_url` (e.g. a CDN origin or `/assets/`) gets a shell whose asset URLs still say `/static/...` and 404. Fix: thread the resolved `settings.static_url` into `PlaygroundState` — snapshot it in `on_ready(ctx)` (ctx carries `settings`) or add a builder field set from settings — and build the prefix as `format!("{}playground/assets", static_url)`. Tagged in code as `// TODO(gaps2 #53)` in `plugins/umbra-playground/src/routes.rs`. Surfaced 2026-06-12 during the embedded→pipeline migration.
