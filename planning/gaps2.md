...
36. [ ] **Rich field-editor JS for `#[umbra(widget = "...")]` — the editor mount + markdown preview + syntax highlighting.** The widget plumbing shipped (features.md #4): `markdown`/`rte` render a tall mono textarea carrying `data-widget`, and the display side has `{{ value | markdown }}`. What's deferred is the actual in-admin editor enhancement: a script that finds `textarea[data-widget="markdown"]` and mounts a split-pane Markdown-it live preview, and `data-widget="rte"` loading TinyMCE/Quill. Should arrive as a third-party-style contribution (a JS module served via `Plugin::static_files` + the `window.umbra` hook in `admin.js`), proving the admin stays editor-agnostic — no core changes. Also here: syntax-highlighted fenced code in `render_markdown` output — ammonia currently strips the `language-*` class pulldown-cmark emits, so highlight.js/Prism have nothing to hook; either widen the ammonia allowlist for `class` on `code`/`pre` (scoped to `language-*`) or pre-highlight server-side (syntect) before sanitizing. Surfaced 2026-06-10 alongside the widget/markdown landing.
