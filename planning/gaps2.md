...
36. [~] **Rich field-editor follow-ups — code-block syntax highlighting; CDN self-hosting.** SHIPPED (features.md #4, 2026-06-10): `admin.js` lazy-mounts EasyMDE (`markdown`), Quill (`rte`), and CodeMirror (`code`, JSON syntax) on every form-render path, themed to the admin tokens; editor previews are sandboxed through DOMPurify (EasyMDE preview + Quill initial load). (a) **SHIPPED 2026-06-20** — syntect server-side highlighting: `render_markdown` rewrites each fenced code block into syntect `hl-` token spans before ammonia runs; the ammonia builder is widened by exactly one inert attribute (`class` on `pre`/`code`/`span`); `fence_lang_is_safe` validates the fence info token to `[a-zA-Z0-9+\-_.#/@]` (max 64 chars) so a hostile info string like `` ```<script>… `` is treated as an unlabelled block rather than a class-injection path. `highlight_styles()` template global emits the `base16-ocean.dark` stylesheet once into `<head>`. `templates::tests::hostile_fence_info_string_is_escaped_and_language_class_survives` pins all four assertions. Commits: `5a33cdf` (CSS), `8d86993` (global), `7a4ded8` (highlighter + ammonia widening), + this commit (fence_lang_is_safe + hostile-info-string test). What remains: (b) **Self-hosting the editor CDNs** (EasyMDE, Quill, CodeMirror, DOMPurify, and EasyMDE's transitive FontAwesome) via `Plugin::static_files`, same call as the htmx/lucide/apexcharts self-hosting already noted in gaps2 #4 — today they load from unpkg/jsdelivr, consistent with the rest of the admin but an offline/air-gapped gap. (c) **EasyMDE image-upload** hook into a future media endpoint. Surfaced 2026-06-10 alongside the widget/markdown landing.
