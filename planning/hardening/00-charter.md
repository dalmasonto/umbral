...
- **`reviews/architecture-modularity.md`** — module boundaries + **file size / cohesion** (QA): the 5 files >2,800 LOC (`queryset/mod.rs` 4846, `migrate.rs` 4660, `umbra-macros/lib.rs` 4521, `dynamic.rs` 3009, `column.rs` 2845) are split candidates — propose **module** splits (a directory of focused files that belong together), not arbitrary file cuts. Dead code, the plugin-contract/facade boundary, dependency direction, duplication.
- **`reviews/static-analysis.md`** — `cargo clippy --workspace --all-targets` landscape (umbra-admin alone had ~44 warnings; umbra-macros has several), `unwrap()`/`expect()`/`panic!` in non-test/prod paths, `unwrap_or_default()` masking, `.ok()` swallowing, `#[allow(...)]` audit, TODO/FIXME inventory.

## Work-lists (snapshot)
- **Crates:** `umbra-core`, `umbra-macros`, `umbra` (facade), `umbra-cli`, `umbra-testing`.
...
- **Largest files (split candidates):** `crates/umbra-core/src/orm/queryset/mod.rs` (4846), `migrate.rs` (4660), `crates/umbra-macros/src/lib.rs` (4521), `orm/dynamic.rs` (3009), `orm/column.rs` (2845), `plugins/umbra-rest/src/lib.rs` (2668), `plugins/umbra-openapi/src/lib.rs` (1658), `forms.rs` (1561), `umbra-cli/src/scaffold.rs` (1467), `app.rs` (1409).
