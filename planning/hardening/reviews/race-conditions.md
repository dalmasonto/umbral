...
**Important (documented):** both are SELECT-then-INSERT/UPDATE with no surrounding transaction (`crates/umbral-core/src/orm/queryset/mod.rs:3886-3996`). Two missers both `create()`. The docstrings already say "pair with a UNIQUE constraint for at-most-one." With a UNIQUE constraint the loser hits the DB and returns `UniqueViolation` (not `(row,false)` as Django would). Without one, two duplicate rows land. Consequence: duplicate rows when the match columns lack a UNIQUE index; otherwise a non-Django error on the race. Fix: wrap in a tx + on `UniqueViolation` re-SELECT and return `(existing,false)` to give true upsert semantics. gaps ref: NEW (or fold into the docstring's known-limitation). Real but low-severity given the documented contract.
...
3. **`update_or_create` / `get_or_create` without a tx** (`crates/umbral-core/src/orm/queryset/mod.rs:3886-3996`) — duplicate rows when match columns lack a UNIQUE constraint; a non-Django `UniqueViolation` error (instead of `(row,false)`) when they have one. Documented as a known limitation but still a correctness gap for the upsert contract.
