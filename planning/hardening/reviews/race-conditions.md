...
- SQLite pools are opened WAL + `busy_timeout=5s` + `foreign_keys=ON` (`crates/umbra-core/src/db.rs:326-357`). WAL gives one writer at a time; the busy-timeout serialises contended writers rather than erroring instantly. Postgres pools get a bounded acquire timeout but default READ COMMITTED isolation.
- The typed `create()` and dynamic `insert_json()` paths both run the DB statement and then `classify_sql_error` (`crates/umbra-core/src/orm/validation.rs:167`) to turn a UNIQUE/FK/NOT-NULL/CHECK violation into a structured `WriteError`. So a SELECT-precheck that loses a race still hits the DB constraint as a backstop — *as long as the constraint exists*.

---

## Sessions

**Critical:** `set_data` read-modify-writes the whole `data` JSON blob — concurrent writes lose keys (`plugins/umbra-sessions/src/lib.rs:400-456`).
...
**Important:** `read_session` lazy-expiry delete races a concurrent refresh, but is benign (`plugins/umbra-sessions/src/lib.rs:243-256`). A expires-and-deletes while B reads the same row; B may still return a row A just deleted, or both delete. Worst case is one extra anonymous round; no corruption. Noted so it isn't re-audited. gaps ref: none.
...
## Membership / junction idempotency (umbra-permissions)

**Important (guarded):** `add_user_to_group` / `grant_user_permission` do `if exists() { return } … create()` (`plugins/umbra-permissions/src/membership.rs:40-52, 130-142`). The SELECT→INSERT gap is real, but `UserGroup`/`UserPermission` carry `unique_together = [["user_id","group_id"]]` / `[["user_id","permission_id"]]` (`plugins/umbra-permissions/src/models.rs:218,248`), so a duplicate INSERT that wins the race hits the DB UNIQUE constraint. The problem: `create()` surfaces that as `WriteError::UniqueViolation` and `add_user_to_group` does **not** catch it — the second concurrent caller gets an error return instead of the documented idempotent `Ok(())`. Consequence: no duplicate row (data stays correct), but a spurious 500/error on a legitimately-idempotent op under concurrency. Fix: match `Err(UniqueViolation)` → `Ok(())` like `set_data` does for its create. gaps ref: NEW.

**Critical:** `set_user_groups` is `delete()` then `bulk_create()` as two separate auto-commit statements — no transaction (`plugins/umbra-permissions/src/membership.rs:77-97`).
...
- Contrast: the typed `M2M::set` (`crates/umbra-core/src/orm/m2m.rs:330-419`) and `set_junction_dynamic` (`:577-637`) both correctly wrap DELETE+INSERT in `p.begin()…tx.commit()`. `set_user_groups` is the one membership path that doesn't. Fix: route it through `umbra::db::transaction(...)` (or reuse `set_junction_dynamic`). gaps ref: NEW.
...
**Important (documented):** both are SELECT-then-INSERT/UPDATE with no surrounding transaction (`crates/umbra-core/src/orm/queryset/mod.rs:3886-3996`). Two missers both `create()`. The docstrings already say "pair with a UNIQUE constraint for at-most-one." With a UNIQUE constraint the loser hits the DB and returns `UniqueViolation` (not `(row,false)` as Django would). Without one, two duplicate rows land. Consequence: duplicate rows when the match columns lack a UNIQUE index; otherwise a non-Django error on the race. Fix: wrap in a tx + on `UniqueViolation` re-SELECT and return `(existing,false)` to give true upsert semantics. gaps ref: NEW (or fold into the docstring's known-limitation). Real but low-severity given the documented contract.
...
1. **`set_data` JSON-blob lost update** (`plugins/umbra-sessions/src/lib.rs:400-456`) — concurrent same-cookie writes silently drop session keys. Read-modify-write with no atomicity. Highest blast radius: every app using session `data` (cart, flash, CSRF state).
2. **`set_user_groups` non-transactional DELETE+INSERT** (`plugins/umbra-permissions/src/membership.rs:77-97`) — opens a window where a concurrent permission check sees zero memberships (transient privilege loss), and concurrent calls diverge the final set. Every sibling junction-replace path is already transactional; this one regressed.
3. **`update_or_create` / `get_or_create` without a tx** (`crates/umbra-core/src/orm/queryset/mod.rs:3886-3996`) — duplicate rows when match columns lack a UNIQUE constraint; a non-Django `UniqueViolation` error (instead of `(row,false)`) when they have one. Documented as a known limitation but still a correctness gap for the upsert contract.
