...
## Completeness (vs Django `contrib.auth`)

| Capability | Status | Notes |
|---|---|---|
| Custom user model | **Complete** | `UserModel` trait, `AuthPlugin<U>` generic, polymorphic `PrimaryKey`. Cleaner than Django's `AUTH_USER_MODEL` swap. Tested for `i64` + `uuid::Uuid` (`tests/uuid_user.rs`, `tests/custom_user.rs`). |
