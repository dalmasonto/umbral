# Review: umbral-playground

Read-only audit, 2026-06-16. Scope: `plugins/umbral-playground/src/` (lib.rs, routes.rs), `build.rs`, `tests/` (m2_build.rs, rust_integration.rs). Cross-referenced against `planning/hardening/backlog.md`, `reviews/security.md`, and `reviews/performance-scalability.md`.
...
| Per-app storage scope | Complete. `app_name` injected as `<meta>` + `window.__UMBRAL_PLAYGROUND_APP__`, escaped. |
...
| `umbral-openapi` spec URL | Partial — see PG-2. Read via registry call with hardcoded fallback. |
| `umbral-rest` dependency | Implicit — the playground is useless without REST + OpenAPI but has no Cargo dep on either. |
...
- `window.__UMBRAL_PLAYGROUND_APP__ = "my\"shop & <test>";` — correct JSON string escaping.
...
It is not an XSS surface: the `<script>` tag contains `window.__UMBRAL_PLAYGROUND_APP__ = <json-string>;` and `<test>` inside a JS string literal cannot break out of the script block (a `</script>` sequence would, but `<test>` alone is safe). The test is correct. This is a clarifying comment opportunity, not a bug.
...
**Fix:** Add an optional `require_staff()` builder (`PlaygroundPlugin::new("app").require_staff()`) that gates the shell route behind `is_staff` using the same `require_staff` guard from `umbral-admin`. When mounted in non-localhost `Dev`, consider logging a boot warning "playground is publicly reachable without authentication".
...
- **No declared Cargo dep on `umbral-rest` or `umbral-openapi`.** The playground is useless without them, but the Cargo manifest lists neither. This makes the dependency graph misleading (a CI that builds `umbral-playground` alone would produce a seemingly-working plugin that renders an empty spec).
- **It is dev/DX tooling, not application logic.** The admin plugin at least provides real CRUD UI an app uses. The playground is only useful for exploring the REST API, which puts it closer to `umbral-livereload` (dev tool) than to `umbral-auth` (core functionality). If the goal is "thin core, plugin-heavy", the playground is legitimately thin — it contributes nothing to the runtime beyond a shell page. It belongs alongside `umbral-openapi` as "optional developer tooling."

**Recommendation:** Keep it. Flag `umbral-openapi` as a soft peer-dependency in the README and docs.

---

## Plugin-contract

- **Facade-only imports:** Complete. `lib.rs:9` uses `umbral::prelude::*` which imports `Plugin`, `StaticDir`, etc. `routes.rs:17` uses `axum::...` directly (allowed — listed as a Cargo dep). No `umbral-core` internals or sibling plugin imports.
