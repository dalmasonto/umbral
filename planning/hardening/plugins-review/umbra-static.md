...
| Template `static()` global | **Complete (in core, not this plugin)** | `umbra-core/src/templates.rs:274` registers `static()` as a minijinja global that resolves against `static_url`. Already filed as docs gap in `helpers.mdx` — **already #66**. |
| Finders (filesystem finder, app-dirs finder, custom finders) | **MISSING** | Django's `STATICFILES_FINDERS` abstraction (pluggable discovery strategies) has no equivalent. The two shapes (filesystem + embedded) are enough for v0. Not stubbed. |
| File hashing / manifest (`ManifestStaticFilesStorage`) | **MISSING** | No content hash in URLs, no manifest JSON, no `{% static 'foo.css' %}` → `/static/foo.abc123.css` rewrite. `collectstatic` copies files as-is. Cache-busting relies entirely on `max_age` settings — a deploy without `--clear` + long max-age will serve stale assets. Not stubbed. |
...
| `STATIC_URL` / `STATIC_ROOT` settings | **Complete** | `settings.static_url` (default `/static/`) and `settings.static_root` (default `staticfiles/`) mirror Django's equivalents. `static_url` normalisation (leading slash, trailing slash) is implemented and tested. |
...
| `X-Content-Type-Options: nosniff` | **MISSING** | Neither the filesystem path nor the embedded path sets `nosniff`. `tower_http::ServeDir` does not add it. `umbra-media` adds it explicitly; umbra-static does not. |
| Media files (`MEDIA_URL`, `MEDIA_ROOT`, upload serving) | **Out of scope** | Owned by `umbra-media`. Correct split. |

## Findings

**[NEW] [Required] `EmbeddedDirService` never emits ETag/304 — every request pays full bandwidth** — `plugins/umbra-static/src/lib.rs:441–472`
...
**[NEW] [Required] `collectstatic` copies symlinks as files but does not detect symlink loops** — `crates/umbra-core/src/static_files.rs:651–689`
...
**[NEW] [Important] No `X-Content-Type-Options: nosniff` header on static responses** — `plugins/umbra-static/src/lib.rs:274–295`, `crates/umbra-core/src/static_files.rs:292–318`

Neither the filesystem `ServeDir` path nor the embedded `EmbeddedDirService` path nor the unified `serve_file()` path adds `X-Content-Type-Options: nosniff`. Older browsers (IE/Edge legacy, some Safari versions) will MIME-sniff a response when the content-type doesn't match what they expect, which can turn a misidentified file into active content. `umbra-media` explicitly sets `nosniff` on its serve route (`plugins/umbra-media/src/lib.rs:481-485`). Static files are intentionally authored (not user-uploaded), so the XSS risk is low compared to media, but parity with `umbra-media`'s security posture is the right baseline. **Fix shape:** add `SetResponseHeaderLayer::if_not_present(X_CONTENT_TYPE_OPTIONS, HeaderValue::from_static("nosniff"))` to the `cache_layer` construction in `routes()`, and add the same to `serve_file()` in `umbra-core`.

**[NEW] [Important] `ServeDir` serves dot-files (`.env`, `.git`, `.htpasswd`) without restriction** — `plugins/umbra-static/src/lib.rs:275–283`
...
**[NEW] [Optional] `defers_to_pipeline()` is evaluated twice per `App::build`** — `plugins/umbra-static/src/lib.rs:228–237`
...
**[NEW] [Nit] `EmbeddedDirService` does not set `Accept-Ranges: none`** — `plugins/umbra-static/src/lib.rs:466–470`
...
**[NEW] [Nit] `cache_headers.rs` describes a dev-mode test scenario that doesn't exist** — `plugins/umbra-static/tests/cache_headers.rs:6–11`
...
- `umbra-core/tests/static_publish.rs` + `static_files.rs` inline tests — registry collision/dedup, `resolve_under_root` (path traversal + symlink escape on Unix), `collect_static` idempotency/clear/missing-source/root-dirs, `static_handler` dev/prod algorithm. Comprehensive.
