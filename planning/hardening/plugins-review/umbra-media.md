# umbra-media — Holistic Review

> Date: 2026-06-16 · Read-only audit

## Verdict

umbra-media is structurally sound for a v0 filesystem-backend plugin: the `Storage` trait is clean, the dependency boundary is correct (depends only on the `umbra` facade, no circular deps), `FileField`/`ImageField` round-trip through TEXT columns, and the serve route's XSS defences (neutralise-active-content, `nosniff`) are well-tested. The two most serious gaps are both architectural: the entire upload pipeline buffers every file body into a `Vec<u8>` in the worker before writing to disk, with no streaming at any layer (multipart parser, `FsStorage::store`, `MediaPlugin::save`), and delete-on-model-delete is completely absent — the admin's `DELETE` handler, the ORM's `delete()`, and the form update path all orphan files on disk with no hook to clean them up. The deferred features (S3, image validation, access control, virus scanning) are clearly labelled; the silent gaps are the memory-buffering and the orphan problem.

---

## Completeness

| Django feature | umbra-media v0 status |
|---|---|
| `FileField` / `ImageField` ORM types | **Exists** — `crates/umbra-core/src/orm/file_field.rs`; TEXT-backed newtypes with `key()`, `url()`, `is_empty()`, `Display`; serde + sqlx impls for both SQLite and Postgres |
...
| `FileSystemStorage` equivalent | **Exists** — `FsStorage` in `umbra-media/src/lib.rs` |
| `Storage` trait (ABC) | **Exists** — `umbra_core::storage::Storage`; `store`, `retrieve`, `delete`, `url` |
| `default_storage` ambient singleton | **Exists** — `OnceLock<Arc<dyn Storage>>` + `set_storage`/`storage`/`storage_opt` in `umbra_core::storage` |
| File upload handling (multipart) | **Exists** — `parse_and_store_multipart` in `umbra_core::web::multipart`; admin create/update route wired |
...
**already #73 — Important — `storage()` panics when MediaPlugin is unregistered** — `crates/umbra-core/src/storage.rs:186`
...
**Upload size cap bypass (already filed in security.md Important)** — `plugins/umbra-media/src/lib.rs:426-433` vs `FsStorage::store:151-183`
...
**Filename sanitization confirmed sound (already filed in security.md FYI)** — `umbra-media/src/lib.rs:161-175`
Strips `/`, `\`, `\0`; UUIDv4 prefix; neutralise-active-content `.txt` appended; `nosniff` set. Tested. (already in security.md)

---

**[NEW] Required — No file deletion on model delete (orphan accumulation)** — `plugins/umbra-admin/src/handlers/crud.rs:664-690`, `crates/umbra-core/src/orm/queryset/mod.rs` (delete paths)
...
**[NEW] Required — No orphan cleanup on file-column update** — `plugins/umbra-admin/src/handlers/crud.rs:503-556`, `plugins/umbra-media/src/lib.rs:420-457`
...
**[NEW] Required — Entire upload pipeline is fully buffered; no streaming** — `crates/umbra-core/src/web/multipart.rs:220-251`, `plugins/umbra-media/src/lib.rs:153-183`
...
**[NEW] Important — `MediaPlugin::save` double-inserts when `self.storage` is itself a `MediaTracking` wrapper** — `plugins/umbra-media/src/lib.rs:420-457`, `242-284`
...
**[NEW] Important — `with_storage` constructor silently sets `dir` to the mount string** — `plugins/umbra-media/src/lib.rs:360-368`
...
**[NEW] Important — No access control on the serve route; private-media pattern is impossible today** — `plugins/umbra-media/src/lib.rs:469-491`
...
**[NEW] Optional — `MediaTracking::store` inserts with client-declared content_type without any validation** — `plugins/umbra-media/src/lib.rs:256-262`
...
**[NEW] Optional — Test `plugin_save.rs` uses raw `sqlx::query` for DDL, violating the ORM-only rule** — `plugins/umbra-media/tests/plugin_save.rs:42-54`
...
**[NEW] Nit — `FsStorage` ignores its `_content_type` parameter entirely** — `plugins/umbra-media/src/lib.rs:156-158`
...
**[NEW] FYI — `MediaPlugin::save` double-insert risk is self-documented but not type-enforced** — `plugins/umbra-media/src/lib.rs:505-509`
...
- `crates/umbra-core/tests/file_image_field.rs` — `FileField`/`ImageField` macro classification, widget defaults, serde wire shape, `url()` fallback without backend.
- `crates/umbra-core/tests/file_field_roundtrip.rs` — ORM `create`/`get` round-trip through SQLite TEXT column; `Option<FileField>` NULL handling.
- `crates/umbra-core/tests/file_field_storage_resolution.rs` — `url()` resolves through a registered ambient backend.
- `crates/umbra-core/tests/file_field_check_missing_storage.rs` — boot fails with `field.storage_backend` error when no plugin provides storage.
- `crates/umbra-core/tests/file_field_check_present_storage.rs` — boot succeeds with a storage-providing plugin.
- `crates/umbra-core/tests/form_file_field.rs` — file/image field form validation; `<input type="file">` render; optional image field.
- `crates/umbra-core/tests/multipart_storage_merge.rs` — `parse_and_store_multipart` stores files, flattens to pairs; empty file parts skipped.
