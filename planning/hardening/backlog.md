...
- **⚑ Session `set_data` lost update** — `plugins/umbral-sessions/src/lib.rs:400-456`. Read-modify-write of the whole session JSON; concurrent same-cookie requests drop each other's keys (cart/flash/CSRF). Also flagged by static-analysis (corrupt-data→empty map, no log, `:389,447`). Fix: atomic merge in a transaction / `SELECT … FOR UPDATE`, and log decode failures. → **NEW #71**
- **⚑ `set_user_groups` non-transactional DELETE+INSERT** — `plugins/umbral-permissions/src/membership.rs:77-97`. Empty-membership window = transient privilege loss; every sibling path is already transactional. Fix: wrap in one tx. → **NEW #71**
- **`update_or_create` / `get_or_create` no transaction** — `crates/umbral-core/src/orm/queryset/mod.rs:3886-3996`. SELECT-then-write → duplicate rows (no UNIQUE) or spurious `UniqueViolation`. Fix: tx + UNIQUE backstop + catch-violation; make `add_user_to_group`/`grant_user_permission` catch the UNIQUE backstop (idempotent under races). → **NEW #71**

### Soft-delete on the dynamic path (reviews/correctness-domain.md)
- **⚑ Dynamic path hard-deletes + lists trashed rows (gaps2 #35)** — `crates/umbral-core/src/orm/dynamic.rs:595-611`. `DynQuerySet` never reads `meta.soft_delete` (correctly populated `migrate.rs:411`), so admin/REST `DELETE FROM` permanently destroys data on the 23 soft-delete website models + returns trashed rows. Highest-impact data-loss item.
...
- **Admin M2M form loads the entire target table** (no LIMIT) on every add/edit render — `plugins/umbral-admin/src/view.rs:511`. The FK picker beside it is paginated; mirror it. → **NEW #72**
- **REST `?format=csv` bypasses the 1000-row cap** and buffers `SELECT *` into memory — `plugins/umbral-rest/src/lib.rs:1748` (`page=None` skips the clamp). Fix: stream + clamp. → **NEW #72**

### Silent wrong-writes (reviews/correctness-domain.md) — NEW
- **Non-i64 M2M child ids dropped from form junction writes** — `crates/umbral-core/src/orm/forms_runtime.rs:226` (reports success, writes nothing). → **NEW #73**
- **Floats bypass `min`/`max` validation** — `orm/dynamic.rs:1348, 2656` (reports success, stores out-of-range). → **NEW #73**
- **`inline_edit` silently writes `""` on parse failure** — `plugins/umbral-admin/src/inline_edit.rs:163` (vs `actions.rs:44` which 400s). → **NEW #73**
...
- `orm/querying.mdx` — `FColExt` claimed in `umbral::prelude` (it's in `umbral::orm`).
- `orm/relationships.mdx` — documents non-existent `#[umbral(m2m = "...")]`.
- `realtime/sse.mdx` + `realtime/scaling.mdx` — stray `</content>`/`</invoke>` tool-gen artifacts break MDX.
- `migrations/checkmigrations.mdx` — `umbral checkmigrations` (no registry) → use `cargo run -- checkmigrations`.
...
- **OAuth: no PKCE + replayable `state`** — `plugins/umbral-oauth/src/routes.rs:143-218`. Add PKCE (S256) + single-use state. → **NEW #74**
- **Empty `SECRET_KEY` silently signs CSRF with an empty HMAC key** — `plugins/umbral-security/src/lib.rs:392-394`. Fail-closed / boot-warn on empty key. → **NEW #75**
- **`password_hash` serde-serialized, guarded only by the block-list** — `plugins/umbral-auth/src/lib.rs:233-234`. One `.expose(["auth_user"])` without `.hide()` leaks argon2 hashes. Fix: `#[umbral(noform)]`-style "never serialize" / auto-hide for `password_hash`. → **NEW #75**
- **Inactive-superuser permission bypass on a live session** — `plugins/umbral-permissions/src/rest.rs:97-105`. Re-check `is_active`. → **NEW #75**
...
- **Per-row registry deep-clone** — `umbral-rest/src/lib.rs:779 → migrate.rs:85` (1000-row page = 1000 full-registry clones). Clone once. → **NEW #72**
- **~12-14 serial permission queries per changelist** — `plugins/umbral-admin` `AdminPerms::load`/`require`; `user_perms()` (one query) is the fix. → **NEW #72**
...
- **⚑ Sessions: corrupt data → empty map, no log** — `umbral-sessions/lib.rs:389,447` (also in P0 set_data fix). → **#71**
- `Masked` malformed key → silent `None` keyring — `crates/umbral-core/src/orm/masked.rs:204`. Surface the error.
- REST CSV writer errors dropped — `umbral-rest`.
...
- **⚑ Boundary violation: `umbral-auth` → `umbral-rest`** for `Authentication`/`Identity` traits — forces `umbral-rest` into every app; contradicts the CLAUDE.md "REST-free app compiles with zero serializer code" contract. Fix: lift those traits into `umbral-core`/facade. → **NEW #76**
- Duplication: `to_snake_case` ×3 (`umbral-macros`, `inspect.rs`, `queryset/mod.rs`), `pascal_case` ×2 (`umbral-openapi`, `umbral-cli`) → an `umbral-naming` internal helper or facade fn. → **NEW #77**

### Docs (Important — long tail, docs-audit/*)
- `.on()` is SQLite-only but used without caveat across pages (e.g. `aggregates.mdx`); add a global note. · REST block-list says "3 tables", code blocks 10 (`rest/exposure.mdx`). · `OrPermission` "strongest error code" vs actual last-write (`rest/permissions.mdx`). · `auth/user-in-templates.mdx` stale "reverse traversal not implemented" callout (it ships) + anonymous sentinel missing `is_staff`/`is_superuser` (`session_user.rs:506`). · `umbral-email` referenced as shipped (crate doesn't exist). · `TenantUser` `fn id(&self) -> i64` vs real `-> <Self as Model>::PrimaryKey`. · `auth/authentication.mdx` `use async_trait::async_trait` → `umbral::async_trait`. · `web/error-pages.mdx` internal `umbral_core::errors::…` path. · `examples/basic.mdx` Cargo.toml missing `"chrono"`. · `templates/helpers.mdx` omits 3 built-in functions. · permissions.mdx example arg-order + `.on(&pool)` that won't compile. → all **doc-fix batch** (cheap, high-value).
...
3. `umbral-macros/src/lib.rs` (4521) → `{derive_model/, field_meta, column_const, kind, derive_form, derive_choices, task_macro}`.
...
- `#[allow(dead_code)]` in prod src: `umbral-rest/src/filtering.rs:94` (`into_condition`), `umbral-admin/src/rows.rs:93` (`too_many_arguments`).
...
- **#76** Plugin-contract fix — lift `Authentication`/`Identity` out of `umbral-rest` into core (REST-free apps).
...
| umbral-livereload | Complete | `to_bytes(body, usize::MAX)` uncapped buffer (report) |
| umbral-health | Complete | `probe_database` raw `sqlx` ORM bypass → add `umbral::db::ping()` (#84); no per-check timeout |
| umbral-playground | Complete (real plugin) | naive multi-`replace()` placeholder chain → injection breakout (#81) |
| umbral-cache | Has gaps | `cache_page` Host/Cookie poisoning (#81); single backend (#82) |
| umbral-static | Has gaps | embedded ETag/304 missing + symlink loop (#81); no manifest hash (#82) |
| umbral-tasks | Lean v1 | fragile `starts_with("handler not found")` non-retriable check (#80); no SKIP-LOCKED/beat/result-backend (#82) |
| umbral-oauth | Real, incomplete | token refresh never implemented (#79); reqwest no-timeout (#81); PKCE = #74 |
| umbral-openapi | Accurate schema, path holes | CRUD paths hardcode `/api/...` ignoring `base_path()` (#79) |
| umbral-security | Solid for scope | `csrf_exempt_paths` segment-boundary bug (#81); empty-key = #75 |
| umbral-media | Has gaps | no file-lifecycle cleanup + fully-buffered `Storage` trait (#82) |
| umbral-admin | Has gaps | `TabularInline` stored-but-never-rendered stub (#79); base-path 404s + view-perm disclosure (#83) |
| umbral-rls | Real but non-functional | emits policy DDL but nothing sets `app.user_id` per request → deny-all (#79, ties #69) |

**Contract:** clean across the board — facade-only imports, no raw `sqlx::query`/`query_as` in plugin `src/` (the lone `umbral-health` `SELECT 1` is the one exception → #84), each owns its migrations. The two dependency-direction smells are #76 (`auth→rest`) and #84 (`realtime→auth` hard dep).
