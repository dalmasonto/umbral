...
**Required:** Default block-list table — doc lists exactly three blocked tables (`auth_user`, `session`, `umbra_migrations`) and says "Three tables are refused even though every other model is served." The actual `DEFAULT_BLOCKED_TABLES` constant at `plugins/umbra-rest/src/lib.rs:86-97` blocks **ten** tables:

```
auth_user, session, umbra_migrations,
...
The doc's "three tables" claim and the accompanying table are wrong. A reader relying on the doc would not know that their `task_row`, `admin_audit_log`, or `permissions_*` models are silently blocked. Fix: expand the table to list all ten, or group them as "framework-internal security/infra tables (10 total)" with the full list inline. Cite: `plugins/umbra-rest/src/lib.rs:86-97`.
...
**Required:** `Authentication` trait signature — doc shows `#[async_trait]` on the trait definition. The actual trait at `plugins/umbra-rest/src/auth.rs:108` does use `#[async_trait]`, so the signature snippet is accurate. However, the doc's `async_trait` import in the example is `use async_trait::async_trait;` (line 153 of authentication.mdx). In the real codebase, `umbra::async_trait` is re-exported and the middleware.rs doc even explicitly says "`#[umbra::async_trait]` is re-exported from the facade, so no direct `async-trait` dependency is required." The authentication doc's `Writing a custom Authentication` example imports `use async_trait::async_trait;` directly, which requires adding `async-trait` as a dependency when it isn't needed. Fix: change the import to `use umbra::async_trait` or `use umbra::prelude::*` in the custom auth example. Not a compilation failure (direct import works if the dep is added) but inconsistent with how the middleware doc instructs users to use it. Cite: `authentication.mdx:136`, `crates/umbra-core/src/middleware.rs:50`.

**Important:** `SessionAuthentication` and `BearerAuthentication` — the doc lists them as built-in classes in the card grid and shows full examples for both. Both live in `plugins/umbra-auth/src/` (not in `umbra-rest/src/auth.rs`). The doc's import paths show `use umbra_auth::SessionAuthentication` and `use umbra_auth::BearerAuthentication` which is correct. But the `#[async_trait]` trait signature box at the top of the page implies the trait definition shown is what a user implements — the trait is actually defined in `umbra-rest`, not `umbra-auth`. The cross-crate relationship is correct in code but the page mixes the two crates' surfaces without a clear seam. Minor clarity issue; no hard drift.
...
**Nit:** `ResourceConfig::for_::<Order>()` — the doc uses this constructor (line 25). It is confirmed present at `plugins/umbra-rest/src/resource.rs:293`. Correct.

**Nit:** The page uses `action_input_schema` and `action_output_schema` as chained methods. Both are confirmed at `plugins/umbra-rest/src/resource.rs:531,542`. Correct.

**FYI:** The validator coverage table ("top-level and nested `type`, `required`, `properties`, `enum`") matches the `validate_against_schema` / `validate_schema_node` / `json_type_matches` implementation at `plugins/umbra-rest/src/lib.rs:1138-1203`. No drift.
...
**Nit:** The `.route` multi-method example passes `&["GET", "POST"]` as the methods slice. The actual `Routes::route` signature at `crates/umbra-core/src/routes.rs:313` takes `methods: &[&'static str]`. The example compiles because `&["GET", "POST"]` is `&[&'static str; 2]` which coerces. Correct.
...
**Nit:** Doc says "`#[umbra::async_trait]` is re-exported from the facade, so no direct `async-trait` dependency is required." Confirmed: `crates/umbra-core/src/middleware.rs:50` uses `#[async_trait]` from `async-trait`. The re-export path exists in the facade (consistent with CLAUDE.md's description of the prelude). Accurate.

**FYI:** Doc says "App-level middleware is added to the stack first, then each plugin's contribution in topological dependency order." This matches the `MiddlewareStack` logic in `crates/umbra-core/src/middleware.rs`. The onion ordering claim (`A.before → B.before → C.before → handler → C.after → B.after → A.after`) is confirmed by `run_stack` at lines 128-174. Accurate.
...
**Important:** `fire_server_error_hook` usage example — the doc at lines 147-158 shows a user calling `umbra_core::errors::fire_server_error_hook(...)` inside a custom `IntoResponse` impl. The function exists at `crates/umbra-core/src/errors.rs:419` and is `pub`. However, it is on `umbra_core`, not on the `umbra` facade. A user would need to add `umbra-core` as a direct dependency (rather than going through the `umbra` facade) to call it, which violates the "plugins import only the facade" principle from CLAUDE.md. Fix: either re-export `fire_server_error_hook` from the `umbra` facade, or change the doc example to use the hook via `AppBuilder::on_server_error` exclusively (the hook-registration path is the normal surface). Cite: `crates/umbra-core/src/errors.rs:419`, doc line 148.

**Nit:** 500 template context table lists `request_path` but the `server_error_panic_handler` at `errors.rs:391` passes an **empty string** for `request_path` on the panic path ("not available in a panic handler"). The doc's table entry says `request_path: "The path of the failing request"` without noting this limitation. The `render_500_middleware` path (non-panic 500) does populate `request_path` correctly. Fix: add a note that `request_path` is empty for panic-path 500s. Cite: `crates/umbra-core/src/errors.rs:391`.
...
**FYI:** `LoggedIn`, `LoginRequired`, `login_required`, `login_required_html` — the page says all ship from `umbra-auth`. Confirmed: `plugins/umbra-auth/src/extractors.rs` defines `OptionalIdentity` and `CurrentIdentity`; the `login_required*` functions and `LoggedIn<U>` exist in `umbra-auth` (confirmed by `extractors.rs` presence and the auth grep results). The page import path `umbra_auth::{AuthUser, login_required::{LoggedIn, LoginRequired, login_required, login_required_html}}` is consistent with the crate structure. Accurate.
...
**Nit:** Doc says Django uses 301 for trailing-slash redirects; umbra picks 308. Confirmed at `crates/umbra-core/src/slash.rs:197` where `StatusCode::PERMANENT_REDIRECT` (308) is used. Accurate.
...
OK — Short page about the `| markdown` filter and `highlight_styles()` template function. No REST or web routing claims. No verifiable drift from this pass (the markdown/syntect integration lives in `crates/umbra-core/src/templates.rs` which is out of scope). No findings.
