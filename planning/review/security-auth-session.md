...
- **Evidence:** `AuthUser.is_staff`/`is_superuser` are plain `bool` with no `#[umbral(noform)]`/`#[umbral(noedit)]` guard (contrast `password_hash`, which is `#[umbral(noform)]`). The shop registers `auth_user` in the admin. The update handler gates only on the generic `change_auth_user` permission; there is no field-level restriction limiting `is_superuser`/`is_staff` edits to existing superusers (Django's `ModelAdmin` does exactly this).
