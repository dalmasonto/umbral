...
> - **Already covered:** AUTH-6 — the admin CSRF compare already routes through `umbral_security::tokens_match`, which is `subtle::ConstantTimeEq` (constant-time). The SecurityPlugin CSRF cookie already sets `Secure` via `csrf_cookie_secure || is_prod`.
...
> - **Closed by design:** AUTH-4 (escalation now blockable via `#[umbral(noedit)]`, enforced by WEB-2), AUTH-7 (double-submit + SameSite is a sound CSRF design).
> - **Deferred (genuine):** AUTH-3 (RLS `set_config` context middleware), AUTH-8 (revoke sessions/tokens on password change).

Scope: `umbral-auth`, `umbral-sessions`, `umbral-permissions`, `umbral-security`, `umbral-rls`, plus auth-related middleware/settings in `umbral-core`.

---

## AUTH-1 — Flagship `examples/shop` ships with NO CSRF protection and NO security headers
> **✓ ALREADY COVERED** — `examples/shop` already mounts SecurityPlugin (CSRF + headers, `/api` exempt); the review predated it.
**Severity: high** · **Verified** (`grep -rn "SecurityPlugin\|umbral_security" examples/shop/` → no hits)

- **File:** `examples/shop/src/main.rs:69-193` (plugin list); the missing piece is `plugins/umbral-security/src/lib.rs:105-127`
...
> **✓ ALREADY COVERED** — SecurityPlugin is mounted and the shop's only POST form carries `{{ csrf_input }}`.
**Severity: high** (compounds AUTH-1) · **Verified** (`grep csrf plugins/umbral-admin/src/handlers/` → only `auth.rs` login)

- **File:** `plugins/umbral-admin/src/handlers/*.rs` (crud, actions, inline_edit, sheet)
...
> **⏳ DEFERRED** — needs a `set_config` context-population middleware (larger change).
**Severity: high** · **Verified** (`grep -rn "set_config\|SET LOCAL\|app.user_id" plugins/umbral-rls/src/` → only **reads** of `current_setting('app.user_id')` in policy DDL; nothing writes it)

- **File:** `plugins/umbral-rls/src/lib.rs` (whole plugin); policies reference `current_setting('app.user_id')` at `:21-23`, `:393`
...
> **🚫 CLOSED — by design.** The enforcement mechanism now exists: WEB-2 (`73ef05d`) makes `#[umbral(noedit)]` / `#[umbral(noform)]` actually block admin writes, so a deployment that registers `auth_user` in the admin marks `is_superuser`/`is_staff` `#[umbral(noedit)]` and they can't be escalated through the form. Per-requester field rules (only-superusers-may-edit) are an application-level policy, not framework default behaviour.
**Severity: medium** (high if a non-superuser holds `change_auth_user`)

- **File:** `plugins/umbral-auth/src/lib.rs:235-237` (model), `examples/shop/src/main.rs:114-119` (admin registration), `plugins/umbral-admin/src/handlers/crud.rs:329-339` (update guard)
- **Evidence:** `AuthUser.is_staff`/`is_superuser` are plain `bool` with no `#[umbral(noform)]`/`#[umbral(noedit)]` guard (contrast `password_hash`, which is `#[umbral(noform)]`). The shop registers `auth_user` in the admin. The update handler gates only on the generic `change_auth_user` permission; there is no field-level restriction limiting `is_superuser`/`is_staff` edits to existing superusers (a mature admin layer does exactly this).
...
- **Fix:** Restrict `is_superuser`/`is_staff` (and group/permission M2M) edits to requesters who are already superusers. At minimum document that registering `auth_user` in the admin without this guard is a privilege-escalation vector. (Note: this and WEB-2 are the same class - server-managed fields writable through a generic path.)

## Lower-severity / hardening
- **AUTH-5 (low)** ✅ FIXED (4cf24e1) — CSRF cookie missing `Secure` (`umbral-security/src/lib.rs:172`, `umbral-admin/src/auth.rs:201`): set `Path=/; SameSite=Lax` with no `Secure`, so it's sent over plain HTTP. The session cookie correctly sets `Secure`. Add `Secure` to the CSRF cookie.
- **AUTH-6 (low)** ✓ ALREADY COVERED (tokens_match is constant-time) — Admin login CSRF compare not constant-time (`auth.rs:143` uses `String ==`), whereas the `SecurityPlugin` middleware correctly uses `subtle::ConstantTimeEq` (`umbral-security/src/lib.rs:280-288`). Route the admin's own check through the same.
- **AUTH-7 (low/info)** 🚫 CLOSED — by design (double-submit-cookie + SameSite=Lax is a sound, standard CSRF design; session-binding is optional hardening) — CSRF is double-submit-cookie, not session-bound (`umbral-security/src/lib.rs:19-31`): the token lives in a non-HttpOnly cookie compared against a header/form field, not tied to the session. If an attacker can set a cookie on the victim's domain (subdomain takeover, sibling-host MITM), the pattern can be defeated; `SameSite=Lax` is the real backstop. Consider binding the token to the session, as the module's own deferred note acknowledges.
- **AUTH-8 (info)** ⏳ DEFERRED — Expiry is enforced but cleanup is lazy: expired sessions deleted only on read (`umbral-sessions/src/lib.rs:245-251`); bearer tokens have **no `expires_at`** (`token.rs:67-92`) and live until revoked. Logout deliberately does not revoke bearer tokens (`auth_routes.rs:333-339`); a password change (`set_password`, `lib.rs:669-685`) does **not** invalidate other sessions/tokens. A stolen bearer token is valid forever; a password reset doesn't lock out an attacker's existing sessions.

## Done well
- **Password hashing correct:** argon2 0.5 with per-password `SaltString::generate(&mut OsRng)`, PHC-encoded, verified via constant-time `Argon2::verify_password` (`umbral-auth/src/lib.rs:529-548`).
...
- **Session fixation defended:** `login_user_id` destroys the pre-auth session row and mints a fresh token before writing the authenticated session (`umbral-sessions/src/lib.rs:431-474`); the layer skips overwriting the rotated cookie.
