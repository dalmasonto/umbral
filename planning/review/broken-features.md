...
**Severity: high** · **Verified** (`claim_one` SELECTs `status='pending'` then UPDATEs `WHERE id = ?` only; no `FOR UPDATE SKIP LOCKED` anywhere in `crates/umbral-core/src`)

- **File:** `plugins/umbral-tasks/src/lib.rs:372-417` (`claim_one`)
- **Evidence:** `SELECT … WHERE status='pending' … LIMIT 1` then `UPDATE … WHERE id = ?` inside `umbral::transaction`, with the comment: *"Wrapped in a transaction so a concurrent worker can't double-claim — SQLite's single-writer model already guarantees this, but the explicit transaction keeps the contract correct for the Postgres backend."* There is **no `FOR UPDATE`/`SKIP LOCKED`** in the ORM, and the UPDATE filters on `ID` only, not `status='pending'`.
...
- **File:** `crates/umbral-core/src/signals.rs:189-220` (also `:155, :169, :443`)
...
- **File:** `plugins/umbral-tasks/src/lib.rs:321-334`
...
## BROKEN-5 — umbral-signals typed handlers silently never fire on deserialize failure
> **✅ FIXED** (`23ad8b0`) — typed handlers log a warning on decode failure instead of `.ok()` swallow.
**Severity: medium**

- **File:** `plugins/umbral-signals/src/lib.rs:145-149` (same at `:172, :201, :229`); core mirror at `signals.rs:233-235`
...
## BROKEN-6 — umbral-email `send()` panics on the console backend when settings aren't initialised
> **✅ FIXED** (`23ad8b0`) — email uses `settings::get_opt`, treats unknown env as not-Dev.
**Severity: medium**

- **File:** `plugins/umbral-email/src/lib.rs:440` vs `:357-362`
- **Evidence:** `load_config` deliberately re-parses `Settings::from_env()` because the ambient `settings::get()` *"would panic before `App::build()` runs"*. But `send()`'s non-Dev warning calls `umbral::settings::get().environment` directly — which is `.expect("umbral: settings not initialised — did you call App::build()?")` (`settings.rs:28`).
...
- **Fix:** Use `umbral::settings::get_opt()` and treat `None` as Dev (or warn that environment is unknown).
...
- **File:** `plugins/umbral-cache/src/cache_page.rs:177-186`
...
- **File:** `crates/umbral-core/src/forms.rs:862-876`
...
- **File:** `plugins/umbral-cache/src/lib.rs:74-75, 466-484`
...
- Signals: typed `m2m_changed`, `disconnect`, cross-process broadcast — umbral-signals module doc deferrals.
