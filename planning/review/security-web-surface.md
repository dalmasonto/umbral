...
Scope: `umbral-rest`, `umbral-admin`, `umbral-playground`, `umbral-openapi`, `umbral-media`, `umbral-static`, and the MiniJinja integration in `umbral-core`. This file holds the only **critical** in the review.
...
- **File:** `plugins/umbral-rest/src/lib.rs:184-188` (`permission_for` → `AllowAny`), `:243` (`NoAuthentication`), `:73`/`:693-710` (`allow` / `DEFAULT_BLOCKED_TABLES`)
- **Evidence:** `RestPlugin::default()` sets `authentication: NoAuthentication`; `permission_for()` returns `AllowAny` for any table without an explicit `.permission(...)`. `allow()` returns `true` for every table except `["auth_user", "session", "umbral_migrations"]`. So `gate()` → `AllowAny.check()` → `Ok(())` for List/Retrieve/Create/Update/Delete.
...
**Severity: high** · **Verified** (`crates/umbral-core/src/orm/dynamic.rs:1039` and `:1290` strip only `col.noform`; no `noedit` branch anywhere on the write path)

- **File:** `plugins/umbral-rest/src/lib.rs:1257-1317` (`create`/`update` → `insert_json`/`update_json`); `crates/umbral-core/src/orm/dynamic.rs:1016-1047`, `:1275-1298`
...
- **Attack path:** A custom user/profile model with `is_superuser` / `is_admin` / `balance` marked `#[umbral(noedit)]` (or REST-`hide()`d) but not `#[umbral(noform)]` is writable via `PATCH /api/profile/1 {"is_superuser": true}`. Combined with WEB-1, this is **unauthenticated privilege escalation**.
...
- **File:** `plugins/umbral-media/src/lib.rs:131-178` (`save`), `:190-210` (`routes`/`ServeDir`)
...
- **File:** `plugins/umbral-rest/src/lib.rs:1065-1069`
...
- **Fix:** Return a generic message in prod, log detail server-side (as `umbral-admin/src/util.rs::sanitise_form_error` already does), gate any detail behind `Environment::Dev`.
...
- **File:** `plugins/umbral-playground/src/routes.rs:130-148`, `plugins/umbral-playground/src/lib.rs:118-124`
...
- **File:** `plugins/umbral-admin/src/handlers/actions.rs:116-236` (`dispatch_action`), `plugins/umbral-admin/src/handlers/fk_picker.rs:35` (`fk_options`)
...
- **Attack path:** With `umbral-permissions` installed, a staff user lacking `change_<model>`/`delete_<model>` can still trigger bulk actions, and can enumerate any model's rows via FK-autocomplete regardless of `view` permission.
- **Fix:** Add `permcheck::require` (Change/Delete for the action, View for `fk_options`) before dispatch.

## Lower-severity notes
- **No CSRF on REST mutations** (`plugins/umbral-rest/`): token-auth APIs don't need it, but a developer wiring session-cookie auth for REST (plausible — the session plugin is shared) makes all `POST/PUT/PATCH/DELETE` CSRF-able. Document that cookie-authenticated REST resources need CSRF or `SameSite` enforcement.
...
## Done well
- Template autoescaping on by default for `.html`/`.htm` in both engines (`crates/umbral-core/src/templates.rs:277-283`, `engine.rs:20`); no `| safe` anywhere in admin templates.
...
- OpenAPI documents only REST-exposed models (gates on `umbral_rest::is_exposed`); dev-only endpoint hints gated to `Environment::Dev`.
- Sensitive built-ins (`auth_user`, `session`, `umbral_migrations`) blocked from REST by default. Admin login is timing-safe.
