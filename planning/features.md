...
85. [ ] **inspectdb data migration: rename-aware, PK-preserving transfer from the source DB** 🟡 Medium
    ...
    > **PHASE 1 SHIPPED (2026-08-16):** `umbral transferdata --from <url> --to <url>` — a resumable, PK-preserving, streaming row copy between two umbral DBs (`crates/umbral-core/src/transfer.rs`, facade `umbral::transfer`). Confirmed the foundation: `insert_json` already binds any real PK (only integer *sentinel* 0/nil-uuid/empty is omitted → autoincrement), so ids round-trip; a new `DynQuerySet::fetch_as_json_on(&pool)` reads a batch from an explicit source pool (unredacted, no M2M echo), `insert_json_in_tx` writes PK-preserved. FK-topological order (parents first, `fk_topo_order`), keyset pagination (never OFFSET), and a per-batch transaction that commits inserts + a `umbral_transfer_state` checkpoint together (crash-safe, exact resume, no dup/PK-conflict). PG sequence reset on table done. Tests: `tests/transfer.rs` (PK+FK preserved across two DBs, FK order, idempotent rerun). Doc: `cli/transferdata.mdx`. Design: `docs/decisions/2026-08-16-data-transfer-engine.md`. **Phase 2 (still open):** M2M junction rows (not registered models — enumerate from `M2M_RELATIONS`), cross-backend SQLite→PG, parallel per-table workers, a `--map` layer reusing inspectdb's rename map for foreign-shaped sources, circular-FK deferred handling, and the `auth_user` password-hash caveat below.
    >
