...
1. [x] **A logs plugin like django-logs** — SHIPPED 2026-06-20: `umbral-logs` plugin — `RequestLog` model + non-blocking fire-and-forget capture layer (`wrap_router`) recording method/path/status/duration/ip/user-agent, path-prefix exclusions + deterministic `sample_rate` + `min_status`, and a read-only `admin_model()` helper for browsing in the admin (`250e984`). Follow-up: `user_id` is header-based (`X-Umbral-User-Id`) to keep the plugin dep-light; auto-resolving it from the request `Identity` extension when auth is mounted is a low-risk enhancement.
...
37. [x] **`earliest()` / `latest()` — convenience wrappers** 🟢 Low
       — Shipped. `QuerySet::earliest("col_name")` = `order_by(col.asc()).first()`; `latest("col_name")` = `order_by(col.desc()).first()`. Takes a `&'static str` column name to match Django's call shape. Tests in `crates/umbral-core/tests/earliest_latest_distinct.rs`.
...
43. [x] **Background task queue (`umbral-tasks`)** 🔴 High — SHIPPED: `umbral-tasks` is a full DB-backed queue — `#[umbral::task]`, `enqueue`, `run_worker`, priority queues, retries, `task_status`/`await_result`. (Remaining Celery gaps tracked in #82.)
...
53. [ ] **Admin bulk actions** 🟡 Medium
    > Why: Checkbox-select rows, then "Delete selected", "Mark as published", "Export to CSV." Django's admin is powerful because of bulk actions. Currently Umbral admin only handles one row at a time.
    >
...
63. [x] **API versioning** 🟢 Low — SHIPPED: umbral-rest DRF-style versioning — `VersioningScheme::UrlPath` + `AcceptHeader`, `RequestContext::version` (opt-in via `.versioning(...)`).
...
82. [ ] **umbral-tasks: remaining Celery gaps — per-task persistence, admin visibility (priority queues DONE)** 🟡 Medium
...
83. [ ] **Custom model validation / sanitization hooks** 🟡 Medium
    > Why: A way to add app-specific field rules on top of the declarative `#[umbral(min_length/email/…)]` validators — content moderation, normalization, format — written in code, like Django's `clean_<field>` / DRF `validate_<field>`. The rules live in the app; the framework provides only the hook (no built-in word lists).
    >
