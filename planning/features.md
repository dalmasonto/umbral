...
39. [~] **Email sending — SMTP and API backends** 🔴 High — PARTIAL: SMTP (lettre) + console + template-driven transactional email shipped via `umbra-email`; HTTP/API backend shipped behind the optional `api` cargo feature (`reqwest`, rustls) covering **Resend** (`POST /emails`) and **SendGrid** (`POST /v3/mail/send`) — provider request mapping lives in the pure, unit-tested `build_api_request(provider, key, msg, default_from) -> ApiRequest{url, bearer, body}` (HTTP construction tested with no network), the `reqwest` POST + non-2xx→`EmailError::ApiResponse{status, body}` mapping wraps it. Selection order: `UMBRA_EMAIL_BACKEND=api` / `email_api_provider`+`email_api_key` → API; else `email_smtp_host` → SMTP; else console (console-refuses-in-prod safety preserved). Settings `email_api_provider` (`resend`/`sendgrid`) + `email_api_key`; `email_default_from` fallback applies; attachments ride as base64 in the JSON. Remaining: Mailgun/SES providers not yet.
