...
7. [x] **Admin dashboard widgets** 🟡 Medium — SHIPPED 2026-07-13. The widget system already had 10 kinds (KPI/card/line/bar/donut/radial/heatmap/progress/table/feed, charts via ApexCharts) and `WidgetSection` grouping. What was missing was the "intelligent" half this entry asks for, and it landed as three commits:

    **Declarative filters** (`901b44e8`). The admin's only filter used to be a `[7d][30d][90d]` chip strip HARDCODED inside `line.html` — eight of the ten kinds could not be filtered at all, and no widget could offer anything but a period, because the strip was markup rather than data. A filter is now declared on the widget and rendered by the admin for every kind: `WidgetFilter::period(..)` (chips), `::date_range()` (two date inputs), `::choice(key, label, opts)` (a `<select>`, read back via `params.choice(key)`). Three kinds because each reaches the closure by a different route (`params.period` / `start`+`end` / `raw[key]`), so the template never guesses which query param a control writes. A chosen filter is STICKY (persisted per user, generalising what period chips already did, with a read-fallback to the legacy `widget_periods` map so nobody's saved period resets on upgrade), and each control CARRIES its neighbours' values in its URL — otherwise clicking "30d" on a widget filtered to `status=paid` would silently drop the status and the strip would lie about what you are looking at. A half-specified date range is ignored rather than guessed.

    **CSV export** (`1fd9012a`). Every widget with rows behind it (line/bar/donut/radial/heatmap/progress/table) exports from its header. It runs the widget's own closure with the SAME resolved filters on screen — an export that ignored them would hand someone a spreadsheet that disagrees with the chart, and they would trust the spreadsheet. Shares `gate_widget` with the data endpoint rather than re-implementing the permission checks (two copies of a security check is how one drifts into a bypass). KPI/feed export as 404, not an empty file. The writer neutralises spreadsheet-formula injection: a cell starting `=`/`+`/`-`/`@` is executed by Excel and Sheets, so a value someone typed into your app becomes code on the machine of whoever opens the export.

    BREAKING: `Widget` gained a `filters` field; use the new `Widget::new(..)` builder instead of a struct literal so the next field is free. Docs: `admin/widgets.mdx` (Filters / Export / Reordering / Reports).

    Still open from the original ask: **user-generated cards from the admin UI** ("recording the filters to a given table") and **cross-table widgets** (customers against sales) — the declarative filter model is the substrate those need, but neither is built. Logged as a follow-up rather than claimed here.
...
8. [x] **Widget reordering (per-user)** 🟢 Low — SHIPPED 2026-07-13 (`f0179a99`). The storage half already existed and was a **no-op**: `PUT /api/dashboard/layout` had been persisting a `Vec<WidgetInstance>` to `AdminUserPref.dashboard_layout` for months and *nothing ever read it*, so the dashboard always rendered in registration order at default spans — saving a layout appeared to work and silently did nothing on the next page load. This is the read side (`view::apply_saved_layout`) plus the drag UI (native HTML5 DnD in `admin.js`; no new dependency, the whole cell is the handle).

    Rules that matter: a widget the layout does not mention keeps its registration order and lands *after* the placed ones (so a newly-installed plugin's widget appears rather than vanishing because it was missing from a layout saved last month); a stale entry renders no ghost cell; a layout is a PREFERENCE, never a permission, so it cannot conjure a widget the permission filter already removed; and a corrupt layout blob costs you your arrangement, not your dashboard.
...
9. [x] **GraphQL plugin** 🟡 Medium — SHIPPED 2026-07-13 as `umbral-graphql`. **The proposed approach below was rejected.** Converting the OpenAPI spec produces `getPost` / `listPosts` — GraphQL in name only, and nobody adopts GraphQL to make the same call with different syntax. They adopt it to traverse a graph. umbral ALREADY has the graph: `Column::fk_target` names the table an FK points at, inverting those edges gives every reverse relation, and it is the same model registry `typegen`/`gen-client`/OpenAPI already read. So the schema is derived from `ModelMeta` directly, with async-graphql's `dynamic` builder (the models are known at RUNTIME, so a derive-macro crate could not fit).

    **Reads.** Forward FK, reverse FK, list + by-id, DataLoader batching (a test asserts a 3-post/2-author list costs 2 reads, not 4 — the client picks the query shape, so the client picks your query count), deny-by-default exposure (an unexposed model is absent from queries, relations AND introspection), `hide()` per field, a 200-row cap, GraphiQL in Dev only.

    **Writes.** `.mutable(table)` / `.mutable_if(...)` → create/update/delete, a SECOND opt-in on top of `expose` (a read you got wrong leaks data; a write you got wrong destroys it). Everything goes through the ORM's dynamic write path, which is the only way it inherits the mass-assignment guard, validators, cleaners, defaults and signals. A `#[umbral(privileged)]` column is not merely stripped from a mutation body — it is ABSENT from the input type, because an input field that silently does nothing is worse than no input field.

    **CSRF.** Mutations made the `/graphql` CSRF exemption a hole (a hostile page submits an HTML `<form>`, the browser attaches the session cookie, the write lands). Every POST must now be `Content-Type: application/json`: a form can only send three content types and none is JSON, and a cross-origin fetch that sends JSON gets preflighted — so the set of forgeable requests is exactly the set refused. Enforced as a route LAYER, because in the handler the `GraphQLRequest` extractor rejected a multipart body first, which meant CSRF was being decided by an extractor that knows nothing about CSRF.

    **Pagination.** Relay cursor connections (`postsConnection(first:, after:, orderBy:, desc:)`). OFFSET is positional, so a delete behind the page boundary makes page 2 start one row late and the row that moved into that slot is never served to anyone — silently. Ties break on the primary key, the cursor encodes the ordering it was minted under, `hasNextPage` costs one extra row. `typed_cmp_condition`/`Cmp` went into the ORM rather than the plugin.

    **Subscriptions.** `.subscribable(table)` → `<model>Changed(id:)` / `<model>Deleted`, over SSE (`POST <path>/sse`) and WebSocket (`GET <path>/ws`), fed by the ORM's write signals — so a row changed by a mutation, a REST call, an admin form or a background task all reach subscribers. The event carries ONLY the primary key and the row is re-read through the ORM: the signal payload is a serde dump of the model and knows nothing about `private`/`secret`/`Masked`/`hide`, so forwarding it would defeat the whole field policy merely because the bytes left over a socket.

    28 tests across 4 suites. Docs: `graphql/index.mdx`. Gaps: gaps3 #70/#71 (field privacy), #72 (pagination), #73 (subscriptions). NOT on crates.io yet — 0.0.7 shipped before the plugin existed, so it is path-dep only until 0.0.8. Original (rejected) proposal below.
...
79. [x] **Factory testing library** — SHIPPED. Aim was "make it easy to write tests that simulate realistic plugin submissions and model updates". `crates/umbral-testing` already had the data half: a `Factory` trait (`build` / `create` / `create_with` / `create_batch`) on a local marker type (the orphan rule is why it hangs off `PluginFactory` rather than `Plugin`), `seq()` for unique columns, a re-exported `fake`, plus `TempPool` / `TestClient` / `TestResponse`.

    What was missing was the SETUP half, and it was the expensive half. Two additions:

    - **`create_tables()`** — the test schema is now derived from the models, via the migration engine (`Snapshot::current()` → `diff` → `render_operation_for`), i.e. the same `ModelMeta` the ORM reads and the same renderer `migrate` uses in production. It replaces the hand-written `CREATE TABLE` that **266 test files** in this repo still carry. That hand-written schema is a *second source of truth* and it drifts: add a column to the model and the test's table silently lacks it, the ORM queries a column that isn't there, and any error-swallowing on the path (`unwrap_or(false)`) turns the failure into a test that PASSES WITH THE WRONG ANSWER. Not hypothetical — a soft-delete column drifting out of a fixture is exactly what made `plugin_directory`'s authorization test report a user was already a moderator when they were not (see `146c34a8`).
    - **`boot(|b| ...)`** — `App::build()` initialises process-wide state and panics if it runs twice, so every test file reinvents a `OnceCell` + `Mutex` dance and gets it subtly wrong in different ways. This is that dance, once, in the library: first call builds, later calls are no-ops, so each `#[tokio::test]` just says what it needs. A plugin's own models get their tables without the test listing them (asserted by `tests/plugin_tables.rs`: `AuthPlugin` → a working `auth_user`, with no schema in the test file).

    Setup went from ~40 lines per test file to `umbral_testing::boot(|b| b.model::<Note>()).await;`. Docs: `testing/setup.mdx` (new), `testing/factories.mdx`, `testing/test-client.mdx`.

    **Follow-up, not done:** migrating the 266 existing hand-written test schemas onto `create_tables()`. Mechanical but large, and each one is a chance to discover a drift bug that was hiding — worth doing in its own pass rather than bundled here. Logged as gaps3 #78.
...
84. [x] **Template traversal of reverse-FK one-to-many lists in the `user` context (0.0.12)** 🟢 Low — SHIPPED. Per-relation opt-in, the design the user picked: `AuthPlugin::with_user_in_templates().expand_list::<Order>()` records the child table; `user_context_layer` (now `from_fn_with_state`, carrying the opt-in list as `Arc<Vec<String>>`) threads it into `expand_relations`, which injects up to `USER_REVERSE_FK_LIST_CAP = 20` child rows — those whose FK points at this row — under `user.<child_table>_set`, scoped by FK target so a list only appears where the FK actually lands. Items are flat (one query per opted-in list per level, no per-item recursion). Not auto-injected: a child list is an unbounded per-request query, so you name the ones worth the cost. Test: `plugins/umbral-auth/tests/reverse_fk_list.rs` (real user + 25 purchases → `user.purchase_set` renders capped at 20 through the real session+middleware+render path; anonymous → no list). Docs: `auth/user-in-templates.mdx` (the opt-in section). Builder + field on `AuthPlugin` (`expand_list::<M: Model>`, `expand_lists: Vec<String>`).

    > (Original)
    > The rest of gap2 #14 is already shipped: `serialize_authenticated_with_relations` / `expand_relations` in `plugins/umbral-auth/src/session_user.rs` expand **forward FKs** and **reverse-OneToOne** children into the serialized `user` up to `USER_RELATION_DEPTH = 2` hops, opt-in via `AuthPlugin::with_user_in_templates()`, with `(table, pk)` cycle detection. So `{{ user.customer.loyalty_points }}` and `{{ user.customer.address.city }}` already work.
    >
    > What's missing: **reverse-FK one-to-many lists** — the *list* of children pointing at the user through a **non-unique** FK, e.g. `{{ user.order_set }}`. It was deliberately left out because it's an unbounded per-request query: a user with 50k orders would load all of them into every authenticated template render, and the whole `with_user_in_templates` contract is "one query per single relation per request" (a bounded, honest cost). A list breaks that bound.
    >
    > Design decisions to settle before building (this is the "work on it" part):
    > - **Row cap.** A hard `LIMIT N` (e.g. 20) so the injected list can never be unbounded. Probably a const like `USER_REVERSE_FK_LIST_CAP`, mirroring `USER_RELATION_DEPTH`. Log when truncated.
    > - **Key naming.** `user.order_set` (Django idiom) vs `user.orders` (pluralized table). The reverse-O2O path already keys on the child *table* name; a list needs a distinct suffix so it doesn't collide with a same-named single relation.
    > - **Opt-in vs automatic.** Given the cost, a reverse-FK list arguably should NOT be auto-injected for *every* child model with a non-unique FK to the user — that could be many tables. Options: (a) auto-inject all, capped; (b) a per-relation opt-in declaration on the builder (`with_user_in_templates().expand_list::<Order>()`); (c) leave it Rust-only (resolve in the handler, as the docs now recommend).
    > - **Ambiguity.** A child with multiple non-unique FKs to the user (rare) — skip, like the reverse-O2O ambiguous case.
    >
    > Where: `plugins/umbral-auth/src/session_user.rs::expand_relations` (add a reverse-FK-list branch after the reverse-O2O branch), a cap const, and a test asserting the capped list appears under the chosen key. Docs already corrected in `documentation/docs/v0.0.1/auth/user-in-templates.mdx` (the "on deck for 0.0.12" callout). Driver: the docs page a user hit that (until now) wrongly claimed forward-FK/reverse-O2O traversal was unimplemented too.

85. [ ] **inspectdb data migration: rename-aware, PK-preserving transfer from the source DB** 🟡 Medium

    > **PHASE 1 SHIPPED (2026-08-16):** `umbral transferdata --from <url> --to <url>` — a resumable, PK-preserving, streaming row copy between two umbral DBs (`crates/umbral-core/src/transfer.rs`, facade `umbral::transfer`). Confirmed the foundation: `insert_json` already binds any real PK (only integer *sentinel* 0/nil-uuid/empty is omitted → autoincrement), so ids round-trip; a new `DynQuerySet::fetch_as_json_on(&pool)` reads a batch from an explicit source pool (unredacted, no M2M echo), `insert_json_in_tx` writes PK-preserved. FK-topological order (parents first, `fk_topo_order`), keyset pagination (never OFFSET), and a per-batch transaction that commits inserts + a `umbral_transfer_state` checkpoint together (crash-safe, exact resume, no dup/PK-conflict). PG sequence reset on table done. Tests: `tests/transfer.rs` (PK+FK preserved across two DBs, FK order, idempotent rerun). Doc: `cli/transferdata.mdx`. Design: `docs/decisions/2026-08-16-data-transfer-engine.md`.
    >
    > **PHASE 2 — M2M junction rows SHIPPED (2026-08-16):** after all models, `collect_junctions` enumerates every junction from `ModelMeta.m2m_relations` (`<parent_table>_<field>`, `(parent_id, child_id)` composite PK) and copies its rows via raw SQL (the junction exception — not a row-level ORM entity), keyset-paginated on the composite key, `ON CONFLICT DO NOTHING` for idempotent resume, through the same per-batch transactional checkpoint. So `Community.software` / `book.tags` links survive the move. Test: `transfer_copies_m2m_junction_rows`.
    >
    > **PHASE 2 — CROSS-BACKEND SQLite↔Postgres SHIPPED (2026-08-16):** the model copy was already backend-dispatched (`fetch_as_json_on` decode + `insert_json_in_tx` coerce), so ids/bool/timestamp/uuid round-trip through JSON to the target's native types. The junction raw path gained a 3-way id classifier (`id_kind`: Int/Uuid/Text) with `read_id_sqlite`/`read_id_pg` + `bind_id_sqlite`/`bind_id_pg` so a UUID junction id decodes/binds natively on each end (pg binds `uuid::Uuid`, not a String). PG target sequence reset is guarded (plain-PK tables no-op). Verified against a LIVE Postgres (`UMBRAL_TEST_POSTGRES_URL`): `transfer_sqlite_to_postgres_cross_backend` copies non-contiguous PKs, a bool (0/1→native), a FK, and an M2M junction from SQLite into PG; checkpoint table shows composite-key junction resume (`book_tags → [10,200]`). Still open: circular-FK deferred handling and the `auth_user` password-hash caveat below.
    >
    > **PHASE 2 — `--map django` + PARALLEL WORKERS SHIPPED (2026-08-16):** `--map django` reads the original Django DB into the umbral target, undoing the reference conventions in reverse — FK column `author_id`→field `author` (via a source-shaped meta + `source_meta_for` key-rename), M2M junction columns `<model>_id`→`parent_id`/`child_id` (`collect_junctions` src cols). Table names unchanged. Closes the loop: inspectdb makes the schema, `transferdata --map django` moves the data. **Parallel workers:** `fk_topo_levels` groups tables into FK levels; within a level up to `--workers N` copy concurrently via `futures_util buffer_unordered` (each `copy_one_model`/`copy_one_junction` owns its txns+checkpoint, so no shared mutable state); parents-before-children holds across levels; junctions run concurrently after all models. Tests: `transfer_map_django_translates_columns`, `transfer_parallel_workers_copies_correctly` (workers=4, batch=1, busy_timeout for concurrent sqlite writers). Cross-backend live test re-verified post-refactor. Docs: `cli/transferdata.mdx`.
    >
    > **PHASE 2 — auth_user PASSWORD-HASH CAVEAT HANDLED (2026-08-16):** a ported DB carries password hashes umbral-auth can't verify (Django `pbkdf2_sha256$…`/`argon2$…`, bcrypt) — `verify_password` *errors* on them (can't parse), so those users can't log in OR get a clean rejection. `umbral_auth::reset_unverifiable_passwords()` + the `resetforeignpasswords` management command scan `AuthUser`, and any hash `argon2 PasswordHash::new()` can't parse is replaced with `random_password_hash()` (a valid argon2 hash of an unknown password → login cleanly returns `false`, recovers via the existing password-reset flow). Idempotent (umbral-native hashes untouched). Test: `plugins/umbral-auth/tests/reset_foreign_passwords.rs`. Docs: transferdata "Porting from Django" callout. Still open: richer `--map` targets, custom-user-model support for the command (AuthUser-only, like createsuperuser).
    >
    > **PHASE 2 — CIRCULAR-FK DEFERRAL SHIPPED (2026-08-16):** a self-referential table (`parent_id` tree, forward refs) or a mutually-referential pair (`Org.lead`↔`Member.org`) can't be inserted in any per-row FK order. Added a `.trusted()` flag on `DynQuerySet` (skips the app-level create validation, incl. the FK-existence SELECT that would reject a cyclic reference; DB constraints still apply) — the transfer uses it on every insert. `fk_topo_plan` splits orderable levels from the cyclic leftover; `copy_cyclic_group` copies the cyclic set in ONE transaction with `defer_foreign_keys` / `SET CONSTRAINTS ALL DEFERRED` so the FK check fires once at commit (every row present). Self-FK tables use the same deferred copy individually. Trade-off: cyclic tables are all-or-nothing (no per-batch resume) — fine for the small tables cycles involve. Postgres needs `DEFERRABLE` constraints (follow-up: emit deferrable FKs). Tests: `transfer_self_referential_forward_reference`, `transfer_mutual_fk_cycle`. Still open: richer `--map` targets, deferrable-FK PG migration support.
    >
    > **Why.** inspectdb gets a developer's *schema* into umbral (models.rs + 0001_initial), but a port isn't done until the *data* moves too. And the transfer isn't a plain copy: inspectdb renames columns (`author_id`→`author`), renames tables (app-prefix strip: `communities_community`→`community`), collapses Django M2M join tables into umbral junctions (feature #86), and can change PK types — so a naive `INSERT … SELECT` won't line up. The developer's premise ("new DB + new migrations, don't reuse the source DB in place") is right for schema *because* the port is structure-changing; the missing piece is a safe data move that survives those renames while keeping every id and FK link intact.
    >
    > **The key insight.** inspectdb already computes the exact old→new mapping (table renames, the `_id` strip, M2M owner/field derivation). That same map is the translation layer a data copy needs — nothing else has to re-derive it.
    >
    > **Design (leanest path first).**
    > - **Emit a `loaddata`-compatible fixture from the SOURCE DB, keyed by NEW model/field names.** umbral already has `dumpdata`/`loaddata` (Django-style fixtures). A new step — `inspectdb --with-data` or a companion `transferdata <source>` command — reads each source table and writes a fixture whose model + field names are the *generated* ones (applying the rename map), so the existing `loaddata` imports it into the fresh DB. No new copy engine; reuse the tested fixture path.
    > - **Preserve primary keys (load-bearing).** Insert each row with its ORIGINAL explicit id, not an autoincrement-assigned one, so every FK reference stays valid with no remap pass. REQUIRES verifying `loaddata` (and the create path) can force a PK onto an autoincrement column — Django's loaddata does this; confirm umbral's does, and add it if not (`insert_json` with an explicit PK value, bypassing the auto-PK). This is the first thing to check before speccing the rest.
    > - **Order + reset.** Insert parents before children — reuse the migration engine's existing FK-graph topological order (`plugin ordering` / cross-plugin FK ordering already exists). After inserting explicit ids on Postgres, `setval()` each sequence to `max(id)` so future inserts don't collide; SQLite AUTOINCREMENT similar via `sqlite_sequence`.
    > - **M2M rows.** Copy the Django join-table rows (`communities_community_software`) into umbral's junction (`community_software`) via the same rename map (#86 gives the owner/field/junction naming).
    >
    > **The one table that can't copy verbatim: users.** Django's `auth_user` password hashes won't validate under umbral-auth's scheme (different hasher/format). Surface this explicitly — either skip `auth_user` rows with a loud warning ("re-create users / trigger password resets"), or offer a documented re-hash shim — rather than silently importing hashes that will never authenticate. Ties to why "AuthUser is a migration, not reuse" (the developer's own reasoning).
    >
    > **Caveats to handle:** type coercions (Django bool-as-int, datetime, JSON) — mostly covered by umbral's type mapping; NULL vs default; very large tables (stream/batch the fixture rather than loading all rows into memory); a `--dry-run` that reports row counts per table before writing.
    >
    > **Where:** `crates/umbral-core/src/inspect.rs` (owns the rename map) + the `dumpdata`/`loaddata` CLI path in `umbral-cli`. First deliverable is the PK-preservation check on `loaddata`, then the source-DB fixture emitter. Driver: the radar + portfolio ports (real Django DBs already introspected).

86. [x] **inspectdb: recover Django M2M join tables as `M2M<T>` fields** 🟡 Medium — SHIPPED. A Django auto-generated M2M through table (`communities_community_software` — a pure junction: exactly two FK columns plus the surrogate `id` PK, nothing else) was landing as a *plain* `#[derive(Model)]` struct, so `Community.software` / `Community.categories` M2M relations were silently lost. `detect_m2m_relations` (Django-only, `crates/umbral-core/src/inspect.rs`) now folds each junction into an `M2M<Target>` field on the owner and drops the join table from the schema — umbral auto-generates its own junction. The owner is the FK target that prefixes the join-table name (`<owner_table>_<field>`), the remainder is the field name (`pick_m2m_owner`, collision-guarded against an existing owner column and skipping an `auth_user`-owned junction). A non-`i64` owner PK spells out the parent-PK generic (`M2M<Software, i32>`, `M2M<Group, uuid::Uuid>`). The initial migration carries it both ways: `ModelMeta.m2m_relations` in the snapshot + an `Operation::CreateM2MTable` per relation (junction `<parent>_<field>`, parent/child PK col+type resolved from the schema). Verified on the real radar (3 M2M) + portfolio (12 M2M, incl. `moderators: M2M<AuthUser, i32>`) Django DBs — join-table structs gone, models compile. Tests: `inspectdb_django_folds_m2m_join_tables`. Docs: `migrations/inspectdb.mdx`. Still deferred: a non-surrogate (composite-PK) through table, and a M2M whose *owner* is `auth_user` (`auth_user_groups`) — left as plain structs.
