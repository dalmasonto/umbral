...
19. [x] **`prefetch_related()` — M2M and reverse-FK batch loading** 🟡 Medium
       — M2M batching shipped. `QuerySet::prefetch_related("tags")` / `prefetch_related_many(&[...])` issue one batched JOIN through the junction table for every parent, group results by `parent_id`, and populate each parent's `M2M.resolved` slot via the new `HydrateRelated::set_m2m_resolved_json` hook. Macro override emits the per-M2M-field arms; new `HydrateRelated::pk_i64` hook (also macro-emitted, only for i64-PK models) feeds the parent-id collection. v1 constraints: M2M only, i64 parent PK only — same as the rest of the M2M plumbing. Tests in `crates/umbra-core/tests/prefetch_related.rs` (3 tests). **Now closed** (this note predated it): reverse-FK collection prefetch (`prefetch_related("comment_set")`) shipped (gaps.md #114); the PK-agnostic hydration refactor (`pk_as_json`) added the Vec-on-parent slot and lifted the i64-only constraint for the reverse path, and the regression tests in `crates/umbra-core/tests/reverse_fk_prefetch.rs` (orm_fixes #1) confirm both single and multi reverse-set prefetch populate. Both halves done.
...
48. [~] **Structured logging / OpenTelemetry** 🟡 Medium
    ...
    >
    > **Foundation shipped** (`umbra-logs::observability`): a set-once `init(ObservabilityConfig::from_env()) -> ObservabilityGuard` helper builds a `tracing_subscriber::registry()` with an `EnvFilter` (`RUST_LOG`, default `info`) and a fmt layer that switches to **structured JSON** on `UMBRA_LOG_FORMAT=json`. Behind the optional `otel` cargo feature it adds a `tracing-opentelemetry` layer exporting over **OTLP/gRPC** to `OTEL_EXPORTER_OTLP_ENDPOINT` (default `http://localhost:4317`, `service.name` from `OTEL_SERVICE_NAME`/config) via a batch span processor; the guard flushes + shuts the exporter down on drop. The base build (no `otel`) pulls none of the otel/tonic deps and degrades to fmt/JSON logging. umbra-core now mounts a `tower_http::TraceLayer` so every request runs in an `http.request` span (`http.method`/`http.route`/`http.status_code`) that the OTel layer exports — one span per request out of the box. Version set pinned deliberately: `opentelemetry`/`opentelemetry_sdk`/`opentelemetry-otlp` 0.32 + `tracing-opentelemetry` 0.33. Scaffold + `examples/shop` wired to `observability::init`. Span export proven without a live collector via the in-memory OTLP exporter test.
    >
...
64. [x] **Multi-tenancy** 🟢 Low — SHIPPED (2026-06-24): `umbra-tenants` delivers schema-per-tenant (one DB, schema/tenant, zero-round-trip routing) AND database-per-tenant (`TenantStrategy::Database`); subdomain/header resolution, `tenant_apps`/`tenant_app(&plugin)`, `create_tenant`/`register_tenant_database`, `migrate_schemas`. Row-level isolation via `umbra-rls` (Postgres RLS). Built on the `DatabaseRouter` foundation (gaps2 #69). Docs `plugins/tenancy.mdx`, example `examples/starknet-explorer`, live-PG-verified.
...
82. [ ] **umbra-tasks: remaining Celery gaps — per-task persistence, admin visibility (priority queues DONE)** 🟡 Medium
    > Periodic/cron "beat" scheduling SHIPPED (a `PeriodicTask` model + `Schedule::cron`/`Schedule::every` + `run_beat`/`tasks-beat`, with an atomic optimistic-claim guard so multiple beat instances can't double-enqueue; see `plugins/umbra-tasks/src/lib.rs` and the tasks doc page). Reliability (eta/delay `run_at`, exponential-backoff retries, per-task timeout, visibility-timeout orphan reclaim) shipped earlier. Result backend + task-status API SHIPPED (this revision): an additive `result` column on `TaskRow`, a generic `register_handler<F, Fut, R: Serialize>` that serializes the handler's return value on success (`Ok(())` stays source-compatible → `null`), and `task_status(id) -> Option<TaskStatus>` / `await_result(id, timeout)` (Celery `AsyncResult` / `.get()`). What's left to call the tasks plugin full Celery parity:
    >
...
83. [ ] **Custom model validation / sanitization hooks** 🟡 Medium
    > Why: A way to add app-specific field rules on top of the declarative `#[umbra(min_length/email/…)]` validators — content moderation, normalization, format — written in code, like Django's `clean_<field>` / DRF `validate_<field>`. The rules live in the app; the framework provides only the hook (no built-in word lists).
    >
