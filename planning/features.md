...
85. [ ] **inspectdb data migration: rename-aware, PK-preserving transfer from the source DB** 🟡 Medium
    ...
    >
    > **PHASE 2 — M2M junction rows SHIPPED (2026-08-16):** after all models, `collect_junctions` enumerates every junction from `ModelMeta.m2m_relations` (`<parent_table>_<field>`, `(parent_id, child_id)` composite PK) and copies its rows via raw SQL (the junction exception — not a row-level ORM entity), keyset-paginated on the composite key, `ON CONFLICT DO NOTHING` for idempotent resume, through the same per-batch transactional checkpoint. So `Community.software` / `book.tags` links survive the move. Test: `transfer_copies_m2m_junction_rows`. Still open: cross-backend SQLite→PG (a UUID junction id needs the native pg type — the raw junction read/binds i64|String today), parallel per-table workers, a `--map` layer reusing inspectdb's rename map for foreign-shaped sources, circular-FK deferred handling, and the `auth_user` password-hash caveat below.
    >
