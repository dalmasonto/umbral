...
10. [ ] **GraphQL has no depth/complexity budget.** Anonymous caller amplifies one request into tens of thousands of DB queries via nesting/aliases. (codex #05; claude triage graphql/schema.rs:439/456)
...
17. [ ] **Storage media is public by default** unless an access policy is set. (codex #10)
...
20. [ ] **REST rustdoc still describes the old unsafe defaults** (pre safe-by-default). (codex #13)
21. [ ] **Redis cache `clear()` uses `FLUSHDB`** → wipes co-tenant keys in a shared Redis. (codex #15)
