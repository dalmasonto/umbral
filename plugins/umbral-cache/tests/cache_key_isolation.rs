...
//!
//! 2. **Session-cookie bypass** — a request carrying an `umbral_session` cookie
...
/// A request that carries `umbral_session=<value>` bypasses the page cache
...
async fn session_cookie_request_is_not_served_from_cache() {
    ...
    // Now send a request bearing an umbral_session cookie.  Even though an
...
/// detected correctly (e.g. `Cookie: pref=dark; umbral_session=tok; lang=en`).
#[tokio::test]
...
/// A cookie whose name merely contains `umbral_session` as a substring but is
/// not exactly `umbral_session=` must NOT trigger the bypass.
#[tokio::test]
async fn non_session_cookie_does_not_bypass_cache() {
    ...
    // `not_umbral_session=abc` is a different cookie; must not bypass.
