...
use umbra::orm::Model;
use umbra_auth::login_required::{current_session_user_pk, resolve_user};
use umbra_auth::{AuthPlugin, UserModel, hash_password, set_password};
use uuid::Uuid;
...
async fn boot_uuid() {
    BOOT_UUID
        .get_or_init(|| async {
            ...
            let settings = umbra::Settings::from_env().unwrap();
...
fn header_map_with_cookie(plaintext: &str) -> http::HeaderMap {
    ...
    headers.insert(
        http::header::COOKIE,
        format!("umbra_session={plaintext}").parse().unwrap(),
    );
    headers
}
...
async fn set_password_updates_the_correct_uuid_keyed_row() {
    ...
    let reread = UuidUser::objects()
        .filter(umbra::orm::Predicate::<UuidUser>::col_eq("id", user.id))
        .first()
        .await
        .expect("re-read")
