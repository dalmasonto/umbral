...
async fn boot() {
    BOOT.get_or_init(|| async {
        ...
        let db_path = tmp.path().join("umbra_login_required.sqlite");
        std::mem::forget(tmp);
        ...
        // Create session table (mirrors umbra-sessions schema).
        sqlx::query(
...
/// the cookie). Stores `sha256(token)` in the DB, same as umbra-sessions.
...
fn req_with_cookie(path: &str, cookie: Option<&str>) -> Request<Body> {
    ...
    if let Some(c) = cookie {
        builder = builder.header("cookie", format!("umbra_session={c}"));
    }
