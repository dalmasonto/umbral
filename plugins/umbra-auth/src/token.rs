...
//! Each token is a long random string with a `umbra_` prefix; only
...
/// is an umbra bearer token (the same trick GitHub uses with `ghp_`).
/// Also lets log scrubbers grep for accidentally-committed tokens.
pub const TOKEN_PREFIX: &str = "umbra_";
...
/// randomness, URL-safe base64 encoded, with the `umbra_` prefix.
...
mod tests {
    ...
    fn digest_is_deterministic_and_unique() {
        let a = digest_token("umbra_AAAAA");
        let b = digest_token("umbra_AAAAA");
        let c = digest_token("umbra_BBBBB");
