...
//!    response HTML contains the sidebar markup (id="umbra-admin-sidebar").
...
async fn boot() -> &'static axum::Router {
    BOOT.get_or_init(|| async {
        let settings =
            umbra::Settings::from_env().expect("figment defaults always load in test env");
...
async fn login_with_valid_creds_sets_session_and_redirects() {
    ...
    assert!(
        new_cookie.is_some_and(|s| s.contains("umbra_session")),
        "login should set umbra_session cookie; got {:?}",
        new_cookie
    );
...
async fn changelist_renders_with_sidebar() {
    ...
    assert!(
        body.contains(r#"id="umbra-admin-sidebar""#),
