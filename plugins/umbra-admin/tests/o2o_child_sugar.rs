...
//! `#[umbra(unique)] pub user: ForeignKey<AuthUser>`. The macro
...
/// THE SUGAR — no `ForeignKey`, no `#[umbra(unique)]`, just
...
async fn boot() {
    BOOT.get_or_init(|| async {
        ...
        // constraints — the exact shape `#[umbra(unique)] pub user:
...
async fn sugar_field_spec_emits_unique_fk_column() {
    ...
    // longhand `#[umbra(unique)] pub user: ForeignKey<AuthUser>`
    // would produce.
    use umbra::orm::Model;
