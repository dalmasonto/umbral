...
const BOUNDARY: &str = "X-UMBRA-EDITOR-UPLOAD";
...
fn upload_request(session: Option<&str>, body: Vec<u8>) -> Request<Body> {
    ...
    if let Some(s) = session {
        b = b.header(header::COOKIE, format!("umbra_session={s}"));
    }
...
async fn non_staff_upload_is_forbidden() {
    ...
    let session = umbra_sessions::create_session(Some(uid.to_string()), None)
        .await
