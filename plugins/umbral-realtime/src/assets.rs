...
//! groups — and a small client helper (`umbral.realtime.subscribe`) that
...
pub const REALTIME_WORKER_JS: &str = r#"// umbral realtime shared worker — ONE EventSource per browser over the union of all groups.
"use strict";
...
function reconcile() {
  ...
  currentUnion = union;
  es = new EventSource("__UMBRAL_RT_BASE__/sse?groups=" + encodeURIComponent(union));
...
/// `umbral.realtime.subscribe(groups, handlers)` (signature unchanged):
///
/// - **Preferred**: a `SharedWorker('/realtime/worker.js', 'umbral-realtime')`
...
pub const REALTIME_CLIENT_JS: &str = r#"// umbral realtime client — share ONE SSE connection across tabs (union routing).
"use strict";
(function () {
  window.umbral = window.umbral || {};
  if (window.umbral.realtime && window.umbral.realtime.subscribe) return;
  ...
  function viaWorker(groups, handlers, names) {
    ...
    var worker = new SharedWorker("__UMBRAL_RT_BASE__/worker.js", "umbral-realtime");
  ...
  function viaEventSource(groups, handlers) {
    var mine = splitGroups(groups);
    var es = new EventSource("__UMBRAL_RT_BASE__/sse?groups=" + groups);
    ...
    model: function (name, handlers, opts) {
      ...
      if (!group) {
        if (window.console && console.warn) {
          console.warn("umbral.realtime.model('" + name + "', …): opts.group is required");
        }
        return noSub();
      }
      var routes = {};
      ...
      return window.umbral.realtime.subscribe(group, routes);
    },
...
/// the `__UMBRAL_RT_BASE__` placeholder in the served JS so a `.at("/rt")`
...
pub async fn worker_js_handler() -> impl axum::response::IntoResponse {
    let body = REALTIME_WORKER_JS.replace("__UMBRAL_RT_BASE__", &rt_base());
    (
        [(
...
/// `GET {base}/client.js` — the `umbral.realtime.subscribe` client helper, with
...
pub async fn client_js_handler() -> impl axum::response::IntoResponse {
    let body = REALTIME_CLIENT_JS.replace("__UMBRAL_RT_BASE__", &rt_base());
    (
        [(
            axum::http::header::CONTENT_TYPE,
            "application/javascript; charset=utf-8",
...
mod tests {
    ...
    fn client_defines_subscribe_and_prefers_worker_over_fallback() {
        assert!(
            REALTIME_CLIENT_JS.contains("umbral.realtime"),
            "exposes the umbral.realtime namespace"
        );
        assert!(
    ...
    fn client_defines_model_sugar_delegating_to_subscribe() {
        ...
        assert!(
            REALTIME_CLIENT_JS.contains("window.umbral.realtime.subscribe(group, routes)"),
    ...
    fn client_defines_presence_sugar_delegating_to_subscribe() {
        assert!(
            ...
            "defines umbral.realtime.presence"
        );
        ...
        let delegations = REALTIME_CLIENT_JS
            .matches("window.umbral.realtime.subscribe(group, routes)")
            .count();
        assert!(
            delegations >= 2,
    ...
    fn served_js_templates_the_configured_base() {
        ...
        let worker = REALTIME_WORKER_JS.replace("__UMBRAL_RT_BASE__", "/rt");
        let client = REALTIME_CLIENT_JS.replace("__UMBRAL_RT_BASE__", "/rt");
        ...
        for (name, js) in [("worker", &worker), ("client", &client)] {
            assert!(
                !js.contains("__UMBRAL_RT_BASE__"),
    ...
    fn default_base_is_byte_identical_to_the_legacy_realtime_urls() {
        ...
        let worker = REALTIME_WORKER_JS.replace("__UMBRAL_RT_BASE__", "/realtime");
        let client = REALTIME_CLIENT_JS.replace("__UMBRAL_RT_BASE__", "/realtime");
        ...
        assert!(!worker.contains("__UMBRAL_RT_BASE__"));
        assert!(!client.contains("__UMBRAL_RT_BASE__"));
    }
    ...
    async fn handlers_serve_javascript() {
        ...
        assert!(String::from_utf8_lossy(&body).contains("umbral.realtime"));
