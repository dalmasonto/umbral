//! Integration tests for the umbra-rls plugin.
//!
...
//! - **PG round trip** (`#[ignore]`'d, needs `UMBRA_TEST_POSTGRES_URL`):
...
async fn rls_plugin_applies_policies_on_postgres() {
    ...
    sqlx::query("DROP TABLE IF EXISTS umbra_phase45_post")
        .execute(&pool)
        .await
        .unwrap();
    sqlx::query(
        "CREATE TABLE umbra_phase45_post ( \
    ...
    let policies: Vec<(String, String)> = sqlx::query_as(
        "SELECT policyname, cmd FROM pg_policies \
         WHERE schemaname = 'public' AND tablename = 'umbra_phase45_post' \
         ORDER BY policyname",
    )
    .fetch_all(&pool)
    .await
    ...
    let rls_enabled: (bool,) = sqlx::query_as(
        "SELECT relrowsecurity FROM pg_class \
         WHERE oid = 'public.umbra_phase45_post'::regclass",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
