...
fn model_schema(
    ...
    for col in &model.fields {
        ...
        // A `#[umbral(private)]` column with no unlock is the exception: it is never RETURNED,
        // but it IS settable (`private` is a read policy — gaps3 #75). Dropping it from the
        // spec would leave a client unable to discover a field it is allowed to send, so it is
        // described with OpenAPI's own word for exactly this shape: `writeOnly`.
        let write_only = umbral_rest::is_write_only(&model.table, &col.name);
        if umbral_rest::is_hidden(&model.table, &col.name) && !write_only {
            continue;
        }
        let mut schema = column_schema_with_refs(col, table_to_schema);
        if write_only {
...
fn openapi_type(ty: SqlType) -> (&'static str, Option<&'static str>) {
    match ty {
        ...
        SqlType::DecimalN(_) => ("string", Some("decimal")),
