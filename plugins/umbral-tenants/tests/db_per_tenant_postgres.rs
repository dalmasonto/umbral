...
//!
//! - `UMBRAL_TENANTS_TEST_PG` — the **default / registry** database URL
//!   (required; if unset the test self-skips).
//! - `UMBRAL_TENANTS_TEST_PG_A` — tenant **A**'s database URL.
//! - `UMBRAL_TENANTS_TEST_PG_B` — tenant **B**'s database URL.
//!
...
//! UMBRAL_TENANTS_TEST_PG_A=postgres://app:apppass@localhost:5433/tnt_a \
//! UMBRAL_TENANTS_TEST_PG_B=postgres://app:apppass@localhost:5433/tnt_b \
//!   cargo test -p umbral-tenants --test db_per_tenant_postgres -- --ignored --nocapture
//! ```

use umbral::db::{DbPool, RouteContext, TenantKey};
use umbral_tenants::{Tenant, TenantStrategy, TenantsPlugin};
...
#[ignore = "needs three test Postgres DBs (UMBRAL_TENANTS_TEST_PG[_A|_B])"]
async fn database_per_tenant_isolation() {
    let Some(base) = base_url() else {
        eprintln!(
            "skipping database_per_tenant_isolation: set UMBRAL_TENANTS_TEST_PG (the default/registry \
             DB) plus UMBRAL_TENANTS_TEST_PG_A and UMBRAL_TENANTS_TEST_PG_B (the two tenant DBs) to run \
    ...
    let (Some(url_a), Some(url_b)) = (
        tenant_url("UMBRAL_TENANTS_TEST_PG_A", &base, "tnt_a"),
        tenant_url("UMBRAL_TENANTS_TEST_PG_B", &base, "tnt_b"),
    ) else {
        eprintln!(
            "skipping database_per_tenant_isolation: could not resolve the two tenant DB URLs. Set \
             UMBRAL_TENANTS_TEST_PG_A and UMBRAL_TENANTS_TEST_PG_B to two distinct, existing Postgres \
             databases."
        );
        return;
    };
    if url_a == url_b || url_a == base || url_b == base {
        eprintln!(
            "skipping database_per_tenant_isolation: the registry DB and the two tenant DBs must be \
             three DISTINCT databases (got overlap). Set UMBRAL_TENANTS_TEST_PG[_A|_B] explicitly."
        );
        return;
    }
    ...
    for t in ["tenant", "umbral_migrations", "tpost"] {
    ...
    for p in [&pool_a, &pool_b] {
        for t in ["tpost", "umbral_migrations"] {
            sqlx::query(&format!("DROP TABLE IF EXISTS public.{t} CASCADE"))
                .execute(p)
                .await
                .ok();
