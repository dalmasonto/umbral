...
//!   cargo test -p umbral-tenants --test data_migration_boundary_postgres \
//!   -- --ignored --nocapture
//! ```
...
use umbral::db::Schema;
use umbral::migrate::{MigrationFile, ModelMeta, Operation};
...
fn write_boundary_data_migration(dir: &Path, snapshot: umbral::migrate::Snapshot) {
...
async fn data_migration_reads_shared_public_writes_tenant_rows() {
    ...
    for t in ["subscription", "plan", "tenant", "umbral_migrations"] {
    ...
    umbral::migrate::make_in(Path::new(&tmp))
        .await
    ...
    // effect.
    let app_snapshot = umbral::migrate::Snapshot::current_for("app");
    ...
    for name in ["tenant_a", "tenant_b"] {
        ...
        umbral::migrate::run_for_schema_in(Path::new(&tmp), &schema, &shared_for_schema)
            .await
