...
//! ## Settings keys
//!
//! All read from `umbral::Settings::extra` (the catch-all
//! `UMBRAL_<KEY>` / `umbral.toml` keys). Defaults in parentheses.
//!
...
//! ## Backend selection
//!
//! `UMBRAL_EMAIL_BACKEND=console` forces the console backend even when
//! other keys are present (useful in CI / tests). Otherwise the order
//! is: `UMBRAL_EMAIL_BACKEND=api` (or both `email_api_provider` +
...
//!   [`umbral::templates::render`] mapping its error into [`EmailError`].
...
//!   `EmailError::Smtp`. Wiring this through `umbral-tasks` lands in a
...
pub enum EmailError {
    ...
    /// "<key>"`), or unset `UMBRAL_EMAIL_BACKEND=api`.
    ApiNotConfigured,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            EmailError::MissingFrom => write!(
                f,
                "umbral-email: missing From and no email_default_from configured"
            ),
            EmailError::NoRecipients => {
                write!(f, "umbral-email: EmailMessage.to is empty")
            }
            EmailError::Address(e) => write!(f, "umbral-email: address: {e}"),
            EmailError::Build(e) => write!(f, "umbral-email: message build: {e}"),
            EmailError::Smtp(e) => write!(f, "umbral-email: smtp: {e}"),
            EmailError::Templates(e) => write!(f, "umbral-email: templates: {e}"),
            EmailError::InvalidAttachmentContentType {
                filename,
                content_type,
            } => write!(
                f,
                "umbral-email: attachment `{filename}` has invalid content type `{content_type}`",
            ),
            EmailError::InvalidHeaderValue {
                ...
                f,
                "umbral-email: {field} contains a forbidden control character \
            ...
            EmailError::ApiNotConfigured => write!(
                f,
                "umbral-email: API backend selected but email_api_provider \
                 and/or email_api_key is missing — set both, or unset \
                 UMBRAL_EMAIL_BACKEND=api",
            ),
            EmailError::ApiTransport(e) => {
                write!(f, "umbral-email: API HTTP transport: {e}")
            }
            EmailError::ApiResponse { status, body } => write!(
                f,
                "umbral-email: API provider returned HTTP {status}: {body}",
            ),
            EmailError::ConsoleBackendInProduction => write!(
                f,
                "umbral-email: console backend refused to send in a non-Dev/Test \
...
fn load_config() -> EmailConfig {
    ...
    // other UMBRAL_-prefixed knob.
    let backend_override = std::env::var("UMBRAL_EMAIL_BACKEND").ok();
    ...
    // Re-parse from env / umbral.toml rather than reaching for the
    ...
    let settings = umbral::Settings::from_env().ok();
    ...
    // `email_smtp_timeout_secs = N` in umbral.toml or
    // `UMBRAL_EMAIL_SMTP_TIMEOUT_SECS=N` in the environment. Set to 0
    ...
    // SMTP (host set) → console. `UMBRAL_EMAIL_BACKEND=console` still
...
pub async fn send(message: &EmailMessage) -> Result<(), EmailError> {
    ...
    match cfg.backend {
        ...
        BackendKind::Console => {
            ...
            if !is_dev_or_test {
                tracing::error!(
                    "umbral-email: console backend refused to deliver in a non-Dev/Test \
...
///
/// The public bridge between the umbral type and the lettre type.
...
/// `umbral-email/api` to compile the `reqwest`-backed delivery path.
#[cfg(not(feature = "api"))]
async fn deliver_api(_request: ApiRequest) -> Result<(), EmailError> {
    Err(EmailError::ApiTransport(
        "the `api` feature is not enabled — rebuild umbral-email with \
...
/// `UMBRAL_EMAIL_BACKEND=console` is in the environment.
...
impl ConsoleBackend {
    ...
    pub fn deliver(&self, message: &Message, original: &EmailMessage) -> Result<(), EmailError> {
        ...
        eprintln!("---- umbral-email (console backend) ----");
...
/// wrapper around [`umbral::templates::render`] so callers get an
...
pub fn render_email_body<C: Serialize>(
    ...
    Ok(umbral::templates::render(template_name, context)?)
}
...
mod tests {
    ...
    fn smtp_timeout_default_is_ten_seconds() {
        ...
        unsafe {
            std::env::remove_var("UMBRAL_EMAIL_SMTP_TIMEOUT_SECS");
        }
        let cfg = load_config();
        assert_eq!(
