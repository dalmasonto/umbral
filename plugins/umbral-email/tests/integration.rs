//! End-to-end coverage for umbral-email.
//!
...
use umbral_email::{EmailError, EmailMessage, EmailPlugin, render_email_body, send};
...
/// `render_email_body` flows through `umbral::templates::render` and
...
async fn render_email_body_uses_the_framework_template_engine() {
    boot().await;
    let ctx = umbral::templates::context!(name => "Alice", site => "umbral");
    let body = render_email_body("email/welcome.txt", &ctx).expect("render");
    assert!(
        body.contains("Hi Alice, welcome to umbral."),
