...
//! ## Design
//!
//! `SessionStore` is the pluggable storage abstraction for umbra sessions.
...
//! umbra-core's ambient pool: set once at boot, read everywhere after that.
...
/// Pluggable storage back-end for umbra sessions.
///
...
// =========================================================================
// Ambient install — mirrors how umbra-core installs the ambient DB pool.
// =========================================================================
...
/// behaviour as umbra-core's pool install).
///
...
pub fn install_store(store: Arc<dyn SessionStore>) {
    if STORE.set(store).is_err() {
        tracing::warn!(
            "umbra-sessions: install_store called more than once; \
