...
//!          SET umbra:session:<sha256hex(token)>  <json>  EX <ttl_secs>
//!   load:  GET umbra:session:<sha256hex(token)>
//!          -> None (nil reply, or expires_at < now)
//!          -> Some(serde_json::from_str(json))
//!   destroy: DEL umbra:session:<sha256hex(token)>
//! ```
//!
//! ## Expiry
//!
...
//! ## Connection
//!
//! Uses `redis::aio::ConnectionManager` (the same pattern as `umbra-cache`'s
...
/// Key prefix for all session keys in Redis. Namespaces umbra sessions
/// away from other data in the same Redis database.
const KEY_PREFIX: &str = "umbra:session:";
...
/// `umbra:session:<sha256hex(token)>` with a native Redis TTL derived
...
/// [`RedisStore::from_env`] convenience that reads `UMBRA_REDIS_URL`.
#[derive(Clone)]
...
impl RedisStore {
    ...
    /// Convenience: reads the Redis URL from the `UMBRA_REDIS_URL`
    ...
    pub async fn from_env() -> Result<Self, SessionError> {
        let url = std::env::var("UMBRA_REDIS_URL").map_err(|_| {
            SessionError::Redis(
                "UMBRA_REDIS_URL environment variable not set".to_string(),
            )
        })?;
...
impl SessionStore for RedisStore {
    ...
    ///
    /// 1. Compute the Redis key: `umbra:session:<sha256(token)>`.
    ...
    /// `SET umbra:session:<hash> <json> EX <ttl_secs>`.
