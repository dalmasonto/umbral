...
//! can mint a valid-looking session), so `new` mirrors umbra-security's
...
impl CookieStore {
    ...
    fn resolve_ambient_key(&self) -> [u8; 32] {
        *self.ambient_key.get_or_init(|| {
            let secret = umbra::settings::get_opt()
                .map(|s| s.secret_key.clone())
                .unwrap_or_default();

            if secret.trim().is_empty() {
                // Mirror umbra-security: error-shout in prod, warn elsewhere.
                // No hard-fail here (the SessionsPlugin boot check owns that),
                // but make the danger impossible to miss in logs.
                match umbra::settings::get_opt().map(|s| &s.environment) {
                    Some(umbra::Environment::Prod) => {
                        tracing::error!(
                            "CookieStore: secret_key is EMPTY in production. Stateless cookie \
                             ...
                             umbra.toml or via UMBRA_SECRET_KEY before deploying."
                        );
                    }
                    _ => {
                        tracing::warn!(
