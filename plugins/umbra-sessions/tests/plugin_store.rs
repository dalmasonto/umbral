...
use umbra_sessions::{COOKIE_NAME, SessionsPlugin};
use umbra_sessions::store::{DbStore, SessionRecord, active_store};
...
async fn session_layer_set_data_persists_via_installed_store() {
    ...
    async fn writer() -> impl IntoResponse {
        umbra_sessions::current_mut(|s| s.set_raw("store_key", serde_json::json!(42)))
