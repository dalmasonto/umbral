...
//! # then:
//! UMBRA_REDIS_URL=redis://localhost:6379/0 \
//!   cargo test -p umbra-sessions --features redis -- --ignored
//! ```

#![cfg(feature = "redis")]

use chrono::{Duration, Utc};
use umbra_sessions::store::{SessionRecord, SessionStore, hash_token_pub};
use umbra_sessions::RedisStore;
...
/// and the Redis key is the expected `umbra:session:<hash>` form.
...
fn key_format_is_correct() {
    ...
    let expected_key = format!("umbra:session:{hash}");
    assert!(
        expected_key.starts_with("umbra:session:"),
        "key starts with the correct prefix"
    );
    assert_eq!(
        expected_key,
        format!("umbra:session:{}", hash_token_pub(token))
    );
}
...
///
/// Requires `UMBRA_REDIS_URL` to be set (e.g. `redis://localhost:6379/0`).
...
async fn live_round_trip() {
    let store = RedisStore::from_env()
        .await
        .expect("connect to Redis (set UMBRA_REDIS_URL)");
