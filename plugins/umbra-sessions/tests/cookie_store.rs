...
use umbra_sessions::store::{SessionRecord, SessionStore};
use umbra_sessions::{COOKIE_NAME, CookieStore, Session, SessionError, SessionsPlugin};
...
async fn end_to_end_through_session_layer_zero_db_rows() {
    ...
    async fn writer() -> impl IntoResponse {
        umbra_sessions::current_mut(|s| s.set_raw("flavor", serde_json::json!("vanilla")))
    ...
    async fn reader() -> impl IntoResponse {
        let flavor = umbra_sessions::current(|s| {
            s.get_raw("flavor")
