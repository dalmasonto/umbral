...
use umbra::web::header;
use umbra_auth::{AuthPlugin, AuthUser};
use umbra_sessions::{COOKIE_NAME, Session, SessionsPlugin};
...
async fn set_data_in_request_reads_back_and_persists_once() {
    ...
    async fn writer(headers: HeaderMap, req: Request) -> impl IntoResponse {
        let token = req
            .extensions()
            .get::<umbra_sessions::SessionToken>()
            .map(|t| t.0.clone())
            .expect("session token in extensions");
        umbra_sessions::set_data(&token, "k", &42i64)
            .await
            .expect("set_data");
        // Same-request read-back via the in-memory record.
        let s = umbra_sessions::current_session(&headers)
            .await
            .expect("current_session ok")
            .expect("in-request session view");
        let v: Option<i64> = umbra_sessions::get_data(&s, "k").expect("get_data");
...
async fn login_rotates_cookie_and_destroys_old_session() {
    ...
    use umbra::plugin::Plugin;
    use umbra::web::HeaderMap;
    use umbra_sessions::{create_session, read_session};
    ...
    umbra_sessions::set_data(&old_token, "cart", &"abc")
        .await
        .expect("seed cart");
    assert!(
    ...
    async fn login_handler(headers: HeaderMap) -> Response {
        ...
        umbra_sessions::login_user_id(&headers, resp.headers_mut(), Some("7".to_string()))
            .await
