...
//!
//! umbra apps previously hand-rolled the tracing subscriber in `main.rs`
...
//!
//! ```ignore
//! use umbra_logs::observability::{init, ObservabilityConfig};
//!
//! #[tokio::main]
//! async fn main() {
...
//! - `UMBRA_LOG_FORMAT=json` — emit structured JSON log lines (with `level`,
...
/// (reads `RUST_LOG`, `UMBRA_LOG_FORMAT`, `OTEL_EXPORTER_OTLP_ENDPOINT`,
...
pub struct ObservabilityConfig {
    ...
    /// `OTEL_SERVICE_NAME`, then to `"umbra"`.
...
impl ObservabilityConfig {
    ...
    /// - `json` is `true` when `UMBRA_LOG_FORMAT=json` (case-insensitive).
    ...
    pub fn from_env() -> Self {
        let json = env::var("UMBRA_LOG_FORMAT")
    ...
    S: tracing::Subscriber + for<'a> tracing_subscriber::registry::LookupSpan<'a>,
{
    ...
    let tracer = provider.tracer("umbra");
...
mod tests {
    ...
    fn json_layer_emits_valid_json() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info!(target: "umbra_logs::observability::tests", answer = 42, "hello json");
        });
        ...
        assert_eq!(parsed["target"], "umbra_logs::observability::tests");
    ...
    fn spans_export_to_in_memory_exporter() {
        ...
        let provider = opentelemetry_sdk::trace::SdkTracerProvider::builder()
            ...
            .build();
        let tracer = provider.tracer("umbra-test");
