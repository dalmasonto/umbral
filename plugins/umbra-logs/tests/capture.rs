//! End-to-end coverage for umbra-logs.
//!
//! Boots a tiny test app (LogsPlugin) over a tempfile-backed sqlite pool —
//! same pattern as the umbra-sessions integration tests — drives synthetic
...
use umbra::prelude::*;
use umbra_logs::{LogsPlugin, RequestLog, flush, request_log};
...
async fn request_log_round_trips_through_orm() {
    ...
    let created = RequestLog::objects()
        .create(RequestLog {
            ...
            user_agent: Some("umbra-test/1.0".to_string()),
            created_at: Utc::now(),
        })
        .await
    ...
    assert_eq!(fetched.user_agent.as_deref(), Some("umbra-test/1.0"));
}
