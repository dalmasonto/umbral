//! End-to-end coverage for umbral-admin. Boot the App once with
...
async fn login_session(router: &axum::Router, username: &str, password: &str) -> String {
    ...
    // Extract the session cookie issued by umbral_sessions::create_session.
    let anon_cookie = headers
        .get(header::SET_COOKIE)
        .and_then(|v| v.to_str().ok())
        .and_then(|s| {
            // The Set-Cookie value is like: umbral_session=<token>; Path=...; ...
            s.split(';')
                .next()
    ...
    let cookie_header = format!("umbral_csrf_token={anon_cookie}");
