...
async fn boot() -> &'static axum::Router {
    BOOT.get_or_init(|| async {
        ...
        // The schema comes from the models — auth_user, session and the admin's own
        // tables are all registered by their plugins, so nothing is spelled out here
        // and nothing can drift out of sync with a model.
