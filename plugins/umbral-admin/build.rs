//! Cargo build script for umbral-admin.
//!
...
//!
//! Manual build: `cd plugins/umbral-admin/css && npm install && npm run build`
...
fn main() {
    ...
    if !css_dir.join("node_modules").exists() {
        println!(
            "cargo:warning=umbral-admin: Tailwind CSS not built (node_modules missing). \
             Run `cd plugins/umbral-admin/css && npm install && npm run build` for production CSS. \
    ...
    match status {
        ...
        Ok(s) => {
            println!(
                "cargo:warning=umbral-admin: Tailwind CSS build exited with status {s}. \
                 Using CDN fallback in dev."
            );
        }
        Err(e) => {
            println!(
                "cargo:warning=umbral-admin: Tailwind CSS build failed ({e}). \
                 Using CDN fallback in dev."
            );
        }
    }
