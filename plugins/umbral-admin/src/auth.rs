...
///
/// On success: returns the authenticated [`umbral_auth::AuthUser`].
...
pub(crate) async fn require_staff(
    headers: &HeaderMap,
    current_path: &str,
) -> Result<umbral_auth::AuthUser, Response> {
...
pub(crate) async fn login_get(
    ...
    if let Ok(Some(user)) = umbral_auth::current_user(&headers).await {
        if user.is_staff {
            let next = params
                .get("next")
...
/// shared `umbral_csrf_token` cookie. If `SecurityPlugin` is installed,
...
pub(crate) async fn login_post(headers: HeaderMap, body: String) -> Response {
    ...
    let cookie_csrf = umbral_security::current_csrf_token(&headers).unwrap_or_default();
    ...
    // field is wrong — timing-safe because umbral_auth hashes the
    // password unconditionally when the username matches.
    let user = match umbral_auth::authenticate::<umbral_auth::AuthUser>(username, password).await {
        Ok(u) => u,
        Err(_) => {
            return bad_login_response_with_csrf(
    ...
    if let Err(e) = umbral_auth::login_with_request(&headers, response.headers_mut(), &user).await {
...
///
/// 1. Ambient (`umbral::templates::current_csrf()`): `SecurityPlugin`
...
fn ensure_csrf_token(headers: &HeaderMap) -> (String, Option<String>) {
    ...
    let mut cookie = format!("umbral_csrf_token={tok}; Path=/; SameSite=Lax");
    if is_prod {
...
mod csrf_tests {
    ...
    async fn ensure_csrf_token_prefers_the_ambient_token() {
        ...
        let (tok, cookie) =
            umbral::templates::with_current_csrf(Some("ambient-token".to_string()), async {
