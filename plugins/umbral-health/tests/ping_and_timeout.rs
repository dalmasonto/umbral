//! Tests for umbral::db::ping() and the per-check timeout in the readiness runner.
...
// ---------------------------------------------------------------------------
// Issue A: umbral::db::ping() via the ORM surface
// ---------------------------------------------------------------------------

/// `umbral::db::ping()` returns `Ok(())` against the live SQLite pool
