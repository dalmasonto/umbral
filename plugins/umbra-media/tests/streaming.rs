//! Behavioral tests for streaming Storage (gaps2 #82): `store_stream` /
//! `retrieve_stream` on `FsStorage`, the mid-stream `max_size` cap on
//! `SizeLimitedStorage` (via the ambient backend), `MediaPlugin::save_stream`
//! recording the ACTUAL streamed byte count, and the buffering default
//! impls for a backend that does not override the streaming methods.
//!
//! Real files in a tempdir, exercised through the public trait — the same
//! harness shape as `fs_storage.rs` / `plugin_save.rs`.
...
/// Build a `ByteStream` from owned chunks. Crucially, the resulting stream
/// carries NO declared total length — the size cap must be enforced as the
/// chunks flow, not from any header. (`futures::stream::iter` reports a
/// `size_hint` of item COUNT, not byte length, so a length-trusting cap
/// would be unable to use it — exactly the point.)
...
/// Drain a `ByteStream` into a single `Vec<u8>`, propagating IO errors.
...
/// `store_stream` writes a MULTI-CHUNK body to disk byte-exact, and
/// `retrieve_stream` reads the same bytes back.
#[tokio::test]
async fn store_stream_round_trips_multi_chunk_body() {
    ...
    // The reported size is the real streamed byte count.
    assert_eq!(stored.size, whole.len() as u64);
    assert!(stored.key.ends_with("-note.txt"));
    assert_eq!(stored.url, fs.url(&stored.key));

    // The file on disk has EXACTLY the streamed bytes.
    let on_disk = std::fs::read(dir.path().join(&stored.key)).unwrap();
    assert_eq!(on_disk, whole);

    // retrieve_stream yields them back, byte-exact.
    let back = fs.retrieve_stream(&stored.key).await.expect("retrieve_stream");
    assert_eq!(collect(back).await.unwrap(), whole);
}

/// retrieve_stream on a missing key maps to NotFound (not a raw IO error).
#[tokio::test]
...
/// chunked body whose cumulative bytes exceed the cap MID-STREAM, returns
/// `TooLarge`, and leaves NO oversized file on disk. The stream declares no
/// total length up front, proving the cap is enforced as bytes flow.
#[tokio::test]
async fn size_limited_store_stream_rejects_oversize_mid_stream() {
    ...
    // Cap at 10 bytes; deliver 30 bytes in three 10-byte chunks. The cap
    // must fire during the second chunk (cumulative 20 > 10), before the
    // body is fully read.
    let max: u64 = 10;
    let limited = MediaPlugin::size_limited_for_test(fs.clone(), max);
    ...
    // NO oversized blob persisted: the only thing that could be on disk is a
    // cleaned-up partial. Assert the media dir holds no file larger than the
    // cap (ideally zero files, but a 0-length leftover would also be fine).
...
/// A body that fits UNDER the cap streams through fine — the cap only fires
/// when the real bytes cross it.
#[tokio::test]
async fn size_limited_store_stream_allows_under_cap() {
    let dir = tempfile::tempdir().unwrap();
    let fs: Arc<dyn Storage> = Arc::new(FsStorage::new("/media", dir.path()));
    let limited = MediaPlugin::size_limited_for_test(fs.clone(), 100);
...
/// The active-content (stored-XSS) guard still applies through the streaming
/// path: a `.html` upload is stored with a `.txt` suffix so ServeDir serves
/// it as inert text/plain. Streaming changes only HOW bytes land, never the
/// filename guards.
#[tokio::test]
...
/// Path-separator sanitisation still applies through streaming — a filename
/// with `/` can't escape the media dir.
#[tokio::test]
async fn store_stream_sanitises_path_separators() {
    ...
    // Separators were dropped; the key is a single path component under dir.
    assert!(!stored.key.contains('/'), "key must not contain a separator: {}", stored.key);
    assert!(dir.path().join(&stored.key).exists());
}

// ---- A non-overriding Storage exercises the buffering DEFAULT impls. ----

/// An in-memory Storage that implements ONLY the buffered `store`/`retrieve`
/// — it does NOT override `store_stream` / `retrieve_stream`, so calling
/// them exercises the trait's default (buffering) impls.
...
/// A Storage that does NOT override the streaming methods still stores and
/// retrieves via the buffering DEFAULT impls — the additive design keeps
/// every existing backend working unchanged.
#[tokio::test]
async fn default_streaming_impls_buffer_and_round_trip() {
    ...
    // Default store_stream buffers, then delegates to `store`.
    let stored = store
    ...
    // Default retrieve_stream wraps `retrieve`'s Vec as a single chunk.
    let back = store.retrieve_stream(&stored.key).await.expect("default retrieve_stream");
    assert_eq!(collect(back).await.unwrap(), whole);
}

/// A mid-stream IO error in the body propagates out of the default
/// `store_stream` (the buffering loop must not swallow it).
#[tokio::test]
...
// ---- MediaPlugin::save_stream end-to-end against a real pool. ----
...
/// `save_stream` records the ACTUAL streamed byte count in `MediaFile.size`,
/// not a declared or guessed length, and the bytes are retrievable.
#[tokio::test]
async fn save_stream_records_accurate_size() {
    ...
    // 25 bytes delivered across three uneven chunks; no declared length.
...
/// `save_stream` with a configured `max_size` rejects an oversize body
/// mid-stream with `TooLarge`, and inserts NO `media_file` row for it.
#[tokio::test]
async fn save_stream_enforces_max_size_mid_stream() {
    boot().await;
    let media_dir = tempfile::tempdir().expect("media dir");
    let fs = Arc::new(FsStorage::new("/media", media_dir.path()));
    let plugin = MediaPlugin::with_storage("/media", fs.clone()).max_size(10);
    ...
    match result {
        Err(umbra_media::MediaError::TooLarge { limit, actual }) => {
            assert_eq!(limit, 10);
            assert!(actual > 10);
        }
    ...
    // No file larger than the cap left behind, and no tracking row.
    ...
    assert!(max_file_len <= 10, "rejected oversize upload left a {max_file_len}-byte file");
}
