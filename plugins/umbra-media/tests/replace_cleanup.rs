//! Replace-cleanup (gaps2 #92): when a row's `FileField` / `ImageField`
//! key is UPDATED to a new value via the typed per-row save path, the OLD
//! blob is deleted from the `Storage` backend, so replacing a file doesn't
//! leak an orphan. Rides the `post_update:<table>` signal the ORM fires on
//! UPDATE (with both the old and new row), which `MediaPlugin` subscribes
//! to in `on_ready` for any model opted into `cleanup_on_delete`.
//!
//! Contract checked here:
//!   - file key A -> B  => blob A deleted, blob B kept
//!   - file key A -> A  => nothing deleted (same key, no replacement)
//!   - non-file update  => no blob deletion
//!   - INSERT (create)  => no blob deletion (post_update never fires)
...
async fn boot() {
    BOOT.get_or_init(|| async {
        ...
        // `cleanup_on_delete` ALSO wires the `post_update` replace-cleanup
        // handler — the exact production wiring (gaps2 #92).
        umbra::App::builder()
            .settings(settings)
            .database("default", pool)
            .model::<ReplaceDoc>()
            .plugin(MediaPlugin::with_storage("/media", fs).cleanup_on_delete::<ReplaceDoc>())
            .build()
            .expect("App::build");
...
/// Replacing a file field with a NEW key deletes the OLD blob, keeps the new.
#[tokio::test]
async fn replacing_file_deletes_old_blob_keeps_new() {
    ...
    // UPDATE the file field A -> B via the typed per-row save path.
...
/// Saving with the SAME file key deletes nothing.
#[tokio::test]
async fn same_key_update_deletes_nothing() {
    ...
    // Change only the title; keep the file key the same.
    let mut updated = doc.clone();
...
/// An UPDATE that doesn't touch the file column leaves the blob intact.
#[tokio::test]
async fn non_file_update_deletes_nothing() {
    ...
    let mut updated = doc.clone();
    updated.title = "t2".into(); // attachment unchanged
    ReplaceDoc::objects()
        .save(updated)
        .await
        .expect("save (update) row");
...
/// INSERT (create) never fires post_update, so no blob is deleted.
#[tokio::test]
