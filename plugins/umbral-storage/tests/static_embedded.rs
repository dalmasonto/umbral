//! Coverage for `StoragePlugin::embedded` — the in-memory variant backed
//! by an `include_dir!()` tree. Moved from umbral-static.
...
async fn embedded_dir_accessor_returns_none() {
    let plugin = StoragePlugin::new().embedded("/static", &FIXTURE);
    assert!(
        plugin.static_dir().is_none(),
        "embedded plugins have no on-disk directory; static_dir() must return None",
    );
    assert_eq!(plugin.static_mount(), Some("/static"));
}
