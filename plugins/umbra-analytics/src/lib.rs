//! umbra-analytics — product-analytics event capture for umbra.
//!
...
//!
//! // In a handler
//! use umbra_analytics::{capture, identify};
//!
...
//! ## Settings keys
//!
//! Read from `UMBRA_POSTHOG_API_KEY` / `UMBRA_POSTHOG_HOST` env vars or
//! `umbra.toml` extra keys `posthog_api_key` / `posthog_host`. Builder
//! overrides win over environment.
//!
//! - `posthog_api_key` / `UMBRA_POSTHOG_API_KEY`. Your project API key.
//!   When absent the plugin is a **no-op**: captures are dropped with a
//!   one-time warning. Never panics, never blocks.
//! - `posthog_host` / `UMBRA_POSTHOG_HOST`. Ingest host
...
/// Mirrors the `umbra-oauth` `http_client()` pattern exactly.
...
impl AnalyticsPlugin {
    ...
    ///
    /// The key wins over `UMBRA_POSTHOG_API_KEY` / `posthog_api_key` in
    ...
    /// variables / `umbra.toml` settings. Equivalent to
    ...
    fn resolve_api_key(&self) -> Option<String> {
        ...
        if let Ok(val) = std::env::var("UMBRA_POSTHOG_API_KEY") {
            if !val.trim().is_empty() {
                return Some(val);
            }
        }

        // umbra.toml extra key last.
        if let Ok(settings) = umbra::Settings::from_env() {
    ...
    fn resolve_host(&self) -> String {
        ...
        if let Ok(val) = std::env::var("UMBRA_POSTHOG_HOST") {
            if !val.trim().is_empty() {
                return val;
            }
        }

        // umbra.toml extra key.
        if let Ok(settings) = umbra::Settings::from_env() {
            ...
            None => {
                warn!(
                    "AnalyticsPlugin registered with no PostHog API key. Set \
                     UMBRA_POSTHOG_API_KEY or pass an explicit key via \
