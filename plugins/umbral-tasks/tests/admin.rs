//! Admin visibility coverage for umbral-tasks (planning/features.md #82):
...
fn admin_model_targets_task_row_read_only_with_retry() {
    let model = umbral_tasks::admin_model();
...
async fn retry_is_a_noop_on_non_failed_or_absent() {
    ...
    let pending_id = enqueue("ok_task", &(), umbral_tasks::EnqueueOptions::default())
        .await
    ...
    drain_queue().await;
    let done_id = enqueue("ok_task", &(), umbral_tasks::EnqueueOptions::default())
        .await
