 ...
 *    per-app Dexie DB (`umbra-playground:<app>`). Generous quota,
...
export const LEGACY_LS_KEY = scopedKey("umbra-playground-settings:v1");
...
export function writeLocalStorageCache(settings: PlaygroundSettings): boolean {
  ...
  try {
    ...
    if (typeof console !== "undefined") {
      console.warn("[umbra-playground] localStorage cache write failed", e);
    }
    return false;
  }
}
...
export async function readSettingsFromDexie(): Promise<PlaygroundSettings | null> {
  try {
    ...
    if (typeof console !== "undefined") {
      console.warn("[umbra-playground] settings Dexie read failed", e);
    }
    return null;
  }
}
...
export async function writeSettingsToDexie(
  ...
  try {
    ...
    if (typeof console !== "undefined") {
      console.warn("[umbra-playground] settings Dexie write failed", e);
    }
    return false;
  }
}
