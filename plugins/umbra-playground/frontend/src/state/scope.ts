 ...
 * gap #71 — two umbra apps served from the same browser (e.g.
 ...
 *
 *  - `<meta name="umbra-playground-app" content="...">`
 *  - `window.__UMBRA_PLAYGROUND_APP__ = "..."`
 *
...
export function getAppScope(): string {
  if (typeof window !== "undefined") {
    const g = (window as unknown as { __UMBRA_PLAYGROUND_APP__?: unknown })
      .__UMBRA_PLAYGROUND_APP__;
    if (typeof g === "string" && g.length > 0) return g;
  }
  if (typeof document !== "undefined") {
    const meta = document.querySelector<HTMLMetaElement>(
      'meta[name="umbra-playground-app"]',
    );
