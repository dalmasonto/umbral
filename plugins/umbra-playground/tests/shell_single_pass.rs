...
use umbra_playground::routes::{PlaygroundState, render_shell};
...
fn app_name_containing_later_placeholder_is_not_re_expanded() {
    ...
    // double-quotes.
    assert!(
        html.contains(r#"window.__UMBRA_PLAYGROUND_APP__ = "__OPENAPI_URL_JSON__";"#),
    ...
    let openapi_line = html
        .lines()
        .find(|l| l.contains("__UMBRA_OPENAPI_URL__"))
        .unwrap_or("");
    // That line must not also contain the app-name literal token.
    assert!(
        !openapi_line.contains("__OPENAPI_URL_JSON__"),
        "the __UMBRA_OPENAPI_URL__ assignment line must not contain the raw \
