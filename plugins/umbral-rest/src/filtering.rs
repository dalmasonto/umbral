...
pub(crate) fn parse_search(
    ...
    for col in columns {
        ...
        // and skipped.
        let predicate: Option<SimpleExpr> = match umbral::migrate::fk_effective_type(col) {
            SqlType::Text => {
...
/// `umbral-openapi` to emit per-column query parameters on list
/// endpoints — clients (Swagger UI, codegen, the umbral-playground)
...
fn build_predicate(col: &Column, lookup: &str, value: &str) -> Result<SimpleExpr, ApiError> {
    ...
    match lookup {
        ...
        "contains" => {
            let pat = format!("%{}%", umbral::orm::escape_like_literal(value));
        ...
        "icontains" => {
            ...
            // the column.
            let pat = format!("%{}%", umbral::orm::escape_like_literal(value)).to_uppercase();
            Ok(
...
fn coerce_value(col: &Column, value: &str) -> Result<sea_query::Value, ApiError> {
    ...
    let v = match umbral::migrate::fk_effective_type(col) {
        SqlType::SmallInt | SqlType::Integer | SqlType::BigInt | SqlType::ForeignKey => {
