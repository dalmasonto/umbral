...
// `Clone` is required: `Order` is `OrderItem.order`'s FK target, and the
// heavy-relations epic's cache-aware to-one accessor (`item.order()`) clones
// the cached row on a `select_related` hit — every `#[derive(Model)]` struct
// is expected to derive `Clone` for exactly this reason.
