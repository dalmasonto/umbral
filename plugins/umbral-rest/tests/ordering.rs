...
async fn ordering_asc_by_score_returns_lowest_first() {
    ...
    let scores: Vec<i64> = rows
        .iter()
        .map(|r| r["score"].as_i64().unwrap_or(0))
        .collect();
    assert_eq!(
        scores,
        vec![10, 20, 30],
        "ascending score order: {scores:?}"
    );
}
...
async fn ordering_unknown_field_is_silently_ignored_and_returns_200() {
    ...
    assert_eq!(
        status,
        StatusCode::OK,
        "unknown ordering field must not produce a 400"
    );
    assert_eq!(
        rows.len(),
        3,
        "all rows still returned when ordering field unknown"
    );
}
