...
use umbra::prelude::{AppContext, Plugin};

/// The processor registry is a single ambient (process-global) seam, like
/// `umbra::storage`. Tests that install processors must not run concurrently
...
fn test_ctx() -> AppContext {
    AppContext {
        ...
        settings: umbra::settings::get().clone(),
    }
}
    ...
    let deadline = tokio::time::timeout(timeout, async {
        loop {
            let row = MediaFile::objects()
                .filter(umbra_storage::media_file::ID.eq(id))
                .first()
                .await
...
async fn save_without_processors_is_ready_immediately() {
    ...
    umbra_storage::clear_processors_for_test();
