...
//! storage backend AND inserts a `media_file` row. Moved from umbra-media.
...
use umbra_storage::{FsStorage, MediaFile, MediaTracking, StoragePlugin};
...
async fn save_writes_bytes_and_inserts_row() {
    ...
    let rows_for_key = MediaFile::objects()
        .filter(umbra_storage::media_file::KEY.eq(&outcome.file.key))
        .count()
        .await
        .expect("count by key");
    assert_eq!(
...
async fn ambient_upload_tracks_exactly_one_row() {
    ...
    let rows_for_key = MediaFile::objects()
        .filter(umbra_storage::media_file::KEY.eq(&stored.key))
        .count()
        .await
        .expect("count by key");
    assert_eq!(
...
async fn save_enforces_max_size() {
    ...
    assert!(
        matches!(err, umbra_storage::MediaError::TooLarge { .. }),
