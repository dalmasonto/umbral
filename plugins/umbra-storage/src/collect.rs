...
//!
//! Where umbra-static's command wrote through core's sync `StaticStorage`
...
use umbra::storage::{STATICFILES, Storage};
...
impl umbra::cli::PluginCommand for CollectStaticCommand {
    ...
    async fn run(&self, matches: &clap::ArgMatches) -> Result<(), umbra::cli::CliError> {
        ...
        let published = umbra::static_files::published_static().ok_or_else(
            || -> umbra::cli::CliError {
                "collectstatic requires a built App; ensure App::build() ran before dispatching \
                 the command (umbra-cli::dispatch is called with the built App)."
                    .into()
            },
        )?;
        ...
        let static_backend: Arc<dyn Storage> = match backend.as_str() {
            "local" => {
                ...
                umbra::storage::try_storage_named(STATICFILES).map_err(
                    |_| -> umbra::cli::CliError {
            ...
            "s3" => {
                ...
                #[cfg(not(feature = "s3"))]
                {
                    return Err("collectstatic --storage s3 requires the umbra-storage `s3` \
        ...
        let summary = umbra::static_files::collect_into_with(
