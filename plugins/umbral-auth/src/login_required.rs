...
/// [`umbral_sessions::current_user_id_str`] instead.
...
/// Resolve the `umbral_session` cookie in `headers` to the
...
///
/// This is the primitive `permission_required` (in `umbral-permissions`)
/// builds on. Callers using a custom user model reach for
/// [`current_session_user_pk`] (the typed generic) or
/// [`umbral_sessions::current_user_id_str`] (the raw string)
