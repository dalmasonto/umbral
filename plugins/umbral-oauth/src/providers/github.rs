...
const USER_AGENT: &str = "umbral-oauth";
...
impl GitHubProvider {
    ...
    /// Build from `UMBRAL_OAUTH_GITHUB_CLIENT_ID` /
    /// `UMBRAL_OAUTH_GITHUB_CLIENT_SECRET`. `None` if either is unset.
    pub fn from_env() -> Option<Self> {
        let id = std::env::var("UMBRAL_OAUTH_GITHUB_CLIENT_ID").ok()?;
        let secret = std::env::var("UMBRAL_OAUTH_GITHUB_CLIENT_SECRET").ok()?;
...
impl OAuthProvider for GitHubProvider {
    ...
    /// GitHub only reports an email as `verified` from `/user/emails` after the
    /// user confirmed ownership, so it is trusted for verified-email
    /// auto-linking.
