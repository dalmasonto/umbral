...
impl GoogleProvider {
    ...
    /// Build from `UMBRAL_OAUTH_GOOGLE_CLIENT_ID` /
    /// `UMBRAL_OAUTH_GOOGLE_CLIENT_SECRET`. Returns `None` if either is
    /// unset, so a consumer can register the provider conditionally.
    pub fn from_env() -> Option<Self> {
        let id = std::env::var("UMBRAL_OAUTH_GOOGLE_CLIENT_ID").ok()?;
        let secret = std::env::var("UMBRAL_OAUTH_GOOGLE_CLIENT_SECRET").ok()?;
