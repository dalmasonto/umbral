...
// shared across every umbral app served to the same browser; now
// each app's localStorage slot is `<app>:<key>`. The full
// `umbral-playground-settings:v1` slot moved to Dexie's `settings`
...
const SELECTED_KEY = scopedKey("umbral-playground:selected-operation:v1");
...
export const usePlayground = create<PlaygroundState>((set, get) => ({
  ...
  loadSpec: async () => {
    ...
    // OpenApiPlugin mount into `window.__UMBRAL_OPENAPI_URL__`
    // (see `plugins/umbral-playground/src/shell.html`), so a
    ...
    const specUrl =
      (typeof window !== "undefined" &&
        (window as typeof window & { __UMBRAL_OPENAPI_URL__?: string })
          .__UMBRAL_OPENAPI_URL__) ||
  ...
  hydrateFromDexie: async () => {
    try {
      ...
      if (typeof console !== "undefined") {
        console.warn("[umbral-playground] Dexie hydration failed", e);
      }
    }
  },
  applyDefaultHeaders: () =>
