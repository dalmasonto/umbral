...
//!   `Plugin::static_dirs()`.
//!
//! So the asset-serving tests now drive `umbral::static_files`'
...
use umbral::static_files::{StaticHandlerState, StaticRegistry, static_handler};
...
async fn shell_follows_configured_static_url() {
    ...
    let state = umbral_playground::routes::PlaygroundState::new(
        "/api/playground",
        "cdn-app",
        false,
        "https://cdn.example.com/s/playground/assets",
    );
    let app = umbral_playground::routes::router(state);
    let req = Request::builder()
        .uri("/api/playground/")
        .body(Body::empty())
        .unwrap();
...
/// two places — a `<meta name="umbral-playground-app">` tag and the
/// `window.__UMBRAL_PLAYGROUND_APP__` global. Both must round-trip the
...
async fn shell_injects_per_app_scope() {
    ...
    let s = String::from_utf8_lossy(&body);
    assert!(
        s.contains(r#"<meta name="umbral-playground-app" content="my-shop" />"#),
        "shell must carry the app meta tag; got: {s}"
    );
    assert!(
        s.contains(r#"window.__UMBRAL_PLAYGROUND_APP__ = "my-shop";"#),
