...
//! [`RateLimiter`](umbra::ratelimit::RateLimiter), no DB round-trip.
//!
...
/// Map a [`RateDecision`](umbra::ratelimit::RateDecision) to the throttle
...
impl Throttle for AnonRateThrottle {
    fn check(&self, ctx: &ThrottleContext) -> Result<(), ThrottleDenied> {
        ...
        // a hole (mirrors umbra-auth's client_ip fallback).
