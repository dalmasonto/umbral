...
//! umbra-cache's redis integration tests:
//!
//!   REDIS_URL=redis://localhost:6379/15 cargo test --features redis -p umbra-realtime --test broker

use umbra_realtime::{Envelope, TargetKind};
...
mod redis_relay {
    ...
    use umbra_realtime::{Broker, Envelope, RedisBroker, Registry, TargetKind};
    ...
    async fn publish_on_one_instance_reaches_a_socket_on_another() {
        ...
        let (_id, mut rx) = reg_b.register(Some(user), HashSet::new(), 8).await;
