...
use umbral_security::test_support::wrap_with_csrf;
...
fn app(signed: bool, secret: Option<&str>) -> Router {
    let routes = Router::new()
        ...
        .route(
            "/with-session-cookie",
            get(|| async { ([(header::SET_COOKIE, "umbral_session=abc; Path=/")], "ok") }),
        );
...
async fn set_cookie_is_appended_not_replaced() {
    ...
    let cookies: Vec<String> = resp
        ...
        .collect();
    assert!(
        cookies.iter().any(|c| c.starts_with("umbral_session=")),
    ...
    let resp = app
        .oneshot(
            Request::post("/form")
                .header(
                    header::COOKIE,
                    format!("umbral_csrf_token={}", "a".repeat(64)),
                )
