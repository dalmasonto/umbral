...
//!
//! Because `umbral::settings::init` is `pub(crate)` and unreachable from
//! plugin tests, we exercise the inner validation logic via
//! `umbral_security::test_support::validate_secret_key`, which takes an
//! explicit `umbral::Settings` reference rather than reading the ambient
//! `SETTINGS` OnceLock.
//!
//! `umbral::Settings` derives `serde::Deserialize` with serde defaults on
