...
//!   cargo test -p umbra-tenants --test m2m_isolation_postgres -- --ignored --nocapture
//! ```
...
async fn m2m_relation_isolated_per_tenant_schema() {
    let Some(url) = pg_url() else {
        eprintln!(
            "skipping m2m_relation_isolated_per_tenant_schema: set UMBRA_TENANTS_TEST_PG \
    ...
    for t in ["tarticle_tags", "tarticle", "ttag", "tenant", "umbra_migrations"] {
