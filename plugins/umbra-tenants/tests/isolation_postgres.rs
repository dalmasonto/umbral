...
//! `UMBRA_TENANTS_TEST_PG` (or `DATABASE_URL`). The real proof:
//!
...
//!   cargo test -p umbra-tenants --test isolation_postgres -- --ignored --nocapture
//! ```
...
fn pg_url() -> Option<String> {
    ...
    // (`UMBRA_TEST_POSTGRES_URL`, used by the other PG-gated suites), or a plain
    // `DATABASE_URL`.
...
async fn schema_per_tenant_isolation() {
    let Some(url) = pg_url() else {
        eprintln!(
            "skipping schema_per_tenant_isolation: set UMBRA_TENANTS_TEST_PG \
    ...
    for t in ["tpost", "tenant", "umbra_migrations"] {
    ...
    for name in ["tenant_a", "tenant_b"] {
        ...
        umbra::migrate::run_for_schema_in(
    ...
    let ctx_a = RouteContext::new().with_tenant(TenantKey::new("tenant_a"));
    let seen_a = umbra::db::route_context_scope(ctx_a, async {
        Tenant::objects().count().await.expect("count tenants under A")
    })
    .await;
    let ctx_b = RouteContext::new().with_tenant(TenantKey::new("tenant_b"));
    let seen_b = umbra::db::route_context_scope(ctx_b, async {
