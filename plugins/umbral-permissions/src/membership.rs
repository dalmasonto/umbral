...
//! in `umbral-auth`, `Group` / `Permission` live in `umbral-permissions`,
//! and the dep arrow runs `umbral-permissions → umbral-auth` so the
...
pub async fn add_user_to_group(user_id: &str, group: &Group) -> Result<(), PermError> {
    ...
    match UserGroup::objects()
        .create(UserGroup {
            ...
            group_id: umbral::orm::ForeignKey::new(group.id),
        })
        .await
    {
    ...
    UserGroup::objects()
        .filter(
            user_group::USER_ID.eq(user_id.to_string())
                & umbral::orm::Predicate::<UserGroup>::col_eq("group_id", group.id),
        )
        .delete()
        .await?;
    Ok(())
...
pub async fn set_user_groups(user_id: &str, group_ids: &[i64]) -> Result<(), PermError> {
    let rows: Vec<UserGroup> = group_ids
        ...
        .map(|gid| UserGroup {
            id: 0,
            user_id: user_id.to_string(),
            group_id: umbral::orm::ForeignKey::new(*gid),
        })
        .collect();
    let user_id = user_id.to_string();
    umbral::db::transaction(|tx| {
        Box::pin(async move {
            UserGroup::objects()
...
pub async fn is_in_group(user_id: &str, group_id: i64) -> Result<bool, PermError> {
    Ok(UserGroup::objects()
        .filter(
            user_group::USER_ID.eq(user_id.to_string())
                & umbral::orm::Predicate::<UserGroup>::col_eq("group_id", group_id),
        )
        .exists()
        .await?)
}
    ...
    UserPermission::objects()
        .filter(
            user_permission::USER_ID.eq(user_id.to_string())
                & umbral::orm::Predicate::<UserPermission>::col_eq(
                    "permission_id",
                    perm.codename.clone(),
                ),
        )
