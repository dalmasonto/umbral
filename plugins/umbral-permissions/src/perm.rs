...
//! so this crate does not depend on `umbral-auth`. If `has_perm` lived on the
//! `UserModel` trait in `umbral-auth`, we'd need `umbral-auth` → `umbral-permissions`
//! (to call the perm query) AND `umbral-permissions` → `umbral-auth` (to read
...
//!
//! ```ignore
//! use umbral_permissions::has_perm;
//!
...
pub enum PermError {
    ...
    Sqlx(sqlx::Error),
    /// A structured umbral ORM write-validator error (UNIQUE violation
...
impl std::fmt::Display for PermError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PermError::Sqlx(e) => write!(f, "umbral-permissions: sqlx: {e}"),
            PermError::Write(e) => write!(f, "umbral-permissions: write: {e}"),
        }
    }
}
...
impl From<umbral::orm::write::WriteError> for PermError {
    ...
    let direct = UserPermission::objects()
        .filter(
            user_permission::USER_ID.eq(user_id_owned.clone())
                & umbral::orm::Predicate::<UserPermission>::col_eq("permission_id", pk.clone()),
        )
        .exists()
        .await?;
    if direct {
