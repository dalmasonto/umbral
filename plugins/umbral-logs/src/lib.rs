...
//!   layer.
//! - [`admin_model`]. A read-only [`umbral_admin::AdminModel`] for `RequestLog`
...
#[cfg(feature = "admin")]
pub use umbral_admin::AdminModel;

// ── Model ───────────────────────────────────────────────────────────────────

/// One captured HTTP request. Lives in the `logs_requestlog` table (the
/// `#[umbral(table = ...)]` override namespaces it under the plugin, the same
...
impl Plugin for LogsPlugin {
    ...
    fn models(&self) -> Vec<umbral::migrate::ModelMeta> {
        vec![umbral::migrate::ModelMeta::for_::<RequestLog>()]
    }
...
///
/// ```ignore
/// AdminPlugin::default().register(umbral_logs::admin_model())
/// ```
///
