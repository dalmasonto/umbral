...
A standalone umbral example: **schema-per-tenant multitenancy where blockchain networks are the tenants.** One Starknet explorer app serves multiple networks (Sepolia testnet + Mainnet), with per-network data isolated per Postgres schema and cross-network data shared in `public`.
...
See `src/main.rs` for the fully-commented walkthrough, and `plugins/umbral-tenants/` + `arch.md` for the design.
