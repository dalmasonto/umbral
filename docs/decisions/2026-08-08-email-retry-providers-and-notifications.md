...
HUMAN REVIEW: One thing missing here is, we have said we are going to extend the list of providers in Part 2 but not the ability to add a custom one since we have more like behance, sentinmail (My custom email/newsletter system) and others. My approach could be, instead of adding some more, its best we build the current 2 that are inbuilt and autoregister them and then ask the developer to develop theirs the way they want. This way, the others that we are planning to add/extend, we can simply build them in another folder ie "email-providers" in which the developer can decide to copy or we simply release them to crates.io as stand-alone installables. This way, the developer can choose to use what they want or build their own on the fly.
