...
- The **operation catalogue** (`CreateTable`, `DropTable`, `AddColumn`, `DropColumn`, `AlterColumn`, indices, constraints, `RunSql`, `RunCode`).
  ...
  - **Shipped since (M8 hardening and later):** **index / constraint ops** (`AddIndex` / `DropIndex`, both composite indexes and composite `UNIQUE` groups, with deterministic `idx_*` / `uniq_*` names); the **`RunSql` data-migration escape hatch** (hand-authored via `makemigrations --empty <plugin>`, carrying optional `reverse_sql` metadata); **rename detection** (`RenameTable` and `RenameColumn`, the same-shape heuristic that disambiguates a rename from a drop+add); database **views** (`CreateView` / `DropView`, including Postgres materialized views); **M2M junction tables** (`CreateM2MTable` / `DropM2MTable`); and **column comments** (`SetColumnComment`, from `#[umbral(help = "...")]`, Postgres-only, no-op on SQLite).
  - **Still deferred:** `AlterColumn` **type changes** (need cast semantics + the table-recreation dance to know whether the SELECT will silently coerce vs fail), **primary-key flips**, and the **`RunCode`** Rust data-migration op (today's data migrations are hand-authored `RunSql`). The table-recreation dance for nullable flips already pays the recreation cost; the same machinery extends naturally once cast safety is modelled.
