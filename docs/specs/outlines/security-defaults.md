...
**Secure-cookie defaults.** `SameSite=Lax` (configurable to `Strict`), `Secure` in `Environment::Prod`, `HttpOnly` for session cookies. The defaults apply to every cookie issued through `umbral::web::Cookie`, not just the session cookie — a plugin that issues its own cookie inherits the safe shape unless it explicitly opts out.

## Promote-to-deep trigger

Promote at **M9 entry**, when `umbral-auth` and `umbral-sessions` re-express as plugins and the first cookie- and token-bearing flows go live. The deep spec locks the CSRF token format, the per-route override mechanism for headers, and the `umbral::sign` API surface that sessions consume.
...
- **CSRF for AJAX.** The cookie-to-header pattern (frontend reads `umbral_csrf` cookie, echoes it in `X-CSRF-Token`) vs a custom header issued out-of-band vs treating Bearer-token endpoints as automatically exempt. Affects how `umbral-rest` interacts with browser-driven SPAs.
...
- **`SECRET_KEY` rotation.** Whether umbral ships a key-rotation helper (old-key list for verify, new key for sign) or treats rotation as an ops concern with a recipe in the docs. Sessions and signed URLs both care.
