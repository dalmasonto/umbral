...
Promote at M8 entry, when `umbra-auth` needs to fire `post_login` for `umbra-sessions` and `umbra-admin` to react to — that is the first place the abstract design has to survive a real cross-plugin event. Promote earlier if any M5–M7 work needs lifecycle hooks before plugins exist.
...
- **Sender filtering.** Django's `sender=Post` argument scopes a handler to one model class. Lifecycle signals in umbra are typed (`Signal<SaveEvent<Post>>`), so per-model scoping falls out of the type system; whether custom signals need a runtime `filter: Fn(&Payload) -> bool` parameter is open.
