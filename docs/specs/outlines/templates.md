...
**Template inheritance.** `{% extends "base.html" %}` and `{% block content %}…{% endblock %}` work the way every Django developer expects. The admin ships a base template (`admin/base.html`) that user projects override by placing a file with the same path earlier in the search path.
