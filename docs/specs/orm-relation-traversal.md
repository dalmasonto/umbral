...
- **`Relation<T>`** — a lazy, single-target traversal handle returned by a to-one accessor (forward FK, forward O2O child-side, reverse O2O parent-side). It:
  - implements `IntoFuture<Output = Result<RelationTarget<T>>>` so `let x = a.b().await?` yields the object directly (honoring decision 2). `RelationTarget<T>` is `T` for a required relation and `Option<T>` for a nullable/reverse-O2O one (see "Return shapes");
  ...
  - offers explicit terminals `.get().await?` (alias of awaiting) and `.exists().await?`.
...
- Required forward FK (`ForeignKey<T>`, NOT NULL) → `Relation<T>` awaiting to `Result<T>`. A missing target row (referential integrity already broken, since the default DDL is RESTRICT) surfaces as an `Err`, never a silent `None`.
- Nullable forward FK (`Option<ForeignKey<T>>`) and reverse O2O parent-side → `Relation<T>` awaiting to `Result<Option<T>>` (absence is legitimate).
