...
| Suggested umbra shape | 1–2 bullets on what it would look like as a plugin. |
...
| **Suggested umbra shape** | Plugin owning a `content_type` table that mirrors Django's. Models opt in via a `#[umbra(generic_relation)]` attribute on the two-column pair. |
...
| **Suggested umbra shape** | Plugin with a `MessagesMiddleware` that reads from / writes to the session, an `umbra::messages::push(...)` helper, and a `{% messages %}` template tag. |
...
| **Suggested umbra shape** | Plugin with a `Site` model, a `current_site()` ambient accessor populated per request, and an `#[umbra(per_site)]` model attribute that injects the FK. |
| **Revisit signal** | A user actually needs one umbra install to serve content scoped per domain. |
...
| **Suggested umbra shape** | Plugin that registers filters on the `umbra::templates` engine: `{{ ts | time_ago }}`, `{{ n | intcomma }}`, etc. |
...
| **Suggested umbra shape** | Plugin with a `Redirect` model, a `RedirectFallbackMiddleware` that checks on 404, admin registration. |
...
| **Suggested umbra shape** | Plugin with a `Sitemap` trait (yields URLs + lastmod), a mount-point setting, per-plugin sitemap registration in `on_ready`. |
| **Revisit signal** | A public-facing umbra app needs SEO. |
...
| **Suggested umbra shape** | Plugin with a `Feed` trait, an `AtomRenderer` and `RssRenderer`, route registration via the plugin. |
...
| **Suggested umbra shape** | Plugin with a `FlatPage` model, a `FlatPageFallbackMiddleware`, admin registration. |
...
| **Suggested umbra shape** | A separate crate `umbra-gis` that depends on the umbra facade. Field types `Point`, `Polygon`. Spatial predicate methods on field columns (`location.within(area)`). |
...
| **Suggested umbra shape** | Plugin `umbra-i18n` that ships a `t!("string")` macro, a `LocaleMiddleware`, and template filters for localised dates and numbers. Probably rides on `fluent` rather than `gettext` so umbra is honest about not being a `.po` shop. |
...
| **Why deferred** | umbra's web layer needs to settle first. WebSockets in axum are already idiomatic; the value-add is the umbra-shape wrapper, which can't be designed before the deep web-layer spec lands. |
| **Complexity** | Medium. The wire protocol is axum's. What umbra owns is the Plugin-contributed handler shape (`fn ws_routes(&self) -> Vec<WsRoute>`?), auth and session integration, the broadcast-channel pattern. |
| **Suggested umbra shape** | Plugin or core extension that adds an `WsRouter` alongside `Router`, with `WsHandler` extractors that receive `Auth<User>` and `Session` automatically. Broadcast via tokio `broadcast::channel`. |
| **Revisit signal** | An umbra app needs real-time features (chat, notifications, collaborative editing). |
...
| **Purpose** | Run umbra against MySQL or Oracle instead of Postgres / SQLite. |
...
| **Suggested umbra shape** | A `MysqlBackend` (or `OracleBackend`) implementing `DatabaseBackend`. Lives in `umbra-core` next to `PostgresBackend` and `SqliteBackend`, or in a separate `umbra-mysql` crate if isolation matters. |
...
| **Purpose** | Let `umbra-tasks` run against Redis or AMQP instead of (or alongside) the default DB-backed broker. |
| **Why deferred** | The DB-backed broker is umbra's default for a reason — no extra infra. PRD §14 lists this as a future. |
...
| **Suggested umbra shape** | `RedisBroker` and `AmqpBroker` implementing the same `Broker` trait. Opt-in via cargo feature. Backend-specific guardrails (Redis can't do strict ordering; AMQP can). |
...
| **Why deferred** | Lower-value once `umbra-openapi` (Swagger UI) ships. Swagger covers most of the same UX with more polish. |
...
| **Suggested umbra shape** | A renderer in `umbra-rest` that activates when `Accept: text/html` is requested. Reuses form rendering from `forms.md`. Off by default in Prod. |
...
| **Purpose** | A first-party way to deploy an umbra app to a managed runtime. |
| **Why deferred** | PRD §14 explicitly out-of-scope. Hosting is the user's choice; umbra ships a binary, not a deploy product. |
| **Complexity** | Large. Cuts across CI, container builds, secrets management, log shipping, scaling rules. |
| **Suggested umbra shape** | Probably never as part of the framework itself. The most likely deliverable is a small `umbra-cli deploy <target>` plugin pattern that emits Dockerfiles, `fly.toml`, k8s manifests as templates. |
...
These two aren't really Django features so much as umbra surfaces that haven't grown big enough to deserve their own spec yet. They live here so they don't get lost.
...
| **Purpose** | A unified `umbra::Error` enum plus `From` impls so handler `?` flows, with a default `IntoResponse` that produces the right HTTP status per variant. |
| **Why deferred** | Currently cross-cutting: spec 03 says `umbra::Error` exists; spec 04 says validators emit `Error::Validation`; the web-layer outline open question #4 says "generic error → response mapping needs a home." No spec owns it end-to-end. |
...
| **Suggested umbra shape** | Probably a deep spec `08-error-model.md` with the enum, the `From` impls, the `IntoResponse` mapping, and an extension hook for plugin-defined variants. |
...
| **Purpose** | A coherent story for what umbra logs by default, what spans handlers create, and what fields go on every log line. |
| **Why deferred** | `tracing` is the obvious answer (`arch.md §2.1` marks it visible to user code) and works fine ad-hoc. Becomes a spec only when umbra needs to mandate a span shape (every request span carrying `request_id`, `user_id`, `plugin`, etc.). |
| **Complexity** | Small. A convention doc plus a small `umbra::tracing` module with span helpers; not a new subsystem. |
| **Suggested umbra shape** | A short outline `logging.md` or a section in `arch.md`. Document the request-span schema, the conventions plugins should follow, the suggested `tracing-subscriber` setup for Dev vs Prod. |
