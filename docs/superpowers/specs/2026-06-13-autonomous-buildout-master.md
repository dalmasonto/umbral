...
**PrimaryKey refactor STARTED** (user-prioritized — "avoid hardcoded i64 for ids"). Prereq ORM-gap queue was already empty. Shipped: **keystone** `pk_as_json` + `pk_key` (620a27c); **hydration lift** reverse-FK + OneToOne hydrators → PK-agnostic (b624594); **ReverseSet win** — a String-slug-PK parent carries a `ReverseSet` and prefetches end-to-end (0569078, `tests/pk_string_reverse_fk.rs`). i64 non-regression green throughout (882 umbra-core tests). **Remaining i64 sites** (see memory project-primary-key-refactor): OneToOne slot + lazy accessor, M2M slot + junction writes + M2M-junction prefetch, in_bulk / join-dedup in queryset/mod.rs, backup.rs restore, dynamic.rs set_m2m_from_strings. Lift one slot-kind per commit with a String-PK e2e test.
