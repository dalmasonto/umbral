...
- [~] **P3 #56–61** — #56/#57/#59 closed (c37e928); **#58 nested writes SHIPPED** (9ab22e8; true-DB-tx → orm_fixes #2); **#61 CSV export SHIPPED** (`?format=csv` on the list endpoint — 8103f70). **Still open:** #60 action input/output schemas; #61 remainder (CSV import, admin bulk-export action, Excel). Realtime SSE+WS retested green after the target wipe (full clean rebuild).
