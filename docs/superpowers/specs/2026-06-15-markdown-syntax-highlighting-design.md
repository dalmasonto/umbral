...
- **Ammonia widened minimally and only for inert attributes.** The markdown path cleans through an `ammonia::Builder` that additionally allows the `class` attribute on `pre`, `code`, and `span` — nothing else. `style`, `on*` handlers, and `javascript:` URLs stay stripped. This is the whole of the "safely" surface. The builder is constructed per call (not a shared static): `ammonia::Builder` is not `Sync` — its boxed `attribute_filter` field is not `Sync`-bound — so it can't sit in a `OnceLock` without a `Mutex`, and a `Mutex` would serialize all markdown rendering. Per-call construction costs the same as the prior `ammonia::clean`, which already built a default builder on every call.
...
- Restricting `class` *values* to the `hl-`/`language-` prefixes (e.g. via an ammonia `attribute_filter`). `class` is inert — it can't execute — so this is not a safety requirement; the only residual is minor CSS-class spoofing in untrusted notes. ammonia's exact-match `allowed_classes` can't express a prefix and syntect's token-class set is large and variable, so this is a deliberate, deferred hardening rather than a gap.
