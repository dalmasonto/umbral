...
```
crates/umbra-core/tests/fk_save_coercion.rs   (NEW — Phase A reproduction + Phase C regression pin)
crates/umbra-core/src/orm/write.rs            (EDIT — Phase B fix, json_to_sea_value FK arm)
crates/umbra-core/src/orm/dynamic.rs          (EDIT — Phase B, only under Suspect B/C branches)
...
**Files:**
- `crates/umbra-core/tests/fk_save_coercion.rs` (NEW)
- Copy the harness shape from `crates/umbra-core/tests/json_form_parse.rs:33-68` (in-memory SQLite pool + `CREATE TABLE` + `App::builder()...build()` + `ModelMeta::for_::<T>()`).
...
- [ ] Add a temporary diagnostic test `probe_fk_meta_shape` that boots, then for the `Child.parent` column prints the three values that disambiguate the suspects:
  ...
  eprintln!("pk_meta(fkparent)    = {:?}", umbra::migrate::pk_meta_for_table("fkparent")); // Suspect C check
  ```
  (Use the facade path `umbra::migrate::pk_meta_for_table` if exported; otherwise `umbra_core::migrate::pk_meta_for_table`.)
- [ ] Run `cargo test -p umbra-core --test fk_save_coercion probe_fk_meta_shape -- --nocapture` and record the printed triple. Interpret:
...
**Files:**
- `crates/umbra-core/src/orm/write.rs:399` (`json_to_sea_value` signature + FK arm at `:428`)
- `crates/umbra-core/src/orm/dynamic.rs:1138` (`insert_json` call site), `:1346` region (`update_json` call site — find the matching `json_to_sea_value(col.ty, ...)` call)
- `crates/umbra-core/src/orm/queryset/write_helpers.rs:67` (typed `create` call site)
...
**Files:**
- `crates/umbra-macros/src/lib.rs:2098` (the `ty` token emission) — verify the FK arm emits `SqlType::ForeignKey`.
- `crates/umbra-core/src/migrate.rs:834` (`From<&FieldSpec> for Column`) — verify `ty: f.ty` is copied, not normalized.
- `crates/umbra-core/src/inspect.rs:848` (the `inspectdb` path that builds columns from DB introspection) — a DB-introspected FK comes back as `INTEGER`/`BigInt` with `fk_target: None`, losing the marker.
...
**Files:**
- `crates/umbra-core/src/migrate.rs:121` (`pk_meta_for_table`), `:128` (the `CACHE` `OnceLock`)
...
**Files:**
- `crates/umbra-core/tests/fk_save_coercion.rs` (extend with the parallel typed-path assertion)
...
- [ ] Add the **parallel typed-path assertion** — the typed `create` path must bind the FK the same way as the dynamic path, so a future change to either can't silently reintroduce the text bind:
  ...
  ```
  (Use whatever the real `ForeignKey<T>` constructor is — `ForeignKey::from_id` / `ForeignKey::new` / `parent.id.into()`; check `crates/umbra-core/src/orm` for the exact API and match it.)
- [ ] Add a direct-bind invariant test asserting BOTH paths funnel through the same coercion, so the assertion can't be satisfied by accident:
  ...
  fn json_fk_arm_coerces_numeric_string_to_bigint() {
      ...
      let v = umbra::orm::write::json_to_sea_value(
      ...
      let t = umbra::orm::write::json_to_sea_value(
...
- [ ] `cd crates && cargo fmt && cargo clippy --all-targets && cargo build && cargo test` — the WHOLE workspace, not just `umbra-core` (a `write.rs` signature change ripples to every `json_to_sea_value` caller across crates; the facade re-export and plugins must still compile). Fix anything red; never `--no-verify`.
- [ ] Run the umbra-admin tests specifically (the original failing consumer surface): `cargo test -p umbra-admin`. They must pass — the admin's `insert_row`/`update_row` → `insert_form`/`update_form` path now binds FKs correctly.
