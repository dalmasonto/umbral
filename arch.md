...
- `reinhardt-db/src/migrations/dependency.rs` — migration dependencies are a first-class enum: `Required { app_label, migration_name }`, plus **`Swappable`** and **`Optional`** (conditional on whether another app is installed). Note what is and isn't new here: umbral **already has a swappable user model** — `umbral_auth::UserModel` is a trait, `LoggedIn<U: UserModel>` is generic over it, and `permissions` keys off a stringified user id precisely so a custom user model needs no adapter. What Reinhardt has that we don't is the *migration-graph* analogue: a dependency that resolves through a setting rather than naming an app directly. umbral defines `Migration::depends_on` and never reads it for ordering at all (see `planning/gaps3.md` #40); that's the real gap, and `Swappable`/`Optional` are what the type should probably grow into once it's load-bearing.
...
One divergence to hold deliberately: Reinhardt gates model attributes on backend cargo features (`#[cfg_attr(feature = "db-sqlite", model(strict = true))]`). We don't, and shouldn't. Django doesn't ask which backend you're on before letting you declare a field; it coerces to the backend's type and fails loudly when it genuinely can't. That's our posture too — the backend is a runtime `DbPool` variant, a field declares which backends it supports, and the **boot-time system check** catches an incompatible field with a clear message (`arch.md §4`). Pushing backend selection into cargo features leaks a deployment concern into the type system and forces every model author to think about it. The easier framework wins on exactly this kind of thing.
