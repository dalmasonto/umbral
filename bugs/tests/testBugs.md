...
- `umbra startproject shop`
- `umbra startapp content` (all content models: Category, Tag, Post, Comment, Page, Faq, Menu, MenuItem, Banner, Testimonial, ContactMessage, Subscriber, MediaAsset, Redirect, SiteSetting)
- `umbra startapp ecommerce` (e-commerce models: Brand, Product, ProductImage, ProductVariant, Customer, Address, Order, OrderItem, Payment, Shipment, Coupon, Review + join tables for M2M)
...
**Where:** `plugins/umbra-rest` — POST to `/api/product/` with `"is_featured": false`
...
**Where:** `crates/umbra-cli/src/scaffold.rs` line 476
**What:** The generated `umbra.toml` has `environment = "dev"` which causes `Settings::from_env()` to fail with: `UnknownVariant("dev", ["Dev", "Test", "Prod"])`.
...
**Where:** `crates/umbra-cli/src/scaffold.rs` in `scaffold_plugin()`
...
### BUG-4: `#[umbra(index)]` attribute not supported by `#[derive(Model)]`
**Where:** `crates/umbra-macros/src/lib.rs` — field-level attribute parser
**What:** The spec documents `index` for column-level indexes, but the macro rejects it with: "unknown field-level umbra attribute `index`".
**Workaround:** Omit `index` attribute; indexes can be added manually via raw SQL.
**Fix suggestion:** Add `index` to `UmbraFieldAttr` and emit `CREATE INDEX` in migration renderer.

### BUG-5: `#[umbra(auto_now_add)]` and `#[umbra(auto_now)]` not supported
**Where:** `crates/umbra-macros/src/lib.rs`
...
**Where:** `crates/umbra-macros/src/lib.rs` — struct-level attribute parser
...
**Where:** `crates/umbra-macros/src/lib.rs` — `classify_field_type()`
...
**Fix suggestion:** Add `ImageField` and `FileField` types that integrate with `umbra-media`.
...
**Where:** `crates/umbra-cli/src/scaffold.rs`
**What:** Generated plugin Cargo.toml depends on `umbra = { git = "..." }`, which doesn't work when developing locally in the same repo.
**Workaround:** Manually change to `path = "../../../crates/umbra"`.
**Fix suggestion:** Add `--local` flag to `startproject` / `startapp` / `startplugin` that emits path dependencies relative to the repo root. Or detect if running inside the umbra repo.

### BUG-18: `LoggedIn<U>` does not implement `Serialize`
**Where:** `plugins/umbra-auth/src/login_required.rs`
...
**Where:** `crates/umbra-cli/src/scaffold.rs` — generated `main.rs` comments
...
**Where:** `plugins/umbra-openapi/src/lib.rs` — `build_spec()`
**What:** The spec generator walks `umbra::migrate::registered_plugins()` and `models_for_plugin()` to emit one path per model table (`/api/{table}/`, `/api/{table}/{id}`). Custom routes contributed by plugins via `Plugin::routes()` — such as `/api/auth/register`, `/api/auth/login`, `/api/auth/logout`, `/api/auth/me` from `AuthPlugin::with_default_routes()` — are invisible to the spec generator. They don't appear in the Swagger UI or the `/openapi/openapi.json` spec.
...
**Where:** `crates/umbra-cli/src/scaffold.rs` — generated `main.rs`
...
### IMP-2: `#[umbra(default = "true")]` on bool stores string literal `true` as TEXT default in SQLite
...
### IMP-3: `#[umbra(min = N)]` / `#[umbra(max = N)]` validators not supported
**Where:** `crates/umbra-macros/src/lib.rs`
...
### IMP-5: No `#[umbra(backend = "postgres")]` field-level gate
**Where:** `crates/umbra-macros/src/lib.rs`
...
**Where:** `plugins/umbra-admin` — `view.rs`, `field_editor.html`, `fk_picker.rs`, ORM `#[derive(Model)]`
...
4. **The ORM lacks a dedicated `admin_search` abstraction for relation lookups.**
   ...
   - The user wants a first-class ORM method — something like `Model::objects().admin_search(query).fetch_for_admin()` — that:
     ...
     - uses the model's `#[umbra(string)]` field (or first non-PK text column) as the display label
...
- Django's `Model.__str__()` is the display label for FK/M2M options. Umbra's equivalent is `#[umbra(string)]` on a field, but there's no uniform "get display string for this row" API.
...
- Add `Manager<T>::admin_display_list() -> Vec<(T::PrimaryKey, String)>` that returns PK + display label, using `#[umbra(string)]` or first non-PK text column.
