...
78. [ ] RestPlugin expand Fks and M2M relationships - This is more like a feature where a user, in the rest plugin setup, can select tables and expand specific relationships (Fks and M2M) to see the related data. Since data can be nested around tables, we need to support this recursively like 1 level by another, the user has to define that.
       — Deferred. The ORM has `select_related` for FKs, but RestPlugin's `fetch_rows` doesn't thread it. Scope: a `ResourceConfig::expand([("author", 1), ("comments", 2)])` builder method, then in the row-building path do per-row JOINs / batched SELECTs (N+1 avoidance via `IN`-list grouping), then nest the result under the FK column name. Depth control needs an explicit limit to stop accidental quadratic explosion. Separate spec; pairs naturally with the OpenAPI plugin so the `$ref` chain reflects expand depth.
