...
13. **`umbra-static` should grow an embedded-assets source.** Today `StaticPlugin::new(mount, dir)` wraps `tower_http::ServeDir`, which requires a real filesystem path. That works for dev (where the assets live next to the source) but is the wrong shape for any plugin that wants to ship its UI inside the binary — the playground hit this when filesystem `dist/` wipes between rebuilds 404ed live browser tabs holding the old asset hashes. The playground now inlines the right pattern via `include_dir!` + a custom in-memory handler (see `plugins/umbra-playground/src/routes.rs::assets`). The framework-level fix is for `umbra-static` to accept either a directory **or** an `&'static Dir` from `include_dir!`, so plugin authors can opt into self-contained asset shipping via `StaticPlugin::embedded(mount, &ASSETS)`. Then the playground (and any future plugin that ships an HTML/CSS/JS UI) drops its bespoke handler and goes through the plugin again — restoring the original dogfood-the-plugin design without giving up the embed-into-binary property.
