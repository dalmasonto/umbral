...
11. [~] **Persist all admin UI state into `AdminUserPref` — filters, sort orders, page sizes, search, per-table preferences.** PARTIAL: filter / search / sort / per_page round-trip shipped in commit `89fba96`. `AdminUserPref` grew `preferences: Option<String>` (nullable; existing rows read as "no prefs yet" without a backfill); new helpers `get_table_pref` / `set_table_pref` read-modify-write the JSON blob keyed at `preferences.tables.<table>`. The changelist handler now (a) 303-redirects to the saved URL on paramless visits, and (b) persists the current shape on every render. Cross-device continuity for free — same user_id reads the same prefs on phone + laptop because the URL gets restored server-side. 4 new round-trip tests in `plugins/umbra-admin/tests/phase4_user_prefs.rs` (None-for-new-user, full-round-trip, per-table namespacing, malformed-JSON-graceful). Remaining v1 slices deferred to follow-up commits: column visibility (per-table `hidden_cols`), `last_path` for `/admin/` landing, dashboard widget-period overrides, "reset to default" affordance for users who want a clean changelist. Original symptom + design preserved below: `plugins/umbra-admin/src/models.rs:43` already has `AdminUserPref { theme, density, sidebar_collapsed, dashboard_layout, updated_at }` and the upsert plumbing (`fetch_or_default`, `upsert`). Only two pieces of UI state currently round-trip through it: theme + sidebar-collapsed (typed columns) and the dashboard layout (a JSON blob in `dashboard_layout: String`). Everything else lives in the URL and dies on refresh / new tab / restart:
