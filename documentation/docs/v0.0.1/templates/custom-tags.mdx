...
Registrars run after the framework's own filters/functions (`img`, `static`, `media_url`, `markdown`, `now`, `currency`, `sanitize`) and in plugin dependency order. Because minijinja's `add_filter` / `add_function` overwrite by name, a plugin *can* deliberately replace a built-in by registering the same name. Do this sparingly and document it.
</Step>
