...
umbra templates run on [minijinja 2](https://docs.rs/minijinja/latest/minijinja/), which ships a Jinja2-compatible filter set out of the box. On top of that standard library, umbra-core registers a handful of custom filters and functions of its own (`img`, `markdown`, `sanitize`, `now`, `currency`) and injects ambient globals into every render (`csrf_token`, `csrf_input`, plus the anonymous-safe `user`). Plugins can add their own — see [Custom template tags](custom-tags).
...
## umbra's own filters and globals

These are registered by umbra-core's template engine (`crates/umbra-core/src/templates.rs`) and are available in every `.html` template the framework renders, on top of the minijinja standard library below.
...
| `user` | global | The current user, injected into every render. Authenticated requests get the full serialized user when `AuthPlugin::with_user_in_templates()` is on; otherwise the anonymous sentinel `{ is_authenticated: false, is_staff: false, is_superuser: false }`. | `{% if user.is_authenticated %}...{% endif %}` |
...
<Callout type="info">
**`None` and `Undefined` render as the empty string.** umbra installs a custom formatter so an `Option<String>` field that is `None`, or an undefined variable, renders as `""` instead of the literal tokens `none` / `undefined`. This means `<input value="{{ form.phone }}">` on a fresh form yields `value=""`, not `value="none"`. Non-null values pass through the normal escaping formatter unchanged.
</Callout>
