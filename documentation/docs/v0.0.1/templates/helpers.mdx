...
umbra templates run on [minijinja 2](https://docs.rs/minijinja/latest/minijinja/), which ships a Jinja2-compatible filter set out of the box. On top of that standard library, umbra-core registers a handful of custom filters of its own (`img`, `markdown`, `sanitize`) and injects two ambient globals into every render (`csrf_token`, `csrf_input`, plus the anonymous-safe `user`).
...
Custom filter registration *by plugins* (through `Plugin::on_ready`) is still deferred. The admin plugin registers its own `naturaltime` filter against its private environment, but that filter is scoped to admin templates - it isn't available in app-level templates.
