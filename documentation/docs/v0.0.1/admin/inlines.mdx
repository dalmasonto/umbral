...
The struct-literal form still works - `InlineModel { model, fk_field, list_display, ..Default::default() }` sets the other knobs to their defaults - but prefer the `::new(...)` constructor plus chainable setters.
