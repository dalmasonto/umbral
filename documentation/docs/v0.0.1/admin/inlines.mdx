...
```rust
use umbra_admin::{AdminPlugin, AdminModel, InlineModel, InlineKind};
...
`"comment"` is the child SQL table. `"post"` is the FK column on `comment` — note that a `ForeignKey<Post>` field named `post` maps to a column named `post` (umbra does **not** append `_id`). `&["text", "rating"]` are the editable columns shown per row; the FK and primary key are managed for you and never rendered.
...
Two layouts, like Django:

<Tabs>
<Tab title="Tabular (default)">
...
Inlines render and save on the admin's **full-page change form** (`/{table}/new` and `/{table}/{id}/edit`) — the path the changelist's "Add" and "Edit" buttons link to. The HTMX sheet variant is a follow-up.
