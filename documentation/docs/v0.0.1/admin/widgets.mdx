...
| `Heatmap` | Activity-by-time / cohort patterns — a 2-D grid of cells colored by magnitude (day × hour signups, retention by cohort). | [`HeatmapPayload`](#heatmap-) |
...
```rust
use umbra_admin::widgets::{RadialPayload, WidgetPayload};
...
Pass two to four tracks to compare related ratios on one tile — the centre then shows their average:
...
Every percent is clamped to `[0, 100]` (non-finite → 0), so an overshoot fills the ring rather than overrunning it. For an explicit arc color, build `RadialTrack { label, value, color: Some("#34d399") }` and pass to `RadialPayload::new(tracks)`.

## Heatmap

A 2-D grid of cells colored by magnitude (ApexCharts `heatmap`). Reach for it when the story is *when* or *where* something concentrates — signups by weekday × hour, retention by cohort × week, load by region × day.

The dense-grid constructor takes row labels, the shared column labels, and a `values[row][col]` matrix:

```rust
use umbra_admin::widgets::{HeatmapPayload, WidgetPayload};
...
`from_grid` is rectangular by construction: a short value row is padded with `0.0` and any extra values past the column labels are dropped, so ragged input never desyncs the grid. For full control build `HeatmapRow { name, cells: vec![HeatmapCell { x, y }] }` and pass to `HeatmapPayload::new(rows)`. ApexCharts shades each cell from a single hue by value; the payload's first row renders at the top.

## Progress

A ranked list of labeled horizontal bars — "top N by metric": revenue by product, traffic by source, completion per category. It's the readable middle ground between a `Donut` (proportions, but circular and capped at ~6 slices) and a `Table` (exact numbers, but no visual weight). Rendered as pure HTML — no chart library mounts.

By default each bar is sized relative to the largest value, so the top item fills the bar:

```rust
use umbra_admin::widgets::{ProgressPayload, WidgetPayload};
