---
title: Permissions
description: Built-in permission classes (AllowAny, IsAuthenticated, IsStaff, ReadOnly), Or / And combinators, and how to write your own.
sidebar_position: 3
icon: shield
...
One permission class is attached **per resource** via `ResourceConfig::permission(...)`. A resource without an explicit class uses `AllowAny`.

## Built-in classes

<CardGrid cols={2}>
  <Card icon="unlock" title="AllowAny" description="The default. Every action allowed, anonymous OK. Use for public catalogues, healthchecks, public read endpoints." />
  <Card icon="user-check" title="IsAuthenticated" description="Require some identity. Anonymous → 401, any authenticated user → allow." />
  <Card icon="shield-check" title="IsStaff" description="Require identity AND is_staff. Anonymous → 401, non-staff → 403, staff → allow. Most admin-adjacent endpoints." />
  <Card icon="eye" title="ReadOnly" description="List / Retrieve open to everyone, Create / Update / Delete → 403. Public-read APIs." />
</CardGrid>
...
Worked example: `IsOwner` - list and retrieve work for everyone, but write actions on a row only succeed if the row's `user_id` matches the caller's `user_id`. The row-level check happens inside the handler (the permission layer runs *before* the row is loaded); the permission class enforces the framing.
...
impl Permission for IsOwnerOrReadOnly {
    fn check(
        ...
        match identity {
            Some(_) => Ok(()),
            None => Err(PermissionError::AuthenticationRequired),
        }
    }
}
```
