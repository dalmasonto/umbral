---
title: Throttling
description: DRF-style API rate limiting — AnonRateThrottle, UserRateThrottle, ScopedRateThrottle, the rate string format, and the 429 + Retry-After response.
sidebar_position: 8
icon: gauge
tags: [rest, throttling, rate-limiting]
---

A throttle answers *how often may this caller hit this resource?*. It's the third request-time gate, run **after** [authentication](/docs/v0.0.1/rest/authentication) resolves an `Identity` and the [permission](/docs/v0.0.1/rest/permissions) check approves the action, but **before** the handler runs (the same order as Django REST Framework). A request that passes auth and permission can still be rejected with **429 Too Many Requests** if the caller is over their rate.
...
<Callout type="info">
Throttles are backed by an in-memory sliding-window limiter (`umbra::ratelimit::RateLimiter`). State lives in the process, so a multi-instance deployment behind a load balancer gives each replica its own counters (effective limit `rate × replicas`). A Redis-backed store is the multi-instance follow-up.
</Callout>
...
```rust
use umbra_rest::{AnonRateThrottle, UserRateThrottle, ResourceConfig, RestPlugin, ScopedRateThrottle};
...
Both `default_throttle` and `.throttle(...)` take a DRF-style `"<num>/<period>"` string. The period token is case-insensitive:
...
On the first throttle that denies, the request short-circuits to **429 Too Many Requests** with a `Retry-After` header (whole seconds, rounded up) and a DRF-shaped body:
...
```rust
use umbra_rest::{Throttle, ThrottleContext, ThrottleDenied};
...
Wrap a `umbra::ratelimit::RateLimiter` to reuse the sliding-window counting the built-ins use.

## See also

- Design rationale: the `umbra-rest` throttle module (`plugins/umbra-rest/src/throttle.rs`) and the core primitive (`crates/umbra-core/src/ratelimit.rs`).
