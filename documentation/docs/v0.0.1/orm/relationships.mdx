...
pub struct Account {                  // a model YOU define
    pub id: i64,
    pub email: String,
    // Reads back the `Profile` whose `#[umbra(unique)] user: ForeignKey<Account>`
    // points here. No `#[umbra(...)]` needed — the back-link is found at runtime.
    #[sqlx(skip)]
