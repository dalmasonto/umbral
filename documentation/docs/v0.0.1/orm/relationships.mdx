...
v1 scope: M2M only (`prefetch_related("comment_set")` for reverse-FK collections is a deferred follow-up); i64 parent PK only (matches the rest of the M2M plumbing).
