...
This is the right shape for `request.user.profile.avatar`-style templating across crates: one method call, one `LIMIT 1` query, no need for the user to know the back-link convention.

To reach the same shape from inside a template (`{{ user.profile.avatar }}`), turn on [user in templates](/v0.0.1/auth/user-in-templates) — `AuthPlugin::with_user_in_templates()` mounts a middleware that injects the AuthUser into every render context. The cross-crate accessor then composes naturally: the handler resolves `user.profile()`, stuffs it into the render context, and the template walks the relation.
