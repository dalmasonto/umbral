...
<Callout type="info">
The column stays plain `TEXT` — `widget` is a rendering hint, not a data type. With no enhancement JavaScript the admin editor degrades to a tall textarea, which is fully usable; the `data-widget="markdown"` attribute is the hook a future split-pane editor mounts on. Syntax-highlighted code blocks and a live preview pane are tracked as follow-ups.
</Callout>
