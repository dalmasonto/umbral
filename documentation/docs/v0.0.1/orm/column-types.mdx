...
| `#[umbra(widget = "markdown" \| "rte" \| "textarea")]` | Presentation hint for form renderers (features.md #4). Metadata only — the column stays `TEXT`, no DDL change. `markdown` / `rte` render a tall editor textarea (carrying `data-widget` for progressive enhancement); pair with the [`\| markdown` filter](#rendering-markdown-safely) on the display side. An unrecognised widget name falls back to the type-derived input, so plugin-defined widgets never break the form. |
