...
The macro reads every field-level attribute below straight off the struct field. Empty cells (`-`) mean the attribute has no effect at that layer; cells with a `✓` mean the framework wires the corresponding behaviour automatically.
...
| `#[umbra(default = "...")]` | DDL default for the column. Required when adding a NOT NULL column to a populated table — Postgres / SQLite reject the ALTER otherwise. Also the value the DB uses when an INSERT omits the column. |
| `#[umbra(help = "...")]` | OpenAPI `description` + admin form help text. Carries the column's intent into both surfaces without a second source of documentation. |
...
| Field | `choices` | The field's type implements [`umbra::orm::ChoiceField`] (typically via `#[derive(Choices)]`). Stored as `TEXT`; admin renders a `<select>`; Postgres gets a `CHECK` constraint. See ["Using a Rust enum"](#derivechoices---using-a-rust-enum-as-a-field-type). |
| Field | `default = "..."` | SQL `DEFAULT` clause for the column. String literal at v1; emitted on both `CREATE TABLE` and `ALTER TABLE ADD COLUMN`. Required when adding a NOT NULL column to a non-empty table — Postgres / SQLite reject the `ADD` otherwise. |
