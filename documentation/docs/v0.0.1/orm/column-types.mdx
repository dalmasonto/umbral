...
`Vec<T>`, `ipnetwork::IpNetwork`, `mac_address::MacAddress`, `umbra::orm::TsVector`, and `rust_decimal::Decimal` (a fixed-point `NUMERIC(19, 4)`, non-nullable only — `Option<Decimal>` isn't supported yet) are Postgres-only. A model that includes one of these fields:
