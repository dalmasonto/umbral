...
| `rust_decimal::Decimal` + `#[umbral(precision = N, scale = M)]` (Postgres-only) | `DecimalN` | `NUMERIC(N, M)` | - (boot fails) |
| `bigdecimal::BigDecimal` (Postgres-only) | `BigDecimal` | `NUMERIC` (unbounded) | - (boot fails) |
| `umbral::orm::gis::Geometry` + `#[umbral(geometry = "...", srid = N)]` (Postgres-only, `postgis` feature) | `Geometry` | `geometry(<kind>, <srid>)` | - (boot fails) |
...
| `#[umbral(precision = N, scale = M)]` | On a `rust_decimal::Decimal` field, render `NUMERIC(N, M)` instead of the default `NUMERIC(19, 4)` - `numeric(5, 2)` for a percentage, `numeric(19, 8)` for an FX rate. Precision must stay within rust_decimal's ~28-digit ceiling; beyond that use `bigdecimal::BigDecimal`. Postgres-only. |
| `#[umbral(geometry = "point", srid = 4326)]` / `#[umbral(geography = "...")]` | On a `umbral::orm::gis::Geometry` field (the `postgis` feature), a PostGIS `geometry(<kind>, <srid>)` / `geography(...)` column. Pair with `#[umbral(index)]` for a GiST index. See [PostGIS geometry](/docs/v0.0.1/orm/gis). Postgres-only. |

### Mapping a field to a differently-named column - `#[sqlx(rename)]`

By default a field's Rust name **is** its database column name. To point a
pretty Rust field at a differently-named column, use sqlx's own attribute -
umbral honors it for the column name (and `sqlx::FromRow` reads from the same
...
pub struct Post {
    pub id: i64,
    #[sqlx(rename = "author_id")]   // Rust field `author`, DB column `author_id`
    pub author: ForeignKey<User>,
}
```

The column constant (`post::AUTHOR`) is still named after the Rust field but
targets the `author_id` column. [`inspectdb`](/docs/v0.0.1/migrations/inspectdb#generated-models-compile)
uses this to escape a column whose name is a Rust keyword — a `type` column
becomes `pub type_` with `#[sqlx(rename = "type")]`.
...
<Callout type="info">
`#[derive(Choices)]` emits its own `serde::Serialize` / `Deserialize` (matching `rename_all`), so the enum must **not** also derive `Serialize` / `Deserialize` or carry `#[serde(...)]` attributes — doing both is a conflicting-implementation error. This single-sources the JSON/wire form with the stored DB value, so a Choices field round-trips the typed write path (`create` / `get_or_create` / `update_or_create`) with only `#[choices(rename_all = "...")]` — no duplicated `#[serde(rename_all)]`.
</Callout>
...
**Filtering by a choice value.** The column constant accepts the **enum variant** directly - the type-safe form - and still accepts the raw stored string for dynamic callers. Both bind the same `rename_all`'d text, because the variant serializes through the same `ChoiceField::as_str` mapping the write path uses, so `#[choices(rename_all = ...)]` stays single-sourced and you never restate the DB literal:

```rust
// All published posts - pass the variant, not a string literal.
let live = Post::objects()
    .filter(post::STATUS.eq(PostStatus::Published))
    .fetch()
    .await?;

// The stored-string form still works (e.g. a value computed at runtime).
let also_live = Post::objects()
...
let active = Post::objects()
    .filter(Q::or(
        post::STATUS.eq(PostStatus::Review),
