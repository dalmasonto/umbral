...
`#[umbral(string)]` is the sibling attribute - it marks one field as the Django-style `__str__`, so the admin's default `list_display` becomes `[<that field>, <other short columns>]` when no explicit config is registered. See [Models - Display hints](./models#display-hints-string-max_length) for the full opt-in story.
...
| `#[umbral(string)]` / `#[umbral(string = true)]` | Mark this field as the Django-style `__str__`. The admin uses it as the label column in the default `list_display` and as the picker label in FK / M2M choosers. See [Models - Display hints](./models#display-hints-string-max_length). |
