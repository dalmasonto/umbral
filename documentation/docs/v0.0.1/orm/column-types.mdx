...
| `ipnetwork::IpNetwork` + `#[umbra(cidr)]` (Postgres-only) | `Cidr` | `CIDR` | - (boot fails) |
| `mac_address::MacAddress` (Postgres-only) | `MacAddr` | `MACADDR` | - (boot fails) |
| `mac_address::MacAddress` + `#[umbra(macaddr)]` (Postgres-only) | `MacAddr` | `MACADDR` | - (boot fails) |
| `String` + `#[umbra(xml)]` (Postgres-only) | `Xml` | `XML` | - (boot fails) |
| `String` + `#[umbra(ltree)]` (Postgres-only) | `Ltree` | `LTREE` | - (boot fails) |
| `String` + `#[umbra(bit)]` (Postgres-only) | `Bit` | `BIT VARYING` | - (boot fails) |
...
| Attribute | Effect |
|---|---|
| `#[umbra(auto_now_add)]` | Stamp `Utc::now()` on INSERT. The required-field check skips the column; clients can omit it from the body and the ORM fills it in at the write seam. Standard `created_at` pattern. |
| `#[umbra(auto_now)]` | Stamp `Utc::now()` on INSERT **and every UPDATE**. `auto_now_add` columns stay frozen at their creation timestamp; `auto_now` columns refresh continuously. Standard `updated_at` pattern. |
| `#[umbra(noform)]` | Client never writes - stripped from `insert_json` / `update_json` bodies. The admin hides the column on every form. Use for `password_hash`, internal tokens, server-managed audit fields. |
| `#[umbra(noedit)]` | Admin edit form shows the field read-only (UX hint, not an API contract). REST writes still accept the value. |
| `#[umbra(index)]` | Single-column index in the DDL. The migration engine emits `CREATE INDEX` on both backends. |
| `#[umbra(unique)]` | UNIQUE constraint in the DDL. UNIQUE violations on insert / update surface as `WriteError::UniqueViolation { field, value }` - REST renders them as `{ "slug": ["A row with slug='widget' already exists."] }`. |
| `#[umbra(default = "...")]` | DDL default for the column - the string lands verbatim in the `DEFAULT` clause (booleans are translated to `1`/`0` on SQLite). For a `choices` field use the DB literal (`"draft"`), **not** the Rust path (`"PostStatus::Draft"`); see [Defaults: two mechanisms](#defaults-two-mechanisms-two-layers). Required when adding a NOT NULL column to a populated table - Postgres / SQLite reject the ALTER otherwise. Also the value the DB uses when an INSERT omits the column. |
| `#[umbra(help = "...")]` | OpenAPI `description` + a hint line under the admin form input. One place to say "use Markdown here" or "ISO-8601 date": it reaches both the API docs and the editor. |
| `#[umbra(widget = "markdown" \| "rte" \| "code" \| "textarea")]` | Presentation hint for form renderers (features.md #4). Metadata only, no DDL change. In the admin: `markdown` mounts an EasyMDE editor (toolbar + sandboxed live preview), `rte` a Quill rich-text editor (stores HTML, render with `\| sanitize`), `code` a CodeMirror editor (JSON syntax + line numbers) for JSON/structured text. Pair `markdown` with the [`\| markdown` filter](#rendering-markdown-safely) on the display side. An unrecognised widget name falls back to the type-derived input, so plugin-defined widgets never break the form. |
| `#[umbra(example = "...")]` | OpenAPI `example` value. Generated clients show it as a placeholder; the playground uses it to seed the request body. |
| `#[umbra(on_delete = "cascade" \| "restrict" \| "set_null" \| "no_action")]` | FK `ON DELETE` clause. Default `no_action` (SQL standard's default - no extra clause emitted). Applies only to `ForeignKey<T>` columns; a typed error fires at derive time if you set it on a non-FK field. |
| `#[umbra(on_update = "...")]` | FK `ON UPDATE` clause. Same vocabulary as `on_delete`. |
| `#[umbra(min = N)]` / `#[umbra(max = N)]` | Numeric range validator. The dynamic-write path returns `WriteError::Validator { field, message }`; Postgres also gets a DDL CHECK so direct DB writes are caught. SQLite skips the CHECK (its CHECK syntax is more friction than safety here). |
| `#[umbra(backend = "postgres")]` / `#[umbra(backend = "sqlite")]` | Restrict this field to a specific backend. The boot-time system check fails when the active backend isn't in the allow-list. Repeat the attribute to allow several. |
| `#[umbra(string)]` / `#[umbra(string = true)]` | Mark this field as the Django-style `__str__`. The admin uses it as the label column in the default `list_display` and as the picker label in FK / M2M choosers. See [Models - Display hints](./models#display-hints-string-max_length). |
...
| `#[umbra(choices)]` | The field's type implements [`umbra::orm::ChoiceField`] (typically via `#[derive(Choices)]`). Stored as TEXT; admin renders a `<select>`; Postgres gets a CHECK constraint. See ["Using a Rust enum"](#derivechoices---using-a-rust-enum-as-a-field-type) below. |
| `#[umbra(cidr)]` | On an `ipnetwork::IpNetwork` field, declare it as Postgres `CIDR` (network address) instead of the default `INET`. Postgres-only. |
| `#[umbra(macaddr)]` | Explicit marker for a `mac_address::MacAddress` field (`MACADDR`). The type already auto-detects, so this only documents intent. Postgres-only. |
| `#[umbra(xml)]` / `#[umbra(ltree)]` / `#[umbra(bit)]` | On a `String` (or `Option<String>`) field, declare it as a Postgres `XML` / `LTREE` / `BIT VARYING` column instead of `TEXT`. Text-backed - the Rust type stays `String`. Postgres-only. See [Text-backed Postgres types](#text-backed-postgres-types---xml-ltree-bit). |
...
pub struct Post {
    ...
    pub title: String,
    pub body: String,                              // unbounded TEXT
    #[umbra(unique, max_length = 80)]              // UNIQUE constraint
    pub slug: String,
    #[umbra(index)]                                // single-column index
    pub status: String,
    #[umbra(min = 0, max = 100_000)]               // bounded integer
    pub view_count: i64,
    ...
    #[umbra(noform)]                               // never appears on any form
...
Foreign derive attributes (`#[serde(...)]`, `#[sqlx(...)]`, `#[validate(...)]`, etc.) are passed through untouched - the derive doesn't reject them, which lets you stack umbra's macro on top of whatever serialization / validation stack you already use.

## All derive macros at a glance

umbra ships four proc-macros. The model derive is the most-used; the choices derive turns a Rust enum into a column field type; the form derive validates request bodies; the task attribute marks background jobs. Each gets a worked example below.
...
|---|---|---|
| `#[umbra(default = "draft")]` on the **field** | Schema. The string lands **verbatim** in the migration's `DEFAULT` clause. | The database, whenever an INSERT omits the column. |
...
| Item | Why |
|---|---|
| `impl umbra::orm::ChoiceField` | Trait carrying `VALUES` (DB strings) + `LABELS` (admin display) + `as_str` + `from_str_ok`. The `Model` derive reads `VALUES` / `LABELS` at compile time to populate the column's `FieldSpec`. |
...
The Rust type drives detection - no `#[umbra(multichoice)]` marker is required; the macro recognises `MultiChoice<E>` directly. `#[umbra(default = "...")]` is a CSV of the initial variants, used both as the SQL `DEFAULT` and as the value `ALTER TABLE ADD COLUMN` applies to existing rows.
...
| Attribute marker | `#[umbra(choices)]` required | Detected from the type - no marker |
...
`Slug` only validates — it doesn't generate. Add `#[umbra(slug_from = "<column>")]` to fill the slug from a sibling column. `slugify` lowercases the source, replaces runs of non-alphanumerics with a single `-`, and trims dashes, so `"Hello, World!"` becomes `"hello-world"`.
...
<Callout type="warning">
`slug_from` runs on the **dynamic write path** — REST endpoints and the admin (anything going through JSON `create` / `update`). The **typed** path `Article::objects().create(obj)` does **not** auto-fill; build the value yourself with `umbra::orm::slugify(&title)`.
</Callout>
...
A `#[umbra(unique)]` slug is enforced by the **database constraint** — umbra runs no application-level pre-check. The dynamic update is `UPDATE … WHERE <pk> = id`, so **re-sending a row its own slug never conflicts** (no *other* row holds that value); you do **not** need to strip the slug from an update payload. A violation fires only when the slug collides with a **different** row — for example, renaming a title so its slug matches an existing post's.
...
`Vec<T>`, `ipnetwork::IpNetwork`, `mac_address::MacAddress`, `umbra::orm::TsVector`, `rust_decimal::Decimal` (a fixed-point `NUMERIC(19, 4)`; both `Decimal` and `Option<Decimal>` are supported), and the text-backed types `XML` / `LTREE` / `BIT VARYING` are Postgres-only. A model that includes one of these fields:
...
pub struct Document {
    pub id: i64,
    #[umbra(xml)]   pub body: String,          // XML
    #[umbra(ltree)] pub path: String,          // LTREE  (e.g. "Top.Science.Astronomy")
    #[umbra(bit)]   pub flags: String,         // BIT VARYING  (e.g. "101")
    #[umbra(xml)]   pub note: Option<String>,  // nullable XML
}
```

Notes and limits at v1:

- `LTREE` requires the `ltree` extension in the target database (`CREATE EXTENSION ltree`). umbra emits the bare `ltree` column type; installing the extension is the operator's job.
- `bit` renders as variable-length `BIT VARYING`. A fixed-width `BIT(n)` needs a hand-written migration until a `#[umbra(bit_len = N)]` attribute lands for a real consumer.
- `#[umbra(macaddr)]` is an explicit marker for a `mac_address::MacAddress` field. The type already auto-detects to `MACADDR`, so the attribute only documents intent.
- `inspectdb` recovers these columns as plain `String` fields (the attribute lives only in the source model, not the database), so re-add `#[umbra(xml)]` / `#[umbra(ltree)]` / `#[umbra(bit)]` if you want the native type back on a re-migrate.
...
Text/body fields (plugin descriptions, usage docs, reviews) are usually authored in Markdown. Capture the source in a plain `String` column, hint the editor with `#[umbra(widget = "markdown")]` + `#[umbra(help = "...")]`, and render it on the display side with the built-in `| markdown` filter:
...
pub struct Plugin {
    pub id: i64,
    pub name: String,
    #[umbra(widget = "markdown", help = "Markdown supported: headings, lists, tables, code.")]
    pub body: String,
}
```
...
`#[umbra(widget = "rte")]` mounts a [Quill](https://quilljs.com) rich-text editor instead. Unlike `markdown`, the `rte` field **stores HTML**, so render it with the `| sanitize` filter (Quill's HTML, cleaned to a safe allowlist, the display companion to `| markdown`):

```rust
#[umbra(widget = "rte", help = "Rich text.")]
...
`#[umbra(widget = "code")]` mounts a [CodeMirror](https://codemirror.net) editor with JSON syntax highlighting and line numbers: the comfortable way to edit a `Json` column or any `String` holding structured text:

```rust
#[umbra(widget = "code", help = "JSON object, see the schema docs.")]
