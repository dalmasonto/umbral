...
Other Django-style attributes (`db_index`, `default`, `on_delete`, `unique`, `nullable`) aren't part of the derive's accept-list. They land as a real plugin or workload surfaces the need. Until then:
...
| `choices = [...]` | `#[derive(Choices)]` on the enum + `#[umbra(choices)]` on the field. The Rust type is the source of truth; Postgres gets a CHECK constraint; the admin renders `<select>`. See ["Using a Rust enum as a field type"](#derivechoices---using-a-rust-enum-as-a-field-type) below. |
| `on_delete = CASCADE` | `ForeignKey<T>` lands the FK + REFERENCES clause; ON DELETE behaviour beyond the default RESTRICT is deferred. Edit the generated migration JSON if you need a different policy. |
