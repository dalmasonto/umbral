...
// comment.plugin.author.name round-trips from that single query; the
// nested hops are cache-served with zero further queries, since
// join_related populates the same slots select_related does.
let plugin = comments[0].plugin().get_opt().await?.unwrap(); // plugin is a nullable FK
let author = plugin.author().await?;
...
let posts = Post::objects()
    ...
    .fetch()
    .await?;
let tags = posts[0].tags().fetch().await?; // cache-served, zero further queries
let cat = tags[0].category().await?;
```
