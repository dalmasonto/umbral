...
The surface lives in `umbra::orm`: `Searchable`, `Search`, and `SearchHit`.
...
These are the **SQL column names** passed as strings, not Rust column constants. For umbra models the column name is the struct field name — the `short_description` field is the `"short_description"` column — so use the field name verbatim.
</Callout>
...
The string is a **static SQL boolean ANDed into the search `WHERE`**. Soft-delete is handled for you: a `#[umbra(soft_delete)]` model automatically excludes `deleted_at IS NULL`, so `filter_sql` is only for business rules.
...
- **FK columns** — compared to the parent's id (umbra's FK column is the field name): `author = 5`, or `category IN (1, 2, 3)`.
