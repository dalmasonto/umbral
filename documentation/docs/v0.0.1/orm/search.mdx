...
The string is a **static SQL boolean ANDed into the search `WHERE`** — it must name real columns and contain **no user input** (it's your constant, never a request value). Soft-delete is handled for you: a `#[umbra(soft_delete)]` model automatically excludes `deleted_at IS NULL`, so `filter_sql` is only for business rules like `"status = 'published'"`.
