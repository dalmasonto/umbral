...
| `post_delete:<table>` | `Manager::delete_instance` (after), **and** per-row from `QuerySet::delete` / `DynQuerySet::delete` (full row when subscribed, else PK-only) | `{ instance, actor }` |
...
## Excluding sensitive fields from payloads

The `instance` / `previous` payloads carry the **whole serialized row**. A subscriber that logs or persists the payload — the natural audit-log shape — would copy every column into its store, including password hashes, tokens, and PII. Mark such fields `#[umbral(signal_skip)]` and they are stripped from every signal payload before it fans out:

```rust
#[derive(sqlx::FromRow, umbral::orm::Model)]
struct Account {
    id: i64,
    email: String,
    #[umbral(signal_skip)]
    password_hash: String, // never appears in any signal payload
}
```

`umbral-auth`'s `AuthUser.password_hash` already carries this. The per-row bulk signals (`bulk_post_save` / `bulk_post_delete`) are unaffected — they only carry primary keys, never field values.
...
## Transactional hooks: atomic "create A ⇒ B exists"

`subscribe` / `subscribe_async` handlers run **after** the write commits and **cannot fail it** — perfect for cache invalidation or an outgoing webhook, wrong for an invariant like "every `AuthUser` must have a `Profile`". If that handler runs post-commit and its own insert fails (or the process restarts first), you get a user with no profile, and nothing surfaces it.

`subscribe_txn` closes that gap. A transactional handler runs **inside the creating transaction**, receives `&mut Transaction` so it can write dependent rows on the *same* tx, and returns a `Result` — an `Err` **rolls the whole create back**. So either the parent and its dependents all commit, or none do.

```rust
use umbral::signals::subscribe_txn;

// Every AuthUser create also creates its Profile — atomically.
subscribe_txn("post_save:auth_user", |payload, tx| Box::pin(async move {
    let uid = payload["instance"]["id"].clone();
    let mut body = serde_json::Map::new();
    body.insert("user_id".into(), uid);
    umbral::orm::DynQuerySet::for_meta(&profile_meta)
        .insert_json_in_tx(&body, tx)
        .await?;      // an Err here rolls the AuthUser create back too
    Ok(())
}));
```

The typed form lives on `on_model` (from `umbral-signals`), which hands you the decoded instance and derives the signal name:
...
Because the future has to borrow the transaction, the handler returns a `Box::pin(async move { … })` rather than being a bare async closure. The `post_save` async subscribers above still fire (post-commit) as well — a model can have both.

<Callout type="info">
Fires from the typed `Manager::create` path (gaps4 #92). A model with a `subscribe_txn` handler runs its create in a transaction; models without one keep the fast, no-extra-transaction path. M2M links set on the created instance flush after commit, so they aren't inside the atomic hook window yet.
</Callout>
...
- **Per-row** signals (`pre_save:<t>`, `post_save:<t>`, `pre_delete:<t>`, `post_delete:<t>`) fire from `Manager::save` and `Manager::delete_instance`. Use these when the handler needs the full instance to do its work (slug auto-generation, cache key derivation, notification subject lines). As of the delete-payload contract, `post_delete:<t>` **also** fires once per deleted row from `QuerySet::delete` and `DynQuerySet::delete` (REST/admin) - carrying the full row when a subscriber exists, PK-only otherwise (see the [Signals plugin](/docs/v0.0.1/plugins/signals) page for the exact shape).
- **Bulk** signals (`bulk_post_save:<t>`, `bulk_post_delete:<t>`) fire once per call from `bulk_create`, `update_values`, `update_expr`, and `QuerySet::delete` with the list of affected primary keys. Use these when the handler can work from just the PKs (audit logs, bulk cache invalidation, downstream re-indexing). A bulk `QuerySet::delete` fires `bulk_post_delete` once **and** the per-row `post_delete` for each row.
