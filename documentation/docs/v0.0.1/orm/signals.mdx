...
The facade module `umbra::signals` re-exports the full application surface: `subscribe`, `subscribe_async`, `emit`, and the actor helpers `with_actor` / `current_actor`.
...
`pre_save` / `post_save` fire on **both** INSERT and UPDATE, and carry only the new instance. A whole class of subscribers — audit-log diffs, change tracking, and umbra-storage's file replace-cleanup — also need the row as it looked **before** the UPDATE. That's what `pre_update:<table>` / `post_update:<table>` add:
...
```rust
use umbra::signals::subscribe_async;
