...
// Restore: clear the timestamp on the trashed row. Updates are not
// auto-filtered (only reads and delete() are), so the predicate alone
// identifies the row.
let mut patch = serde_json::Map::new();
patch.insert("deleted_at".into(), serde_json::Value::Null);
Post::objects().filter(post::ID.eq(id)).update_values(patch).await?;
...
<Callout type="warning">
`update_values` is **not** auto-scoped. Only reads (`fetch` / `first` / `get` / `count` / `exists`) and `delete()` inject the `deleted_at IS NULL` filter. A bulk `Post::objects().update_values(...)` with no predicate mutates trashed rows too, and `.with_deleted()` / `.only_deleted()` are currently silent no-ops on the update path (tracked as `gaps2 #34`). Always pin the rows you mean to touch with an explicit `.filter(...)` predicate — as the restore example above does — rather than relying on the auto-filter to spare the trash.
</Callout>

Pinned by `crates/umbra-core/tests/soft_delete.rs` (feature #72); behaviour reference in the `Model::SOFT_DELETE` docs in `crates/umbra-core/src/orm/model.rs`.
