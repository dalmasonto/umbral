...
The admin ships a full trash UI for any `#[umbra(soft_delete)]` model, with no per-model config:
...
Pinned by `crates/umbra-core/tests/soft_delete.rs` (feature #72, gaps2 #34), `crates/umbra-core/tests/soft_delete_dynamic.rs` (gaps2 #35), and `plugins/umbra-admin/tests/soft_delete_admin.rs` (the admin trash UI); behaviour reference in the `Model::SOFT_DELETE` docs in `crates/umbra-core/src/orm/model.rs`.
