...
The table is created for you: declaring any model `audited` registers `umbral_audit` automatically, so `makemigrations` picks it up through the normal [migration loop](../migrations/managed-migrations). No setup.
...
- the same writes made inside a transaction via `Model::objects().on_tx(tx)`
- REST — `POST` / `PATCH` / `DELETE`
- the admin, including bulk actions
- background tasks and CLI commands

<Callout type="info">
  **Transactional writes are audited atomically.** A write made inside `db::transaction(...)` through `on_tx(tx)` records its audit row **through the same transaction** — so the audit entry commits or rolls back with the change it describes. A transaction that rolls back leaves no audit trail for a write that never happened, and the update's after-image is read on the transaction's own connection, so it reflects the in-flight change rather than the stale pre-write value.
</Callout>
