...
Django's chainable `annotate()` over a relation — `Post.objects.filter(...).annotate(comments=Count("comments"), rating=Avg("reviews__rating"))`. The annotation is **query-builder state**: it renders as a correlated scalar subquery inside the one SELECT every terminal builds, so it stacks, composes with `.filter` / `.order_by` / `.limit`, and shows up in `.to_sql()` and `.explain()` out of the box. One query, no N+1, ever.
...
When the child model is `#[umbra(soft_delete)]`, `annotate_count` folds `AND <child>.deleted_at IS NULL` into the correlated subquery for you. A comment trashed via the real soft-delete `delete()` path silently drops out of the count — you never count rows the rest of the app treats as gone. No opt-in; the child's `soft_delete` flag rides through to the annotation.
...
```text
ambiguous reverse relation `order` on `Store` — candidates: [ad_order.buyer_store, ad_order.seller_store];
declare a `#[umbra(reverse_fk = "<fk>")] ReverseSet<Child>` field to disambiguate
```
