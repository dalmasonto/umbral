...
## Multi-hop aggregates: aggregating across relation paths

`annotate_count` also accepts a deep, `__`-separated relation path — `"posts__comments"` counts comments across every post a user has, in one correlated subquery. `annotate_sum` / `annotate_avg` / `annotate_min` / `annotate_max` do the same for a column reached through a path: the last `__` segment names the column, the prefix names the path to it.

```rust
use umbral::orm::Cmp;

let rows = User::objects()
    .annotate_sum("price_total", "posts__price")     // SUM(price) over every post
    .annotate_count("posts__comments")               // COUNT across every post's comments
...
Each of these deep-path annotations compiles to its **own** correlated subquery — the same JOIN-walking engine relation traversal and `select_related` use internally, wrapped in a `SELECT <AGG>(...)` instead of projected as rows. There is no shared JOIN and no `GROUP BY` on the base table.

<Callout type="info">
  Stacking a `SUM` over one relation and a `COUNT` over a different (or overlapping) relation can never inflate one against the other. A naive `JOIN posts JOIN comments` approach multiplies: a post's price would be counted once per comment on it. Because each annotation is its own independent correlated subquery, `price_total` and `posts__comments_count` are each computed against their own JOIN, with nothing to multiply against — the classic multi-aggregate JOIN-multiplication bug is impossible by construction.
</Callout>

### Filtering on an annotation: `filter_annotation`

An annotation alias is a SELECT-list expression, not a real column, so neither SQLite nor Postgres accepts it directly in a `WHERE`. `filter_annotation(alias, cmp, value)` is a portable stand-in for SQL `HAVING`: it wraps the whole annotated query as a derived table and filters that table's alias column, applied exactly once regardless of how many rows come back.

```rust
use umbral::orm::Cmp;

// Only users with at least one comment across their posts.
...
`Cmp` is the small, portable comparison set both backends render identically: `Eq`, `Ne`, `Gt`, `Gte`, `Lt`, `Lte` — the same enum `typed_cmp_condition`'s keyset-pagination cursors use. Multiple `filter_annotation` calls AND together, the same way `.filter()` does.

`.limit()` / `.offset()` compose correctly **regardless of where they appear in the chain relative to `filter_annotation`** — pagination always applies to the FILTERED set, never the pre-filter one. (Earlier drafts of this page said to call them after `filter_annotation`; that workaround is no longer needed — `build_query_for` moves any `.limit()`/`.offset()` onto the post-filter wrap itself.)

An alias that was never annotated (a typo, or a `filter_annotation`/`order_by_annotation` before the matching `annotate_*` call) fails loudly at query time rather than emitting SQL against a column the derived table never produced.

See `docs/specs/orm-heavy-relations-epic.md` (sub-project B, "Multi-hop aggregates") for the full design rationale, including why the subquery-per-annotation shape was chosen over a shared-JOIN `GROUP BY`.
...
`order_by` is typed against the model's own columns, so it can't name a computed alias. `order_by_annotation` can — which is what a leaderboard needs:

```rust
let top = AuthUser::objects()
    .annotate_count("post_set")
    .order_by_annotation("post_set_count", true)   // true = descending
    .limit(20)
    .fetch_annotated()
    .await?;
```

The sort happens **in SQL**, so it composes with `limit` — you get the top 20 by post count, not 20 arbitrary rows sorted afterwards.

<Callout type="info">
  A misspelled alias is a loud error naming both the alias you asked for and the ones that exist. It never emits `ORDER BY` against a column the database has never heard of — or, worse, one that happens to exist and returns confidently wrong rows.
</Callout>

## Typed annotation rows

`fetch_annotated` hands back `(Model, Map<String, Value>)`, so a flat row means poking JSON per field. `fetch_annotated_as::<T>()` deserializes the model's columns **and** its annotations into one struct of your own:
...
let top: Vec<Leader> = AuthUser::objects()
    .annotate_count("post_set")
    .order_by_annotation("post_set_count", true)
    .limit(20)
    .fetch_annotated_as::<Leader>()
    .await?;
```

`annotate_as::<T>()` does the same for the GROUP BY rollup:

```rust
#[derive(Deserialize)]
struct ByAuthor { author: i64, posts: i64 }

let rows: Vec<ByAuthor> = Post::objects()
    .annotate_as::<ByAuthor>(&["author"], &[("posts", Aggregate::count())])
    .await?;
```

A field the query didn't produce is a deserialize error, not a silent default — a typo in your struct shouldn't read as "zero posts".
