...
An annotation alias is a SELECT-list expression, not a real column, so neither SQLite nor Postgres accepts it directly in a `WHERE`. `filter_annotation(alias, op, value)` is a portable stand-in for SQL `HAVING`: it wraps the whole annotated query as a derived table and filters that table's alias column, applied exactly once regardless of how many rows come back.
...
`Op` is the small, portable comparison set both backends render identically: `Eq`, `Ne`, `Gt`, `Gte`, `Lt`, `Lte`. Multiple `filter_annotation` calls AND together, the same way `.filter()` does. Call `.limit()` / `.offset()` **after** `filter_annotation` in the chain so pagination applies to the filtered set.
