...
<Callout type="info">
**Single base per model, for now.** `#[model(base = ...)]` takes exactly one base today. A model that composes **several** bases uses the nested `#[umbral(flatten)]` form below (see [Composing several bases](#composing-several-bases)) — multi-base support for `#[model(...)]` is planned.
</Callout>
