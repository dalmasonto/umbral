...
**Typed column constants for base fields are not generated yet.** A model's *own* fields get typed constants like `article::TITLE` for building `filter` / `order_by` predicates. Base-**inherited** columns don't have them yet, so you can't currently `filter`/`order_by` a base column through the typed `QuerySet` API:

```rust
Article::objects().filter(article::TITLE.eq("Hello"))   // own field — works
// article::CREATED_AT does not exist yet — see the note below
```

The columns are fully functional for **migrations, inserts, auto-stamping, and reads** today — this only affects *typed query building* on the inherited columns. If you need to filter or order by a shared column right now, either declare that one column on the model directly, or reach it through the dynamic query API (`DynQuerySet::for_meta`, which the admin/REST layer uses and addresses columns by name). Tracked as planning `gaps5 #105`.
</Callout>
