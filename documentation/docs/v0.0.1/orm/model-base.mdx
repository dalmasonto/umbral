...
**Typed column constants for base fields are not generated yet.** For a model's *own* fields you get typed constants like `article::TITLE` for query building. Base-inherited columns don't have them yet, so filter or order them by name instead:

```rust
// own field — typed constant:
Article::objects().filter(article::TITLE.eq("Hello"))
// base field — order/filter by column name:
Article::objects().order_by("-created_at")
```

This is a follow-up (planning `gaps5 #105`); the columns are fully functional for migrations, inserts, auto-stamping and reads today.
</Callout>
