...
That's broad on purpose — it grew organically as the ORM shipped. A future split lands the read surface, the expressions surface, and the write surface on three focused pages (`querying.mdx`, `expressions.mdx`, `writes.mdx`); the design is captured in `docs/decisions/2026-06-06-orm-docs-restructure.md`. Until then, the H2 sections below group related material so the page is at least skimmable: **F-expressions** and **Q-objects** for composable predicates; **values**, **DB-function helpers**, and **QuerySet sugar** for read-side composition; **Mutate-side terminals** for the write-and-return-row APIs; **Composition** for subqueries + set ops + `in_bulk`; **JSON column ops** at the end.
