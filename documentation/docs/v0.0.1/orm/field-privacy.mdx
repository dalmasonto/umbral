...
```
POST /api/product/   {"name": "Widget", "cost": "0.01"}   → 201, and the response has no `cost`
GET  /api/product/1                                        → still no `cost`
GET  /api/product/1  (as staff, with an unlock)            → "cost": "0.01"
```

To stop a column being **written** from an untrusted body, reach for a different attribute:
...
<Callout type="warning">
  If you marked a column `private` expecting it to be unwritable, mark it `#[umbral(privileged)]` as well. `private` hides it; `privileged` protects it. Conflating the two used to make an honest `POST` fail with `cost: ["This field is required."]` — for a field the client had demonstrably just sent.
</Callout>
...
`GET /api/product/1` now returns `cost` to staff and omits it for everyone else — same URL, same handler. The unlock governs **reads**, and only reads: see [above](#private-is-a-read-policy-it-does-not-guard-writes). Anyone who may write the resource may set `cost`; `#[umbral(privileged)]` is what stops them.
