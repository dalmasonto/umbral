...
**Indexes are automatic.** A `tsvector` column is useless for search without a GIN index, so the migration engine emits one for you — `CREATE INDEX "idx_<table>_<col>_gin" ON "<table>" USING GIN ("<col>")` — for every `TsVector` field, on both `CreateTable` and a later `AddColumn`. You don't write `#[umbra(index)]` and you don't hand-author the GIN DDL; `migrate` does it. (The column you *populate* a `tsvector` from — running `to_tsvector(...)` over your text columns via a trigger or a generated column — is still yours to set up; umbra stores and queries the `tsvector`, it doesn't yet generate it from source text at write time.)
...
Full-text search matches **lexemes**: whole words, normalised by the language config (stemming, case-folding, stop-word removal). It is **not** substring search and **not** fuzzy/reverse matching. This trips people up, so concretely, against a document `"The best product ever made"`:
...
So a "reverse" or arbitrary substring like `tseb` will **never** match `best` — no full-text engine does that. The closest thing to partial-word matching is the prefix form (`prod:*` via `.matches`, or just typing a leading word in `.matches_websearch`). These rows are pinned by a live-Postgres test (`fts_matches_lexemes_not_substrings_or_reverses`).

### Then why use it, if `?search=` can match `rodu` and it can't?

Because matching `rodu` inside `product` is almost never what you want — it's noise. A substring scan (`?search=`, `LIKE '%term%'`) is literal and naive; a `tsvector` understands *words*, and the trade-off mostly runs the other way:
...
The attribute lands at M3.1; per-field attributes for `db_index`, `default`, `on_delete` are scheduled as the plugins that need them ship. (`choices` is shipped - `#[derive(Choices)]` on the enum + `#[umbra(choices)]` on the field; see [Column types - Using a Rust enum](./column-types#derivechoices---using-a-rust-enum-as-a-field-type).)
...
- `#[umbra(string)]` - the Django-style `__str__`, **opt-in**. Tagging one field per model switches the admin's default `list_display` to that single column when no explicit config is registered. Without any field tagged, the changelist shows every column - the legacy behaviour. The PK isn't rendered as a separate column because every row in the admin is clickable into the sheet, so the id is already reachable. The attribute is never required; it's how you ask for the compact single-column form. Equivalent shapes: `#[umbra(string)]` and `#[umbra(string = true)]`.
