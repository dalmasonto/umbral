...
Read the table name back with `User::table_name()` (→ `"auth_user"`) instead of hardcoding the string — handy when the table name diverges from the struct name (`UserProfile` → `profile`). It's a convenience over the `Model::TABLE` associated const; `Model` is in the prelude, so `User::table_name()` resolves directly.
...
Every model has to reach the registry, or it's invisible to `makemigrations`, the admin, and REST. The explicit way names each one:
...
`auto_models()` replaces the whole list — `#[derive(Model)]` self-registers, so the information is already there:
...
The two compose: a model named explicitly *and* discovered automatically is registered once, so you can add `auto_models()` to an existing app without touching the `.model::<T>()` calls you already have.

<Callout type="warning">
  **Why it's opt-in rather than the default.** Discovery is *link-time*. A model in your **binary** crate is always linked and always found. A model in a **library** crate that nothing else references can be dropped by the linker — and it would then be missing from the registry, which means missing from `makemigrations`, which means a table that silently never gets created. That failure is invisible until production, so it isn't something to switch on for you.

  If your models live in a library crate, either `use` it from `main.rs` (so the linker keeps it) or keep naming them with `.model::<T>()`.
</Callout>
