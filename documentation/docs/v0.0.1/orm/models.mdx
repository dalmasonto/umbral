...
The derive defaults `ipnetwork::IpNetwork` to `Inet`. Opting into `CIDR` semantics is deferred to a future `#[umbra(cidr)]` attribute; until then, edit the generated migration directly if you need the stricter constraint.
...
Each field gets a typed column constant under the snake_case module: `post::id`, `post::title`, `post::body`, `post::published_at`. The comparison methods are checked at compile time.
