...
- `#[umbra(string)]` — the Django-style `__str__`. The admin's default `list_display` is `[pk, <first string field>]` when no explicit config is registered, so the changelist shows a human label instead of every column. Equivalent shapes: `#[umbra(string)]` and `#[umbra(string = true)]`.
...
If no field on the model is tagged `string`, the admin falls back to the first non-PK `Text` column as the label. If no `Text` column exists either, the changelist shows every field — the legacy behaviour.
