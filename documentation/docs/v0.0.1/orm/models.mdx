...
- One primary-key field. By default that is a field named `id`; alternatively, mark any field with `#[umbra(primary_key)]` (e.g. a string codename). Supported PK types: every Rust integer width (`i8` / `i16` / `i32` / `i64` and `u8` / `u16` / `u32` / `u64`, with `i64` the auto-increment default), `uuid::Uuid` (no auto-generation, you supply the value), and `String` (no auto-generation, you supply the value). A custom newtype works too with one line: `impl umbra::orm::PrimaryKey for MyId {}` (the type must lower to a `sea_query::Value`).
...
The derive defaults `ipnetwork::IpNetwork` to `Inet`. Opt into the stricter `CIDR` semantics with the field-level `#[umbra(cidr)]` attribute (the Rust type stays `ipnetwork::IpNetwork`; only the column type changes):
...
Beyond `table`, the struct-level `#[umbra(...)]` also accepts `plugin`, `display`, `icon`, `database`, `singleton`, `soft_delete`, `unique_together = [[...]]`, `indexes = [[...]]`, and `ordering = ["-col", "col"]`. The field-level `#[umbra(...)]` accepts `primary_key`, `unique`, `index`, `default = "..."`, `on_delete = "..."` / `on_update = "..."`, `auto_now` / `auto_now_add`, `max_length = N`, `min = N` / `max = N`, `string`, `noform`, `noedit`, `help = "..."`, `example = "..."`, `widget = "..."`, `backend = "..."`, `cidr`, `db_constraint = false`, `slug_from = "..."`, `choices`, `reverse_fk = "..."`, and `no_reverse`. (`choices` pairs `#[derive(Choices)]` on the enum with `#[umbra(choices)]` on the field; see [Column types - Using a Rust enum](./column-types#derivechoices---using-a-rust-enum-as-a-field-type).)
...
- `#[umbra(max_length = N)]`: soft truncation cap for display. The admin trims the value at `N` characters when rendering it in `list_display`, appending `…` so the cut is visible. Counted in characters, not bytes, so emoji and combining characters survive intact.
...
Four write primitives complement the read side. All return a `Result<_, umbra::orm::WriteError>` whose variants name the offending field on type mismatch / null-on-required-column / unknown-column errors.
