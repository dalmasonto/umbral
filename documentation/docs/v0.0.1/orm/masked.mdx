...
<Callout type="danger">
**The encrypt-on-serialize guarantee only holds on the typed `create`/`bulk_create` path.** The *dynamic* JSON write path — `DynQuerySet::insert_json` / `update_json`, which is what the **REST plugin's create/update endpoints and the admin auto-CRUD form submit use** — binds a masked column as a plain `TEXT` value taken straight from the request body. That JSON never passes through `Masked`'s serde `Serialize`, so **it is stored as plaintext, not ciphertext.** Until this is fixed in the write path, do **not** expose a masked field on a writable REST `ModelViewSet` or admin form: hide it from writes (`.hide([...])`) and set the value server-side with `Masked::new(...)` on the typed path. See the ORM audit finding `core-orm #1`.
</Callout>
