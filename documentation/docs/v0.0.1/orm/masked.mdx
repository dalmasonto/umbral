...
### Forms treat a masked field as write-only

Deserialization reads input as new plaintext. A form that re-submits a redacted value would overwrite the stored one, so render masked fields as write-only (set a new value, never edit the existing ciphertext). OAuth tokens are server-set, so this doesn't arise there.
