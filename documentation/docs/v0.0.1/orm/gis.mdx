...
```rust
// Facilities within 5 km of a point (metres on a `geography` column;
// SRID units on `geometry`).
let near = Facility::objects()
    .filter(facility::LOCATION.dwithin("SRID=4326;POINT(36.8172 -1.2864)", 5000.0))
    .fetch_pg(&pool)
    .await?;
...
Available predicates: `dwithin(other, distance)`, `intersects(other)`, `contains(other)`, `within(other)`, and `bbox_overlaps(other)` (the index-accelerated `&&` bounding-box pre-filter), plus `is_null` / `is_not_null` and ordering.
