...
Now `Booking.payment_total` updates itself whenever an `Rsvp` row is created, changed via a per-row write (`.save()`), or deleted - no code at the write sites.
...
<Callout type="warning">
  **Set-based bulk updates on a source are NOT covered in v1.** The handler subscribes to per-row `post_save` / `post_delete`, which `create`, `.save()`, and a per-row `delete()` fire. A **set-based** update on the source - `Source::objects().filter(...).update_values(...)` or `.update_expr(...)` - emits only the bulk signal (`bulk_post_save`, which carries matched pks but no row instances), so the recompute handler never runs and the target silently goes stale. If you bulk-update a source table, recompute the affected targets explicitly afterward. A bulk-aware handler is a tracked follow-up (see the design spec).
</Callout>
