...
description: Take over how umbra picks the database and read/write target per request — read replicas and custom per-request routing.
sidebar_position: 15
...
umbra ships **no multitenancy system** — no `Tenant` model, no per-tenant schema migrations, no tenant provisioning. Don't reach for the router expecting turnkey schema- or database-per-tenant tenancy; it gives you the low-level routing primitive (which database a query uses), not the management around it.
</Callout>
