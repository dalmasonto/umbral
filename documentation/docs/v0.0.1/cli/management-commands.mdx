...
[`umbral_cli::dispatch`](#how-it-works); `cargo run -- <command>` is
how you invoke them.

This is a project-local entry point that runs your own project code,
so the commands see your models and your plugins. The framework-global [`umbral`](./startproject) binary handles the commands that don't need your models -
scaffolding (`startproject`, `startplugin`, `startcommand`, `plugin add`) plus `maskkeygen` and `doctor`. It can't run `migrate` or the other model-aware commands, because it doesn't know about your models.

## Getting help and the version

`umbral help`, `umbral --help`, and `umbral -h` all print the **same** unified
catalog — every built-in subcommand plus every command your plugins and project
register — grouped and aligned. Run any of them **inside a project** and the
listing includes your plugin-contributed commands (`createsuperuser`,
`tasks-worker`, your own scaffolded commands); run them outside a project and
you get the built-in catalog with a note that plugin commands appear inside a
project.

```bash
umbral --help            # the full grouped catalog (same as `umbral help`)
umbral --version         # e.g. `umbral 0.0.12`
umbral migrate --help    # command-specific help: every flag and required arg
```

For a **specific** command, put `--help` after it: `umbral migrate --help`
renders that command's own flags and required arguments (including the ones your
plugin or project command declares). `cargo run -- --help` / `cargo run -- migrate --help`
work identically from inside a project.
...
<CardGrid cols={2}>
  ...
  <Card icon="shield-check" title="checkmigrations" description="Classify pending operations SAFE/WARNING/UNSAFE for zero-downtime deploys. Exits non-zero on unsafe ops, a CI gate before migrate." />
  <Card icon="file-code" title="typegen" description="Emit TypeScript interfaces and unions for every registered model. --out writes a file; --check exits non-zero when it has drifted from the models." />
  ...
  <Card icon="git-merge" title="squashmigrations" description="Collapse a plugin's migration history into one file, preserving the current schema." />
  <Card icon="copy" title="transferdata" description="Resumable PK/FK-preserving row copy from one umbral database to another." />
  <Card icon="stethoscope" title="doctor" description="Diagnose common misconfigurations (settings, keys, pool, migration state) before you deploy." />
  ...
  <Card icon="table" title="importcsv" description="Load a CSV file into one table, coercing each cell to its column type through the validated write path. Inverse of the ?format=csv export." />
  <Card icon="key" title="maskkeygen" description="Generate an X25519 keypair for Masked<T> field encryption and print the two env-var lines to configure it." />
  <Card icon="refresh-cw" title="dev" description="Auto-reload loop: wraps cargo-watch to rebuild and restart the binary on every Rust-source save." />
</CardGrid>
...
<CardGrid cols={2}>
  <Card icon="user-plus" title="createsuperuser" description="umbral-auth. Create an is_staff + is_superuser user with an argon2id-hashed password. --noinput for CI." />
  <Card icon="cpu" title="tasks-worker" description="umbral-tasks. Drain the task queue. --once runs one batch and exits (cron-friendly)." />
  <Card icon="clock" title="tasks-beat" description="umbral-tasks. Run the periodic-task scheduler. --once syncs schedules, runs one tick, exits." />
  <Card icon="trash-2" title="clearsessions" description="umbral-sessions. Delete all expired session rows from the database." />
  <Card icon="folder-down" title="collectstatic" description="umbral-storage. Collect every plugin's static assets into static_root. --clear / --hashed / --storage." />
  <Card icon="wand-2" title="startpermission / startauthentication / startpagination / startthrottle" description="umbral-rest. Scaffold one of REST's four extension classes, registered and ready to wire. See rest/scaffolding-classes." />
</CardGrid>
...
A backfill, an import, a nightly reconcile — anything you'd reach for a script for. [`umbral startcommand`](./startcommand) scaffolds it with its arguments and registers it, either on your project (`App::builder().commands(...)`) or inside one of your plugins (`Plugin::commands()`). It then runs as `cargo run -- <name>` and lists in `umbral help` alongside everything above.
...
Without `UMBRAL_DATABASE_URL` set, the management subcommands fall back
...
| `--allow-destructive` | off | Apply a migration that contains destructive ops (`DROP TABLE` / `DROP COLUMN`). Without it, `migrate` refuses them - the guard that stops a missing `.model::<T>()` registration from silently dropping a table. |
| `--allow-in-memory` | off | Permit `migrate` to run against an in-memory SQLite URL (`:memory:` / `mode=memory`). Off by default because an in-memory database is discarded at process exit, so migrating one is almost always a mistake. |
...
Full tier rules + the expand-contract pattern: [Checking migrations for zero-downtime safety](/docs/v0.0.1/migrations/checkmigrations).
...
```bash
# Point at a specific database (a URL or a SQLite file path) and scaffold
# models into a plugin dir.
cargo run -- inspectdb ./legacy.sqlite3 --output plugins/imported

# Onboard a Django database: strip the `_id` off FK columns and the `<app>_`
# prefix off struct names, and map `auth_user` onto umbral-auth's AuthUser.
cargo run -- inspectdb ./django.sqlite3 --framework django --output plugins/imported

# Introspect the app's ambient database instead (no positional argument),
# and mark the migration already-applied so the next `migrate` is a no-op.
...
|---|---|---|
| `[database]` (positional) | app's ambient DB | The source to introspect: a `sqlite://` / `postgres://` URL, or a path to a SQLite file (opened read-only). Omit it to use `UMBRAL_DATABASE_URL`. |
| `--framework <name>` | - (verbatim names) | Undo a source ORM's conventions. `django` strips `_id` off FK fields and `<app>_` off struct/table names, and maps `auth_user` to umbral-auth's `AuthUser`. |
| `--with-table-names` | off | Strip an app-prefix off generated struct names (`blog_post` → `Post`) and preserve the real table with a `#[umbral(table = "...")]` macro. Off by default, struct names round-trip to their table verbatim (`BlogPost`). |
...
The inverse of the REST list endpoint's [`?format=csv` export](/docs/v0.0.1/rest/csv-export). Reads a CSV file and inserts its rows into one table, coercing each cell to the column's type and routing through the same validated write path as a REST POST - so validators, `auto_now`, `slug_from`, and FK-existence checks all apply per row.
...
<Callout type="warning">
`UMBRAL_MASK_PRIVATE_KEY` is the key that **decrypts every masked column** — treat it like a database password. Because `maskkeygen` prints it to stdout, capture it straight into your secret store (Vault, AWS/GCP secret manager, a sealed CI variable) and keep it out of shell history, terminal scrollback, screen shares, and any committed `.env`. If you must redirect it to a file, redirect to one you never commit and delete it once the secret is stored. Losing the private key crypto-shreds the data; leaking it exposes every masked value.
</Callout>
...
See [Scaffolding a project](./startproject) for the global `umbral`
