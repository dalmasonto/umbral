...
plugins. The framework-global [`umbral`](./startproject) binary handles
...
<CardGrid cols={2}>
  ...
  <Card icon="clock" title="tasks-beat" description="umbral-tasks. Run the periodic-task scheduler (Celery beat). --once syncs schedules, runs one tick, exits." />
  <Card icon="trash-2" title="clearsessions" description="umbral-sessions. Delete all expired session rows from the database (Django parity)." />
