...
<CardGrid cols={2}>
  ...
  <Card icon="folder-down" title="collectstatic" description="umbra-static. Collect every plugin's static assets into static_root. --clear / --hashed / --storage." />
</CardGrid>
...
| `--storage <backend>` | `local` | Where to write: `local` (the on-disk `static_root`) or `s3` (requires the umbra-static `s3` feature and `UMBRA_STATIC_BUCKET` / `UMBRA_STATIC_REGION`). Overrides `UMBRA_STATIC_STORAGE`. |
