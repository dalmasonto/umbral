...
<CardGrid cols={2}>
  <Card icon="user-plus" title="createsuperuser" description="umbra-auth. Create an is_staff + is_superuser user with an argon2id-hashed password. --noinput for CI." />
  <Card icon="cpu" title="tasks-worker" description="umbra-tasks. Drain the task queue. --once runs one batch and exits (cron-friendly)." />
  <Card icon="clock" title="tasks-beat" description="umbra-tasks. Run the periodic-task scheduler (Celery beat). --once syncs schedules, runs one tick, exits." />
  <Card icon="trash-2" title="clearsessions" description="umbra-sessions. Delete all expired session rows from the database (Django parity)." />
  <Card icon="folder-down" title="collectstatic" description="umbra-storage. Collect every plugin's static assets into static_root. --clear / --hashed / --storage." />
</CardGrid>
...
| Flag | Default | Effect |
|---|---|---|
| `--addr <host:port>` | `127.0.0.1:8000` | Override `settings.bind_addr`. `0.0.0.0:PORT` listens on every interface; `127.0.0.1:PORT` only loopback. Equivalent to setting `UMBRA_BIND_ADDR`. |
...
# -> UMBRA_MASK_PUBLIC_KEY=...
# -> UMBRA_MASK_PRIVATE_KEY=...
```
...
## tasks-beat - `umbra-tasks`

Run the periodic-task scheduler (the Celery *beat* equivalent). On startup it syncs the registered periodic specs to `PeriodicTask` rows, then each tick claims every due row atomically and enqueues the underlying task. Run it as its **own** process alongside `tasks-worker` — beat fills the queue, the worker drains it.
...
## clearsessions - `umbra-sessions`

Delete every expired session row from the database (Django's `clearsessions`). Run it periodically (cron / a scheduled task) to keep the session table from accumulating dead rows.
...
## collectstatic - `umbra-storage`

Collect every plugin's static assets — namespaced `static_dirs()` plus site-level `static_root_dirs()` — into `settings.static_root`. Django's `collectstatic`.
...
| `--hashed` | off | Write content-hashed copies (`app.<hash>.css`) alongside each asset plus a `staticfiles.json` manifest (Django's `ManifestStaticFilesStorage`). Use in production. |
| `--storage <backend>` | `local` | Where to write: `local` (the on-disk `static_root`) or `s3` (requires the umbra-storage `s3` feature and `UMBRA_STATIC_BUCKET` / `UMBRA_STATIC_REGION`). Overrides `UMBRA_STATIC_STORAGE`. |
