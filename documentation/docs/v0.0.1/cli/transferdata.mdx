...
FK-deferred transaction so the cycle resolves at commit — on Postgres this
needs the constraints to be `DEFERRABLE`. Still on the list: richer `--map`
targets beyond Django. The design lives in
