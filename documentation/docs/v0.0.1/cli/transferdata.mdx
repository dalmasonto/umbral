...
Registered models (single-column PK) and their M2M junction rows are covered.
Still on the list: cross-backend moves (SQLite → Postgres, where a UUID junction
id needs the native type), parallel per-table workers, and circular-FK tables
(mutually-referential pairs need deferred-constraint handling). The design lives
in `docs/decisions/2026-08-16-data-transfer-engine.md`.
