...
Registered models (single-column PK), their M2M junction rows, and cross-backend
moves (SQLite ↔ Postgres) are covered. Still on the list: parallel per-table
workers, a `--map` layer for reading a foreign-shaped source (reusing
`inspectdb`'s rename map), and circular-FK tables (mutually-referential pairs
need deferred-constraint handling). The design lives in
