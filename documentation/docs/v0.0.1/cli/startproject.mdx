...
├── plugins/            # local app plugins land here (umbra startapp <name>)
│   ├── .gitkeep
│   └── README.md
...
An empty home for your local app plugins. Run `umbra startapp <name>` (below) to create one - it scaffolds the crate under `plugins/<name>/` and auto-wires it into your project's `Cargo.toml`.
...
Generates a **Django-app-style per-concern layout** - one file per concern, so a new plugin opens to obvious "what's in which file" structure:
...
`urls.rs` is the app's URL conf - the route table that maps paths to `views::` handlers, like Django's `urls.py`:
...
| Choosing... | Use |
|---|---|
| A local app plugin for your project | `startapp` (Django-app-style `lib`/`models`/`views`/`urls`, auto-wired into your `Cargo.toml`) |
