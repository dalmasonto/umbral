...
description: "The `umbral` global binary creates new projects (`startproject`) and plugin crates (`startplugin`)."
sidebar_position: 1
icon: layers
---
...
It does two things: `startproject` and `startplugin`. Everything else (`serve`, `migrate`, `createsuperuser`, …) runs against your project's own binary via [`cargo run -- <command>`](./management-commands).
