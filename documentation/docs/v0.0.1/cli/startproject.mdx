...
description: The `umbral` global binary creates new projects (`startproject`), minimal plugin stubs (`startapp`), and richer distributable-plugin scaffolds (`startplugin`). Like `django-admin startproject` and `python manage.py startapp`.
sidebar_position: 1
icon: layers
---
...
A plugin in umbral is what Django calls an app. `startapp` scaffolds a
