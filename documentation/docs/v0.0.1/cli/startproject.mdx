...
```
myblog/
├── Cargo.toml          # Cargo WORKSPACE root: [workspace.dependencies] + the app package
├── src/
...
### One workspace, one version of each dependency

The generated `Cargo.toml` is a **Cargo workspace** (a root package plus `members = ["plugins/*"]`). Every shared dependency — `umbral`, each built-in plugin, `sqlx`, `serde`, `chrono`, `tokio` — is pinned once in `[workspace.dependencies]`, and both the app and every plugin you scaffold inherit it by writing `<crate>.workspace = true`:
...
# …every built-in + third-party dep, single-sourced
...
Why this matters: `#[derive(sqlx::FromRow)]` forces **every** crate that declares a model to depend on `sqlx` directly (its expansion hardcodes `::sqlx::` paths — no `#[sqlx(crate = "…")]` escape hatch), so a multi-plugin project has many `sqlx` deps. With `[workspace.dependencies]` there is exactly **one** version to change, and a plugin can never drift from the app — the class of bug [`umbral doctor`](./doctor) exists to catch (gaps4 #90/#65). A plugin generated **outside** a workspace still gets explicit version pins, so it builds standalone.
...
```
plugins/widgets/
├── Cargo.toml              # deps inherited from the workspace (umbral/serde/sqlx/chrono/async-trait .workspace = true)
