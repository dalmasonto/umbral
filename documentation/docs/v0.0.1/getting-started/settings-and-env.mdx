...
# Named secondary database URLs. App::build() opens a lazy pool for each
# entry automatically (see the callout below); a model routes to an alias
# via `#[umbral(database = "alias")]` or per-plugin via `Plugin::database()`.
[databases]
