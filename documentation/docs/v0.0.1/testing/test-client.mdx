...
`umbral-testing` gives you Django's `TestCase` + `Client` ergonomics in Rust. The repeated work in every integration test (spin up a pool, build the router, send a request, read the response) collapses into three types:
