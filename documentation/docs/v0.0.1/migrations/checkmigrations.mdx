...
`RENAME TABLE` / `RENAME COLUMN` (not atomic with a code deploy — one code version references the missing name), `ALTER COLUMN` (a type change rewrites the column and locks a large table; a NOT NULL tightening fails on existing NULLs), and adding a NOT NULL column **with no default** (old inserts that omit it fail).
</Step>
