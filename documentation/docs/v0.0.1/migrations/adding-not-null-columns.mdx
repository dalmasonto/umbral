...
Adding a new NOT NULL column to a table that already has rows is the most common "your migration won't apply" scenario. umbra catches it earlier than most: `makemigrations` itself refuses to write the migration when it would brick a populated table. This page covers the safe paths umbra knows about and the backend differences you'll hit.
...
When a model gains a NOT NULL field that is *not* `Option<T>`, *not* `#[umbra(default = "...")]`, and *not* `#[umbra(auto_now)]` / `#[umbra(auto_now_add)]` (and isn't the primary key), the autodetector stops with an `UnsafeAlter` error instead of generating an `AddColumn` op that would fail on every populated row:

```text
umbra migrate: unsafe column change on `Order.region`: adding NOT NULL column
...
`#[umbra(default = "...")]` so the migration backfills, or (c) add
...
2. **Adjust the model** to one of those shapes (`Option<T>`, `#[umbra(default = "...")]`, or an `auto_now*` timestamp).
