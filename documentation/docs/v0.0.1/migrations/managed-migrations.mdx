...
- Nullable flip → `AlterColumn` (rendered via the SQLite table-recreation dance:
  CREATE _umbra_new, INSERT SELECT, DROP, RENAME - Postgres uses native
...
```
umbra makemigrations: rename detected (column-shape match): `foo` -> `bar` - please verify this is a rename and not a coincidental column-shape match between two unrelated models
```
