...
Most migrations are **schema** migrations — umbra auto-detects them from a model change (`CreateTable`, `AddColumn`, …). A **data migration** changes *rows*, not the schema: backfill a new column, normalise existing values, seed lookup rows. umbra ships this as `Operation::RunSql`, a hand-authored raw-SQL op.
...
- `crates/umbra-core/src/migrate.rs` (the `Operation::RunSql` variant) and `docs/specs/06-migration-engine.md` for the design.
