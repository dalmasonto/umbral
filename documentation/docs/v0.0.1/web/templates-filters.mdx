...
description: minijinja built-in filters, umbra's custom filters and template functions, and how to register your own from app or plugin code.
sidebar_position: 11
icon: filter
tags: [templates, filters, minijinja, plugins]
---

Umbra templates run on [minijinja](https://docs.rs/minijinja/latest/minijinja/), a faithful Jinja2 port. Every filter and function minijinja ships is available without any setup. On top of that umbra registers its own filters and globals at engine-build time, and any plugin can contribute more through the `Plugin::template_registrars` hook.
...
<Callout type="info">
  Umbra enables autoescape for every `.html` / `.htm` template. Values piped through `| safe` (or returned as `minijinja::Value::from_safe_string` in Rust) bypass the escaping — all other output is HTML-escaped automatically.
</Callout>

Date and time formatting uses minijinja's `now()` built-in. Umbra replaces that global with its own `now()` (see below), which accepts a chrono `strftime` format string for consistent, UTC-based rendering.

## Umbra filters and template functions

These are registered by umbra itself at engine-build time. They are available in every template, in every plugin, without any import or configuration.
...
|---|---|
| `user` | The current request user. Always present: anonymous requests get `{ is_authenticated: false, is_staff: false, is_superuser: false }`. Attributes like `user.email`, `user.is_staff` are available when `umbra-auth` + sessions are installed. |
| `csrf_token` | The CSRF token string (injected when `umbra-security`'s CSRF middleware is mounted). |
...
- [`crates/umbra-core/src/templates.rs`](https://github.com/dalmas-ogembo/umbra/tree/main/crates/umbra-core/src/templates.rs) — source for all built-in filters and functions.
