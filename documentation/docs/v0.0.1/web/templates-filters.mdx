...
For date and time formatting, umbra registers its own `now()` global (see below), which accepts a chrono `strftime` format string for consistent, UTC-based rendering.
