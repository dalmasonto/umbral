...
Both hooks have a default pass-through, so implement only the one you need. `#[umbra::async_trait]` is re-exported from the facade — no direct `async-trait` dependency required.
...
A `before_request` that returns `Err(response)` stops the chain: the handler and every **later** middleware are skipped. Only the middleware whose `before_request` already ran get an `after_response`, still in reverse. So if `B` short-circuits in the stack above:
