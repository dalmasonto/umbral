...
pub async fn submit_contact(form: Form<ContactMessage>) -> impl IntoResponse {
    let msg = match form.into_result() {
        Ok(m)     => m,
        Err(errs) => return render_form_with_errors(errs),
    };
...
```rust
fn render_form_with_errors(errs: FormErrors) -> Response {
    let mut ctx = errs.as_template_ctx();
    // Synthesise a form-level banner when no non-field error exists
    // but per-field errors did fire (the common case).
    if !ctx.contains_key("form") {
        ctx.insert("form".into(), serde_json::json!(
            "Please fix the highlighted fields and send again."
        ));
    }
    let body = umbra::templates::render(
        "contact.html",
        &context!(form = ContactMessage::default(), errors = ctx),
    ).unwrap();
    (StatusCode::UNPROCESSABLE_ENTITY, Html(body)).into_response()
}
```

For templates that need to render **every** error per field (multiple validators firing on one input), call `errs.field_errors()` and pass the `HashMap<String, Vec<String>>` directly.
