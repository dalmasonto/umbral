...
- Body size: capped at **16 MiB by default**, configurable via `UMBRA_MAX_FORM_BODY_BYTES` (or `Settings::max_form_body_bytes` in code / `umbra.toml`). Set it to `0` to **disable the cap entirely** — handy in dev. Over-limit requests return 413.
