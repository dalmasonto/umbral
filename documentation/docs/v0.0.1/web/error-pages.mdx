...
In Django you drop a `404.html` or `500.html` in your templates dir and the framework picks it up. umbral ships the same shape - two builder methods, both opt-in, both pointing at templates the existing engine renders. Out of the box, umbral also ships its own Tailwind-styled default pages that fire automatically when you haven't provided your own.
...
```bash
UMBRAL_ENVIRONMENT=Prod cargo run
```
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        ...
        umbral_core::errors::fire_server_error_hook(
            &Some(my_hook.clone()),
            &self.0.to_string(),
            "/",
        );
