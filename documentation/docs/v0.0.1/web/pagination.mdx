...
```rust
use umbra::prelude::*;
use umbra::pagination::Paginator;
use umbra::templates::{render, context};
...
A reusable `_pagination.html` partial ships under `crates/umbra-core/templates/`. Copy it into your project `templates/` dir and `{% include %}` it from a list template; it renders a minimal, CSS-framework-agnostic `<nav>` with `aria` attributes from two context vars: `page` (the `PageContext` above) and `base_query` (the current querystring).
