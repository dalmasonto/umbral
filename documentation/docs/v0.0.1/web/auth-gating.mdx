...
Two complementary shapes for the Django `@login_required` pattern: an axum **extractor** for per-handler gating and a tower::Layer **middleware** for gating whole Router subtrees. Both ship from `umbral-auth`. Both share one `LoginRequired` config that picks between two error responses - JSON 401 for APIs, 302 redirect for HTML pages.
...
- [`umbral-auth` plugin docs](/docs/v0.0.1/plugins/auth) - full reference for `LoginRequired`, `LoggedIn<U>`, `LoginRequiredLayer`.
