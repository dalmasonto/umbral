...
The full reference lives under the auth plugin: [Guarding views](/v0.0.1/plugins/auth#guarding-views).
...
`LoggedIn<U>` is generic over any `U: UserModel`, so a [custom user model](/v0.0.1/plugins/auth#swapping-the-user-model) works without adapters:
...
- [`umbra-auth` plugin docs](/v0.0.1/plugins/auth) - full reference for `LoginRequired`, `LoggedIn<U>`, `LoginRequiredLayer`.
- [Users and passwords](/v0.0.1/auth/users-and-passwords) - the underlying user / session story.
- [Sessions](/v0.0.1/plugins/sessions) - how the session cookie is created and validated.
