...
`.with_user_in_templates()` flips a flag on the plugin. At build time, `AuthPlugin::wrap_router` (a [`Plugin::wrap_router`](/docs/v0.0.1/plugins/the-plugin-trait) override) wraps the whole merged router in `axum::middleware::from_fn(user_context_layer)`:
...
- [Login, logout, request.user](/docs/v0.0.1/auth/login-and-request-user): extractors for handler-level user access
- [Users and passwords](/docs/v0.0.1/auth/users-and-passwords): the `AuthUser` model + password hashing
- [Plugin trait](/docs/v0.0.1/plugins/the-plugin-trait): what `wrap_router` is and how middleware is contributed
