...
Django's templates have `{% if user.is_authenticated %}` and `{% if user.is_staff %}` because Django's `auth` middleware injects `request.user` into every template context for free. Umbral ships the same shape, opt-in, one builder call.
