...
<Callout type="warning">
  **Reverse-FK one-to-many lists are not expanded.** `{{ user.order_set }}` — the *list* of children pointing at the user through a **non-unique** FK — is deliberately not injected: it's an unbounded per-request query (a user with 50k orders would load all of them into every template render). It's [on deck for 0.0.12](https://github.com/dalmasonto/umbral/blob/main/changelog/README.md), with a row cap.

  **What to do today** for a child list: resolve it in the handler and pass it in explicitly:

  ```rust
  let orders = Order::objects().filter(order::USER.eq(&user.0.id)).limit(20).fetch().await?;
  render("me.html", &context!(username, orders))
  ```
</Callout>

Everything else — columns on `AuthUser` itself (`is_authenticated`, `is_staff`, `username`, `email`, `date_joined`, …) plus the FK/reverse-O2O relations above — is available on `user` directly. M2M sets and computed fields still have to be resolved in the handler.
