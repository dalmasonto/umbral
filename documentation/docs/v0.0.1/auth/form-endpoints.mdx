...
<Callout type="info">Flash messages require an active session. On a brand-new visitor's very first failed submit — before any session cookie has been set — the flash is dropped (tracked as gaps3 #4); serving your login page behind CSRF middleware (which umbral does by default) ensures a session exists before the form is ever submitted.</Callout>
