...
sidebar_position: 4
tags: [auth, oauth, social-login, google, github]
---
...
`from_settings` reads the conventional keys — `oauth_redirect_base` (your app's **public origin**; each provider's callback is `{redirect_base}/oauth/{provider}/callback` and must match the provider's console), plus `oauth_google_client_id`/`_secret` and `oauth_github_client_id`/`_secret`. In practice those come from `UMBRAL_OAUTH_*` env vars. A provider with no credentials is simply skipped, so it's safe to leave the plugin on with nothing set; a provider with only HALF a credential pair is skipped **with a warning** (that's a typo, not a choice).

For manual control, the granular builders remain — `provider_opt` keeps an env-driven setup a straight chain:

```rust
use umbral_oauth::providers::{GoogleProvider, GitHubProvider};

OAuthPlugin::new("https://example.com")
    .provider_opt(GoogleProvider::from_env())
    .provider_opt(GitHubProvider::from_env())
    .login_redirect("/dashboard")
```

`.login_redirect(path)` is where the browser lands after a successful login (default `/`).
...
<Callout type="danger">
**A mask keyring is required.** The callback encrypts the provider's access/refresh tokens into [`Masked`](../orm/masked) columns on `SocialAccount` — for most apps this is the **first masked write** they ever do. With no keyring configured that write fails and the callback returns a generic **500**. Generate one with `umbral maskkeygen` and set `UMBRAL_MASK_PUBLIC_KEY` (add `UMBRAL_MASK_PRIVATE_KEY` too if you later `.reveal()` the tokens to call the provider's API). When a provider is registered but no keyring is set, the plugin **warns at boot** — `oauth: providers are registered but no mask keyring is configured …` — so you catch it before the first user hits sign-in, not after.
</Callout>
...
There are exactly two ways to run social login, and picking the wrong path is the biggest source of confusion. Decide up front which one you're building:

<Tabs>
<Tab title="Fullstack (same origin)">
umbral serves both the app **and** the API on one origin. This is the happy path: the callback's `login_user_id(...)` sets the session cookie on that same origin, so there's **no `?next`, no fragment token, no BFF handoff**.

Set `oauth_redirect_base`, the client id/secret, **and a mask keyring**, and choose your post-login destination with the `.login_redirect("/dashboard")` builder (it defaults to `/`) — then just link to `/oauth/{provider}/login` and land there.
</Tab>
<Tab title="API + separate-origin SPA">
umbral is the API; a Next/Vite SPA on a **different origin** owns the UI. Here you use **token mode**: allowlist the SPA origin, the SPA starts login with `?next=<spa-callback>`, the callback appends `#token=<bearer>` to that URL, and the SPA exchanges it for its own session. This is the flow documented below — every step matters, and skipping one fails silently.
</Tab>
</Tabs>

## SPA login (bearer token)

The fullstack flow above ends in a cookie session, perfect for a server-rendered app. A single-page app on a **different origin** (Vite, Next, a mobile client) can use the *same* routes and get back a **bearer token** instead, so it can call the API with `Authorization: Bearer`.
...
The allowlist is **env-driven** like every other OAuth deploy knob — set `oauth_allow_return` (i.e. `UMBRAL_OAUTH_ALLOW_RETURN`), a comma-separated list of prefixes, so moving the SPA origin dev→prod is a config change, not a recompile. `from_settings` reads it automatically:

```bash
# .env
UMBRAL_OAUTH_ALLOW_RETURN=https://app.example.com/auth/callback,http://localhost:5173/auth/callback
```

The builder form still works (and appends onto any env-driven prefixes) when you want it in code:

```rust
let oauth = OAuthPlugin::from_settings(&settings)
    .allow_return("https://app.example.com/auth/callback");
```

When token mode is enabled (a non-empty allowlist), the plugin logs an info line at boot spelling out the `?next=` requirement.
</Step>
...
<Callout type="warning">
The `?next=` is **mandatory** for the token flow. A login started with **no** `?next` only establishes a same-origin session and mints **no token** — a separate-origin SPA that links straight to `/oauth/{provider}/login` lands back empty and looks "logged in on the backend but not the SPA." When token mode is on, a login that completes with no allowlisted `?next` is logged as a warning so the missing parameter is diagnosable.
</Callout>
</Step>
...
<Callout type="tip">
**Prefer an httpOnly cookie over `localStorage`?** For a Next.js-style app with a backend-for-frontend (BFF), don't keep the bearer in `localStorage` (readable by any script). Instead, in your SPA callback route read the fragment token, `POST` it to your own BFF endpoint, have the BFF validate it (`GET /api/auth/me` with `Authorization: Bearer <token>`), and on success set an **httpOnly** session cookie the browser can't read. The token never touches client-readable storage; the fragment is scrubbed immediately after the handoff. The four moving parts — read fragment → POST to BFF → validate via `/api/auth/me` → set httpOnly cookie — are the whole recipe.
</Callout>

<Callout type="warning">
A `?next=` that doesn't match an `allow_return` prefix is rejected with **400**, so an attacker can't point the flow at their own origin to harvest a freshly minted token. With **no** `allow_return` configured the allowlist is empty, so **any** present `?next=` matches nothing and is likewise rejected with **400** — token mode is off by default, and a plain login (no `?next=`) keeps its server-rendered session behavior (safe by default). Token mode is login-only: a connect flow never mints a token (the user already holds one).
</Callout>
...
<Step title="Verified-email link">If a **trusted** provider asserts a **verified** email matching an existing user, link to that user. Two gates: the email must be verified *and* the provider must be trusted to assert verification. An unverified address — or a verified one from an untrusted provider — can't capture an account.</Step>
<Step title="Auto-create">Otherwise mint a new `AuthUser` (unique username; the password is unusable until they set one).</Step>
</Steps>

<Callout type="warning">
Email-based auto-linking happens **only** for a provider-verified email from a provider you trust. This prevents account takeover: an attacker can't pre-register an unverified address to capture a future real signup, and a custom provider that lies about `email_verified` can't seize a matching account. Built-in **Google** and **GitHub** are trusted (they verify email ownership). A custom provider is **untrusted by default** — its logins auto-create a separate account instead of linking. Opt in by overriding `fn trusts_verified_email(&self) -> bool { true }` only when you trust the provider to assert ownership honestly.
</Callout>
...
`Identity { uid, email, email_verified, display_name }` is what the policy consumes; set `email_verified` honestly, since it gates auto-linking. A custom provider is **untrusted for verified-email linking by default** (safe by default): even a `email_verified: true` identity won't link to an existing account until you opt in with a fourth method, `fn trusts_verified_email(&self) -> bool { true }`. Add it only when your provider genuinely verifies email ownership.
