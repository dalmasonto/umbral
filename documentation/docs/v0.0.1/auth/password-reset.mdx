...
Always returns `202 Accepted` — regardless of whether the email belongs to a registered account. The response body and timing are identical in both cases to prevent account enumeration. When the account exists, a single-use token is generated and the reset link is emailed via the configured mailer.
...
3. **Revokes all bearer tokens** for the user.
4. Returns `200 OK`.

The session and token revocation happens atomically with the password write. A stolen reset link cannot silently coexist with an attacker's live session.
