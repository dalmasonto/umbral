...
AuthPlugin::new().min_password_length(12)

// Or replace the whole set with a hand-built policy.
AuthPlugin::new().password_validators(
...
// legacy import only.
AuthPlugin::new().disable_password_validation()
```
...
App::builder()
    ...
    // .plugin(AuthPlugin::new().with_default_routes_at("/auth"))
    .build()?;
```
...
```rust
AuthPlugin::new().disable_throttle()
```
