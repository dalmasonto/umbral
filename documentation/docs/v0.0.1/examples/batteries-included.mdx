...
/// worker (`cargo run -- tasks-worker`) can dispatch it. Enqueue work
/// with `umbra_tasks::enqueue("send_welcome", &payload).await?`.
#[umbra::task]
...
| Background tasks | `umbra_tasks::TasksPlugin` + `#[umbra::task]` (self-registering) + `tasks-worker` |
