...
The settings loader only reads `UMBRA_`-prefixed keys, so the database URL goes under `UMBRA_DATABASE_URL` (a bare `DATABASE_URL` would be ignored and the app would fall back to `sqlite::memory:`).

```bash
UMBRA_DATABASE_URL=postgres://api:api@localhost:5432/api_service
UMBRA_BIND_ADDR=0.0.0.0:8000
UMBRA_SECRET_KEY=replace-me-run-openssl-rand-hex-32
...
# The URL lives under UMBRA_DATABASE_URL; psql wants a bare connection string.
psql "$UMBRA_DATABASE_URL" <<'SQL'
