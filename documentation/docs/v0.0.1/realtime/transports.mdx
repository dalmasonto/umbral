...
`umbra-realtime` rejects cross-origin WebSocket upgrades **by default, in production, with no configuration**. The guard runs before the upgrade and returns `403 cross-origin WebSocket rejected`. Its rules, in order:
