...
Always re-check authorization inside `on_message` before publishing to any client-supplied room (as above: `Realtime::can_send(ctx.user_id, &room)`, or your own membership lookup). Treat `text` as untrusted: parse without `unwrap`, and validate every field you route on.
</Callout>
