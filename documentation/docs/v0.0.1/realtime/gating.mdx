...
<Tabs>
<Tab title="umbral-auth sessions">

If you use `umbral-auth`, enable the `auth` feature and call `with_auth_sessions()`. It reads the session cookie at the handshake and resolves the logged-in user's id:
...
### Membership / multi-tenant - check the DB at the handshake

For a `tenant:{id}` or a `team:{id}` room, gate on a real membership check. `group_policy_async_fn` takes an **async** closure, so the ORM query is a plain `.await` — it runs once, at the SSE/WS handshake. Fail closed (`false`) on a lookup error:
...
The async closure receives owned `Option<String>` / `String` (it outlives the handshake's stack). The sync `group_policy_fn` stays the right tool for shape-only rules like the ones above.
...
When a closure isn't enough (you want state, or shared helpers across methods), implement the `GroupPolicy` trait on your own type and pass it to `group_policy`. The trait is `async` (via `async_trait`), so a method can await the DB directly:

```rust
use umbral_realtime::{GroupPolicy, RealtimePlugin, async_trait};
...
impl GroupPolicy for AppPolicy {
    async fn can_join(&self, user_id: Option<&str>, group: &str) -> bool {
        ...
        match user_id {
            Some(uid) => self.allows(uid, group).await,
            None => false,
        }
    }
}
...
| Check the join | `group_policy_fn(...)` / `group_policy_async_fn(...)` / `group_policy(...)` | allow → stream; deny → `403` |
