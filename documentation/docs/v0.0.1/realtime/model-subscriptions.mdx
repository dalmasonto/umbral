...
Route each event to a group computed from the row, so a client can watch a single record. The closure receives the `ModelEvent`; `ev.pk()` returns the row's `id` when it's an integer PK:
...
```rust
use umbra_realtime::{Expose, ModelAction, RealtimePlugin};
...
Use `umbra.realtime.model(table, handlers, { group })`. It's sugar over `subscribe` that wires the `created` / `updated` / `deleted` event names for you. **`group` is required** — name the group you exposed to; there's no magic default:
...
| `ev.pk()` | the row's `id` as `Option<i64>` (integer PKs only) |
