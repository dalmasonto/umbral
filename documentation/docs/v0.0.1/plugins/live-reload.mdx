---
title: Live reload
description: Edit a template, stylesheet, or asset and the browser refreshes itself over SSE, with CSS hot-swapped in place. The Vite dev-loop feel for a server-rendered Rust app.
sidebar_position: 18
...
The plugin mounts `GET /__umbra/livereload`, a Server-Sent Events stream. SSE is one-way (server to client), which is all a reload signal needs, and `EventSource` reconnects on its own.
</Step>
