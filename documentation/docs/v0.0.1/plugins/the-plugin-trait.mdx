...
`dispatch` fires the hooks for `serve` — after `auto_migrate_on_serve()` has applied migrations — and for every command that runs against live data (`createsuperuser`, `worker`, your own management commands). It **skips them entirely** for the schema commands (`migrate`, `makemigrations`, `showmigrations`, `inspectdb`, `typegen`, …).
