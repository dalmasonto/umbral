...
impl Plugin for BlogPlugin {
    ...
    fn models(&self) -> Vec<umbral::migrate::ModelMeta> {
        // One line for the whole crate: every #[derive(Model)] in THIS crate,
        // via the link-time registry. Or list them explicitly —
        // `vec![umbral::migrate::ModelMeta::for_::<Post>()]` — if a table
        // must never depend on the linker keeping the model's object file.
...
`App::build()` topologically sorts the registered plugins, runs each plugin's `system_checks()` alongside the
framework's, merges every plugin's `routes()` into the app router, and
fires `on_ready` in dependency order.

The sort reads two edge sets: the `dependencies()` you declare, and the cross-plugin foreign keys it finds on your models. A `ForeignKey<T>` renders `REFERENCES "<T::TABLE>"` inside `CREATE TABLE`, so the plugin owning that table is migrated first whether or not you declared the edge.

<Callout type="info">
  You still want to declare `dependencies()` — it documents intent, and it orders plugins that depend on each other for reasons the schema can't see (an `on_ready` hook that reads another plugin's registry, say). The FK derivation only guarantees that a missing declaration can't silently produce a `relation "auth_user" does not exist` on the first deploy to a fresh database.
</Callout>

A `#[umbral(db_constraint = false)]` column renders no `REFERENCES` clause, so it imposes no ordering. And if the foreign keys themselves form a cycle, the build fails with `BuildError::ForeignKeyCycle`, naming the columns that close the loop rather than a bare `PluginCycle` you never wrote.
...
<CardGrid cols={3}>
  ...
  <Card icon="git-branch" title="dependencies" description="&'static [&'static str]. Plugins that must load first. The topological sort unions this with the cross-plugin foreign keys read off your models; declared cycles surface as BuildError::PluginCycle, FK-implied ones as BuildError::ForeignKeyCycle." />
  ...
  <Card icon="zap" title="on_ready" description="Sync hook fired in dependency order once the app is actually up — never during migrate/makemigrations. Seed content, spawn background work, wire signals, seal late registrations." />
</CardGrid>
...
| `routes_builder` | `fn routes_builder(&self) -> Option<Routes>` | `None` | Preferred alternative to `routes`/`route_paths`: return a `Routes` builder and umbral derives both the router **and** the declared paths from one source, so the two can't drift. |
...
`on_ready` means *the application is up*, and a hook that seeds content or backfills rows needs a migrated schema. So a CLI binary builds with `build_deferred()` and lets `dispatch` decide:
...
let app = App::builder()
    ...
    .build_deferred()?;      // wire everything, fire nothing

umbral_cli::dispatch(app).await
```

`dispatch` fires the hooks for `serve` — after `auto_migrate_on_serve()` has applied migrations — and for every command that runs against live data (`createsuperuser`, `tasks-worker`, your own management commands). It **skips them entirely** for the schema commands (`migrate`, `makemigrations`, `showmigrations`, `inspectdb`, `typegen`, …).

<Callout type="warning">
  `App::build()` still fires `on_ready` itself, which is what you want in a test or when embedding umbral under your own axum tree. Handing *that* app to `dispatch` means the hooks ran before argv was even parsed — so `cargo run -- migrate` against a fresh database seeds tables it hasn't created yet. `dispatch` prints a warning when it sees this; the fix is one word, `build()` → `build_deferred()`.
</Callout>

Driving the app yourself? `App::serve()` calls `App::ready()` for you. Call `ready()` by hand only if you hold an `App` and neither serve nor dispatch it.
