...
Opt in with `.with_user_in_templates()` and every template render in the app gets `{{ user }}` for free - Django's `request.user` shape, no per-handler ctx threading. The method lives on `AuthPlugin`, not `SessionsPlugin`: the layer needs to name the concrete user model to hydrate it, which is `umbral-auth`'s job. `SessionsPlugin` still has to be registered for the underlying session lookup to work:
