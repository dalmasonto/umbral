...
- **Redis / external store.** The pluggable `SessionStore` trait is in place (`DbStore` and `CookieStore` ship), but a Redis-backed store is deferred until a real use case asks for it. Implement `SessionStore` yourself and pass it to `.store(...)` in the meantime.

The spec lives in `docs/specs/02-plugin-contract.md`. Source: `plugins/umbra-sessions/src/lib.rs` and `plugins/umbra-sessions/src/cookie_store.rs`.
