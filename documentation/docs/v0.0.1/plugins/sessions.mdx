...
- **Key.** The 256-bit key is `SHA-256(secret_key)`. **`secret_key` is required**: a stateless cookie session derived from an empty key is trivially forgeable (anyone can mint a valid cookie for any `user_id`). With an empty `secret_key`, `CookieStore` currently only *logs* - it warns in dev/test and emits an error line in production, but still boots and serves forgeable sessions. The hard boot-fail on an empty key comes from the separate `umbral-security` plugin (`validate_secret_key`); `SessionsPlugin` does not enforce it on its own. **If you deploy `CookieStore`, register `umbral-security` too and set `secret_key` in `umbral.toml` or via `UMBRAL_SECRET_KEY` before deploying** - do not rely on `CookieStore` alone to refuse an empty key.
