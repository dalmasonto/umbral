...
## Assigning groups and permissions in code

`has_perm` / `user_perms` answer the read side. A set of free functions in `umbral_permissions` handles the write side - putting users in groups and granting or revoking permissions - without hand-writing SQL. Each takes the stringified user id, matching `has_perm`.

```rust
use umbral_permissions::{
    add_user_to_group, remove_user_from_group, set_user_groups,
    groups_for_user, group_ids_for_user, is_in_group,
    grant_user_permission, revoke_user_permission,
    direct_permissions_for_user, has_direct_user_permission,
};

// Group membership
add_user_to_group(&user_id, &editors).await?;              // idempotent add
remove_user_from_group(&user_id, &editors).await?;
set_user_groups(&user_id, &[editors.id, authors.id]).await?; // replace the whole set
let groups = groups_for_user(&user_id).await?;             // Vec<Group>
let ids = group_ids_for_user(&user_id).await?;             // Vec<i64>
let member = is_in_group(&user_id, editors.id).await?;     // bool

// Direct (non-group) permission grants
grant_user_permission(&user_id, &publish).await?;
revoke_user_permission(&user_id, &publish).await?;
let direct = direct_permissions_for_user(&user_id).await?;             // Vec<Permission>
let has = has_direct_user_permission(&user_id, "publish_post").await?; // ignores groups
```

When you have the app label and codename as separate values, `has_perm_scoped(user_id, app_label, codename)` checks a grant without building the `"app.codename"` string yourself (use `has_perm` when you already have that combined form).
