...
**The `is_active` check applies only to the superuser bypass, not to the ordinary permission path.** For a non-superuser, the layer resolves the session, then calls `has_perm(user_id, perm)` directly - it does **not** re-check `is_active`. A user who was deactivated but still holds a valid, unexpired session cookie keeps passing `permission_required` for every permission they were granted until that session is invalidated. If deactivation must revoke access immediately, invalidate the user's sessions on the deactivation path (session validity is `umbral-auth`'s responsibility, not this layer's).
</Callout>
