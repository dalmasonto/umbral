---
title: Email plugin
description: SMTP + template-driven transactional email with file attachments.
sidebar_position: 12
icon: mail
---
...
The HTTP-API senders are behind a feature: `cargo add umbral-email --features api`.
...
**Selection order:** a custom backend registered with [`EmailPlugin::with_backend`](#bring-your-own-sender) overrides all of this. Otherwise `UMBRAL_EMAIL_BACKEND=console` wins (the safety valve), then: API (env-forced, or both `email_api_provider` + `email_api_key` set) → SMTP (`email_smtp_host` set) → console.
...
The built-in Console / SMTP / API backends cover most apps, but you can send through **your own transport** — a provider's SDK, an in-house relay, a queue, or a test spy — without forking the plugin. Implement the `EmailBackend` trait and register it with `EmailPlugin::with_backend(...)`. Once registered, your backend takes over `send()` for the whole app.
...
impl EmailBackend for ResendClient {
    async fn deliver(&self, message: &EmailMessage) -> Result<(), EmailError> {
        // message.from is already resolved and every header value has been
        // validated against CRLF/header injection. Build and POST your
        // provider's request here; map any failure to EmailError::Backend(..).
...
App::builder()
    .plugin(EmailPlugin::with_backend(ResendClient { key: api_key }))
    // ...
    .build()?;
```

That's the whole contract: a struct, one `deliver` method, one builder call. `deliver` receives the high-level `EmailMessage` (its `from` filled in), so you compose whatever shape your provider wants.

<Callout type="info">
Your backend fully replaces the settings-driven dispatch — a bare app still gets Console in dev / SMTP in prod with zero code, but once you register a custom backend it is used everywhere `send()` is called. The header-injection guard still runs first, so a custom transport is never handed an unvalidated subject or recipient. Report failures with `EmailError::Backend(String)`.
</Callout>
...
- **No retry queue.** Transient SMTP failures bubble up. Wiring sends
  through `umbral-tasks` is the natural follow-on.
