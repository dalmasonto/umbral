...
description: A `Plugin` is a self-contained, swappable unit that bundles models, routes, and logic into one builder call.
sidebar_position: 2
icon: package
---
...
| CLI subcommands | `Plugin::commands()`; `umbral-auth` contributes `createsuperuser` this way (see the [Plugin trait page](/docs/v0.0.1/plugins/the-plugin-trait)) |
...
<Callout type="warning">
  A plugin crate's own `Cargo.toml` needs `sqlx` directly — every `#[derive(Model)]`
  struct also needs `sqlx::FromRow`, and sqlx's derive macro can't be routed through
  a re-export. **Pin the same `sqlx` version umbral-core itself resolves.** A second,
  incompatible `sqlx` major anywhere in the dependency tree produces an inscrutable
  unsatisfied `FromRow<PgRow>` trait bound rather than an error that names "sqlx" at
  all. Run [`umbral doctor`](/docs/v0.0.1/cli/doctor) after adding a plugin (yours or
  a third party's) to catch the drift before it bites.
</Callout>
