...
When the OpenAPI spec advertises filter parameters (umbra-openapi emits them when `ResourceConfig::enable_filters()` is on), the Params table shows them with proper types and lookups. When it doesn't, the playground falls back to chip suggestions inferred from the response item schema — `[+ eq]` / `[+ contains]` / `[+ gte]` / `[+ in]` etc. per field, scoped by type.
