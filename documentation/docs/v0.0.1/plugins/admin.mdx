...
The admin plugin creates two tables on first boot via `Plugin::on_ready`:
...
No migrations are required for these tables. They are created with `CREATE TABLE IF NOT EXISTS` on every boot.
...
**Two built-in widgets** seed the catalog on every boot:

- `umbra_total_models` (KPI) - count of all registered models.
- `umbra_recent_users` (Feed) - last 5 `auth_user` signups.
