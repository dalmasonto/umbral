...
Every successful create, update, delete, and sheet-create operation writes one row to `admin_audit_log`. Bulk actions write one row summarizing the operation. The write is fire-and-forget (errors are logged but do not fail the originating request).
