...
| `GET /admin/` | Dashboard - a widget grid (may redirect to the last-visited changelist) |
...
Inline related-model editors and paginated list views **have shipped** - see [Inlines](/docs/v0.0.1/admin/inlines) and the paginated row list above.

<Callout type="info">
The Django-style form-layout conveniences are **not** part of the admin config yet - they are not stored on `AdminModel`:

- **`fieldsets`** - grouping fields into named sections on the edit form.
- **`prepopulated_fields`** - slug autofill from another field.
- **`raw_id_fields`** - FK rendered as a raw id input instead of a picker.

When you need that level of layout control today, model the page with a [custom view](/docs/v0.0.1/admin/custom-views).
</Callout>
...
- `builtin_total_models_widget()` → key `umbral_total_models` (Bar) - row counts across all registered models.
