...
export UMBRA_S3_BUCKET=my-assets
export UMBRA_S3_REGION=us-east-1
# Optional: UMBRA_S3_ENDPOINT (MinIO/R2), UMBRA_S3_PREFIX, plus either explicit
# UMBRA_S3_ACCESS_KEY / UMBRA_S3_SECRET_KEY or the standard AWS credential chain.

cargo run -- collectstatic --hashed --storage s3
```

<Callout type="info">
The S3-backend env vars are now `UMBRA_S3_*`. The legacy `UMBRA_STATIC_BUCKET` / `_REGION` / `_ENDPOINT` / `_PREFIX` / `_PUBLIC_BASE` names still work as a deprecated fallback (a one-time warning is logged when only the old name is set). The static *pipeline* settings `UMBRA_STATIC_URL` / `UMBRA_STATIC_ROOT` are a separate concern and are unchanged.
</Callout>
...
// From the environment (the UMBRA_S3_* vars + explicit or AWS-chain credentials):
...
```bash
export UMBRA_S3_BUCKET=my-bucket
export UMBRA_S3_REGION=us-east-1
# Optional: UMBRA_S3_ENDPOINT (MinIO/R2/B2/Spaces), UMBRA_S3_PREFIX, UMBRA_S3_PUBLIC_BASE
# Explicit creds (else the AWS chain): UMBRA_S3_ACCESS_KEY / UMBRA_S3_SECRET_KEY
export UMBRA_S3_ACCESS_KEY=…  UMBRA_S3_SECRET_KEY=…
```
...
The `UMBRA_S3_*` env vars target **any** S3-compatible provider. Set the bucket + region for AWS; add an endpoint (and usually path-style) for everyone else. Credentials come from explicit `UMBRA_S3_ACCESS_KEY` / `UMBRA_S3_SECRET_KEY` when both are set, otherwise the AWS chain.
...
```bash
export UMBRA_S3_BUCKET=my-bucket
export UMBRA_S3_ENDPOINT=https://minio.example.com
export UMBRA_S3_REGION=us-east-1
export UMBRA_S3_PATH_STYLE=true        # MinIO needs path-style addressing
export UMBRA_S3_ACCESS_KEY=…  UMBRA_S3_SECRET_KEY=…
```
</Tab>
...
```bash
export UMBRA_S3_BUCKET=my-bucket
export UMBRA_S3_ENDPOINT=https://<account-id>.r2.cloudflarestorage.com
export UMBRA_S3_REGION=auto
export UMBRA_S3_ACCESS_KEY=…  UMBRA_S3_SECRET_KEY=…
```
</Tab>
...
```bash
export UMBRA_S3_BUCKET=my-bucket
export UMBRA_S3_ENDPOINT=https://s3.<region>.backblazeb2.com
export UMBRA_S3_REGION=<region>        # e.g. us-west-004
export UMBRA_S3_ACCESS_KEY=…  UMBRA_S3_SECRET_KEY=…
```
</Tab>
<Tab title="DigitalOcean Spaces">
```bash
export UMBRA_S3_BUCKET=my-space
export UMBRA_S3_ENDPOINT=https://<region>.digitaloceanspaces.com
export UMBRA_S3_REGION=<region>        # e.g. nyc3
export UMBRA_S3_ACCESS_KEY=…  UMBRA_S3_SECRET_KEY=…
```
</Tab>
</Tabs>
...
<Callout type="info">
**Credentials precedence.** If both `UMBRA_S3_ACCESS_KEY` and `UMBRA_S3_SECRET_KEY` are set, the storage backend uses them (plus optional `UMBRA_S3_SESSION_TOKEN`) — handy when you want storage to use a different provider's keys without colliding with `AWS_*` used by other services. Otherwise it falls back to the standard AWS chain (`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`, an IAM role, `~/.aws/credentials`).

**Path-style.** Set `UMBRA_S3_PATH_STYLE=true` (or `.path_style(true)`) for MinIO and most self-hosted stores, which serve `endpoint/bucket/key`. Leave it off for AWS, which uses virtual-hosted addressing (`bucket.s3.region.amazonaws.com`).
</Callout>
...
```bash
export UMBRA_S3_PRESIGN_TTL=900    # url() returns links valid for 900 seconds
```
...
**Enabling S3 safely.**
- **Credentials** come from explicit `UMBRA_S3_ACCESS_KEY` / `UMBRA_S3_SECRET_KEY` or the standard AWS chain (IAM role, `~/.aws/credentials`) — never hard-code keys in source or `settings`.
- **Public vs signed.** Without a presign TTL, `S3Storage::url()` returns `<public_base>/<key>` (or the bare key) — built for **public-read** buckets fronted by a CDN; set `UMBRA_S3_PUBLIC_BASE` (or `.public_base(...)`) to that origin. For **private** buckets, set `UMBRA_S3_PRESIGN_TTL` (or `.presign(secs)`) and `url()` returns signed, time-limited URLs instead.
...
Real uploads often need work *after* the bytes land — generate a thumbnail, transcode a video, strip EXIF, scan for malware. Doing that inline blocks the upload response on slow work. umbra-storage runs it in the background instead, and tracks where each file is in its lifecycle with `MediaFile.status`:
...
When a background task flips `status` through the ORM, the ORM fires `post_save:media_file`. umbra-storage does **not** depend on umbra-realtime — instead a developer who wants the frontend notified just exposes the model over realtime, and the status change is pushed automatically:
...
Background work runs via an in-process `tokio::spawn`. That's simple and has **no plugin dependency** — umbra-storage doesn't import umbra-tasks. The trade-off: a process crash mid-processing loses the in-flight work (the file stays `processing`). For crash-durable processing, have the processor **enqueue an `umbra-tasks` job** instead of doing the work inline:

```rust
.on_upload(|media: MediaFile| async move {
    ProcessUpload { media_id: media.id }.enqueue().await?;  // your umbra-tasks job
    Ok(())
})
```

The job survives a restart because it's persisted in the task queue; the `on_upload` closure just hands work off to it. umbra-storage stays decoupled from both umbra-tasks and umbra-realtime — the developer composes them.
...
The storage-backend trait `gaps.md #49` called for **has shipped**: it landed as `Storage` (in `umbra-core`, see above), with the filesystem `FsStorage` impl, the `s3`-gated `S3Storage` (now with `UMBRA_S3_*` config, explicit credentials, path-style addressing, and **presigned URLs** for private buckets), and `.media_with_storage(...)` for swapping backends. One thing still deferred:
