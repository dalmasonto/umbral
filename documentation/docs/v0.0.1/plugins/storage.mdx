...
`{{ static('css/app.css') }}` resolves to `<static_url>css/app.css`: `/static/css/app.css` by default, or `https://cdn.example.com/s/css/app.css` when `static_url` points at a CDN. This is umbral's equivalent of Django's `{% static "..." %}` tag.
