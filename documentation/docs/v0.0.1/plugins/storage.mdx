...
The S3 backend is behind a feature: `cargo add umbral-storage --features s3`.
...
|---|---|---|---|
| `static_url` | `UMBRAL_STATIC_URL` | `/static/` | URL base every asset is served under. May be an absolute CDN origin (`https://cdn.example.com/s/`). |
| `static_root` | `UMBRAL_STATIC_ROOT` | `staticfiles/` | On-disk dir `collectstatic` writes to and production serves from. |
...
Uploads are capped at **25 MiB by default** (`DEFAULT_MAX_UPLOAD_SIZE`) even without a `.max_size(..)` call; raise/lower it with `.max_size(bytes)` or remove it deliberately with `.max_size_unlimited()` (only for trusted upload paths - an uncapped media side lets any client fill the disk).
...
export UMBRAL_S3_REGION=us-east-1
export UMBRAL_S3_ACCESS_KEY=…  UMBRAL_S3_SECRET_KEY=…
# No endpoint, no path-style - virtual-hosted addressing is the AWS default.
```
</Tab>
<Tab title="MinIO">
```bash
export UMBRAL_S3_BUCKET=my-bucket
export UMBRAL_S3_ENDPOINT=https://minio.example.com
export UMBRAL_S3_REGION=us-east-1
export UMBRAL_S3_PATH_STYLE=true        # MinIO needs path-style addressing
export UMBRAL_S3_ACCESS_KEY=…  UMBRAL_S3_SECRET_KEY=…
```
</Tab>
...
```bash
export UMBRAL_S3_BUCKET=my-bucket
export UMBRAL_S3_ENDPOINT=https://<account-id>.r2.cloudflarestorage.com
export UMBRAL_S3_REGION=auto
export UMBRAL_S3_ACCESS_KEY=…  UMBRAL_S3_SECRET_KEY=…
```
</Tab>
...
```bash
export UMBRAL_S3_BUCKET=my-bucket
export UMBRAL_S3_ENDPOINT=https://s3.<region>.backblazeb2.com
export UMBRAL_S3_REGION=<region>        # e.g. us-west-004
export UMBRAL_S3_ACCESS_KEY=…  UMBRAL_S3_SECRET_KEY=…
```
</Tab>
<Tab title="DigitalOcean Spaces">
```bash
export UMBRAL_S3_BUCKET=my-space
export UMBRAL_S3_ENDPOINT=https://<region>.digitaloceanspaces.com
export UMBRAL_S3_REGION=<region>        # e.g. nyc3
export UMBRAL_S3_ACCESS_KEY=…  UMBRAL_S3_SECRET_KEY=…
```
</Tab>
</Tabs>
...
- **Public vs signed.** Without a presign TTL, `S3Storage::url()` returns `<public_base>/<key>` (or the bare key) - built for **public-read** buckets fronted by a CDN; set `UMBRAL_S3_PUBLIC_BASE` (or `.public_base(...)`) to that origin. For **private** buckets, set `UMBRAL_S3_PRESIGN_TTL` (or `.presign(secs)`) and `url()` returns signed, time-limited URLs instead.
- **Feature-gated.** The `rust-s3` AWS client compiles only with the `s3` feature, so a local-only app never pulls it in.
- **Active-content guard applies on the S3 backend too.** `S3Storage::store` (and the streaming path, which delegates to it) applies the same stored-XSS defence as the local `FsStorage`: an upload with an active-content extension (`.html` / `.svg` / `.js`, ...) is renamed with a `.txt` suffix **and** stored with `Content-Type: text/plain`, so a public / CDN-fronted bucket serves it inert instead of executing script on the serving origin. Defence in depth still applies for untrusted uploads: prefer **presigned URLs on a private bucket** (`.presign(secs)`), or configure the bucket/CDN to force `Content-Disposition: attachment` for user-upload prefixes. Note the guard covers the key-generating `store` path (what `save` / admin uploads use); the exact-key `put` records the caller's content type verbatim.
</Callout>
...
Both `save`/`store` (buffered) and `save_stream`/`store_stream` (streaming) apply the **same** filename guards - path-separator/control-char sanitisation and active-content (`.html`/`.svg`/`.js` → `.txt`) neutralisation - on **every backend**, so streaming never weakens the stored-XSS defence. On the S3 backend the defanged upload is additionally stored with `Content-Type: text/plain` so a public bucket can't serve it inline as HTML; see the S3 safety note above.
...
| `status`     | Meaning                                                                                  |
|--------------|------------------------------------------------------------------------------------------|
| `ready`      | Done - the file is stored and any processors finished. The default for a plain upload.   |
| `processing` | A background task is running (processors, or a deferred write). The URL may not resolve yet. |
| `failed`     | A processor (or a deferred write) errored. For `save`, the **original is still stored**.  |
...
| `status`        | `String`                        | `ready` \| `processing` \| `failed` - the [background-processing](#background-uploads--processing) lifecycle. Defaults to `ready`, so a plain upload is usable immediately. |
| `owner`         | `Option<String>`                | The PK (as a string) of the user who owns the file, or `NULL` for a public/unowned file. Stamp it with `set_media_owner`; it's what [`.media_access_owner()`](#owner-only--media_access_owner) checks. |

The model is registered through `Plugin::models()`, so the migration engine creates the table on the next `migrate` and the admin lists it under the **storage** sidebar group automatically. Every column is `#[umbral(noedit)]` - the admin shows the row read-only because mutating it from the UI would diverge from what's on disk. The `owner` column is PK-agnostic (a `String`) so it holds an `i64`, `Uuid`, or string primary key equally; `set_media_owner(key, owner)` writes it and returns `false` if no row tracks that key.
...
That plain `<mount>/<key>` URL is right for **public** files. A **private** file lives behind a [gate](#access-control-for-private-uploads); to embed one in a page without a session, mint a short-lived signed URL in the handler and pass it to the template:

```rust
// handler: hand the template a link good for 5 minutes
use std::time::Duration;
ctx.insert("doc_url", &umbral_storage::signed_media_url("/media", &doc.key, Duration::from_secs(300)));
```

```jinja
<a href="{{ doc_url }}" download="{{ doc.filename }}">Download</a>
```
...
| `.max_size(bytes)`              | 25 MiB (`DEFAULT_MAX_UPLOAD_SIZE`) | Hard cap on upload size, returned as `MediaError::TooLarge` when exceeded. `save` checks the buffered length; `save_stream` enforces it **mid-stream** (no full buffering, no trusting `Content-Length`). |
| `.max_size_unlimited()`         | off     | Removes the cap entirely. Only for trusted/authenticated upload paths (or when an upstream proxy enforces its own body limit) - an uncapped media side is a disk-exhaustion DoS surface. |
...
| `.media_access(callback)`       | off (public) | Gate the media GET route: the async callback receives the request headers + the requested key and returns `true` to serve or `false` for a 403. Off by default (every file public). |
| `.media_access_identity(callback)` | off (public) | Same, but the callback receives the resolved `Option<Identity>` instead of raw headers — the ergonomic auth check. |
| `.media_access_owner()`         | off (public) | Owner-only: serve a file only to the user whose PK string matches its recorded `owner`. Stamp owners with `set_media_owner`. |
| `.media_signed_urls()`          | off (public) | Serve only requests carrying a valid HMAC-signed, time-bounded query. Mint links with `signed_media_url(mount, key, ttl)`. |

## Access control for private uploads

By default the media route serves **every** file under the mount to anyone with the URL — correct for public assets (avatars, logos), but an IDOR for private uploads (a leaked or shared URL grants permanent, unauthenticated access). Four gates, from most turnkey to most bespoke:

### Owner-only — `.media_access_owner()`

The tracked `media_file` row carries an `owner` column. Stamp it once at upload with `set_media_owner`, and the gate resolves the caller (through the app-wide auth backend) and serves each file only to its owner — no file→owner join to hand-write:
...
// …at your upload site (a handler with `State(storage): State<StoragePlugin>`),
// the one extra line that records ownership:
let outcome = storage.save("avatar.png", "image/png", &bytes).await?;
umbral_storage::set_media_owner(&outcome.file.key, &identity.user_id).await?;
```

Anonymous callers, and files with no recorded owner (a `NULL` after the additive migration, or a file with no tracked row), are denied — fail closed.

### Time-bounded share links — `.media_signed_urls()`

Hand someone a private file for a bounded window with **no session** — the signed link *is* the access grant. Mint it with `signed_media_url`; the signature binds the key AND the expiry to your `secret_key`, so it can't be edited to point elsewhere or extended:

```rust
StoragePlugin::new().media("/media", "./media").media_signed_urls();

// In a handler — a link good for 5 minutes:
use std::time::Duration;
let url = umbral_storage::signed_media_url("/media", &key, Duration::from_secs(300));
```

Signed URLs **compose** with the other gates (a valid signature grants access outright, otherwise the closure decides) and are the one gate that also works through the non-FS proxy below.

### Custom logic — `.media_access_identity(...)` / `.media_access(...)`

For a rule the presets don't cover, resolve the caller yourself. `media_access_identity` hands you the resolved `Option<Identity>` (via `App::builder().authentication(...)`); `media_access` hands you the raw headers (a signed header token, a custom scheme):
...
StoragePlugin::new()
    ...
    .media_access_identity(|caller, key| async move {
        ...
        my_custom_rule(&user, &key).await
    });
```

Every gate runs on `GET <mount>/<key>` **before any bytes are served** (Range and HEAD requests too, and it sees the full nested key for subdirectory files); a deny short-circuits with a 403 and the contents never leave the server. It's async, so a database lookup is fine.

<Callout type="info">
**Gates work on S3 / custom backends too, via a proxy.** A non-FS backend serves its own URLs, so with no gate the framework mounts nothing there. But when you configure a gate, it mounts a route that runs the gate and **streams the bytes through the backend** — so private custom backends serve gated files, and signed URLs work everywhere. For a high-traffic *public* mount, a private bucket + [presigned URLs](#presigned-urls-private-buckets) (`.presign(secs)` on the S3 builder) still avoids the proxy round-trip.
</Callout>

## What v0 does not ship

The storage-backend trait `gaps.md #49` called for **has shipped**: it landed as `Storage` (in `umbral-core`, see above), with the filesystem `FsStorage` impl, the `s3`-gated `S3Storage` (now with `UMBRAL_S3_*` config, explicit credentials, path-style addressing, and **presigned URLs** for private buckets), and `.media_with_storage(...)` for swapping backends. Access control for private uploads has also shipped — the four [gates](#access-control-for-private-uploads) (`.media_access_owner()`, `.media_signed_urls()`, `.media_access_identity()`, `.media_access()`), including the gated proxy that lets S3/custom backends serve private files. Built-in [thumbnails / resizing](#thumbnails-and-resizing) shipped too, behind the `images` feature. Deferred bits of the image library:

1. **EXIF-strip and format-probe.** Thumbnail generation is in (`thumbnails(...)`), but a *built-in* EXIF-metadata stripper and a byte-level format prober are not yet. The [`on_upload`](#background-uploads--processing) hook (with the `processing`/`ready`/`failed` lifecycle and `save_deferred`) runs any pipeline you write, so you can strip EXIF today in your own processor.
...
The size cap stops a 20 MB file. It does **not** stop a 2 MB `.exe` renamed to `avatar.png` from landing in an `ImageField`. `accept(...)` does:

```rust
StoragePlugin::default()
    .media("/media", fs)
    .accept_images()                                   // png / jpeg / gif / webp
    // or an explicit list:
    .accept(&["image/png", "application/pdf"])
```

<Callout type="warning">
  **It sniffs the bytes, not the header.** A client-declared `Content-Type` is trivially spoofed, so a policy that only reads the declaration stops nobody. Three things must hold: the declared type is on the list, the bytes' real signature is on the list, and **the two agree**. An executable claiming `image/png` fails the second check; a JPEG declared as a PNG fails the third.
</Callout>

Enforcement is a **storage decorator**, the same shape as the size cap — so the admin, form handling, REST, and any upload route you write yourself all inherit it, *including routes added after you set the policy*. A check in a handler only ever covers the handlers that existed the day it was written.

`accept_images()` deliberately **excludes SVG**. An SVG is a script-execution vector, not a picture; an image allow-list shouldn't invite one in. (Active content that arrives some other way is still defanged on write — see above.)


## Thumbnails and resizing

Behind the `images` cargo feature, so an app with no user-uploaded images never compiles a codec:

```toml
umbral-storage = { version = "0.0.6", features = ["images"] }
```

```rust
use umbral_storage::{Thumbnail, thumbnails, variant_key};

static SIZES: &[Thumbnail] = &[
    Thumbnail::new("thumb", 200, 200),
    Thumbnail::new("medium", 800, 800),
];

StoragePlugin::default()
    .media("/media", fs)
    .accept_images()
    .on_upload(thumbnails(SIZES))
```

Every image upload now gets its variants written automatically, at a **derived key**:
...
A variant's key is a *pure function* of the original's — `variant_key(&file.key, "thumb")` — so a template builds a thumbnail URL with **no extra column, no join, and no second lookup**.

<Callout type="info">
  **It never upscales.** A 40×40 avatar asked for a 200×200 thumbnail stays 40×40: enlarging it invents detail that was never there and costs bytes to say nothing. The size is a **bounding box** the image is fitted *within* — aspect ratio is always preserved, never stretched to a shape.
</Callout>

Non-image uploads pass straight through: a PDF in the same bucket isn't an error, it's just not a thing to thumbnail. Decoding runs on a blocking thread (it's CPU-bound and would otherwise stall unrelated requests), bounded by `UMBRAL_MEDIA_PROCESSING_CONCURRENCY`.

Variants are written with `Storage::store_at` — an exact-key write, as opposed to `store`, which mints a fresh UUID key (right for a user upload, where two users' `avatar.png` must not collide; wrong for a derived object whose key you computed). They're deliberately **not** recorded as `media_file` rows: a thumbnail isn't a second upload, and giving it a row would double every upload in the admin and let orphan cleanup delete a variant out from under its original.
