...
- **Active-content guard is NOT applied by the S3 backend.** The `.html` / `.svg` / `.js` → `.txt` rename that defends against stored XSS is performed by the local `FsStorage` backend (and the local `/media` serve path also sends `X-Content-Type-Options: nosniff`). `S3Storage::store` uploads the object under a UUID key with the **client-declared `Content-Type` verbatim** and does *not* rename active-content extensions. If your S3 bucket is public / CDN-fronted, an uploaded `evil.html` (Content-Type `text/html`) is served inline and can execute script on the serving origin. Serve S3-hosted user uploads through **presigned URLs on a private bucket** (`.presign(secs)`), or configure the bucket/CDN to force a safe `Content-Disposition: attachment` (or override `Content-Type` to `application/octet-stream`) for user-upload prefixes. Do not host untrusted uploads on a public bucket without one of these controls.
</Callout>
...
On the **filesystem** backend, both `save`/`store` (buffered) and `save_stream`/`store_stream` (streaming) apply the **same** filename guards - path-separator sanitisation and active-content (`.html`/`.svg`/`.js` → `.txt`) neutralisation - so streaming never weakens the stored-XSS defence. The **S3** backend does not perform the active-content rename in `store`/`store_stream` (it keeps the client's `Content-Type`); see the S3 safety note above for how to serve untrusted S3-hosted uploads safely.
