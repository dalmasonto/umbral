...
On writes the plugin is stricter than REST: an unauthorized attempt to set a private column is refused **by name** (`not authorized to set 'cost'`) rather than silently dropped.
