...
- **Admin visibility.** No admin UI into the queue (pending/failed counts,
  retrying a failed task). Also #82. (The **result backend** and **task-status
  API** — `task_status` / `await_result` — are done; see "Task results &
  status" above.)
