...
- **Result backend.** No place to read a task's return value back, and no
  task-status query API beyond inspecting the `task_row` row directly. The
  remaining Celery gap, deferred to the deep spec (`planning/features.md` #82).
- **Priority queues.** No priority ordering beyond `run_at` then id. Also #82.
