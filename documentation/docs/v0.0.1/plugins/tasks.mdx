...
That's the whole wiring: **the handler is discovered automatically**. The
macro registers the task in a link-time slice, and `TasksPlugin` (plus the
`tasks-worker` / `tasks-beat` commands) installs every discovered handler at
startup — there is nothing to call by hand, so a forgotten registration can
no longer surface as a runtime `HandlerNotFound` in production.

The macro also emits, for when you want manual control:

1. The original `async fn send_welcome` unchanged, so you can call it directly
   in tests or other code.
2. A companion `pub fn register_send_welcome()` that installs the handler by
   hand — useful with `TasksPlugin::no_discover()`, which turns discovery off
   for the whole process and returns you to explicit registration.

Enqueue work anywhere in your request handlers:
...
// Discovery registers it under the custom name; enqueue uses it too:
