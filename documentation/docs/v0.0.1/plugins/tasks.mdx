...
minutes — register a periodic schedule, Celery `beat_schedule` style, and run the
**beat** process. Beat is the scheduler; it does not run handlers itself. Each
time a schedule is due, beat enqueues a normal task row; the **worker** drains it.
Run them as two separate processes (exactly like Celery's `worker` and `beat`).

Register schedules on the plugin builder with `.periodic(name, schedule, task, payload)`:

```rust
use std::time::Duration;
use umbra_tasks::{Schedule, TasksPlugin};
...
`result` column — the **result backend**, Celery's `AsyncResult` payload. Query
a task's outcome by the id `enqueue` returned:

```rust
use umbra_tasks::{enqueue, register_handler, task_status, await_result, TaskState, EnqueueOptions};
use std::time::Duration;
...
// Blocking with a timeout: poll until terminal (Celery's AsyncResult.get()).
...
See `arch.md` and `plugins/umbra-tasks/src/lib.rs` for the design rationale.

## Admin visibility

Operators browse and inspect the task queue in the admin. `umbra_tasks::admin_model()` returns a **read-only** `AdminModel` for the `task_row` table — register it on the admin alongside your own models:
...
App::builder()
    .plugin(TasksPlugin::default())
    .plugin(AdminPlugin::default().register(umbra_tasks::admin_model()))
    .build()?;
```
...
let requeued: bool = umbra_tasks::retry_task(42).await?;
```

The `admin_model()` function and the `umbra-admin` dependency are gated behind the `admin` feature (on by default). A tasks-only app that doesn't run the admin builds with `default-features = false` and never pulls the admin into its dependency graph:

```toml
umbra-tasks = { version = "0.0.1", default-features = false }
```
