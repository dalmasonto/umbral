---
title: Request logs
description: Request logging for umbral. A non-blocking capture layer records each HTTP request to a RequestLog model, browsable in the admin.
sidebar_position: 21
...
Each row carries `method`, `path`, `status`, `duration_ms`, plus best-effort `user_id`, `ip` (the client address, taken from `X-Forwarded-For` under the configured trusted-proxy-hops policy), `user_agent`, and a `created_at` timestamp. The model lives in the `logs_requestlog` table - `migrate` creates it the same way the built-in auth and sessions tables are created (from the plugin's `models()`).

<Callout type="info">
**How `user_id` is resolved.** The session/identity surface lives in optional sibling plugins (`umbral-auth` / `umbral-sessions`) that `umbral-logs` deliberately doesn't depend on, so a logs-only app stays slim. The logger reads the **trusted `umbral_logs::LoggedUserId(String)` request extension** - never a request header, which a client could forge. Your auth middleware inserts it (`req.extensions_mut().insert(LoggedUserId(id.to_string()))`) once it has authenticated the caller; otherwise `user_id` is `None`. It is stored as text, so string and UUID primary keys are preserved, not just `i64`s.
</Callout>
