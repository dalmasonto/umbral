...
<Callout type="info" title="Bulk save methods fire bulk signals, not per-row signals">
`Manager::bulk_create(vec)` and `QuerySet::update_values(map)` fire ONLY a **bulk signal**, never per-row `post_save`:
...
**`QuerySet::delete()` and `DynQuerySet::delete()` (REST/admin) fire BOTH**: `bulk_post_delete:<table>` once with `{ ids }`, AND a per-row `post_delete:<table>` for every deleted row. The per-row payload is PK-only (`{ "instance": { "<pk>": <id> } }`) unless something is actually subscribed to `post_delete:<table>` - a subscriber present means the DELETE pays for `RETURNING *` and the payload carries the FULL row instead, same shape `Manager::delete_instance` uses. The full row still honors `#[umbral(signal_skip)]` on both paths - a skipped field never reaches the payload, whether the delete went through the typed or the dynamic (REST/admin) path. `pre_delete` is still `Manager::delete_instance`-only; there's no single moment to fire it for a multi-row DELETE.

This is intentionally verbose - bulk deletes are usually hot paths where the cost of serialising every row to JSON and calling N signal handlers would be a surprise when nobody's listening. That's why the full-row cost is gated on `post_delete:<table>` actually having a subscriber.
</Callout>
...
## Transactional writes (`on_tx`)

A write made through `Model::objects().on_tx(tx)` (inside `db::transaction(...)`, `db::transaction_on(...)`, `db::transaction_sqlite(...)`, or `db::transaction_pg(...)`) fires its `post_save` / `post_delete` / `bulk_post_save` / `bulk_post_delete` signals **after the transaction commits**, not while it's open. This matters because a naive "emit inline, mid-transaction" implementation is a phantom-signal bug: the signal (and any side effect a handler triggers) would fire even if the transaction later rolls back, telling subscribers about a write that never actually happened. Instead, each `on_tx()` write buffers its signal on the transaction, and the runner flushes that buffer only once `commit()` succeeds - a rollback simply drops the buffer, so a rolled-back write fires nothing.

```rust
db::transaction(|tx| Box::pin(async move {
    Order::objects().on_tx(tx).create(new_order).await?;
    // post_save:order fires only after this closure returns Ok and the
    // transaction commits - never if it returns Err and rolls back.
    Ok::<_, MyError>(())
})).await?;
```

**Boundary:** this only applies to a transaction opened through one of the `db::transaction*` helpers, because those are the ones that own the commit call. A transaction you `begin()` and commit yourself (`let mut tx = db::begin().await?; ...; tx.commit().await?;`) is outside umbral's lifecycle - it has no hook to flush from, so any signals buffered on it are silently dropped when the `Transaction` value is dropped. Use `db::transaction(...)` (or its `_on` / `_sqlite` / `_pg` siblings) whenever you need signals to fire.
