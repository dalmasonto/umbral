...
`UserModel::id()` returns `i64` today. The framework's stored-id surfaces match: `umbra-sessions` writes `Option<i64>`, `Identity` from `umbra-rest` carries `user_id: i64`. A custom user model with a `uuid::Uuid` PK isn't fully supported through this trait yet; a follow-up generalises `id()` to an associated `Pk` type.
