...
description: umbral-auth plugin - AuthUser model, argon2 password hashing, session login helpers, and the custom user model swap.
sidebar_position: 3
...
Django has a `@login_required` decorator that gates a view behind authentication. umbral ships the same idea in two complementary shapes: a per-handler extractor and a per-Router layer. Both share one config struct that controls the unauthenticated response.
...
This is umbral's equivalent of Django's `AUTH_USER_MODEL`. In Django you set a string (`AUTH_USER_MODEL = "myapp.CustomUser"`); in umbral you use the type parameter directly, so the swap is validated at compile time with no runtime reflection involved.
...
Then `authenticate::<EmailUser>("alice@example.com", "s3cret").await?` looks the row up by `email`. This is the same role Django's `USERNAME_FIELD` setting plays - except in umbral it's a trait method on your struct, not a separate config string.
