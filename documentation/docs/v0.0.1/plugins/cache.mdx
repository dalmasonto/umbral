...
`cache_page` and `umbra_cache::ambient()` both read the process-wide ambient handle. The idiomatic way to install it is to register `CachePlugin::new(cache)` on the builder - `on_ready` wires the carried cache as the ambient handle at boot:
