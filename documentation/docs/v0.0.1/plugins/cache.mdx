...
- Request carries an `umbral_session` cookie (logged-in / personalised requests are never served from, nor written to, the page cache)

If the ambient cache was not initialised, misses and stores are silently skipped - the handler fires normally.

<Callout type="warning">
**`cache_page` is safe only for genuinely anonymous, non-personalised pages.** Its personalisation gate recognises exactly one signal: the `umbral_session` cookie. It does **not** inspect `Authorization` headers, HTTP Basic auth, or any other cookie name, and on the response side it honours only `Cache-Control: no-store` — **not** `private`, `no-cache`, or `max-age=0`. The cache key has no user dimension.

Consequences you must design around:

- If you authenticate with a **bearer token / `Authorization` header** (JWT, API key) rather than the `umbral_session` cookie, a per-user `200` response wrapped in `cache_page` will be stored and served to other users. Do not wrap authenticated routes.
- Marking a response `Cache-Control: private` does **not** keep it out of this store. Use `Cache-Control: no-store` (or a `Set-Cookie`) to force a bypass.
- `cache_page` is not `Vary`-aware. A response whose body depends on `Accept-Language`, `Authorization`, or any other request header will be served across those variants.
