...
umbral's cache plugin (`umbral-cache`) mirrors Django's cache framework: one `Cache` handle over a swappable `CacheBackend` trait, three built-in backends (memory, SQLite, Redis), and a view-level `cache_page` middleware that caches full GET responses for a route subtree.
