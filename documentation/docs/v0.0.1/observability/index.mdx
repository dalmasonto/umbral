...
**Even with no init at all, `serve` is never silent.** `App::serve` installs a plain fmt subscriber (honoring `RUST_LOG`, default `info`) when no global subscriber exists yet, so a fresh app always prints its `umbral serving on ...` line — a forgotten init used to make a booting server indistinguishable from a hung one. Any subscriber installed earlier in `main` (including `observability::init`) wins; the fallback is strictly last-resort console logging with no OTLP export.
