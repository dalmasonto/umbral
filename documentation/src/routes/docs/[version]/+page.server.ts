...
import { getCachedVersions, getCachedAllDocs, getProducts, getEffectiveConfig, link } from 'specra';
...
export const load: PageServerLoad = async ({ params }) => {
  ...
  if (matchedProduct) {
    ...
    redirect(302, link(`/docs/${version}/${activeVersion}`));
  }
